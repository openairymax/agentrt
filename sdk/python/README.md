...
│   ├── agent.py                # AgentRT/AsyncAgentRT 同步/异步客户端
│   ├── task.py                 # Task 领域模型
...
client = AgentRT(endpoint="http://localhost:18789", timeout=30, api_key="key")
...
async with AsyncAgentRT(endpoint="http://localhost:18789") as client:
...
```python
from agentrt.utils.token_optimizer import TokenOptimizer
...
async def main():
    async with AsyncAgentRT() as client:
        task = await client.submit_task("Analyze data")
        result = await task.wait(timeout=30)
        print(result.output)
...
```python
from agentrt import AgentRT, TaskManager, MemoryManager
