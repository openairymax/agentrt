...
#define AGENTRT_STORAGE_VERSION "1.0.0"
#define AGENTRT_MAX_PATH_LEN 512
#define AGENTRT_MAX_RECORDS 1000000
#define AGENTRT_INDEX_FILE "index.json"

typedef struct storage_record {
    char record_id[64];
    char file_path[AGENTRT_MAX_PATH_LEN];
    size_t data_len;
    time_t created_at;
    time_t updated_at;
...
typedef struct builtin_storage {
    char base_path[AGENTRT_MAX_PATH_LEN];
...
{
    char tmp[AGENTRT_MAX_PATH_LEN] = {0};
...
{
    builtin_storage_t *st = (builtin_storage_t *)AGENTRT_CALLOC(1, sizeof(builtin_storage_t));
    if (!st)
        return NULL;
...
#pragma GCC diagnostic ignored "-Wformat-truncation"
    ...
    char data_dir[AGENTRT_MAX_PATH_LEN];
    snprintf(data_dir, sizeof(data_dir), "%s/data", st->base_path);
#pragma GCC diagnostic pop
    ensure_dir(data_dir);

    st->record_capacity = 1024;
    st->records = (storage_record_t *)AGENTRT_CALLOC(st->record_capacity, sizeof(storage_record_t));
    if (!st->records) {
        AGENTRT_FREE(st);
        return NULL;
    }
...
void builtin_storage_destroy(builtin_storage_t *st)
{
    if (!st)
        return;
    if (st->records)
        AGENTRT_FREE(st->records);
    AGENTRT_FREE(st);
}

agentrt_error_t builtin_storage_write(builtin_storage_t *st, const void *data, size_t len,
                                      const char *metadata_json, char **out_record_id)
{
    ...
    if (st->record_count >= st->record_capacity) {
        ...
        if (new_cap > AGENTRT_MAX_RECORDS)
            return AGENTRT_ENOMEM;
        if (new_cap > SIZE_MAX / sizeof(storage_record_t))
            return AGENTRT_ENOMEM;
        storage_record_t *tmp =
            (storage_record_t *)AGENTRT_REALLOC(st->records, new_cap * sizeof(storage_record_t));
        if (!tmp)
            return AGENTRT_ENOMEM;
        st->records = tmp;
...
agentrt_error_t builtin_storage_get(builtin_storage_t *st, const char *record_id, void **out_data,
                                    size_t *out_len)
{
...
agentrt_error_t builtin_storage_delete(builtin_storage_t *st, const char *record_id)
{
...
agentrt_error_t builtin_storage_touch(builtin_storage_t *st, const char *record_id)
{
    if (!st || !record_id)
        return AGENTRT_EINVAL;
    for (size_t i = 0; i < st->record_count; i++) {
