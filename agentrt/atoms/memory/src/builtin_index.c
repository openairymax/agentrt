...
{
    builtin_index_t *idx = (builtin_index_t *)AGENTRT_CALLOC(1, sizeof(builtin_index_t));
    return idx;
}
        ...
        while (t) {
            ...
            if (t->record_ids) {
                for (size_t j = 0; j < t->record_count; j++) {
                    AGENTRT_FREE(t->record_ids[j]);
                }
                AGENTRT_FREE(t->record_ids);
            }
            AGENTRT_FREE(t);
            t = next;
        }
...
agentrt_error_t builtin_index_add(builtin_index_t *idx, const char *record_id,
                                  const char *metadata_json, size_t data_len)
{
    ...
    index_entry_t *entry = (index_entry_t *)AGENTRT_CALLOC(1, sizeof(index_entry_t));
    if (!entry)
            ...
            if (isalnum((unsigned char)*p) || *p == '_') {
                ...
                if (ti > 0) {
                    ...
                    if (strlen(token) >= 2) {
                        ...
                        while (te) {
                            if (strcmp(te->token, token) == 0)
                                break;
                            te = te->next;
                        }

                        if (!te) {
                            te = (token_entry_t *)AGENTRT_CALLOC(1, sizeof(token_entry_t));
                            if (te) {
                                snprintf(te->token, sizeof(te->token), "%s", token);
                                te->record_capacity = 16;
                                te->record_ids =
                                    (char **)AGENTRT_CALLOC(te->record_capacity, sizeof(char *));
                        ...
                        if (te) {
                            if (te->record_count >= te->record_capacity) {
                                ...
                                char **tmp = (char **)AGENTRT_REALLOC(te->record_ids,
                                                              te->record_capacity * sizeof(char *));
                                if (tmp)
                                    te->record_ids = tmp;
                            }
                            if (te->record_count < te->record_capacity) {
                                te->record_ids[te->record_count++] = AGENTRT_STRDUP(record_id);
                            }
                        }
                    }
                    ti = 0;
...
agentrt_error_t builtin_index_remove(builtin_index_t *idx, const char *record_id)
{
    if (!idx || !record_id)
        return AGENTRT_EINVAL;
...
agentrt_error_t builtin_index_lookup(const builtin_index_t *idx, const char *record_id,
                                     int *out_found)
{

    if (!idx || !record_id || !out_found)
        return AGENTRT_EINVAL;
...
agentrt_error_t builtin_index_search(const builtin_index_t *idx, const char *query, uint32_t limit,
                                     char ***out_record_ids, float **out_scores, size_t *out_count)
{
    ...
    *out_count = 0;
    ...
    char **results = (char **)AGENTRT_CALLOC(result_cap, sizeof(char *));
    float *scores = (float *)AGENTRT_CALLOC(result_cap, sizeof(float));
                ...
                if (strlen(token) >= 2) {
                    ...
                    if (te) {
                        for (size_t i = 0; i < te->record_count && result_count < result_cap; i++) {
                            ...
                            if (!already) {
                                results[result_count] = AGENTRT_STRDUP(te->record_ids[i]);
...
agentrt_error_t builtin_index_compact(builtin_index_t *idx)
{
    if (!idx)
        return AGENTRT_EINVAL;
