...
Memory 是 AgentRT 的免费开源内存子系统，提供 L1 原始存储与 L2 特征索引的基础能力。采用**可拔插提供商架构**，通过统一的 `agentrt_memory_provider_t` 函数指针表实现提供商的热替换——内置免费提供商（builtin_provider）提供 L1+L2 基础功能，商业提供商 MemoryRovol 实现 L1-L4 全功能 PRO 记忆系统。

Memory 模块以纯 C 实现，无 FAISS / SQLite / 外部向量数据库依赖，是 AgentRT 八层架构中 Atoms 微核心层的核心存储底座。
...
Memory 位于 Atoms 层，是整个 AgentRT 记忆能力的微核心抽象。上层模块（CoreLoopThree、Frameworks 等）通过 `agentrt_memory_provider_t*` 指针访问记忆功能，无需感知底层提供商的具体实现。
...
```
memory/
├── CMakeLists.txt          # 构建配置（静态库 agentrt_memory）
├── src/
...
#### 能力标记 (`agentrt_memory_capabilities_t`)
...
#### 查询结果 (`agentrt_memory_query_result_t`)
...
#### 提供商 vtable (`agentrt_memory_provider_t`)
...
| `capabilities` | `agentrt_memory_capabilities_t` | 能力标记 |
| `impl` | `void*` | 提供商私有实现数据 |
| `sync_target` | `agentrt_memory_provider_t*` | 同步目标提供商 |

---

### 2. 内置提供商 (`builtin_provider.c`)

实现 `agentrt_memory_provider_t` 接口的免费提供商，提供 L1+L2 基础功能。
...
└── stats      → agentrt_memory_stats_t (统计信息)
```
...
| 接口 | 说明 |
|------|------|
| `agentrt_memory_provider_register(provider)` | 注册内存提供商 |
| `agentrt_memory_provider_get_active()` | 获取当前活跃提供商 |
| `agentrt_memory_provider_set_active(provider)` | 设置活跃提供商 |
| `agentrt_memory_provider_unregister()` | 注销并销毁活跃提供商 |

### 内置提供商工厂

| 接口 | 说明 |
|------|------|
| `agentrt_builtin_memory_provider_init(storage_path)` | 初始化内置提供商并注册为活跃 |
| `agentrt_builtin_provider_create()` | 创建内置提供商实例（不自动注册） |

### 资源释放

| 接口 | 说明 |
|------|------|
| `agentrt_memory_provider_free_query_results(ids, scores, count)` | 释放查询结果 |
| `agentrt_memory_query_result_free(result)` | 释放查询结果结构体 |
...
int main(void) {
    ...
    if (err != AGENTRT_SUCCESS) return 1;

    agentrt_memory_provider_t *mem = agentrt_memory_provider_get_active();
    ...
    /* 清理 */
    agentrt_memory_provider_free_query_results(ids, scores, count);
...
Memory 模块编译为静态库 `agentrt_memory`，依赖 commons 层的 error/compat/platform 头文件。
...
# 仅构建 memory 模块
cmake --build build --target agentrt_memory
```
...
**安装**: 头文件安装至 `include/agentrt/memory/`，库文件安装至 `lib/`。
...
| 模块 | 关系 |
|------|------|
| **memoryrovol** | 商业提供商，实现同一 `agentrt_memory_provider_t` 接口，提供 L1-L4 全功能 |
| **coreloopthree** | 上层消费者，在认知-执行循环中读写记忆 |
| **frameworks** | 通过 Frameworks 层的 `AGENTRT_FW_MEMORY` 框架类型封装 Memory |
| **commons/error** | 提供错误码定义（`agentrt_error_t`） |
| **commons/compat** | 提供内存兼容宏（`AGENTRT_MALLOC/CALLOC/FREE/STRDUP`） |
