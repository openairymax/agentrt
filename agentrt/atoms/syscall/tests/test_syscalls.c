...
int test_sys_init_cleanup(void)
{
    ...
    TEST_ASSERT(err == AGENTRT_SUCCESS, "init should return SUCCESS");

    agentrt_syscalls_cleanup();

    err = agentrt_syscalls_init();
    TEST_ASSERT(err == AGENTRT_SUCCESS, "re-init should succeed");
...
int test_sys_invoke_invalid_number(void)
{
    ...
    result = agentrt_syscall_invoke(0, NULL, 0);
    TEST_ASSERT(result != NULL, "syscall 0 should not be NULL");
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "syscall 0 should return EINVAL");

    result = agentrt_syscall_invoke(-1, NULL, 0);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "negative should return EINVAL");

    result = agentrt_syscall_invoke(SYS_MAX, NULL, 0);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "SYS_MAX should return EINVAL");

    result = agentrt_syscall_invoke(99999, NULL, 0);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "large number should return EINVAL");
...
int test_sys_invoke_wrong_argc(void)
{
    ...
    result = agentrt_syscall_invoke(SYS_TASK_SUBMIT, args, 0);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "wrong argc for submit");

    /* memory_write 需要4个参数，给2个 */
    result = agentrt_syscall_invoke(SYS_MEMORY_WRITE, args, 2);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "wrong argc for write");

    /* session_create 需要2个参数，给5个 */
    result = agentrt_syscall_invoke(SYS_SESSION_CREATE, args, 5);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "wrong argc for session_create");

    /* agent_spawn 需要2个参数，给0个 */
    result = agentrt_syscall_invoke(SYS_AGENT_SPAWN, args, 0);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "wrong argc for spawn");

    /* memory_search 需要5个参数，给3个 */
    result = agentrt_syscall_invoke(SYS_MEMORY_SEARCH, args, 3);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "wrong argc for search");

    /* agent_invoke 需要4个参数，给1个 */
    result = agentrt_syscall_invoke(SYS_AGENT_INVOKE, args, 1);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "wrong argc for invoke");
...
{
    agentrt_error_t init_err = agentrt_syscalls_init();
    ...
    args[3] = &out_task;
    result = agentrt_syscall_invoke(SYS_TASK_SUBMIT, args, 4);
    printf("    (task_submit returned %ld)\n", (long)(intptr_t)result);

    /* task_cancel - 1 arg */
    args[0] = (void *)"fake-task-id";
    result = agentrt_syscall_invoke(SYS_TASK_CANCEL, args, 1);

    /* memory_delete - 1 arg */
    args[0] = (void *)"fake-record-id";
    result = agentrt_syscall_invoke(SYS_MEMORY_DELETE, args, 1);

    /* session_close - 1 arg */
    args[0] = (void *)"fake-session-id";
    result = agentrt_syscall_invoke(SYS_SESSION_CLOSE, args, 1);

    /* agent_terminate - 1 arg */
    args[0] = (void *)"fake-agent-id";
    result = agentrt_syscall_invoke(SYS_AGENT_TERMINATE, args, 1);

    /* telemetry_metrics - 1 arg */
    char *out_metrics = NULL;
    args[0] = &out_metrics;
    result = agentrt_syscall_invoke(SYS_TELEMETRY_METRICS, args, 1);
    if (out_metrics) {
        free(out_metrics);
    }
...
int test_sys_all_syscalls_reachable(void)
{
    ...
    for (int num = 1; num < SYS_MAX; num++) {
        void *args[5];
        AGENTRT_MEMSET(args, 0, sizeof(args));
        void *r = agentrt_syscall_invoke(num, args, 0);
        total++;
...
int test_sys_boundary_numbers(void)
{
    ...
    result = agentrt_syscall_invoke(1, args, 0);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "syscall 1 with 0 args -> EINVAL");

    result = agentrt_syscall_invoke(SYS_MAX - 1, args, 0);
    TEST_ASSERT((intptr_t)result == AGENTRT_EINVAL, "last valid syscall with 0 args -> EINVAL");
