 ...
 * @brief 创建限流器
 */
agentrt_error_t agentrt_sys_rate_limiter_create(int capacity, double rate)
{
    if (g_rate_limiter)
        return AGENTRT_EALREADY;

    token_bucket_t *bucket = (token_bucket_t *)AGENTRT_CALLOC(1, sizeof(token_bucket_t));
    if (!bucket)
    ...
    bucket->last_refill = (time_t)(agentrt_time_ms() / 1000ULL);
    bucket->lock = agentrt_mutex_create();
    ...
    AGENTRT_LOG_INFO("Rate limiter created (capacity=%d, rate=%.2f/s)", bucket->capacity,
                     bucket->refill_rate);
 ...
 * @brief 尝试获取令牌
 */
agentrt_error_t agentrt_sys_rate_limiter_acquire(int tokens)
{
    if (!g_rate_limiter)
    ...
    agentrt_mutex_lock(g_rate_limiter->lock);
    ...
    if (g_rate_limiter->tokens >= tokens) {
        ...
        agentrt_mutex_unlock(g_rate_limiter->lock);
        return AGENTRT_SUCCESS;
    }

    agentrt_mutex_unlock(g_rate_limiter->lock);
    AGENTRT_LOG_DEBUG("Rate limit exceeded (available=%d, requested=%d)", g_rate_limiter->tokens,
                      tokens);
 ...
 * @brief 获取限流器状态
 */
agentrt_error_t agentrt_sys_rate_limiter_get_status(char **out_json)
{
 ...
 * @brief 重置限流器
 */
void agentrt_sys_rate_limiter_reset(void)
{
    if (!g_rate_limiter)
        return;
    ...
    g_rate_limiter->last_refill = (time_t)(agentrt_time_ms() / 1000ULL);
    agentrt_mutex_unlock(g_rate_limiter->lock);

    AGENTRT_LOG_INFO("Rate limiter reset");
}

/**
 * @brief 销毁限流器
 */
void agentrt_sys_rate_limiter_destroy(void)
{
    if (!g_rate_limiter)
        return;

    AGENTRT_LOG_INFO("Rate limiter destroyed");

    if (g_rate_limiter->lock) {
        agentrt_mutex_free(g_rate_limiter->lock);
    }
    AGENTRT_FREE(g_rate_limiter);
    g_rate_limiter = NULL;
}
