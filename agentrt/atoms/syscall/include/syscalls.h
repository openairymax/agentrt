...
#ifndef AGENTRT_SYSCALL_H
#define AGENTRT_SYSCALL_H
 ...
 */
AGENTRT_API agentrt_error_t agentrt_syscalls_init(void);

/**
 * @brief 清理系统调用层资源
 */
AGENTRT_API void agentrt_syscalls_cleanup(void);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_task_submit(const char *input, size_t input_len,
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_task_query(const char *task_id, int *out_status);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_task_wait(const char *task_id, uint32_t timeout_ms,
                                                  char **out_result);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_memory_write(const void *data, size_t len,
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_memory_search(const char *query, uint32_t limit,
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_memory_get(const char *record_id, void **out_data,
                                                   size_t *out_len);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_session_create(const char *metadata, char **out_session_id);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_session_get(const char *session_id, char **out_info);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_session_list(char ***out_sessions, size_t *out_count);
...
AGENTRT_API agentrt_error_t agentrt_sys_session_get_persist_status(
    const char *session_id, session_persist_status_t *out_status, agentrt_error_t *out_error);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_telemetry_traces(const char *trace_id, char **out_spans);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_agent_spawn(const char *agent_spec, char **out_agent_id);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_agent_invoke(const char *agent_id, const char *input,
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_agent_list(char ***out_agent_ids, size_t *out_count);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_skill_install(const char *skill_url, char **out_skill_id);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_skill_execute(const char *skill_id, const char *input,
                                                      char **out_output);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_skill_list(char ***out_skills, size_t *out_count);
 ...
 * @return agentrt_error_t
 */
AGENTRT_API agentrt_error_t agentrt_sys_skill_uninstall(const char *skill_id);
...
} agentrt_syscall_num_t;

/**
 * @brief 工具执行参数结构体
 * @details 由 tool_executor_run 构造，经 agentrt_sandbox_invoke 传递给
 * sys_tool_execute 处理函数。output_buffer 由调用者分配（保证 cap_size
 * 字节可写），sys_tool_execute 将 agentrt_process_run_capture 的返回值
...
typedef struct {
    ...
    int exec_result;          /**< 输出：agentrt_process_run_capture 返回值
 ...
 * @param ptr 内存指针
 */
AGENTRT_API void agentrt_sys_free(void *ptr);

#ifdef __cplusplus
}
#endif

#endif /* AGENTRT_SYSCALL_H */
