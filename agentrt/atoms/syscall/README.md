...
Syscall 层是 AgentRT 中**用户态与内核态之间的唯一通道**，提供统一的系统调用抽象接口。它将底层硬件的异构性和操作系统的差异性封装为标准的 API 集合，确保上层模块能以统一的方式访问系统资源。Syscall 层不仅提供 5 类标准系统调用（任务、内存、会话、遥测、代理），还包含 Skill 管理子系统、沙箱隔离机制、熔断器和速率限制器，构成完整的安全防护体系。
...
| 接口 | 功能 | 参数 |
|------|------|------|
| `agentrt_sys_task_submit()` | 提交任务到内核 | 输入数据（JSON）、超时时间 |
| `agentrt_sys_task_query()` | 查询任务状态 | 任务 ID |
| `agentrt_sys_task_wait()` | 等待任务完成 | 任务 ID、超时时间 |
| `agentrt_sys_task_cancel()` | 取消任务 | 任务 ID |

### 2. 内存管理调用

| 接口 | 功能 | 参数 |
|------|------|------|
| `agentrt_sys_memory_write()` | 写入记忆数据 | 数据指针、长度、元数据（JSON） |
| `agentrt_sys_memory_search()` | 搜索记忆 | 查询字符串、返回数量限制 |
| `agentrt_sys_memory_get()` | 获取记忆记录 | 记录 ID |
| `agentrt_sys_memory_delete()` | 删除记忆记录 | 记录 ID |

### 3. 会话管理调用

| 接口 | 功能 | 参数 |
|------|------|------|
| `agentrt_sys_session_create()` | 创建会话 | 元数据（JSON，可为 NULL） |
| `agentrt_sys_session_get()` | 获取会话信息 | 会话 ID |
| `agentrt_sys_session_close()` | 关闭会话 | 会话 ID |
| `agentrt_sys_session_list()` | 列出所有会话 | — |
| `agentrt_sys_session_get_persist_status()` | 获取会话持久化状态 | 会话 ID |
...
| 接口 | 功能 | 参数 |
|------|------|------|
| `agentrt_sys_telemetry_metrics()` | 获取系统指标 | — |
| `agentrt_sys_telemetry_traces()` | 获取链路追踪 | 追踪 ID |

### 5. Agent 管理调用

| 接口 | 功能 | 参数 |
|------|------|------|
| `agentrt_sys_agent_spawn()` | 创建 Agent 实例 | Agent 规格（JSON） |
| `agentrt_sys_agent_terminate()` | 销毁 Agent 实例 | Agent ID |
| `agentrt_sys_agent_invoke()` | 调用 Agent 执行任务 | Agent ID、输入数据 |
...
| 接口 | 功能 | 参数 |
|------|------|------|
| `agentrt_sys_skill_install()` | 安装技能 | 技能 URL（file:// 或 http://） |
| `agentrt_sys_skill_execute()` | 执行技能 | 技能 ID、输入数据 |
| `agentrt_sys_skill_list()` | 列出已安装技能 | — |
| `agentrt_sys_skill_uninstall()` | 卸载技能 | 技能 ID |

### 7. 辅助函数

| 接口 | 功能 |
|------|------|
| `agentrt_sys_free()` | 释放系统调用分配的内存 |
| `agentrt_syscalls_init()` | 初始化系统调用层 |
| `agentrt_syscalls_cleanup()` | 清理系统调用层资源 |
...
```c
agentrt_error_t ret = agentrt_sys_task_submit(
...
| agentrt_types.h | commons | 统一类型定义 |
...
int main(void) {
    ...
    char *session_id;
    agentrt_sys_session_create(NULL, &session_id);
    ...
    char *record_id;
    agentrt_sys_memory_write("sample data", 11,
                             "{\"type\":\"text\"}", &record_id);
