...
extern agentrt_execution_unit_t *agentrt_browser_unit_create(void);
extern int agentrt_browser_unit_set_agent(agentrt_execution_unit_t *unit, const char *agent_id);
extern int agentrt_browser_launch(const char *browser_path, int port, int headless,
                                  const char *user_data_dir);
extern int agentrt_browser_close(void);
extern int agentrt_browser_get_state(void);
extern int agentrt_browser_create_context(const char *agent_id, char *out_context_id,
                                          size_t ctx_size);
extern int agentrt_browser_destroy_context(const char *context_id);

static void test_browser_unit_create(void)
{
    agentrt_execution_unit_t *unit = agentrt_browser_unit_create();
...
static void test_browser_unit_set_agent(void)
{
    ...
    int ret = agentrt_browser_unit_set_agent(unit, "test-agent-001");
    if (ret != 0) {
    ...
    ret = agentrt_browser_unit_set_agent(NULL, "test-agent-002");
    if (ret == 0) {
    ...
    ret = agentrt_browser_unit_set_agent(unit, NULL);
    if (ret == 0) {
...
static void test_browser_execute_navigate(void)
{
    ...
    if (err != AGENTRT_SUCCESS || output == NULL) {
...
static void test_browser_execute_navigate_no_url(void)
{
    ...
    void *output = NULL;
    agentrt_error_t err = unit->execution_unit_execute(unit, "navigate", &output);

    if (err != AGENTRT_EINVAL) {
        TEST_FAIL("browser_execute_navigate_no_url", "expected AGENTRT_EINVAL");
        if (output)
            AGENTRT_FREE(output);
        unit->execution_unit_destroy(unit);
        return;
...
static void test_browser_execute_navigate_unsafe_url(void)
{
    ...
    if (err != AGENTRT_EPERM) {
        TEST_FAIL("browser_execute_navigate_unsafe", "expected AGENTRT_EPERM");
        if (output)
            AGENTRT_FREE(output);
        unit->execution_unit_destroy(unit);
        return;
...
static void test_browser_execute_evaluate(void)
{
    ...
    void *output = NULL;
    agentrt_error_t err = unit->execution_unit_execute(unit, "evaluate script=1+1", &output);
...
static void test_browser_execute_click(void)
{
    ...
    void *output = NULL;
    agentrt_error_t err = unit->execution_unit_execute(unit, "click selector=#btn", &output);
...
static void test_browser_execute_screenshot(void)
{
    ...
    void *output = NULL;
    agentrt_error_t err = unit->execution_unit_execute(unit, "screenshot format=png", &output);
...
static void test_browser_execute_wait(void)
{
    ...
    void *output = NULL;
    agentrt_error_t err = unit->execution_unit_execute(unit, "wait timeout=1000", &output);
...
static void test_browser_execute_unsupported(void)
{
    ...
    void *output = NULL;
    agentrt_error_t err = unit->execution_unit_execute(unit, "scroll down", &output);
...
static void test_browser_execute_null_input(void)
{
    ...
    void *output = NULL;
    agentrt_error_t err = unit->execution_unit_execute(unit, NULL, &output);

    if (err != AGENTRT_EINVAL) {
        TEST_FAIL("browser_execute_null_input", "expected AGENTRT_EINVAL");
        unit->execution_unit_destroy(unit);
        return;
    }
...
static void test_browser_execute_null_output(void)
{
    ...
    agentrt_error_t err = unit->execution_unit_execute(unit, "navigate https://example.com", NULL);

    if (err != AGENTRT_EINVAL) {
        TEST_FAIL("browser_execute_null_output", "expected AGENTRT_EINVAL");
        unit->execution_unit_destroy(unit);
        return;
    }
...
{
    int state = agentrt_browser_get_state();
    if (state != 0) {
...
{
    int ret = agentrt_browser_launch(NULL, 0, 1, NULL);
    if (ret == 0) {
...
{
    char ctx_id[64] = {0};
    int ret = agentrt_browser_create_context(NULL, ctx_id, sizeof(ctx_id));
    if (ret == 0) {
...
{
    int ret = agentrt_browser_destroy_context(NULL);
    if (ret == 0) {
...
static void test_browser_no_enotsup(void)
{
    ...
    for (size_t i = 0; i < sizeof(commands) / sizeof(commands[0]); i++) {
        void *output = NULL;
        agentrt_error_t err = unit->execution_unit_execute(unit, commands[i], &output);
        if (err == AGENTRT_ENOTSUP || err == AGENTRT_ENOSYS) {
            char msg[128];
