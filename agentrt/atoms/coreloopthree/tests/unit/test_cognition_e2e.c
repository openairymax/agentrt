...
static void test_e2e_sustained_processing(void)
{
    ...
    for (int i = 0; i < 5; i++) {
        agentrt_task_plan_t *plan = NULL;
        err = agentrt_cognition_process(engine, inputs[i], strlen(inputs[i]), &plan);
...
static void test_e2e_parallel_dispatcher(void)
{
    ...
    if (err == AGENTRT_SUCCESS && plan && plan->task_plan_node_count > 0) {
...
static void test_e2e_context_processor(void)
{
    ...
    agentrt_cognition_set_context_take(engine, (void *)ctx_data, NULL);
