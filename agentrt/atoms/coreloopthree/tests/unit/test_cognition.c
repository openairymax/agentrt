...
{
    agentrt_error_t err = agentrt_cognition_create_take(NULL, NULL, NULL, NULL);
...
static void test_cognition_process_simple_input(void)
{
    ...
    err = agentrt_cognition_process(engine, input, input_len, &plan);

    if (err == AGENTRT_SUCCESS && plan != NULL) {
...
static void test_cognition_process_empty_input(void)
{
    ...
    agentrt_task_plan_t *plan = NULL;
    err = agentrt_cognition_process(engine, "", 0, &plan);
...
{
    agentrt_cognition_engine_t *engine = NULL;
    agentrt_cognition_create_take(NULL, NULL, NULL, &engine);

    agentrt_error_t err = agentrt_cognition_process(engine, NULL, 10, NULL);
    if (err != AGENTRT_SUCCESS) {
...
static void test_cognition_set_fallback_null(void)
{
    ...
    agentrt_cognition_set_fallback_plan(engine, NULL);
...
static void test_cognition_stats(void)
{
    ...
    char *stats = NULL;
    size_t stats_len = 0;
    agentrt_error_t err = agentrt_cognition_stats(engine, &stats, &stats_len);
...
static void test_cognition_health_check(void)
{
    ...
    char *json = NULL;
    agentrt_error_t err = agentrt_cognition_health_check(engine, &json);
...
static void test_intent_parser_create_destroy(void)
{
    ...
    agentrt_error_t err = agentrt_intent_parser_create(&parser);

    if (err == AGENTRT_SUCCESS && parser != NULL) {
...
static void test_intent_parser_parse(void)
{
    ...
    agentrt_error_t err = agentrt_intent_parser_parse(parser, input, len, &intent);
...
static void test_intent_parser_add_rule(void)
{
    ...
    agentrt_error_t err =
        agentrt_intent_parser_add_rule(parser, "*search*", "search_intent", 0.85f, 0);
...
static void test_intent_parser_stats_and_reset(void)
{
    ...
    char *stats = NULL;
    agentrt_intent_parser_stats(parser, &stats);
    if (stats)
        free(stats);

    agentrt_intent_parser_reset_stats(parser);
...
static void test_intent_parser_health_check(void)
{
    ...
    char *json = NULL;
    agentrt_error_t err = agentrt_intent_parser_health_check(parser, &json);
    if (err == AGENTRT_SUCCESS && json) {
        free(json);
...
{
    assert(sizeof(agentrt_cognition_config_t) >= sizeof(uint32_t) * 2 + sizeof(void *) * 2);
    assert(sizeof(agentrt_intent_t) >=
           sizeof(char *) * 2 + sizeof(size_t) * 2 + sizeof(uint32_t) + sizeof(void *));
    assert(sizeof(agentrt_task_node_t) >= sizeof(char *) * 2 + sizeof(size_t) * 3 +
                                              sizeof(char **) + sizeof(uint32_t) +
                                              sizeof(void *) * 2);
    assert(sizeof(agentrt_task_plan_t) >=
           sizeof(char *) + sizeof(size_t) * 3 + sizeof(agentrt_task_node_t **) + sizeof(char **));
