 ...
 *
 * 验证 ACC-DT26 验收标准：cognition engine 关键事件调用 agentrt_hook_trigger，
...
static void test_hook_system_mechanics(void)
{
    ...
    if (agentrt_hook_init() != 0) {
        TEST_FAIL("agentrt_hook_init failed");
        return;
    }
    ...
    if (agentrt_hook_register("t_a1", HOOK_TYPE_PRE_EXEC, cb_continue,
                              &cnt_a1, 100, true) != 0) {
        TEST_FAIL("register t_a1 failed");
        agentrt_hook_shutdown();
        return;
    }
    if (agentrt_hook_register("t_a2", HOOK_TYPE_PRE_EXEC, cb_continue,
                              &cnt_a2, 50, true) != 0) {
        TEST_FAIL("register t_a2 failed");
        agentrt_hook_unregister("t_a1");
        agentrt_hook_shutdown();
        return;
    }

    /* 验证注册总数 */
    if (agentrt_hook_count() != 2) {
        char msg[128];
        snprintf(msg, sizeof(msg), "Part A: hook_count=%zu (expected 2)", agentrt_hook_count());
        TEST_FAIL(msg);
        agentrt_hook_unregister("t_a1");
        agentrt_hook_unregister("t_a2");
        agentrt_hook_shutdown();
        return;
    }
    ...
    hook_decision_t decision_a = agentrt_hook_trigger(&ctx);
    ...
    /* 注销 Part A 的 hook */
    if (agentrt_hook_unregister("t_a2") != 0) {
        TEST_FAIL("Part A: unregister t_a2 failed");
        agentrt_hook_unregister("t_a1");
        agentrt_hook_shutdown();
        return;
    }
    if (agentrt_hook_unregister("t_a1") != 0) {
        TEST_FAIL("Part A: unregister t_a1 failed");
        agentrt_hook_shutdown();
        return;
    }
    if (agentrt_hook_count() != 0) {
        char msg[128];
    ...
    if (agentrt_hook_register("t_abort", HOOK_TYPE_PRE_EXEC, cb_abort,
                              &cnt_abort, 100, true) != 0) {
        TEST_FAIL("Part B: register t_abort failed");
        agentrt_hook_shutdown();
        return;
    }
    if (agentrt_hook_register("t_cont_post", HOOK_TYPE_PRE_EXEC, cb_continue,
                              &cnt_continue_post, 50, true) != 0) {
        TEST_FAIL("Part B: register t_cont_post failed");
        agentrt_hook_unregister("t_abort");
        agentrt_hook_shutdown();
        return;
    }
    ...
    hook_decision_t decision_b = agentrt_hook_trigger(&ctx);
    ...
    if (cnt_abort.fired != 1) {
        ...
        TEST_FAIL(msg);
        agentrt_hook_unregister("t_abort");
        agentrt_hook_unregister("t_cont_post");
        agentrt_hook_shutdown();
        return;
    }
 ...
 *
 * 设计说明：agentrt_cognition_create_take(NULL, NULL, NULL, &engine) 传入 NULL
 * plan strategy，process 在 Phase 1 planning 必然失败（err=AGENTRT_EUNKNOWN），
...
static void test_cognition_engine_hook_integration(void)
{
    ...
    if (err != AGENTRT_SUCCESS || engine == NULL) {
        TEST_FAIL("agentrt_cognition_create_take failed");
        return;
    }
    ...
    if (agentrt_hook_register("integ_pre", HOOK_TYPE_PRE_EXEC, cb_continue,
                              &pre_cnt, 100, true) != 0) {
        TEST_FAIL("register integ_pre failed");
        agentrt_cognition_destroy(engine);
        return;
    }
    if (agentrt_hook_register("integ_on_err", HOOK_TYPE_ON_ERROR, cb_continue,
                              &on_error_cnt, 100, true) != 0) {
        TEST_FAIL("register integ_on_err failed");
        agentrt_hook_unregister("integ_pre");
        agentrt_cognition_destroy(engine);
        return;
    }
    ...
    if (err == AGENTRT_SUCCESS) {
        ...
        if (plan) agentrt_task_plan_free(plan);
        agentrt_hook_unregister("integ_pre");
        agentrt_hook_unregister("integ_on_err");
        agentrt_cognition_destroy(engine);
        return;
    }
...
static void test_pre_exec_abort_short_circuits(void)
{
    ...
    if (agentrt_hook_register("abort_pre", HOOK_TYPE_PRE_EXEC, cb_abort,
                              &pre_abort_cnt, 100, true) != 0) {
        TEST_FAIL("register abort_pre failed");
        agentrt_cognition_destroy(engine);
        return;
    }
    if (agentrt_hook_register("on_err", HOOK_TYPE_ON_ERROR, cb_continue,
                              &on_error_cnt, 100, true) != 0) {
        TEST_FAIL("register on_err failed");
        agentrt_hook_unregister("abort_pre");
        agentrt_cognition_destroy(engine);
        return;
    }
    ...
    if (pre_abort_cnt.fired != 1) {
        ...
        if (plan) agentrt_task_plan_free(plan);
        agentrt_hook_unregister("abort_pre");
        agentrt_hook_unregister("on_err");
        agentrt_cognition_destroy(engine);
        return;
    }
