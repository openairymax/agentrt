...
static void test_loop_create_default_config(void)
{
    ...
    if (err == AGENTRT_SUCCESS && loop != NULL) {
...
{
    agentrt_error_t err = agentrt_loop_create(NULL, NULL);
    if (err != AGENTRT_SUCCESS) {
...
static void test_loop_get_engines_partial(void)
{
    ...
    agentrt_execution_engine_t *execution = NULL;
    agentrt_loop_get_engines(loop, NULL, &execution, NULL);
...
{
    agentrt_cognition_engine_t *c = NULL;
    agentrt_execution_engine_t *e = NULL;
    agentrt_memory_engine_t *m = NULL;

    agentrt_loop_get_engines(NULL, &c, &e, &m);
...
{
    assert(sizeof(agentrt_loop_config_t) >= sizeof(uint32_t) * 5 + sizeof(void *) * 3);
...
static void test_loop_config_checkpoint_custom(void)
{
    ...
    snprintf(config.loop_config_checkpoint_path, sizeof(config.loop_config_checkpoint_path),
             AGENTRT_TMP_DIR "/test_checkpoints");
    config.loop_config_checkpoint_interval_ms = 5000;

    assert(config.loop_config_checkpoint_enabled == 1);
    assert(strcmp(config.loop_config_checkpoint_path, AGENTRT_TMP_DIR "/test_checkpoints") == 0);
...
static void test_loop_checkpoint_disabled_by_default(void)
{
    ...
    agentrt_error_t list_err = agentrt_loop_list_checkpoints(loop, NULL, NULL);
    if (list_err == AGENTRT_ENOTINIT) {
...
static void test_loop_restore_without_checkpoint(void)
{
    ...
    agentrt_error_t restore_err = agentrt_loop_restore_task(loop, "nonexistent_task", &restored_id);
    if (restore_err == AGENTRT_ENOENT || restore_err == AGENTRT_ENOTINIT) {
