...
static size_t count_all_hooks(void)
{
    ...
    for (int t = 0; t < HOOK_TYPE_COUNT; t++) {
        total += agentrt_hook_count_by_type((hook_type_t)t);
    }
    return total;
}
...
static void test_builtin_handlers_register(void)
{
    ...
    if (agentrt_hook_init() != 0) {
        TEST_FAIL("builtin_handlers_register", "agentrt_hook_init failed");
    }

    /* 注册内置 handler */
    if (agentrt_hook_register_builtin_handlers() != 0) {
...
{
    /* 记录触发前的计数 */
    uint64_t pre_exec_before = agentrt_hook_metrics_get_count(HOOK_TYPE_PRE_EXEC);
    uint64_t on_error_before = agentrt_hook_metrics_get_count(HOOK_TYPE_ON_ERROR);

    /* 构造 PRE_EXEC hook 上下文并触发 */
    hook_context_t ctx1;
    AGENTRT_MEMSET(&ctx1, 0, sizeof(ctx1));
    ctx1.type = HOOK_TYPE_PRE_EXEC;
    ctx1.source_daemon = "test";
    ctx1.operation = "test_pre_exec";
    ctx1.timestamp_ns = 1000000000ULL;
    hook_decision_t dec1 = agentrt_hook_trigger(&ctx1);
    ...
    hook_context_t ctx2;
    AGENTRT_MEMSET(&ctx2, 0, sizeof(ctx2));
    ctx2.type = HOOK_TYPE_ON_ERROR;
    ctx2.source_daemon = "test";
    ctx2.operation = "test_on_error";
    ctx2.timestamp_ns = 2000000000ULL;
    hook_decision_t dec2 = agentrt_hook_trigger(&ctx2);
    if (dec2 != HOOK_DECISION_CONTINUE) {
        TEST_FAIL("builtin_handlers_trigger", "ON_ERROR should return CONTINUE");
    }

    /* 验证计数器递增 */
    uint64_t pre_exec_after = agentrt_hook_metrics_get_count(HOOK_TYPE_PRE_EXEC);
    uint64_t on_error_after = agentrt_hook_metrics_get_count(HOOK_TYPE_ON_ERROR);
...
{
    char buf[512];
    int n = agentrt_hook_metrics_dump(buf, sizeof(buf));
    if (n <= 0) {
    ...
    /* 验证缓冲区不足时返回 -1 */
    char tiny[8];
    if (agentrt_hook_metrics_dump(tiny, sizeof(tiny)) != -1) {
...
{
    const int N = 5;
    uint64_t before = agentrt_hook_metrics_get_count(HOOK_TYPE_POST_TOOL);
    ...
    uint64_t after = agentrt_hook_metrics_get_count(HOOK_TYPE_POST_TOOL);
    if (after - before != (uint64_t)N) {
        TEST_FAIL("metrics_count", "count increase != N");
    }

    /* 验证越界 type 返回 0 */
    if (agentrt_hook_metrics_get_count((hook_type_t)99) != 0) {
        TEST_FAIL("metrics_count", "out-of-range type should return 0");
    }
    if (agentrt_hook_metrics_get_count((hook_type_t)-1) != 0) {
...
static void test_builtin_handlers_unregister(void)
{
    ...
    uint64_t before = agentrt_hook_metrics_get_count(HOOK_TYPE_PRE_LLM);
    ...
    uint64_t after = agentrt_hook_metrics_get_count(HOOK_TYPE_PRE_LLM);
    if (after != before) {
        TEST_FAIL("builtin_handlers_unregister", "count changed after unregister");
    }

    /* 重新注册（为后续测试恢复状态） */
    agentrt_hook_register_builtin_handlers();
