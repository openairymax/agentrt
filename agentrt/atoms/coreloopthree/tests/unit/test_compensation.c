...
static void test_compensation_register_single(void)
{
    ...
    int test_value = 42;
    agentrt_error_t err = agentrt_compensation_register(mgr, "action_write_db",
...
static void test_compensation_register_multiple(void)
{
    ...
    for (int i = 0; i < 3; i++) {
        agentrt_error_t err =
            agentrt_compensation_register(mgr, actions[i], compensators[i], &values[i]);
        if (err != AGENTRT_SUCCESS)
            all_ok = 0;
    }
...
static void test_compensation_register_null_action_id(void)
{
    ...
    int val = 0;
    agentrt_error_t err = agentrt_compensation_register(mgr, NULL, "some_compensator", &val);
...
static void test_compensation_compensate_existing(void)
{
    ...
    int val = 77;
    agentrt_compensation_register(mgr, "action_to_undo", "undo_handler", &val);

    agentrt_error_t err = agentrt_compensation_compensate(mgr, "action_to_undo");
    if (err == AGENTRT_SUCCESS) {
...
static void test_compensation_compensate_nonexistent(void)
{
    ...
    agentrt_error_t err = agentrt_compensation_compensate(mgr, "nonexistent_action");
    if (err != AGENTRT_SUCCESS) {
...
{
    agentrt_error_t err = agentrt_compensation_compensate(NULL, "any");
    if (err != AGENTRT_SUCCESS) {
...
static void test_compensation_human_queue_null_mgr(void)
{
    char **actions = NULL;
    size_t count = 0;
    agentrt_error_t err = agentrt_compensation_get_human_queue(NULL, &actions, &count);
    if (err != AGENTRT_SUCCESS) {
...
{
    agentrt_compensation_result_free(NULL);
...
static void test_compensation_manager_fields(void)
{
    ...
    AGENTRT_MEMSET(&mgr, 0, sizeof(mgr));
...
{
    agentrt_compensation_result_t result;
    AGENTRT_MEMSET(&result, 0, sizeof(result));

    assert(result.status == AGENTRT_SUCCESS);
...
static void test_compensation_register_bulk(void)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        int val = i;
        agentrt_error_t err = agentrt_compensation_register(mgr, action_id, comp_id, &val);
        if (err == AGENTRT_SUCCESS)
            ok_count++;
    }
