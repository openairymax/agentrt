...
static agentrt_error_t mock_s1_exp(const char *content, size_t content_len, const char *critique,
                                   size_t critique_len, float *out_score,
                                   tc3_verdict_t *out_verdict, char **out_opinion,
...
static void assert_stat(const tc3_coordinator_t *coord, uint32_t exp_accept, uint32_t exp_minor,
                        uint32_t exp_major, uint32_t exp_reject, const char *test_name)
{
    tc3_stats_t s;
    if (tc3_coordinator_get_stats(coord, &s) != 0) {
        TEST_FAIL(test_name, "get_stats failed");
        return;
    }
...
static void test_tc3_happy_path_accept(void)
{
    ...
    if (!c) {
        TEST_FAIL("happy", "create failed");
        return;
    }

    char *out = NULL;
    size_t out_len = 0;
    agentrt_error_t err = tc3_coordinator_execute(c, "input", 5, &out, &out_len);
    if (err != 0) {
        TEST_FAIL("happy", "execute error");
        tc3_coordinator_destroy(c);
        return;
    }

    if (mc.generate_call_count < 1)
        TEST_FAIL("happy", "S2 never called");
    if (mc.correction_requested_count > 0)
        TEST_FAIL("happy", "unexpected correction triggered");
...
static void test_tc3_minor_fix_correction(void)
{
    ...
    tc3_coordinator_t *c = make_coord(&mc);
    if (!c) {
        TEST_FAIL("minor_fix", "create failed");
        return;
    }
    ...
    agentrt_error_t err = tc3_coordinator_execute(c, "input", 5, &out, &out_len);
    if (err != 0) {
        TEST_FAIL("minor_fix", "execute error");
        tc3_coordinator_destroy(c);
        return;
    }
...
static void test_tc3_max_rounds_exhaustion(void)
{
    ...
    tc3_coordinator_t *c = make_coord(&mc);
    if (!c) {
        TEST_FAIL("max_rounds", "create failed");
        return;
    }
    ...
    agentrt_error_t err = tc3_coordinator_execute(c, "input", 5, &out, &out_len);
    if (err != 0) {
        TEST_FAIL("max_rounds", "execute error");
        tc3_coordinator_destroy(c);
        return;
    }
...
static void test_tc3_expert_callback_registered(void)
{
    ...
    tc3_coordinator_t *c = make_coord(&mc);
    if (!c) {
        TEST_FAIL("expert_reg", "create failed");
        return;
    }
    ...
    agentrt_error_t err = tc3_coordinator_execute(c, "input", 5, &out, &out_len);
    if (err != 0) {
        TEST_FAIL("expert_reg", "execute error");
        tc3_coordinator_destroy(c);
        return;
    }
...
static void test_tc3_multi_semantic_units(void)
{
    mock_ctx_t mc;
    mock_reset(&mc);
    mc.generate_output = "This first sentence is long enough to be a complete thought. "
    ...
    tc3_coordinator_t *c = make_coord(&mc);
    if (!c) {
        TEST_FAIL("multi_units", "create failed");
        return;
    }
    ...
    agentrt_error_t err = tc3_coordinator_execute(c, "input", 5, &out, &out_len);
    if (err != 0) {
        TEST_FAIL("multi_units", "execute error");
        tc3_coordinator_destroy(c);
        return;
    }
...
static void test_tc3_statistics_and_reset(void)
{
    ...
    tc3_coordinator_t *c = make_coord(&mc);
    if (!c) {
        TEST_FAIL("stats_reset", "create failed");
        return;
    }
    ...
    if (s1.total_units == 0)
        TEST_FAIL("stats_reset", "total_units=0 after first exec");
    if (s1.total_time_ns == 0)
        TEST_FAIL("stats_reset", "total_time_ns=0");
    ...
    if (s3.total_units != 0)
        TEST_FAIL("stats_reset", "reset: total_units not zero");
    if (s3.accepted_units != 0)
        TEST_FAIL("stats_reset", "reset: accepted_units not zero");
    if (s3.total_corrections != 0)
        TEST_FAIL("stats_reset", "reset: corrections not zero");
    if (s3.total_time_ns != 0)
        TEST_FAIL("stats_reset", "reset: total_time_ns not zero");
...
static void test_tc3_iterative_convergence(void)
{
    ...
    tc3_coordinator_t *c = make_coord(&mc);
    if (!c) {
        TEST_FAIL("iterative", "create failed");
        return;
    }

    char *out = NULL;
    size_t out_len = 0;
    agentrt_error_t err = tc3_coordinator_execute(c, "start", 5, &out, &out_len);
    if (err != 0) {
        TEST_FAIL("iterative", "execute error");
        tc3_coordinator_destroy(c);
        return;
    }
...
static void test_tc3_null_params(void)
{
    tc3_coordinator_t *coord = NULL;
    /* NULL config → 使用默认值 */
    agentrt_error_t err = tc3_coordinator_create(NULL, NULL, NULL, &coord);
    if (err != 0) {
        TEST_FAIL("null_params", "create with NULL config failed");
        return;
    }

    err = tc3_coordinator_execute(coord, NULL, 0, NULL, NULL);
    if (err == 0)
        TEST_FAIL("null_params", "accepts NULL input without error");

    err = tc3_coordinator_execute(NULL, "test", 4, NULL, NULL);
    if (err == 0)
        TEST_FAIL("null_params", "accepts NULL coordinator");

    err = tc3_coordinator_get_stats(NULL, NULL);
    if (err == 0)
        TEST_FAIL("null_params", "get_stats with NULL params should fail");
...
static void test_tc3_no_s2_callback(void)
{
    ...
    agentrt_error_t err = tc3_coordinator_create(&config, NULL, NULL, &coord);
    if (err != 0 || !coord) {
        TEST_FAIL("no_s2", "create failed");
        return;
    }

    err = tc3_coordinator_execute(coord, "test", 4, NULL, NULL);
    if (err == 0)
        TEST_FAIL("no_s2", "should fail without s2_generate");
...
static void test_tc3_empty_input(void)
{
    ...
    tc3_coordinator_t *c = make_coord(&mc);
    if (!c) {
        TEST_FAIL("empty_input", "create failed");
        return;
    }

    char *out = NULL;
    size_t out_len = 0;
    agentrt_error_t err = tc3_coordinator_execute(c, "", 0, &out, &out_len);
    if (err != 0) {
        TEST_FAIL("empty_input", "execute error");
        tc3_coordinator_destroy(c);
        return;
    }

    if (!out)
        TEST_FAIL("empty_input", "no output for empty input");
    if (out)
        free(out);
    tc3_coordinator_destroy(c);
}
...
static void test_tc3_correction_prompt_contains_critique(void)
{
    ...
    tc3_coordinator_t *c = make_coord(&mc);
    if (!c) {
        TEST_FAIL("corr_prompt", "create failed");
        return;
    }

    char *out = NULL;
    size_t out_len = 0;
    agentrt_error_t err = tc3_coordinator_execute(c, "test", 4, &out, &out_len);
    if (err != 0) {
        TEST_FAIL("corr_prompt", "execute error");
        tc3_coordinator_destroy(c);
        return;
    }
...
static void test_tc3_long_content_accept(void)
{
    ...
    char *out = NULL;
    size_t out_len = 0;
    agentrt_error_t err = tc3_coordinator_execute(c, "fact", 4, &out, &out_len);
    if (err != 0) {
