...
static double sample_reward_fn(const cog_experience_t *exp, void *user_data)
{
    ...
    if (exp->feedback == COG_FEEDBACK_POSITIVE)
        return 1.0;
    if (exp->feedback == COG_FEEDBACK_NEGATIVE)
        return -1.0;
    return 0.1;
}
                                        ...
                                        cog_feedback_t feedback)
{
    ...
    AGENTRT_MEMSET(&exp, 0, sizeof(exp));
    AGENTRT_STRNCPY_TERM(exp.id, "exp_001", sizeof(exp.id));
    AGENTRT_STRNCPY_TERM(exp.domain, domain, sizeof(exp.domain));
...
static void test_record_single_experience(void)
{
    ...
    if (!evo) {
        TEST_FAIL("record experience", "create failed");
        return;
    }
...
static void test_record_multiple_experiences(void)
{
    cog_evolution_t *evo = cog_evolution_create(COG_LEVEL_LEARNING);
    if (!evo) {
        TEST_FAIL("record multiple", "create failed");
        return;
    }
    ...
    for (int i = 0; i < 10; i++) {
        ...
        AGENTRT_STRNCPY_TERM(exp.domain, "multi_test", sizeof(exp.domain));
...
static void test_extract_patterns_basic(void)
{
    cog_evolution_t *evo = cog_evolution_create(COG_LEVEL_LEARNING);
    if (!evo) {
        TEST_FAIL("extract patterns", "create failed");
        return;
    }
    ...
    for (int i = 0; i < 20; i++) {
        ...
        AGENTRT_STRNCPY_TERM(exp.domain, "pattern_test", sizeof(exp.domain));
...
static void test_evolve_strategies_basic(void)
{
    cog_evolution_t *evo = cog_evolution_create(COG_LEVEL_LEARNING);
    if (!evo) {
        TEST_FAIL("evolve strategies", "create failed");
        return;
    }
    ...
    for (int i = 0; i < 30; i++) {
        ...
        AGENTRT_STRNCPY_TERM(exp.domain, "strategy_test", sizeof(exp.domain));
...
static void test_select_strategy_basic(void)
{
    cog_evolution_t *evo = cog_evolution_create(COG_LEVEL_LEARNING);
    if (!evo) {
        TEST_FAIL("select strategy", "create failed");
        return;
    }
...
static void test_transfer_knowledge_basic(void)
{
    cog_evolution_t *evo = cog_evolution_create(COG_LEVEL_REASONING);
    if (!evo) {
        TEST_FAIL("transfer knowledge", "create failed");
        return;
    }
    ...
    for (int i = 0; i < 15; i++) {
        ...
        AGENTRT_STRNCPY_TERM(exp.domain, "source_domain", sizeof(exp.domain));
...
static void test_evaluate_level_progression(void)
{
    cog_evolution_t *evo = cog_evolution_create(COG_LEVEL_PERCEPTION);
    if (!evo) {
        TEST_FAIL("evaluate level", "create failed");
        return;
    }
    ...
    for (int i = 0; i < 50; i++) {
        ...
        AGENTRT_STRNCPY_TERM(exp.domain, "learning_test", sizeof(exp.domain));
...
static void test_get_counts_and_fitness(void)
{
    cog_evolution_t *evo = cog_evolution_create(COG_LEVEL_LEARNING);
    if (!evo) {
        TEST_FAIL("counts/fitness", "create failed");
        return;
    }
    ...
    for (int i = 0; i < 25; i++) {
        ...
        AGENTRT_STRNCPY_TERM(exp.domain, "stats_test", sizeof(exp.domain));
...
static void test_full_lifecycle_workflow(void)
{
    ...
    if (!evo) {
        TEST_FAIL("full workflow", "create failed");
        return;
    }
    ...
    for (int i = 0; i < 40; i++) {
        ...
        AGENTRT_STRNCPY_TERM(exp.domain, "workflow_domain", sizeof(exp.domain));
    ...
    cog_evolution_select_strategy(evo, "workflow_domain", "{\"task\":\"full_run\"}", &sel);

    cog_knowledge_t *know = NULL;
    cog_evolution_transfer_knowledge(evo, "workflow_domain", "target_domain", &know);
