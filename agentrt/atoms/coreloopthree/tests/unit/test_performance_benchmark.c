...
static agentrt_error_t mock_executor(const char *tool_name __attribute__((unused)),
...
static void bench_parallel_dispatcher_single(void)
{
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        size_t count = 0;
        agentrt_parallel_dispatcher_dispatch(d, &call, 1, &results, &count);
        if (results)
...
static void bench_parallel_dispatcher_multi(void)
{
    ...
    agentrt_parallel_dispatcher_t *d = agentrt_parallel_dispatcher_create(8);
    assert(d != NULL);
    agentrt_parallel_dispatcher_set_executor(d, mock_executor, NULL);

    agentrt_tool_call_t calls[10];
    ...
    for (int i = 0; i < 10; i++) {
        ...
        calls[i].safety_class = AGENTRT_TOOL_READ_ONLY;
    }
    ...
    for (int i = 0; i < iterations; i++) {
        agentrt_tool_result_t *results = NULL;
        size_t count = 0;
        int rc = agentrt_parallel_dispatcher_dispatch(d, calls, 10, &results, &count);
        (void)rc;
        if (results)
            free_results(results, count);
    }
...
static void bench_delegate_create_destroy(void)
{
    ...
    uint64_t t_start = bench_time_ns();
    for (int i = 0; i < iterations; i++) {
        agentrt_delegate_task_t *task = agentrt_delegate_create("benchmark task", &config);
        if (task)
...
static void bench_delegate_assign_collect(void)
{
    ...
    uint64_t t_start = bench_time_ns();
    for (int i = 0; i < iterations; i++) {
        agentrt_delegate_task_t *task = agentrt_delegate_create("bench task", &config);
        if (!task)
            continue;
        ...
        char *result = NULL;
        size_t rlen = 0;
        agentrt_delegate_collect(task, &result, &rlen);
        if (result)
            free(result);
        agentrt_delegate_destroy(task);
    }
...
static void bench_mac_batch_register_1000(void)
{
    ...
    for (int i = 0; i < 1000; i++) {
        AGENTRT_MEMSET(&agents[i], 0, sizeof(mac_agent_info_t));
...
static void bench_mutex_lock_unlock(void)
{
    ...
    agentrt_mutex_t m;
    agentrt_mutex_init(&m);

    const int iterations = 1000000;
    uint64_t t_start = bench_time_ns();
    for (int i = 0; i < iterations; i++) {
        agentrt_mutex_lock(&m);
        agentrt_mutex_unlock(&m);
    }
    uint64_t t_end = bench_time_ns();
    print_ns(t_end - t_start, "total");
