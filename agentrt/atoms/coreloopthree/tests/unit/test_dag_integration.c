 ...
 *   3. test_dag_param_validation — 参数验证：NULL 参数返回 AGENTRT_EINVAL
                                ...
                                const char *input_json, char **output_json, void *user_data)
{
    ...
    if (st) {
        st->call_count++;
        AGENTRT_STRNCPY_TERM(st->last_node_id, node_id ? node_id : "", sizeof(st->last_node_id));
        AGENTRT_STRNCPY_TERM(st->last_input, input_json ? input_json : "", sizeof(st->last_input));
    }
    if (output_json) {
        char buf[256];
                                     ...
                                     const char *name, const char *handler_name)
{
    ...
    AGENTRT_STRNCPY_TERM(wf->id, id, sizeof(wf->id));
    AGENTRT_STRNCPY_TERM(wf->name, name, sizeof(wf->name));
    AGENTRT_STRNCPY_TERM(wf->description, "W18 integration test workflow",
    ...
    AGENTRT_STRNCPY_TERM(wf->nodes[0].name, "Start Node", sizeof(wf->nodes[0].name));
    wf->nodes[0].type = TASKFLOW_NODE_TASK;
    wf->nodes[0].state = TASKFLOW_STATE_PENDING;
    wf->nodes[0].task_handler_name = AGENTRT_STRDUP(handler_name);
...
static void test_dag_basic(void)
{
    ...
    err = agentrt_loop_create(&config, &loop);
    if (err != AGENTRT_SUCCESS || !loop) {
        TEST_FAIL(test_name, "agentrt_loop_create failed");
        return;
    }

    AGENTRT_MEMSET(&hstate, 0, sizeof(hstate));
    err = agentrt_loop_dag_register_handler(loop, "h_basic", mock_handler_success, &hstate);
    ...
    char *exec_id = NULL;
    err = agentrt_loop_submit_dag(loop, &wf, "{\"input\":\"test_data\"}", &exec_id);
    if (err != AGENTRT_SUCCESS || !exec_id) {
    ...
    err = agentrt_loop_dag_wait(loop, exec_id, 0, &result_json);
    if (err != AGENTRT_SUCCESS) {
        TEST_FAIL(test_name, "dag_wait failed");
        AGENTRT_FREE(exec_id);
        AGENTRT_FREE(result_json);
        agentrt_loop_destroy(loop);
        return;
    }
...
static void test_dag_status_query(void)
{
    ...
    agentrt_loop_dag_register_handler(loop, "h_status", mock_handler_success, NULL);

    taskflow_workflow_t wf;
    dag_workflow_init_single(&wf, "wf_status_001", "Status Query Test", "h_status");

    char *exec_id = NULL;
    err = agentrt_loop_submit_dag(loop, &wf, NULL, &exec_id);
    if (err != AGENTRT_SUCCESS || !exec_id) {
        TEST_FAIL(test_name, "submit_dag failed");
        if (wf.nodes) {
            AGENTRT_FREE(wf.nodes[0].task_handler_name);
    ...
    /* 等待完成 */
    char *result = NULL;
    agentrt_loop_dag_wait(loop, exec_id, 0, &result);
    AGENTRT_FREE(result);

    /* 查询状态 */
    char *state = NULL;
    double progress = -1.0;
    err = agentrt_loop_dag_status(loop, exec_id, &state, &progress);
    if (err != AGENTRT_SUCCESS) {
...
static void test_dag_param_validation(void)
{
    ...
    char *exec_id = NULL;
    err = agentrt_loop_submit_dag(NULL, NULL, NULL, &exec_id);
    ...
    err = agentrt_loop_submit_dag(loop, NULL, NULL, &exec_id);
    if (err != AGENTRT_EINVAL) {
        failures++;
    ...
    err = agentrt_loop_dag_register_handler(NULL, "x", mock_handler_success, NULL);
    if (err != AGENTRT_EINVAL) {
        failures++;
    ...
    err = agentrt_loop_dag_register_handler(loop, NULL, mock_handler_success, NULL);
    if (err != AGENTRT_EINVAL) {
        failures++;
    ...
    /* dag_wait NULL loop */
    err = agentrt_loop_dag_wait(NULL, "x", 0, NULL);
    if (err != AGENTRT_EINVAL) {
        failures++;
    ...
    err = agentrt_loop_dag_status(NULL, "x", NULL, NULL);
    if (err != AGENTRT_EINVAL) {
        failures++;
...
static void test_dag_handler_output(void)
{
    ...
    agentrt_loop_dag_register_handler(loop, "h_output", mock_handler_success, NULL);

    taskflow_workflow_t wf;
    dag_workflow_init_single(&wf, "wf_output_001", "Handler Output Test", "h_output");

    char *exec_id = NULL;
    err = agentrt_loop_submit_dag(loop, &wf, "{\"task\":\"generate_output\"}", &exec_id);
    if (err != AGENTRT_SUCCESS || !exec_id) {
        TEST_FAIL(test_name, "submit_dag failed");
        if (wf.nodes) {
            AGENTRT_FREE(wf.nodes[0].task_handler_name);
