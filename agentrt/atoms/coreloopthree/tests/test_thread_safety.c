...
static agentrt_cognition_engine_t *create_engine_with_feedback(void)
{
 ...
 * 辅助: 错误码可读字符串
 * ============================================================================ */
static const char *error_str(agentrt_error_t err)
{
    if (err == AGENTRT_SUCCESS) return "AGENTRT_SUCCESS";
    if (err == AGENTRT_EINVAL)  return "AGENTRT_EINVAL";
    if (err == AGENTRT_ENOMEM)  return "AGENTRT_ENOMEM";
    return "UNKNOWN";
}
...
static void *thread_worker(void *arg)
{
    ...
    if (err == AGENTRT_SUCCESS && engine != NULL) {
        ...
        err = agentrt_cognition_process(engine, args->input,
        ...
        char *stats = NULL;
        size_t stats_len = 0;
        err = agentrt_cognition_stats(engine, &stats, &stats_len);
        ...
        err = agentrt_cognition_health_check(engine, &health_json);
        if (err == AGENTRT_SUCCESS && health_json) {
...
/* 模拟 coordinator 函数 */
static agentrt_error_t mock_coord_func(const char **prompts, size_t count,
                                       void *context, char **out_result)
{
    (void)context;
    if (!prompts || !out_result) return AGENTRT_EINVAL;
    ...
    *out_result = (char *)malloc(total_len);
    if (!*out_result) return AGENTRT_ENOMEM;
    snprintf(*out_result, total_len, "%s%s%s", header, count_buf, trailer);

    return AGENTRT_SUCCESS;
}

static void mock_coord_destroy(agentrt_coordinator_strategy_t *s)
{
    free(s);
}

/* 模拟 dispatcher 函数 */
static agentrt_error_t mock_disp_func(const agentrt_task_node_t *task,
                                      ...
                                      void *context, char **out_agent_id)
{
    ...
    (void)candidates;
    (void)context;
    if (!out_agent_id) return AGENTRT_EINVAL;

    *out_agent_id = strdup("agent-0");
    if (!*out_agent_id) return AGENTRT_ENOMEM;

    return AGENTRT_SUCCESS;
}

static void mock_disp_destroy(agentrt_dispatching_strategy_t *s)
{
    free(s);
}
...
static void *thread_strategy_worker(void *arg)
{
    ...
    /* 2. 创建 dispatcher 策略 */
    agentrt_dispatching_strategy_t *disp =
        (agentrt_dispatching_strategy_t *)calloc(1, sizeof(*disp));
    if (!disp) {
        free(coord);
    ...
    /* 3. 创建引擎 */
    agentrt_cognition_engine_t *engine = NULL;
    agentrt_error_t err = agentrt_cognition_create_take(NULL, coord, disp, &engine);
    if (err == AGENTRT_SUCCESS && engine != NULL) {
...
static void *thread_mix_worker(void *arg)
{
    ...
    switch (args->type) {
    case MIX_TYPE_BASIC:
        err = agentrt_cognition_create_take(NULL, NULL, NULL, &engine);
        break;
    ...
    case MIX_TYPE_STRATEGY: {
        ...
        err = agentrt_cognition_create_take(NULL, coord, disp, &engine);
        coord->destroy(coord);
        disp->destroy(disp);
        break;
    }
 ...
 *
 * 断言:
 *   - NULL 输入返回 AGENTRT_EINVAL
 *   - NULL 引擎返回 AGENTRT_EINVAL
 *   - 错误码正确传播，不被吞没
 * ============================================================================ */
...
TEST(int02_2_null_input_handling)
{
    ...
    /* 1. NULL engine → AGENTRT_EINVAL */
    agentrt_task_plan_t *plan = NULL;
    agentrt_error_t err = agentrt_cognition_process(NULL, "test", 4, &plan);
    assert(err != AGENTRT_SUCCESS);
    printf("    NULL engine → err=%s (expected: non-success)\n", error_str(err));

    /* 2. NULL input → AGENTRT_EINVAL */
    agentrt_cognition_engine_t *engine = create_default_engine();
    err = agentrt_cognition_process(engine, NULL, 0, &plan);
    assert(err != AGENTRT_SUCCESS);
    printf("    NULL input → err=%s (expected: non-success)\n", error_str(err));

    /* 3. NULL out_plan → AGENTRT_EINVAL */
    err = agentrt_cognition_process(engine, "test", 4, NULL);
    assert(err != AGENTRT_SUCCESS);
    printf("    NULL out_plan → err=%s (expected: non-success)\n", error_str(err));

    /* 4. NULL out_engine → AGENTRT_EINVAL */
    err = agentrt_cognition_create_take(NULL, NULL, NULL, NULL);
    assert(err != AGENTRT_SUCCESS);
...
TEST(int02_2_null_engine_handling)
{
    ...
    /* 1. agentrt_cognition_destroy(NULL) 应该安全无操作 */
    agentrt_cognition_destroy(NULL);
    printf("    destroy(NULL) → no crash (safe no-op)\n");

    /* 2. agentrt_cognition_set_fallback_plan(NULL, NULL) */
    agentrt_cognition_set_fallback_plan(NULL, NULL);
    printf("    set_fallback_plan(NULL, NULL) → no crash\n");

    /* 3. agentrt_cognition_set_context_take(NULL, NULL, NULL) */
    agentrt_cognition_set_context_take(NULL, NULL, NULL);
    printf("    set_context(NULL, NULL, NULL) → no crash\n");

    /* 4. agentrt_cognition_set_memory(NULL, NULL) */
    agentrt_cognition_set_memory(NULL, NULL);
    printf("    set_memory(NULL, NULL) → no crash\n");

    /* 5. agentrt_cognition_stats(NULL, NULL, NULL) → 错误 */
    agentrt_error_t err = agentrt_cognition_stats(NULL, NULL, NULL);
    assert(err != AGENTRT_SUCCESS);
    printf("    stats(NULL) → err=%s (expected: non-success)\n", error_str(err));

    /* 6. agentrt_cognition_health_check(NULL, NULL) → 错误 */
    err = agentrt_cognition_health_check(NULL, NULL);
    assert(err != AGENTRT_SUCCESS);
    printf("    health_check(NULL) → err=%s (expected: non-success)\n", error_str(err));

    /* 7. agentrt_task_plan_free(NULL) 应该安全无操作 */
    agentrt_task_plan_free(NULL);
...
TEST(int02_2_error_code_propagation)
{
    ...
    assert(err == AGENTRT_SUCCESS);
    printf("    Valid input → err=%s (expected: AGENTRT_SUCCESS)\n",
           error_str(err));
    if (plan) {
        agentrt_task_plan_free(plan);
    }
    ...
    for (int i = 0; i < 3; i++) {
        plan = NULL;
        err = agentrt_cognition_process(engine, invalid_inputs[i],
...
TEST(int02_2_create_ex_error_handling)
{
    ...
    agentrt_cognition_engine_t *engine = NULL;
    agentrt_error_t err = agentrt_cognition_create_ex_take(NULL, NULL, NULL, NULL,
                                                      &engine);
    assert(err == AGENTRT_SUCCESS);
    assert(engine != NULL);
    ...
    err = agentrt_cognition_create_ex_take(NULL, NULL, NULL, NULL, NULL);
    assert(err != AGENTRT_SUCCESS);
    ...
    err = agentrt_cognition_create_ex_take(&config, NULL, NULL, NULL, NULL);
    assert(err != AGENTRT_SUCCESS);
...
TEST(int02_2_intent_parser_error_handling)
{
    ...
    err = agentrt_intent_parser_create(&parser);
    assert(err == AGENTRT_SUCCESS);
    assert(parser != NULL);

    err = agentrt_intent_parser_parse(parser, NULL, 0, NULL);
    assert(err != AGENTRT_SUCCESS);
...
TEST(int02_2_memory_engine_error_handling)
{
    ...
    /* 3. 创建引擎并测试 NULL 写入 */
    agentrt_memory_engine_t *mem = NULL;
    err = agentrt_memory_create(NULL, &mem);
    assert(err == AGENTRT_SUCCESS);
    assert(mem != NULL);

    err = agentrt_memory_write(mem, NULL, NULL);
    assert(err != AGENTRT_SUCCESS);
    printf("    memory_write(NULL record) → err=%s\n", error_str(err));

    agentrt_memory_destroy(mem);
}
