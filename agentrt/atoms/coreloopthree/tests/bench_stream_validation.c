...
{
    char *buf = (char *)AGENTRT_MALLOC(target_len + 1);
    if (!buf)
        return NULL;
...
static void bench_int04_1(void)
{
    ...
    for (size_t si = 0; si < num_sizes; si++) {
        ...
        agentrt_error_t err = sc_stream_critic_create(NULL, &critic);
        assert(err == AGENTRT_SUCCESS);
        assert(critic != NULL);

        /* 分配延迟样本数组 */
        double *latencies = (double *)AGENTRT_MALLOC(
