...
TEST(int01_1_five_phase_pipeline)
{
    ...
    agentrt_error_t err = agentrt_cognition_process(engine, input, input_len, &plan);
...
TEST(int01_1_sustained_processing)
{
    ...
    for (int i = 0; i < 5; i++) {
        agentrt_task_plan_t *plan = NULL;
        agentrt_error_t err = agentrt_cognition_process(engine, inputs[i],
...
/* 模拟坐标函数: 合并多个模型输出 */
static agentrt_error_t mock_coordinate_func(const char **prompts, size_t count,
...
TEST(int01_2_coordinator_status)
{
    ...
    agentrt_error_t err = coord_strategy->coordinate(
    ...
    /* 3. 使用该策略创建认知引擎 */
    agentrt_cognition_engine_t *engine = NULL;
    err = agentrt_cognition_create_take(NULL, coord_strategy, NULL, &engine);
    assert(err == AGENTRT_SUCCESS);
    assert(engine != NULL);
...
TEST(int01_4_metacognition_scoring)
{
    ...
    agentrt_task_plan_t *plan = NULL;
    agentrt_error_t err = agentrt_cognition_process(engine, input, strlen(input), &plan);
    assert(err == AGENTRT_SUCCESS);
    if (plan) {
...
TEST(int01_extra_memory_integration)
{
    ...
    char *record_id = NULL;
    err = agentrt_memory_write(mem_engine, &record, &record_id);
    assert(err == AGENTRT_SUCCESS);
    ...
    err = agentrt_memory_health_check(mem_engine, &mem_health);
    assert(err == AGENTRT_SUCCESS);
...
TEST(int01_extra_intent_parser)
{
    ...
    /* 2. 添加自定义规则 */
    err = agentrt_intent_parser_add_rule(parser, "calculate|compute|math",
    ...
    err = agentrt_intent_parser_parse(parser, test_input, strlen(test_input), &intent);
    assert(err == AGENTRT_SUCCESS);
    assert(intent != NULL);
    ...
    /* 4. 获取解析器统计 */
    char *stats = NULL;
    err = agentrt_intent_parser_stats(parser, &stats);
    assert(err == AGENTRT_SUCCESS);
    assert(stats != NULL);
    ...
    /* 5. 健康检查 */
    char *health = NULL;
    err = agentrt_intent_parser_health_check(parser, &health);
    assert(err == AGENTRT_SUCCESS);
    assert(health != NULL);
