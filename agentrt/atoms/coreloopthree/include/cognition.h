...
#ifndef AGENTRT_COGNITION_H
#define AGENTRT_COGNITION_H
...
/* 前向声明 */
typedef struct agentrt_cognition_engine agentrt_cognition_engine_t;
typedef struct agentrt_memory_engine agentrt_memory_engine_t;
typedef struct agentrt_memory_provider agentrt_memory_provider_t;
typedef struct agentrt_intent agentrt_intent_t;
typedef struct agentrt_task_plan agentrt_task_plan_t;
typedef struct agentrt_plan_strategy agentrt_plan_strategy_t;
typedef struct agentrt_coordinator_strategy agentrt_coordinator_strategy_t;
typedef struct agentrt_dispatching_strategy agentrt_dispatching_strategy_t;
typedef struct agentrt_intent_parser agentrt_intent_parser_t;
typedef struct agentrt_task_checkpoint agentrt_task_checkpoint_t;
 ...
 */
typedef void (*agentrt_feedback_callback_t)(int level, const char *module, const char *event,
                                            const char *data, size_t data_len, void *user_data);

/**
 * @brief 认知引擎配置
 */
typedef struct agentrt_cognition_config {
    uint32_t cognition_default_timeout_ms;         /**< 默认任务超时（毫秒） */
    uint32_t cognition_max_retries;                /**< 最大重试次数 */
    agentrt_feedback_callback_t feedback_callback; /**< 反馈回调函数（可选） */
 ...
 * @param out_plan [out] 输出计划 (OWNER - caller must call agentrt_task_plan_free).
 * @return agentrt_error_t
 */
typedef agentrt_error_t (*agentrt_plan_func_t)(const agentrt_intent_t *intent, void *context,
 ...
 */
typedef void (*agentrt_plan_destroy_t)(agentrt_plan_strategy_t *strategy);

/**
 * @brief 规划策略对象
 */
struct agentrt_plan_strategy {
    agentrt_plan_func_t plan;       /**< 规划函数 */
    agentrt_plan_destroy_t destroy; /**< 释放函数 */
    void *data;                     /**< 私有数据 */
};
 ...
 * @return agentrt_error_t
 */
typedef agentrt_error_t (*agentrt_coordinate_func_t)(const char **prompts, size_t count,
 ...
 * @param strategy [in] 策略对象 (TRANSFER - function takes ownership and frees).
 */
typedef void (*agentrt_coordinator_destroy_t)(agentrt_coordinator_strategy_t *strategy);

/**
 * @brief 协同策略对象
 */
struct agentrt_coordinator_strategy {
    agentrt_coordinate_func_t coordinate;
    agentrt_coordinator_destroy_t destroy;
    void *data;
};
 ...
 * @return agentrt_error_t
 */
typedef agentrt_error_t (*agentrt_dispatch_func_t)(const agentrt_task_node_t *task,
 ...
 * @param strategy [in] 策略对象 (TRANSFER - function takes ownership and frees).
 */
typedef void (*agentrt_dispatch_destroy_t)(agentrt_dispatching_strategy_t *strategy);

/**
 * @brief 调度策略对象
 */
struct agentrt_dispatching_strategy {
    agentrt_dispatch_func_t dispatch;
    agentrt_dispatch_destroy_t destroy;
    void *data;
};
 ...
 * @param out_engine [out] 输出引擎句柄 (OWNER - caller must call agentrt_cognition_destroy).
 * @return agentrt_error_t AGENTRT_SUCCESS 成功，其他为错误码
 *
 ...
 *
 * @see agentrt_cognition_create_ex_take(), agentrt_cognition_destroy()
 */
/* _take: caller transfers ownership */
AGENTRT_API agentrt_error_t agentrt_cognition_create_take(agentrt_plan_strategy_t *plan_strategy,
 ...
 * - 创建完成后，引擎实例可以被多个线程使用，但需要外部同步
 *
 * @see agentrt_cognition_create_take(), agentrt_cognition_destroy()
 */
/* _take: caller transfers ownership */
AGENTRT_API agentrt_error_t agentrt_cognition_create_ex_take(
    const agentrt_cognition_config_t *manager, agentrt_plan_strategy_t *plan_strategy,
    agentrt_coordinator_strategy_t *coord_strategy, agentrt_dispatching_strategy_t *disp_strategy,
    agentrt_cognition_engine_t **out_engine);
 ...
 * - 销毁后，引擎实例不应再被任何线程使用
 *
 * @see agentrt_cognition_create_take(), agentrt_cognition_create_ex_take()
 */
 ...
 * @reentrant 否
 */
AGENTRT_API void agentrt_cognition_set_fallback_plan(agentrt_cognition_engine_t *engine,
 ...
 * @reentrant 否
 */
AGENTRT_API void agentrt_cognition_set_plan_strategy(agentrt_cognition_engine_t *engine,
 ...
 * @reentrant 否
 */
AGENTRT_API void agentrt_cognition_set_coord_strategy(agentrt_cognition_engine_t *engine,
 ...
 * @reentrant 否
 */
AGENTRT_API void agentrt_cognition_set_disp_strategy(agentrt_cognition_engine_t *engine,
 ...
 * @param out_plan [out] 输出任务计划 (OWNER - caller must call agentrt_task_plan_free).
 * @return agentrt_error_t AGENTRT_SUCCESS 成功，其他为错误码
 *
 ...
 * - 处理过程中，引擎实例不应被其他线程修改
 *
 * @see agentrt_task_plan_free()
 */
AGENTRT_API agentrt_error_t agentrt_cognition_process(agentrt_cognition_engine_t *engine,
 ...
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_cognition_process()
 */
AGENTRT_API void agentrt_task_plan_free(agentrt_task_plan_t *plan);
 ...
 */
/* _take: caller transfers ownership */
AGENTRT_API void agentrt_cognition_set_context_take(agentrt_cognition_engine_t *engine, void *context,
                                                    void (*destroy)(void *));
 ...
 */
AGENTRT_API void agentrt_cognition_set_memory(agentrt_cognition_engine_t *engine,
 ...
 */
AGENTRT_API void agentrt_cognition_set_memory_provider(agentrt_cognition_engine_t *engine,
 ...
 */
AGENTRT_API void agentrt_cognition_set_llm_service(agentrt_cognition_engine_t *engine,
 ...
 */
AGENTRT_API void agentrt_cognition_set_llm_adapter(agentrt_cognition_engine_t *engine,
 ...
 */
AGENTRT_API void agentrt_cognition_set_llm_streaming(agentrt_cognition_engine_t *engine,
                                                     int enabled,
 ...
 */
AGENTRT_API void agentrt_cognition_set_tool_service(agentrt_cognition_engine_t *engine,
 ...
 * @ownership adapter: BORROW
 */
AGENTRT_API void agentrt_cognition_set_tool_adapter(agentrt_cognition_engine_t *engine,
 ...
 */
AGENTRT_API tool_service_t *agentrt_cognition_get_tool_service(
 ...
 */
AGENTRT_API void agentrt_cognition_enable_checkpoint(agentrt_cognition_engine_t *engine,
 ...
 * @reentrant 否
 */
AGENTRT_API agentrt_error_t agentrt_cognition_stats(agentrt_cognition_engine_t *engine,
 ...
 * @reentrant 否
 */
AGENTRT_API agentrt_error_t agentrt_cognition_health_check(agentrt_cognition_engine_t *engine,
                                                           char **out_json);
...
/**
 * @brief 创建意图解析器
 * @param out_parser [out] 输出解析器句柄 (OWNER - caller must call agentrt_intent_parser_destroy).
 * @return agentrt_error_t
 *
 * @ownership out_parser: OWNER
 */
AGENTRT_API agentrt_error_t agentrt_intent_parser_create(agentrt_intent_parser_t **out_parser);
 ...
 */
AGENTRT_API void agentrt_intent_parser_destroy(agentrt_intent_parser_t *parser);
 ...
 * @param out_intent [out] 输出意图结构 (OWNER - caller must call agentrt_intent_free).
 * @return agentrt_error_t
 *
 * @ownership out_intent: OWNER
 */
AGENTRT_API agentrt_error_t agentrt_intent_parser_parse(agentrt_intent_parser_t *parser,
                                                        const char *input, size_t input_len,
                                                        agentrt_intent_t **out_intent);
 ...
 */
AGENTRT_API void agentrt_intent_free(agentrt_intent_t *intent);
 ...
 */
AGENTRT_API agentrt_error_t agentrt_intent_parser_add_rule(agentrt_intent_parser_t *parser,
                                                           const char *pattern,
 ...
 * @ownership out_stats: OWNER
 */
AGENTRT_API agentrt_error_t agentrt_intent_parser_stats(agentrt_intent_parser_t *parser,
                                                        char **out_stats);
 ...
 */
AGENTRT_API void agentrt_intent_parser_reset_stats(agentrt_intent_parser_t *parser);
 ...
 * @ownership out_json: OWNER
 */
AGENTRT_API agentrt_error_t agentrt_intent_parser_health_check(agentrt_intent_parser_t *parser,
                                                               char **out_json);
