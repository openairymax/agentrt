...
#ifndef AGENTRT_LOOP_H
#define AGENTRT_LOOP_H
...
/* 前向声明 */
typedef struct agentrt_core_loop agentrt_core_loop_t;
typedef struct agentrt_checkpoint_stats agentrt_checkpoint_stats_t;
...
typedef struct agentrt_loop_config {
    ...
    agentrt_plan_strategy_t *loop_config_plan_strategy;         /**< 规划策略（可选） */
    agentrt_coordinator_strategy_t *loop_config_coord_strategy; /**< 协同策略（可选） */
    agentrt_dispatching_strategy_t *loop_config_disp_strategy;  /**< 调度策略（可选） */
 ...
 *
 * @ownership out_loop 由调用者负责通过 agentrt_loop_destroy() 释放
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_loop_destroy()
 */
 ...
 *
 * 与 agentrt_loop_submit 不同，此函数在任务计划生成后自动保存检查点，
 ...
 * @return agentrt_error_t AGENTRT_SUCCESS 恢复成功，AGENTRT_ENOENT 无检查点
 *
 * @ownership out_restored_task_id 由调用者负责释放
 * @threadsafe 是（内部使用互斥锁保护）
 * @reentrant 否
 * @see agentrt_loop_submit_persistent()
 */
 ...
 *
 * 从指定路径的快照文件恢复任务ID，然后通过 agentrt_loop_restore_task
 * 恢复完整的任务状态。
 *
 * @param loop [in] 循环句柄（非NULL）
 * @param snapshot_path [in] 快照文件路径（非NULL）
 ...
 *
 * 与 agentrt_loop_submit（自然语言单任务）不同，此函数接受结构化的
 ...
 * agentrt_loop_dag_register_handler() 注册对应的处理器。
 *
 * @param loop [in] 循环句柄（非NULL）
 ...
 * @reentrant 否
 * @see agentrt_loop_dag_wait(), agentrt_loop_dag_register_handler()
 */
 ...
 * @return agentrt_error_t AGENTRT_SUCCESS 成功，AGENTRT_ETIMEOUT 超时
 *
 * @threadsafe 是
 * @see agentrt_loop_submit_dag()
 */
 ...
 * @return agentrt_error_t AGENTRT_SUCCESS 成功，AGENTRT_ENOENT 执行实例不存在
 *
 * @threadsafe 是
 */
