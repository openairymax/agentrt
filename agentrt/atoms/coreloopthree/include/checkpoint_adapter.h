 ...
 *   agentrt_loop_set_checkpoint_adapter(loop, ckpt);
 *
 *   // 3. 每 N 轮自动保存检查点
 *   //    N 由 agentrt.yaml 的 multi_agent.checkpoint.interval_turns 配置
 *
 *   // 4. 崩溃后恢复
...
#ifndef AGENTRT_CORELOOPTHREE_CHECKPOINT_ADAPTER_H
#define AGENTRT_CORELOOPTHREE_CHECKPOINT_ADAPTER_H
 ...
 * checkpoint_adapter.c 实现端仍保留 include（调用 agentrt_checkpoint_save /
 * agentrt_checkpoint_restore 等 API 与 agentrt_task_checkpoint_t 类型）。
...
typedef struct {
    const char *storage_path;         /**< 检查点存储路径，默认 "/var/lib/agentrt/checkpoints" */
 ...
 * @return 0 成功，AGENTRT_ENOENT 无检查点，其他为错误
 *
...
#endif /* AGENTRT_CORELOOPTHREE_CHECKPOINT_ADAPTER_H */