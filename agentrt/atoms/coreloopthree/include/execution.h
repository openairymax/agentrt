...
#ifndef AGENTRT_EXECUTION_H
#define AGENTRT_EXECUTION_H
...
#define AGENTRT_EXEC_MAX_OUTPUT_LEN 65536
...
/* 前向声明 */
typedef struct agentrt_execution_engine agentrt_execution_engine_t;
...
#endif
typedef struct agentrt_compensation agentrt_compensation_t;
...
struct agentrt_execution_unit {
     ...
     */
    agentrt_error_t (*execution_unit_execute)(agentrt_execution_unit_t *unit, const void *input,
                                              void **out_output);
    /**
     * @brief 释放单元资源
     */
    void (*execution_unit_destroy)(agentrt_execution_unit_t *unit);
    /**
     * @brief 获取单元元数据
     */
    const char *(*execution_unit_get_metadata)(agentrt_execution_unit_t *unit);
};
 ...
 *
 * @ownership out_engine 由调用者负责通过 agentrt_execution_destroy() 释放
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_execution_destroy()
 */
AGENTRT_API agentrt_error_t agentrt_execution_create(uint32_t max_concurrency,
 ...
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_execution_create()
 */
AGENTRT_API void agentrt_execution_destroy(agentrt_execution_engine_t *engine);
 ...
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_execution_unregister_unit()
 */
AGENTRT_API agentrt_error_t agentrt_execution_register_unit(agentrt_execution_engine_t *engine,
                                                            const char *name,
 ...
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_execution_register_unit()
 */
AGENTRT_API void agentrt_execution_unregister_unit(agentrt_execution_engine_t *engine,
                                                   const char *name);
 ...
 * - 调用者无需额外的同步措施
 *
 * @see agentrt_execution_query(), agentrt_execution_wait()
 */
AGENTRT_API agentrt_error_t agentrt_execution_submit(agentrt_execution_engine_t *engine,
 ...
 * @threadsafe 是（内部使用互斥锁保护）
 * @reentrant 否
 * @see agentrt_execution_submit()
 */
AGENTRT_API agentrt_error_t agentrt_execution_query(agentrt_execution_engine_t *engine,
                                                    const char *task_id,
 ...
 * @return agentrt_error_t AGENTRT_SUCCESS 成功，其他为错误码
 *
 * @ownership out_result 由调用者负责通过 agentrt_task_free() 释放
 * @threadsafe 是（内部使用条件变量和互斥锁）
 * @reentrant 否
 * @see agentrt_task_free()
 */
AGENTRT_API agentrt_error_t agentrt_execution_wait(agentrt_execution_engine_t *engine,
 ...
 * @see agentrt_execution_submit()
 */
AGENTRT_API agentrt_error_t agentrt_execution_cancel(agentrt_execution_engine_t *engine,
                                                     const char *task_id);
 ...
 * @see agentrt_task_free()
 */
AGENTRT_API agentrt_error_t agentrt_execution_get_result(agentrt_execution_engine_t *engine,
                                                         const char *task_id,
                                                         agentrt_task_t **out_result);
 ...
 * @reentrant 否
 * @see agentrt_execution_wait(), agentrt_execution_get_result()
 */
AGENTRT_API void agentrt_task_free(agentrt_task_t *task);
 ...
 * @return agentrt_error_t AGENTRT_SUCCESS 成功，其他为错误码
 *
 * @ownership out_manager 由调用者负责通过 agentrt_compensation_destroy() 释放
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_compensation_destroy()
 */
 ...
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_compensation_create()
 */
 ...
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_compensation_compensate()
 */
 ...
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_compensation_register()
 */
 ...
 * @reentrant 否
 */
AGENTRT_API agentrt_error_t agentrt_execution_health_check(agentrt_execution_engine_t *engine,
                                                           char **out_json);
 ...
 *   - 2 (跨轮次): 统计信息更新
 */
AGENTRT_API void agentrt_execution_set_feedback_callback(agentrt_execution_engine_t *engine,
                                                         agentrt_feedback_callback_t callback,
                                                         void *user_data);
