...
#ifndef AGENTRT_CORELOOPTHREE_MEMORY_H
#define AGENTRT_CORELOOPTHREE_MEMORY_H
...
typedef struct agentrt_memory_record agentrt_memory_record_t;
typedef struct agentrt_memory_query agentrt_memory_query_t;
...
typedef struct agentrt_memory_record {
    ...
    agentrt_memory_type_t memory_record_type; /**< 记忆类型 */
 ...
 * @brief 检索结果项
 */
typedef struct agentrt_memory_result_item {
    char *memory_result_item_record_id;
    float memory_result_item_score;
    agentrt_memory_record_t *memory_result_item_record;
} agentrt_memory_result_item_t;

typedef struct agentrt_memory_result_ext {
    agentrt_memory_result_item_t **memory_result_items;
 ...
 *
 * @ownership out_engine 由调用者负责通过 agentrt_memory_destroy() 释放
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_memory_destroy()
 */
AGENTRT_API agentrt_error_t agentrt_memory_create(const char *config_path,
                                                  agentrt_memory_engine_t **out_engine);
 ...
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_memory_create()
 */
AGENTRT_API void agentrt_memory_destroy(agentrt_memory_engine_t *engine);

/**
 * @brief 切换记忆引擎的底层提供商（P3.11-C9: 统一 memory engine + bridge provider）
 *
 * engine 在 create 时通过 agentrt_memory_provider_get_active() 获取全局 active
 ...
 * @reentrant 否
 */
AGENTRT_API agentrt_error_t agentrt_memory_set_provider(agentrt_memory_engine_t *engine,
 ...
 * @reentrant 否
 */
AGENTRT_API agentrt_error_t agentrt_memory_write(agentrt_memory_engine_t *engine,
 ...
 * @return agentrt_error_t AGENTRT_SUCCESS 成功，其他为错误码
 *
 * @ownership out_result 由调用者负责通过 agentrt_memory_result_free() 释放
 * @threadsafe 是（内部使用互斥锁保护）
 * @reentrant 否
 * @see agentrt_memory_result_free()
 */
AGENTRT_API agentrt_error_t agentrt_memory_query(agentrt_memory_engine_t *engine,
 ...
 * @return agentrt_error_t AGENTRT_SUCCESS 成功，其他为错误码
 *
 * @ownership out_record 由调用者负责通过 agentrt_memory_record_free() 释放
 * @threadsafe 是（内部使用互斥锁保护）
 * @reentrant 否
 *
 * @concurrency 并发合约：
 ...
 *
 * @see agentrt_memory_record_free()
 */
AGENTRT_API agentrt_error_t agentrt_memory_get(agentrt_memory_engine_t *engine,
                                               const char *record_id, int include_raw,
                                               agentrt_memory_record_t **out_record);
 ...
 * @reentrant 否
 */
AGENTRT_API agentrt_error_t agentrt_memory_mount(agentrt_memory_engine_t *engine,
 ...
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_memory_query()
 */
AGENTRT_API void agentrt_memory_result_free(agentrt_memory_result_ext_t *result);
 ...
 * @threadsafe 否（内部未使用线程安全措施）
 * @reentrant 否
 * @see agentrt_memory_get()
 */
AGENTRT_API void agentrt_memory_record_free(agentrt_memory_record_t *record);
 ...
 * @reentrant 否
 */
AGENTRT_API agentrt_error_t agentrt_memory_evolve(agentrt_memory_engine_t *engine, int force);
 ...
 * @reentrant 否
 */
AGENTRT_API agentrt_error_t agentrt_memory_health_check(agentrt_memory_engine_t *engine,
                                                        char **out_json);
...
#endif /* AGENTRT_CORELOOPTHREE_MEMORY_H */
