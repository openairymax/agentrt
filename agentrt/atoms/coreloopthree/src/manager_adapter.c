...
struct agentrt_manager_adapter_s {
    char yaml_path[256];                     /* agentrt.yaml 路径 */
 ...
 */
static void bridge_reload_callback(const agentrt_yaml_config_t *old_config,
                                   const agentrt_yaml_config_t *new_config,
                                   void *user_data)
{
    agentrt_manager_adapter_t *adapter =
        (agentrt_manager_adapter_t *)user_data;
    if (!adapter) return;

    AGENTRT_LOG_INFO("ManagerAdapter: config reload detected, "
                         ...
                         agentrt_manager_adapter_t **out_adapter)
{
    ...
    agentrt_manager_adapter_t *adapter =
        (agentrt_manager_adapter_t *)AGENTRT_CALLOC(1, sizeof(*adapter));
    if (!adapter) {
        AGENTRT_LOG_ERROR("ManagerAdapter: init FAILED (OOM)");
        return -1;
    }

    const char *path = yaml_path ? yaml_path : "./configs/agentrt.yaml";
    safe_strcpy(adapter->yaml_path, sizeof(adapter->yaml_path), path);

    AGENTRT_LOG_INFO("ManagerAdapter: init START (path=%s)", path);

    /* P1.1.1 + P1.1.2: 通过 config_loader 加载 YAML 配置 */
    int ret = agentrt_config_init(path);
    if (ret != 0) {
        AGENTRT_LOG_WARN("ManagerAdapter: config_init failed for %s, "
                         "using defaults", path);
        /* 非致命：使用默认配置继续 */
    }
    ...
    agentrt_config_on_reload(bridge_reload_callback, adapter);

    AGENTRT_LOG_INFO("ManagerAdapter: init OK (path=%s, config_loaded=%d)",
...
void manager_adapter_shutdown(agentrt_manager_adapter_t *adapter)
{
    ...
    AGENTRT_LOG_INFO("ManagerAdapter: shutdown START");
    ...
    AGENTRT_LOG_INFO("ManagerAdapter: shutdown OK");
}
...
int manager_adapter_reload(agentrt_manager_adapter_t *adapter)
{
    ...
    AGENTRT_LOG_INFO("ManagerAdapter: reload triggered for %s",
                     adapter->yaml_path);

    return agentrt_config_reload(adapter->yaml_path);
}
    ...
    agentrt_manager_adapter_t *adapter)
{
    ...
    return agentrt_config_get_global();
}
                                ...
                                uint32_t interval_ms)
{
    ...
    AGENTRT_LOG_INFO("ManagerAdapter: starting config watch "
                     "(path=%s, interval=%ums)",
                     adapter->yaml_path, interval_ms);

    int ret = agentrt_config_watch_start(adapter->yaml_path, interval_ms);
    if (ret == 0) {
        adapter->watch_running = true;
        AGENTRT_LOG_INFO("ManagerAdapter: config watch started OK");
    } else {
        AGENTRT_LOG_WARN("ManagerAdapter: config watch start FAILED");
    }
...
void manager_adapter_stop_watch(agentrt_manager_adapter_t *adapter)
{
    ...
    AGENTRT_LOG_INFO("ManagerAdapter: stopping config watch");
    agentrt_config_watch_stop();
    adapter->watch_running = false;
}
    ...
    agentrt_loop_config_t *out_loop_config)
{
    ...
    /* 清零并设置默认值 */
    AGENTRT_MEMSET(out_loop_config, 0, sizeof(*out_loop_config));
    ...
    AGENTRT_LOG_DEBUG("ManagerAdapter: yaml_to_loop_config "
...
{
    if (!adapter || !out_loop) return AGENTRT_EINVAL;

    AGENTRT_LOG_INFO("ManagerAdapter: creating loop from YAML config");

    const agentrt_yaml_config_t *yaml_config =
        manager_adapter_get_config(adapter);
    if (!yaml_config) {
        AGENTRT_LOG_WARN("ManagerAdapter: no config loaded, "
                         "creating loop with defaults");
        return agentrt_loop_create(NULL, out_loop);
    }

    /* 转换 YAML 配置为 Loop 配置 */
    agentrt_loop_config_t loop_config;
    manager_adapter_yaml_to_loop_config(yaml_config, &loop_config);

    /* 创建 Loop */
    agentrt_error_t err = agentrt_loop_create(&loop_config, out_loop);
    if (err == AGENTRT_SUCCESS) {
        AGENTRT_LOG_INFO("ManagerAdapter: loop created OK from YAML config");
    } else {
        AGENTRT_LOG_WARN("ManagerAdapter: loop creation FAILED (err=%d)", err);
    }
    ...
    void *user_data)
{
    ...
    AGENTRT_LOG_DEBUG("ManagerAdapter: registered config change callback "
...
{
    if (!adapter) {
        AGENTRT_LOG_WARN("C-L01: MANAGER-ADAPTER-STATS unavailable");
        return;
    }

    AGENTRT_LOG_INFO("C-L01: MANAGER-ADAPTER-STATS path=%s "
