...
/* 跨平台路径常量（AGENTRT_DATA_DIR / AGENTRT_LOG_DIR / AGENTRT_CONFIG_DIR） */
#include "platform.h"
...
static void safe_strcpy(char *dst, const char *src, size_t dst_size) {
    ...
    AGENTRT_MEMCPY(dst, src, len);
    dst[len] = '\0';
}

static void parse_llm_provider(struct yaml_node *node,
                               agentrt_llm_provider_config_t *prov) {
    if (!node || !prov) return;
    ...
    if (models_node && models_node->type == YAML_NODE_SEQUENCE) {
        ...
        if (count > AGENTRT_LLM_MAX_MODELS_PER_PROVIDER)
            count = AGENTRT_LLM_MAX_MODELS_PER_PROVIDER;
...
void agentrt_yaml_config_defaults(agentrt_yaml_config_t *config) {
    if (!config) return;
    ...
    /* 默认 openai provider */
    agentrt_llm_provider_config_t *openai = &config->llm.providers[0];
    ...
    safe_strcpy(config->memory.storage_path, AGENTRT_DATA_DIR "/memory",
    ...
    safe_strcpy(config->hooks.hook_dirs[0], AGENTRT_CONFIG_DIR "/hooks.d/",
...
static void parse_llm_config(struct yaml_node *root,
    ...
    if (providers && providers->type == YAML_NODE_MAPPING) {
        ...
        for (size_t i = 0; i < count && i < AGENTRT_LLM_MAX_PROVIDERS; i++) {
        ...
        for (int p = 0; known_providers[p]; p++) {
            ...
            if (prov) {
                ...
                if (idx >= AGENTRT_LLM_MAX_PROVIDERS) break;
    ...
    if (routing) {
        ...
        if (fb && fb->type == YAML_NODE_SEQUENCE) {
            ...
            if (fb_count > AGENTRT_LLM_MAX_FALLBACK)
                fb_count = AGENTRT_LLM_MAX_FALLBACK;
...
static void parse_memory_config(struct yaml_node *root,
                                agentrt_memory_config_t *mem_cfg) {
...
static void parse_observability_config(struct yaml_node *root,
                                        agentrt_observability_config_t *obs) {
...
int agentrt_yaml_parse(const char *yaml_content,
    ...
    if (hooks) {
        ...
        if (dirs && dirs->type == YAML_NODE_SEQUENCE) {
            ...
            if (dc > AGENTRT_HOOK_MAX_DIRS) dc = AGENTRT_HOOK_MAX_DIRS;
    ...
    if (plugins) {
        ...
        if (dirs && dirs->type == YAML_NODE_SEQUENCE) {
            size_t dc = yaml_size(dirs);
            if (dc > AGENTRT_PLUGIN_MAX_DIRS) dc = AGENTRT_PLUGIN_MAX_DIRS;
            for (size_t i = 0; i < dc; i++) {
                struct yaml_node *item = yaml_get_index(dirs, i);
                const char *d = yaml_as_string(item, "");
...
int agentrt_yaml_load(const char *yaml_path,
                      agentrt_yaml_config_t *config) {
    if (!yaml_path || !config) return -1;

    /* 先填充默认值 */
    agentrt_yaml_config_defaults(config);
    ...
    int ret = agentrt_yaml_parse(content, config);
    AGENTRT_FREE(content);

    /* 解析后应用环境变量覆盖 */
    if (ret == 0) {
        agentrt_yaml_env_override(config);
        /* 平台路径映射（P1.12.4） */
        agentrt_yaml_resolve_platform_paths(config);
    }
...
int agentrt_yaml_env_override(agentrt_yaml_config_t *config) {
    if (!config) return -1;

    AGENTRT_LOG_DEBUG("C-L01: YAML-ENV-OVERRIDE: START");
    int override_count = 0;

    const char *val;

    /* LLM 默认提供商 */
    val = getenv("AGENTRT_LLM_DEFAULT_PROVIDER");
    if (val) {
        safe_strcpy(config->llm.default_provider, val,
                    sizeof(config->llm.default_provider));
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_LLM_DEFAULT_PROVIDER=%s", val);
        override_count++;
    }

    /* LLM 路由策略 */
    val = getenv("AGENTRT_LLM_ROUTING_STRATEGY");
    if (val) {
        safe_strcpy(config->llm.routing.strategy, val,
                    sizeof(config->llm.routing.strategy));
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_LLM_ROUTING_STRATEGY=%s", val);
        override_count++;
    }

    /* LLM 成本预算 */
    val = getenv("AGENTRT_LLM_ROUTING_COST_BUDGET_DAILY_USD");
    if (val) {
        config->llm.routing.cost_budget_daily_usd = atof(val);
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_LLM_ROUTING_COST_BUDGET_DAILY_USD=%.2f",
                         config->llm.routing.cost_budget_daily_usd);
        override_count++;
    }

    /* 各提供商的 API Key 环境变量覆盖 */
    /* 格式: AGENTRT_LLM_PROVIDERS_<NAME>_API_KEY 或 <NAME>_API_KEY */
    ...
    for (int p = 0; provider_env_names[p]; p++) {
        /* 构造 AGENTRT_LLM_PROVIDERS_<NAME>_API_KEY 环境变量名 */
        char env_key[128];
        snprintf(env_key, sizeof(env_key), "AGENTRT_LLM_PROVIDERS_%s_API_KEY",
        ...
        for (uint32_t i = 0; i < config->llm.provider_count; i++) {
            ...
            if (strcasecmp(config->llm.providers[i].name,
                ...
                AGENTRT_LOG_DEBUG("C-L01: env override set api_key_env=%s for provider %s",
    ...
    /* 记忆存储路径 */
    val = getenv("AGENTRT_MEMORY_STORAGE_PATH");
    if (val) {
        safe_strcpy(config->memory.storage_path, val,
                    sizeof(config->memory.storage_path));
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_MEMORY_STORAGE_PATH=%s", val);
        override_count++;
    }

    /* 网关 HTTP 端口 */
    val = getenv("AGENTRT_GATEWAY_HTTP_PORT");
    if (val) {
        int port = atoi(val);
        if (port > 0 && port <= 65535) {
            config->gateway.http.port = (uint16_t)port;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_GATEWAY_HTTP_PORT=%u",
                             config->gateway.http.port);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_GATEWAY_HTTP_PORT=%s ignored (invalid port)", val);
        }
    }

    /* Metrics 端口 */
    val = getenv("AGENTRT_OBSERVABILITY_METRICS_PORT");
    if (val) {
        int port = atoi(val);
        if (port > 0 && port <= 65535) {
            config->observability.metrics.port = (uint16_t)port;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_OBSERVABILITY_METRICS_PORT=%u",
                             config->observability.metrics.port);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_OBSERVABILITY_METRICS_PORT=%s ignored (invalid port)", val);
        }
    }

    /* 日志级别 */
    val = getenv("AGENTRT_OBSERVABILITY_LOGGING_LEVEL");
    if (val) {
        safe_strcpy(config->observability.logging.level, val,
                    sizeof(config->observability.logging.level));
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_OBSERVABILITY_LOGGING_LEVEL=%s", val);
        override_count++;
    }

    /* 安全模式 */
    val = getenv("AGENTRT_SECURITY_MODE");
    if (val) {
        safe_strcpy(config->security.mode, val,
                    sizeof(config->security.mode));
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_SECURITY_MODE=%s", val);
        override_count++;
    }

    /* 内核内存限制 */
    val = getenv("AGENTRT_KERNEL_MEMORY_MAX_ALLOC_MB");
    if (val) {
        int mb = atoi(val);
        if (mb > 0) {
            config->kernel.memory.max_alloc_mb = (uint32_t)mb;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_MEMORY_MAX_ALLOC_MB=%u",
                             config->kernel.memory.max_alloc_mb);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_MEMORY_MAX_ALLOC_MB=%s ignored (invalid value)", val);
        }
    }

    /* 内核 IPC 配置 */
    val = getenv("AGENTRT_KERNEL_IPC_MAX_MESSAGE_SIZE");
    if (val) {
        int sz = atoi(val);
        if (sz > 0) {
            config->kernel.ipc.max_message_size = (uint32_t)sz;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_IPC_MAX_MESSAGE_SIZE=%u",
                             config->kernel.ipc.max_message_size);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_IPC_MAX_MESSAGE_SIZE=%s ignored (invalid value)", val);
        }
    }

    val = getenv("AGENTRT_KERNEL_IPC_SHM_POOL_SIZE_MB");
    if (val) {
        int mb = atoi(val);
        if (mb > 0) {
            config->kernel.ipc.shm_pool_size_mb = (uint32_t)mb;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_IPC_SHM_POOL_SIZE_MB=%u",
                             config->kernel.ipc.shm_pool_size_mb);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_IPC_SHM_POOL_SIZE_MB=%s ignored (invalid value)", val);
        }
    }

    /* 内核调度器配置 */
    val = getenv("AGENTRT_KERNEL_SCHEDULER_MAX_TASKS");
    if (val) {
        int tasks = atoi(val);
        if (tasks > 0) {
            config->kernel.scheduler.max_tasks = (uint32_t)tasks;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_SCHEDULER_MAX_TASKS=%u",
                             config->kernel.scheduler.max_tasks);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_SCHEDULER_MAX_TASKS=%s ignored (invalid value)", val);
        }
    }

    val = getenv("AGENTRT_KERNEL_SCHEDULER_TIME_SLICE_MS");
    if (val) {
        int ms = atoi(val);
        if (ms > 0) {
            config->kernel.scheduler.time_slice_ms = (uint32_t)ms;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_SCHEDULER_TIME_SLICE_MS=%u",
                             config->kernel.scheduler.time_slice_ms);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_SCHEDULER_TIME_SLICE_MS=%s ignored (invalid value)", val);
        }
    }

    /* 内核 OOM 水位 */
    val = getenv("AGENTRT_KERNEL_MEMORY_OOM_WATERMARK_PERCENT");
    if (val) {
        int pct = atoi(val);
        if (pct >= 50 && pct <= 100) {
            config->kernel.memory.oom_watermark_percent = (uint32_t)pct;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_MEMORY_OOM_WATERMARK_PERCENT=%u",
                             config->kernel.memory.oom_watermark_percent);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_KERNEL_MEMORY_OOM_WATERMARK_PERCENT=%s ignored (out of range [50,100])", val);
        }
    }

    /* 记忆系统提供商 */
    val = getenv("AGENTRT_MEMORY_PROVIDER");
    if (val) {
        /* P1.11.2: 通过环境变量切换 memory provider */
        /* 值: builtin | memoryrovol | auto */
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_MEMORY_PROVIDER=%s", val);
        override_count++;
    }

    /* 多 Agent 配置 */
    val = getenv("AGENTRT_MULTI_AGENT_MAX_CONCURRENT");
    if (val) {
        int n = atoi(val);
        if (n > 0) {
            config->multi_agent.max_concurrent_agents = (uint32_t)n;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_MULTI_AGENT_MAX_CONCURRENT=%u",
                             config->multi_agent.max_concurrent_agents);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_MULTI_AGENT_MAX_CONCURRENT=%s ignored (invalid value)", val);
        }
    }

    /* 安全沙箱类型 */
    val = getenv("AGENTRT_SECURITY_SANDBOX_TYPE");
    if (val) {
        safe_strcpy(config->security.sandbox.type, val,
                    sizeof(config->security.sandbox.type));
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_SECURITY_SANDBOX_TYPE=%s", val);
        override_count++;
    }

    /* 审计日志路径 */
    val = getenv("AGENTRT_SECURITY_AUDIT_LOG_PATH");
    if (val) {
        safe_strcpy(config->security.audit.log_path, val,
                    sizeof(config->security.audit.log_path));
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_SECURITY_AUDIT_LOG_PATH=%s", val);
        override_count++;
    }

    /* 可观测性追踪导出器 */
    val = getenv("AGENTRT_OBSERVABILITY_TRACING_EXPORTER");
    if (val) {
        safe_strcpy(config->observability.tracing.exporter, val,
                    sizeof(config->observability.tracing.exporter));
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_OBSERVABILITY_TRACING_EXPORTER=%s", val);
        override_count++;
    }

    /* P1.6: Checkpoint 环境变量覆盖 */
    val = getenv("AGENTRT_CHECKPOINT_ENABLED");
    if (val) {
        config->multi_agent.checkpoint_enabled =
            (strcmp(val, "1") == 0 || strcasecmp(val, "true") == 0 ||
             strcasecmp(val, "yes") == 0);
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_CHECKPOINT_ENABLED=%s -> %d",
                         val, config->multi_agent.checkpoint_enabled);
        override_count++;
    }

    val = getenv("AGENTRT_CHECKPOINT_INTERVAL_MS");
    if (val) {
        int ms = atoi(val);
        if (ms >= 1000) {
            config->multi_agent.checkpoint_interval_ms = (uint32_t)ms;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_CHECKPOINT_INTERVAL_MS=%u",
                             config->multi_agent.checkpoint_interval_ms);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_CHECKPOINT_INTERVAL_MS=%s ignored (< 1000ms)", val);
        }
    }

    val = getenv("AGENTRT_CHECKPOINT_INTERVAL_TURNS");
    if (val) {
        int turns = atoi(val);
        if (turns > 0) {
            config->multi_agent.checkpoint_interval_turns = (uint32_t)turns;
            AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_CHECKPOINT_INTERVAL_TURNS=%u",
                             config->multi_agent.checkpoint_interval_turns);
            override_count++;
        } else {
            AGENTRT_LOG_WARN("C-L01: ENV-OVERRIDE AGENTRT_CHECKPOINT_INTERVAL_TURNS=%s ignored (invalid value)", val);
        }
    }

    val = getenv("AGENTRT_CHECKPOINT_PATH");
    if (val) {
        safe_strcpy(config->multi_agent.checkpoint_path, val,
                    sizeof(config->multi_agent.checkpoint_path));
        AGENTRT_LOG_INFO("C-L01: ENV-OVERRIDE AGENTRT_CHECKPOINT_PATH=%s", val);
        override_count++;
    }

    AGENTRT_LOG_INFO("C-L01: YAML-ENV-OVERRIDE: DONE — %d env var(s) applied", override_count);
    return 0;
}
...
void agentrt_yaml_config_free(agentrt_yaml_config_t *config) {
    if (!config) return;
    /* 目前所有字段都是栈上的固定大小数组，无需额外释放 */
    /* 未来如果改为动态分配，在此处释放 */
}

int agentrt_yaml_validate(const agentrt_yaml_config_t *config) {
    if (!config) {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: NULL config pointer");
        return -1;
    }

    AGENTRT_LOG_DEBUG("C-L01: YAML-VALIDATE: START");

    /* 验证 version */
    if (config->version[0] == '\0') {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — version is empty");
        return -2;
    }

    /* 验证 LLM 配置 */
    if (config->llm.default_provider[0] == '\0') {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — llm.default_provider is empty");
        return -3;
    }
    if (config->llm.provider_count == 0) {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — llm.provider_count is 0");
        return -4;
    }

    /* 验证路由策略 */
    if (config->llm.routing.strategy[0] == '\0') {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — llm.routing.strategy is empty");
        return -5;
    }
    if (config->llm.routing.fallback_count == 0) {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — llm.routing.fallback_count is 0");
        return -6;
    }
    ...
    if (config->llm.cache.enabled) {
        if (config->llm.cache.ttl_seconds == 0) {
            AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — llm.cache.ttl_seconds is 0");
            return -7;
        }
        if (config->llm.cache.max_entries == 0) {
            AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — llm.cache.max_entries is 0");
            return -8;
        }
    }
    ...
    if (config->memory.enabled) {
        if (config->memory.storage_path[0] == '\0') {
            AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — memory.storage_path is empty");
            return -9;
        }
    }
    ...
    if (config->security.enabled) {
        if (config->security.mode[0] == '\0') {
            AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — security.mode is empty");
            return -10;
        }
    }
    ...
    if (config->gateway.enabled) {
        if (config->gateway.http.port == 0) {
            AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — gateway.http.port is 0");
            return -11;
        }
    }
    ...
    if (config->observability.metrics.enabled) {
        if (config->observability.metrics.port == 0) {
            AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — observability.metrics.port is 0");
            return -12;
        }
    }
    ...
    if (config->kernel.memory.oom_watermark_percent > 100) {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — oom_watermark_percent=%u > 100",
                         config->kernel.memory.oom_watermark_percent);
        return -13;
    }
    if (config->kernel.memory.oom_watermark_percent < 50) {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — oom_watermark_percent=%u < 50",
                         config->kernel.memory.oom_watermark_percent);
        return -14;
    }
    if (config->kernel.ipc.max_message_size == 0) {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — ipc.max_message_size is 0");
        return -15;
    }
    if (config->kernel.ipc.shm_pool_size_mb == 0) {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — ipc.shm_pool_size_mb is 0");
        return -16;
    }
    if (config->kernel.scheduler.max_tasks == 0) {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — scheduler.max_tasks is 0");
        return -17;
    }
    if (config->kernel.scheduler.time_slice_ms == 0) {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — scheduler.time_slice_ms is 0");
        return -18;
    }
    if (config->kernel.memory.max_alloc_mb == 0) {
        AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — memory.max_alloc_mb is 0");
        return -19;
    }
    ...
    if (config->multi_agent.enabled) {
        if (config->multi_agent.max_concurrent_agents == 0) {
            AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — multi_agent.max_concurrent_agents is 0");
            return -20;
        }
        if (config->multi_agent.communication.protocol[0] == '\0') {
            AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — multi_agent.communication.protocol is empty");
            return -21;
        }
        if (config->multi_agent.collaboration.default_pattern[0] == '\0') {
            AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — multi_agent.collaboration.default_pattern is empty");
            return -22;
        }
    }
    ...
    if (config->hooks.enabled) {
        for (uint32_t i = 0; i < config->hooks.global_hooks.on_tool_call_count; i++) {
            if (config->hooks.global_hooks.on_tool_call[i].hook[0] == '\0') {
                AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — hooks.on_tool_call[%u].hook is empty", i);
                return -23;
            }
        }
    }
    ...
    if (config->observability.logging.level[0] != '\0') {
        ...
        if (!valid) {
            AGENTRT_LOG_WARN("C-L01: YAML-VALIDATE: FAIL — invalid logging level '%s'",
    ...
    AGENTRT_LOG_DEBUG("C-L01: YAML-VALIDATE: OK (version=%s, providers=%u, routing=%s, "
 ...
 *   /var/lib/agentrt/ → %ProgramData%\AgentRT\data\
 *   /var/log/agentos/ → %ProgramData%\AgentRT\logs\
 *   /etc/agentrt/     → %ProgramData%\AgentRT\config\
...
int agentrt_yaml_resolve_platform_paths(agentrt_yaml_config_t *config) {
    if (!config) return -1;

    /* 记忆存储路径 */
    platform_resolve_path(config->memory.storage_path,
                          sizeof(config->memory.storage_path),
                          "/var/lib/agentrt/", "data\\memory\\");
    ...
    for (uint32_t i = 0; i < config->hooks.hook_dir_count; i++) {
        if (strstr(config->hooks.hook_dirs[i], "/etc/agentrt/")) {
            platform_resolve_path(config->hooks.hook_dirs[i],
                                  sizeof(config->hooks.hook_dirs[i]),
                                  "/etc/agentrt/", "config\\hooks.d\\");
