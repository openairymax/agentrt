 ...
 * 发送到 llm_d。认知引擎经 agentrt_cognition_set_llm_adapter() 注入本适配器。
 */
...
static char *build_llm_request_json(const llm_request_config_t *config)
{
    ...
    /* 构建完整 JSON-RPC 请求 */
    char *json = (char *)AGENTRT_MALLOC(131072);
    if (!json) return NULL;
...
static llm_response_t *parse_llm_response_json(const char *json)
{
    ...
    llm_response_t *resp =
        (llm_response_t *)AGENTRT_CALLOC(1, sizeof(llm_response_t));
    if (!resp) return NULL;
    ...
    if (content_start) {
        ...
        if (content_start) {
            ...
            if (len > 0) {
                resp->choices =
                    (llm_message_t *)AGENTRT_CALLOC(1, sizeof(llm_message_t));
                if (resp->choices) {
                    resp->choices[0].role = AGENTRT_STRDUP("assistant");
                    resp->choices[0].content =
                        (char *)AGENTRT_MALLOC(len + 1);
                    if (resp->choices[0].content) {
                        AGENTRT_MEMCPY((void *)resp->choices[0].content, content_start, len);
    ...
    if (finish_start) {
        ...
        if (finish_start) {
            ...
            if (len > 0 && len < 64) {
                resp->finish_reason = (char *)AGENTRT_MALLOC(len + 1);
                if (resp->finish_reason) {
                    AGENTRT_MEMCPY(resp->finish_reason, finish_start, len);
    ...
    if (model_start) {
        ...
        if (model_start) {
            ...
            if (len > 0 && len < 64) {
                resp->model = (char *)AGENTRT_MALLOC(len + 1);
                if (resp->model) {
                    AGENTRT_MEMCPY(resp->model, model_start, len);
                    resp->model[len] = '\0';
                }
            }
        }
...
llm_svc_adapter_t *llm_svc_adapter_create(const llm_svc_adapter_config_t *config)
{
    llm_svc_adapter_t *adapter =
        (llm_svc_adapter_t *)AGENTRT_CALLOC(1, sizeof(llm_svc_adapter_t));
    if (!adapter) {
        AGENTRT_LOG_ERROR("C-L02: Failed to allocate LLM adapter");
        return NULL;
    }
    ...
    if (!adapter->bsd) {
        AGENTRT_LOG_WARN("C-L02: SD bootstrap failed for '%s', "
    ...
    if (!adapter->bipc) {
        AGENTRT_LOG_ERROR("C-L02: IPC bootstrap failed for '%s'", ch_name);
        if (adapter->bsd) {
    ...
    /* llm_service_t 由 llm_d daemon 内部构造和管理（见 daemons/llm_d/src/service.h，
     ...
     * agentrt_cognition_set_llm_adapter() 注入 adapter。
     * 旧实现在此 AGENTRT_MALLOC(1) 分配 1 字节占位符桩并作为 llm_service_t 返回，
    ...
    AGENTRT_LOG_INFO("C-L02: LLM adapter created (service=%s, channel=%s, "
...
void llm_svc_adapter_destroy(llm_svc_adapter_t *adapter)
{
    ...
    AGENTRT_LOG_INFO("C-L02: LLM adapter destroyed (requests=%llu errors=%llu)",
...
llm_service_t *llm_svc_adapter_get_service(llm_svc_adapter_t *adapter)
{
     ...
     * 见 daemons/llm_d/src/service.h:17），coreloopthree 无法真实构造。
     * 本函数保留为接口兼容，始终返回 NULL。认知引擎应通过
     * agentrt_cognition_set_llm_adapter() 注入 IPC adapter，由 adapter 通过
     * IPC 调用 llm_d，而非直接持有 llm_service_t 句柄。
     * 旧实现返回 1 字节 AGENTRT_MALLOC(1) 占位符桩，若被 llm_service_complete()
     * 解引用会导致堆越界读——已移除。 */
    (void)adapter;
    return NULL;
}
                             ...
                             llm_response_t **out_response)
{
    ...
    *out_response = NULL;
    ...
    if (!adapter->connected || !adapter->bipc) {
        AGENTRT_LOG_WARN("C-L02: Adapter not connected, cannot send request");
    ...
    if (adapter->bsd) {
        ...
        if (sdh) {
            ...
            if (sd_ret != 0) {
                AGENTRT_LOG_WARN("C-L02: llm_d '%s' not found in SD, "
    ...
    if (!request_json) {
        AGENTRT_LOG_ERROR("C-L02: Failed to build request JSON");
        adapter->total_errors++;
        return -1;
    }
    ...
    if (ipc_ret != 0) {
        AGENTRT_LOG_ERROR("C-L02: IPC request failed: ret=%d", ipc_ret);
        adapter->total_errors++;
        return -1;
    }
    ...
    if (!*out_response) {
        AGENTRT_LOG_WARN("C-L02: Failed to parse LLM response");
        adapter->total_errors++;
        return -1;
    }
                                    ...
                                    llm_response_t **out_response)
{
    ...
    *out_response = NULL;
    ...
    if (!adapter->connected || !adapter->bipc) {
        AGENTRT_LOG_WARN("C-L02: Adapter not connected for streaming");
        adapter->total_errors++;
        return -1;
    }
    ...
    if (ipc_ret != 0) {
        AGENTRT_LOG_ERROR("C-L02: Streaming IPC request failed: ret=%d", ipc_ret);
        adapter->total_errors++;
        return -1;
    }
