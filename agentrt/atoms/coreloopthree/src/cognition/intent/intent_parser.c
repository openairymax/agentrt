...
static size_t extract_keywords(const char *text, char **keywords, size_t max_keywords)
{
    ...
    while (token && count < max_keywords) {
        // 过滤短词
        if (strlen(token) > 2) {
            keywords[count] = AGENTRT_STRDUP(token);
            if (keywords[count]) {
                                         ...
                                         float confidence, uint32_t flags)
{
    ...
    intent_rule_t *rule = (intent_rule_t *)AGENTRT_CALLOC(1, sizeof(intent_rule_t));
    if (!rule) {
        AGENTRT_LOG_ERROR("Failed to allocate intent rule");
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
    }

    rule->pattern = AGENTRT_STRDUP(pattern);
    rule->pattern_len = strlen(pattern);
    rule->intent_name = AGENTRT_STRDUP(intent_name);
    ...
    if (!rule->pattern || !rule->intent_name) {
        if (rule->pattern)
            AGENTRT_FREE(rule->pattern);
        if (rule->intent_name)
            AGENTRT_FREE(rule->intent_name);
        AGENTRT_FREE(rule);
        AGENTRT_LOG_ERROR("Failed to duplicate strings for intent rule");
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
    }
 ...
 */
static agentrt_error_t add_rule_to_parser(agentrt_intent_parser_t *parser, intent_rule_t *rule)
{
    if (!parser || !rule)
        ATM_RET_ERR(AGENTRT_EINVAL);

    agentrt_mutex_lock(parser->lock);

    // 添加到链表头部
    rule->next = parser->rule_list;
    parser->rule_list = rule;

    agentrt_mutex_unlock(parser->lock);

    AGENTRT_LOG_DEBUG("Added intent rule: %s -> %s", rule->pattern, rule->intent_name);
 ...
 * @return AGENTRT_SUCCESS 找到匹配，其他为未找到
 */
static agentrt_error_t match_intent_by_rules(agentrt_intent_parser_t *parser, const char *text,
                                             char **out_intent_name, float *out_confidence)
{
    ...
    char *lower_text = AGENTRT_STRDUP(text);
    if (!lower_text)
    ...
    while (rule) {
        ...
        if (contains_ignore_case(lower_text, rule->pattern)) {
            if (rule->confidence > best_confidence) {
                ...
                if (best_intent)
                    AGENTRT_FREE(best_intent);
                best_intent = AGENTRT_STRDUP(rule->intent_name);
                if (!best_intent) {
                    AGENTRT_FREE(lower_text);
                    agentrt_mutex_unlock(parser->lock);
                    ATM_RET_ERR(AGENTRT_ENOMEM);
                }
            }
                                         ...
                                         size_t max_entries)
{
    ...
    for (size_t i = 0; i < keyword_count && count < max_entries; i++) {
        ...
        // 检查是否为时间相关
        else if (contains_ignore_case(keyword, "time") || contains_ignore_case(keyword, "hour") ||
                 contains_ignore_case(keyword, "minute") ||
                 contains_ignore_case(keyword, "second") || contains_ignore_case(keyword, "day") ||
                 contains_ignore_case(keyword, "week") || contains_ignore_case(keyword, "month") ||
        ...
        // 检查是否为文件相关
                 ...
                 contains_ignore_case(keyword, "report") || contains_ignore_case(keyword, "data")) {
            type = "file";
            confidence = 0.7f;
        }

        if (type) {
            entities[count].type = AGENTRT_STRDUP(type);
            entities[count].value = AGENTRT_STRDUP(keyword);
            if (!entities[count].type || !entities[count].value) {
                AGENTRT_FREE(entities[count].type);
                AGENTRT_FREE(entities[count].value);
                continue;
            }
 ...
 */
agentrt_error_t agentrt_intent_parser_create(agentrt_intent_parser_t **out_parser)
{
    if (!out_parser)
        ATM_RET_ERR(AGENTRT_EINVAL);

    agentrt_intent_parser_t *parser =
        (agentrt_intent_parser_t *)AGENTRT_CALLOC(1, sizeof(agentrt_intent_parser_t));
    if (!parser) {
        AGENTRT_LOG_ERROR("Failed to allocate intent parser");
        ATM_RET_ERR(AGENTRT_ENOMEM);
    }

    parser->lock = agentrt_mutex_create();
    if (!parser->lock) {
        AGENTRT_LOG_ERROR("Failed to create mutex for intent parser");
        AGENTRT_FREE(parser);
        ATM_RET_ERR(AGENTRT_ENOMEM);
    }
    ...
    // 生成唯一 ID
    char uuid_buf[64];
    if (agentrt_generate_uuid(uuid_buf) == AGENTRT_SUCCESS) {
        parser->parser_id = AGENTRT_STRDUP(uuid_buf);
    } else {
        parser->parser_id = NULL;
    }
    if (!parser->parser_id) {
        AGENTRT_LOG_WARN("Failed to generate UUID for intent parser, using default");
        parser->parser_id = AGENTRT_STRDUP("intent_parser_default");
    }
    ...
    if (rule)
        add_rule_to_parser(parser, rule);
    ...
    *out_parser = parser;
    ...
    AGENTRT_LOG_INFO("Intent parser created: %s", parser->parser_id);
    return AGENTRT_SUCCESS;
}
 ...
 * @param parser 解析器句柄
 */
void agentrt_intent_parser_destroy(agentrt_intent_parser_t *parser)
{
    if (!parser)
        return;

    AGENTRT_LOG_DEBUG("Destroying intent parser: %s", parser->parser_id);

    free_rule_list(parser->rule_list);

    if (parser->lock) {
        agentrt_mutex_free(parser->lock);
    }

    if (parser->parser_id) {
        AGENTRT_FREE(parser->parser_id);
    }
 ...
 * @return agentrt_error_t
 */
agentrt_error_t agentrt_intent_parser_parse(agentrt_intent_parser_t *parser, const char *input,
                                            size_t input_len, agentrt_intent_t **out_intent)
{
    ...
    // 记录开始时间
    uint64_t start_time_ns = (uint64_t)agentrt_time_monotonic_ms() * 1000000ULL;

    // 更新统计
    parser->total_parsed++;

    agentrt_intent_t *intent = (agentrt_intent_t *)AGENTRT_CALLOC(1, sizeof(agentrt_intent_t));
    if (!intent) {
        AGENTRT_LOG_ERROR("Failed to allocate intent structure");
        parser->failure_count++;
        ATM_RET_ERR(AGENTRT_ENOMEM);
    }

    intent->intent_raw_text = (char *)AGENTRT_MALLOC(input_len + 1);
    if (!intent->intent_raw_text) {
        AGENTRT_LOG_ERROR("Failed to allocate raw text buffer");
        AGENTRT_FREE(intent);
        parser->failure_count++;
        ATM_RET_ERR(AGENTRT_ENOMEM);
    }
    ...
    agentrt_error_t match_result = match_intent_by_rules(parser, input, &intent_name, &confidence);

    if (match_result == AGENTRT_SUCCESS && intent_name) {
        ...
        if (entity_count > 0) {
            ...
            if (context_json) {
                ...
                for (size_t i = 0; i < entity_count; i++) {
                    ...
                    AGENTRT_FREE(entities[i].type);
                    AGENTRT_FREE(entities[i].value);
                }
        ...
        AGENTRT_LOG_DEBUG("Intent parsed successfully: %s (confidence: %.2f)", intent_name,
                          confidence);

        parser->success_count++;
    } else {
        intent->intent_goal = AGENTRT_STRDUP("unknown");
        ...
        AGENTRT_LOG_WARN("No intent matched for input: %.*s", (int)input_len, input);

        parser->failure_count++;
    }

    uint64_t end_time_ns = (uint64_t)agentrt_time_monotonic_ms() * 1000000ULL;
    uint64_t duration_ns = end_time_ns - start_time_ns;
    parser->total_time_ns += duration_ns;

    *out_intent = intent;

    if (match_result == AGENTRT_SUCCESS) {
        char feedback_json[256];
        snprintf(feedback_json, sizeof(feedback_json),
                 "{\"intent\":\"%s\",\"confidence\":%.2f,\"duration_ns\":%llu}", intent_name,
                 confidence, (unsigned long long)duration_ns);
    }
 ...
 * @param intent 意图结构
 */
void agentrt_intent_free(agentrt_intent_t *intent)
{
    if (!intent)
        return;

    if (intent->intent_raw_text)
        AGENTRT_FREE(intent->intent_raw_text);
    if (intent->intent_goal)
        AGENTRT_FREE(intent->intent_goal);
    if (intent->intent_context)
        AGENTRT_FREE(intent->intent_context);
 ...
 * @return agentrt_error_t
 */
agentrt_error_t agentrt_intent_parser_add_rule(agentrt_intent_parser_t *parser, const char *pattern,
                                               ...
                                               uint32_t flags)
{
    ...
    if (confidence < 0.0f || confidence > 1.0f) {
        AGENTRT_LOG_ERROR("Confidence must be between 0.0 and 1.0");
        ATM_RET_ERR(AGENTRT_EINVAL);
    }

    intent_rule_t *rule = create_intent_rule(pattern, intent_name, confidence, flags);
    if (!rule) {
        AGENTRT_LOG_ERROR("Failed to create intent rule");
        ATM_RET_ERR(AGENTRT_ENOMEM);
    }

    agentrt_error_t result = add_rule_to_parser(parser, rule);
    if (result != AGENTRT_SUCCESS) {
 ...
 * @return agentrt_error_t
 */
agentrt_error_t agentrt_intent_parser_stats(agentrt_intent_parser_t *parser, char **out_stats)
{
    if (!parser || !out_stats)
        ATM_RET_ERR(AGENTRT_EINVAL);
 ...
 * @param parser 解析器
 */
void agentrt_intent_parser_reset_stats(agentrt_intent_parser_t *parser)
{
    if (!parser)
        return;
    ...
    AGENTRT_LOG_INFO("Intent parser stats reset: %s", parser->parser_id);
}
 ...
 * @return agentrt_error_t
 */
agentrt_error_t agentrt_intent_parser_health_check(agentrt_intent_parser_t *parser, char **out_json)
{
    if (!parser || !out_json)
        ATM_RET_ERR(AGENTRT_EINVAL);
    ...
    cJSON_AddNumberToObject(health_json, "timestamp_ns", agentrt_get_monotonic_time_ns());
