...
static agentrt_intent_type_t check_greeting(const char *lower_input, float *score)
{
    for (int i = 0; g_greet_keywords[i]; i++) {
        if (intent_contains_ignore_case(lower_input, g_greet_keywords[i])) {
            *score = 0.95f;
            return AGENTRT_INTENT_GREETING;
        }
    }
    return AGENTRT_INTENT_UNKNOWN;
}

static agentrt_intent_type_t check_farewell(const char *lower_input, float *score)
{
    for (int i = 0; g_farewell_keywords[i]; i++) {
        if (intent_contains_ignore_case(lower_input, g_farewell_keywords[i])) {
            *score = 0.95f;
            return AGENTRT_INTENT_FAREWELL;
        }
    }
    return AGENTRT_INTENT_UNKNOWN;
}

static agentrt_intent_type_t check_query(const char *lower_input, float *score)
{
    int query_count = 0;
    ...
    if (query_count > 0) {
        *score = 0.8f + (query_count > 2 ? 0.15f : 0.05f);
        return AGENTRT_INTENT_QUERY;
    }
    return AGENTRT_INTENT_UNKNOWN;
}

static agentrt_intent_type_t check_command(const char *lower_input, float *score)
{
    int cmd_count = 0;
                            ...
                            agentrt_intent_classification_t *result)
{
    ...
    if (!atomic_load_explicit(&g_classifier_initialized, memory_order_acquire)) {
        agentrt_intent_classifier_init();
    }

    char *lower_input = (char *)AGENTRT_MALLOC(input_len + 1);
    if (!lower_input) {
        return AGENTRT_EINVAL;
    }
    ...
    result->type = AGENTRT_INTENT_UNKNOWN;
    result->confidence = 0.0f;

    float max_score = 0.0f;
    agentrt_intent_type_t detected_type = AGENTRT_INTENT_UNKNOWN;

    typedef agentrt_intent_type_t (*intent_checker_t)(const char *, float *);
    ...
    for (int i = 0; checkers[i]; i++) {
        ...
        agentrt_intent_type_t current_type = checkers[i](lower_input, &current_score);

        if (current_type != AGENTRT_INTENT_UNKNOWN) {
    ...
    result->type_name = agentrt_intent_type_name(detected_type);

    AGENTRT_FREE(lower_input);
    return 0;
}
