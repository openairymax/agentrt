...
#ifndef AGENTRT_TRIPLE_COORDINATOR_H
#define AGENTRT_TRIPLE_COORDINATOR_H
...
typedef agentrt_error_t (*tc3_s2_generate_fn)(const char *input, size_t input_len, char **output,
                                              size_t *output_len, void *user_data);

typedef agentrt_error_t (*tc3_s1_verify_fn)(const char *content, size_t content_len,
                                            float *out_score, int *out_acceptable,
                                            char **out_critique, size_t *out_critique_len,
                                            void *user_data);

typedef agentrt_error_t (*tc3_s1_expert_fn)(const char *content, size_t content_len,
...
AGENTRT_API agentrt_error_t tc3_coordinator_create(const tc3_config_t *config,
                                                   agentrt_thinking_chain_t *chain,
                                                   agentrt_metacognition_t *meta,
                                                   tc3_coordinator_t **out_coord);

AGENTRT_API void tc3_coordinator_destroy(tc3_coordinator_t *coord);

AGENTRT_API agentrt_error_t tc3_coordinator_execute(tc3_coordinator_t *coord, const char *input,
                                                    size_t input_len, char **out_output,
                                                    size_t *out_output_len);

AGENTRT_API agentrt_error_t tc3_coordinator_execute_streaming(tc3_coordinator_t *coord,
                                                              const char *input, size_t input_len,
                                                              char **out_output,
                                                              size_t *out_output_len);

AGENTRT_API agentrt_error_t tc3_coordinator_get_stats(const tc3_coordinator_t *coord,
                                                      tc3_stats_t *out_stats);

AGENTRT_API agentrt_error_t tc3_coordinator_reset(tc3_coordinator_t *coord);

#ifdef __cplusplus
}
#endif

#endif /* AGENTRT_TRIPLE_COORDINATOR_H */
