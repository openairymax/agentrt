 ...
 * @param out_text  [out] 响应文本（AGENTRT_MALLOC 分配，调用方释放）
 * @param out_len   [out] 响应文本长度
 * @return AGENTRT_SUCCESS 或错误码
 */
static agentrt_error_t tc3_llm_call(const tc3_llm_ctx_t *ctx, const char *system,
                                    ...
                                    char **out_text, size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    if (ctx->lock) {
        agentrt_mutex_lock(ctx->lock);
    }
    adapter = ctx->llm_adapter;
    svc = ctx->llm_svc;
    if (ctx->lock) {
        agentrt_mutex_unlock(ctx->lock);
    }

    if (!adapter && !svc) {
        AGENTRT_LOG_DEBUG("TC3-LLM: no LLM handle available (adapter=%p svc=%p)",
    ...
    if (adapter) {
        ...
        AGENTRT_LOG_DEBUG("TC3-LLM: IPC adapter complete: ret=%d", ret);
    } else if (svc) {
        ret = llm_service_complete(svc, &llm_cfg, &resp);
        AGENTRT_LOG_DEBUG("TC3-LLM: direct service complete: ret=%d", ret);
    }

    if (ret != 0 || !resp || !resp->choices || resp->choice_count == 0) {
        AGENTRT_LOG_WARN("TC3-LLM: LLM call failed (ret=%d resp=%p choices=%p count=%u)",
                         ret, (void *)resp,
    ...
    if (!content || content[0] == '\0') {
        AGENTRT_LOG_WARN("TC3-LLM: LLM returned empty content");
        llm_response_free(resp);
        return AGENTRT_ESERVICE;
    }
    ...
    if (content_len > TC3_LLM_MAX_RESPONSE_LEN) {
        ...
        AGENTRT_LOG_WARN("TC3-LLM: LLM response truncated (%zu -> %d)",
                         strlen(content), TC3_LLM_MAX_RESPONSE_LEN);
    }

    /* 复制响应文本（调用方负责释放） */
    char *text = (char *)AGENTRT_MALLOC(content_len + 1);
    if (!text) {
        AGENTRT_LOG_ERROR("TC3-LLM: OOM copying response (%zu bytes)", content_len + 1);
        llm_response_free(resp);
    ...
    *out_len = content_len;
    ...
    AGENTRT_LOG_DEBUG("TC3-LLM: LLM call success (%zu bytes, model=%s tokens=%u)",
 ...
 * 处理基本转义 (\" \\ \n \t)。
 *
 * @return 找到返回 1 并设置 *out_str (AGENTRT_MALLOC 分配)，否则返回 0
 */
                                ...
                                const char *field, char **out_str)
{
    ...
    while (q < end && *q != '"') {
        if (*q == '\\' && q + 1 < end) {
            ...
            if (pos + 1 >= buf_sz) {
                buf_sz *= 2;
                char *nb = (char *)AGENTRT_REALLOC(buf, buf_sz);
                if (!nb) {
                    AGENTRT_FREE(buf);
                    return 0;
                }
...
agentrt_error_t tc3_llm_s2_generate(const char *input, size_t input_len, char **output,
                                    size_t *output_len, void *user_data)
{
    if (!input || input_len == 0 || !output || !output_len) {
        AGENTRT_LOG_ERROR("TC3 s2_generate: invalid params (input=%p input_len=%zu "
    ...
    *output_len = 0;
    ...
    if (!ctx) {
        AGENTRT_LOG_ERROR("TC3 s2_generate: NULL user_data");
        return AGENTRT_EINVAL;
    }

    /* 首轮: 复用 seed text（避免重复 LLM 调用） */
    if (ctx->seed_text && ctx->seed_len > 0 && !ctx->seed_consumed) {
        char *seed_copy = (char *)AGENTRT_MALLOC(ctx->seed_len + 1);
        if (!seed_copy) {
            AGENTRT_LOG_ERROR("TC3 s2_generate: OOM copying seed (%zu bytes)", ctx->seed_len + 1);
            return AGENTRT_ENOMEM;
        }
        ...
        *output_len = ctx->seed_len;
        ...
        AGENTRT_LOG_INFO("TC3 s2_generate: returned seed text (%zu bytes, first call)",
                        ctx->seed_len);
        return AGENTRT_SUCCESS;
    }
    ...
    agentrt_error_t err = tc3_llm_call(ctx, S2_SYSTEM_PROMPT, input, ctx->s2_model,
                                        output, &prompt_len);
    if (err != AGENTRT_SUCCESS) {
        AGENTRT_LOG_WARN("TC3 s2_generate: LLM call failed (err=%d round=%d), "
    ...
    *output_len = prompt_len;
    AGENTRT_LOG_INFO("TC3 s2_generate: LLM generated %zu bytes (correction round)",
                    *output_len);
    return AGENTRT_SUCCESS;
}
...
agentrt_error_t tc3_llm_s1_verify(const char *content, size_t content_len, float *out_score,
                                  int *out_acceptable, char **out_critique,
                                  size_t *out_critique_len, void *user_data)
{
    if (!content || content_len == 0 || !out_score) {
        AGENTRT_LOG_ERROR("TC3 s1_verify: invalid params (content=%p content_len=%zu "
                         "out_score=%p)",
    ...
    *out_score = 0.5f;
    ...
    tc3_llm_ctx_t *ctx = (tc3_llm_ctx_t *)user_data;
    if (!ctx) {
        AGENTRT_LOG_ERROR("TC3 s1_verify: NULL user_data");
        return AGENTRT_EINVAL;
    }
    ...
    if (!prompt) {
        AGENTRT_LOG_ERROR("TC3 s1_verify: OOM building prompt (%zu bytes)", prompt_sz);
        return AGENTRT_ENOMEM;
    }
    ...
    if (written <= 0 || (size_t)written >= prompt_sz) {
        AGENTRT_LOG_ERROR("TC3 s1_verify: prompt encoding error (written=%d buf=%zu)",
                         written, prompt_sz);
        AGENTRT_FREE(prompt);
    ...
    agentrt_error_t err = tc3_llm_call(ctx, S1_VERIFY_SYSTEM_PROMPT, prompt, ctx->s1_verify_model,
                                       &llm_resp, &llm_resp_len);
    AGENTRT_FREE(prompt);

    if (err != AGENTRT_SUCCESS || !llm_resp || llm_resp_len == 0) {
         ...
         *
         * 返回 AGENTRT_SUCCESS 表示"验证已完成"（只是结果是不通过），
         * 调用方 verify_with_metacognition 会据此降级到 metacognition
         * 或 default_s1_verify 做进一步评估。 */
        AGENTRT_LOG_WARN("TC3 s1_verify: LLM unavailable (err=%d), applying conservative "
        ...
        *out_score = 0.3f;
        ...
        if (out_critique && out_critique_len) {
            ...
            char *fb_copy = (char *)AGENTRT_MALLOC(fb_len + 1);
            if (fb_copy) {
...
#ifdef AGENTRT_HAS_CJSON
        /* 用 cJSON 解析（更健壮） */
        char *json_copy = (char *)AGENTRT_MALLOC(json_len + 1);
        if (json_copy) {
            ...
            if (root) {
                ...
                if (j_critique && cJSON_IsString(j_critique) && j_critique->valuestring[0]) {
                    ...
                    critique = (char *)AGENTRT_MALLOC(cl + 1);
                    if (critique) {
    ...
    if (!parse_ok) {
        ...
        AGENTRT_LOG_WARN("TC3 s1_verify: JSON parse failed, raw response: %.200s", llm_resp);
        score = 0.5f; /* 保守回退 */
        acceptable = 0;
    } else {
    ...
    *out_score = score;
    ...
    if (out_critique && out_critique_len) {
        if (critique) {
            ...
            *out_critique_len = strlen(critique);
            ...
            if (fl > 0) {
                char *fb_copy = (char *)AGENTRT_MALLOC(fl + 1);
                if (fb_copy) {
    ...
    AGENTRT_LOG_INFO("TC3 s1_verify: score=%.2f acceptable=%d critique_len=%zu (parse_ok=%d)",
                    score, acceptable,
                    (out_critique && *out_critique) ? *out_critique_len : 0,
                    parse_ok);

    AGENTRT_FREE(llm_resp);
    return AGENTRT_SUCCESS;
}
...
agentrt_error_t tc3_llm_s1_expert(const char *content, size_t content_len, const char *critique,
                                  ...
                                  size_t *out_opinion_len, void *user_data)
{
    if (!content || content_len == 0 || !out_score || !out_verdict) {
        AGENTRT_LOG_ERROR("TC3 s1_expert: invalid params (content=%p content_len=%zu "
    ...
    *out_verdict = TC3_RESULT_MAJOR_FIX;
    ...
    tc3_llm_ctx_t *ctx = (tc3_llm_ctx_t *)user_data;
    if (!ctx) {
        AGENTRT_LOG_ERROR("TC3 s1_expert: NULL user_data");
        return AGENTRT_EINVAL;
    }
    ...
    char *prompt = (char *)AGENTRT_MALLOC(prompt_sz);
    if (!prompt) {
        AGENTRT_LOG_ERROR("TC3 s1_expert: OOM building prompt (%zu bytes)", prompt_sz);
        return AGENTRT_ENOMEM;
    }
    ...
    if (written <= 0 || (size_t)written >= prompt_sz) {
        AGENTRT_LOG_ERROR("TC3 s1_expert: prompt encoding error (written=%d buf=%zu)",
                         written, prompt_sz);
        AGENTRT_FREE(prompt);
        return AGENTRT_EUNKNOWN;
    }
    ...
    char *llm_resp = NULL;
    size_t llm_resp_len = 0;
    agentrt_error_t err = tc3_llm_call(ctx, S1_EXPERT_SYSTEM_PROMPT, prompt, ctx->s1_expert_model,
                                       &llm_resp, &llm_resp_len);
    AGENTRT_FREE(prompt);

    if (err != AGENTRT_SUCCESS || !llm_resp || llm_resp_len == 0) {
        /* LLM 不可用 — 回退到保守判决（MAJOR_FIX） */
        AGENTRT_LOG_WARN("TC3 s1_expert: LLM unavailable (err=%d), "
        ...
        if (json_copy) {
            ...
            if (root) {
                ...
                if (j_opinion && cJSON_IsString(j_opinion) && j_opinion->valuestring[0]) {
                    ...
                    opinion = (char *)AGENTRT_MALLOC(ol + 1);
                    if (opinion) {
    ...
    if (!parse_ok) {
        AGENTRT_LOG_WARN("TC3 s1_expert: JSON parse failed, raw response: %.200s", llm_resp);
        /* 保持保守默认 */
    }
    ...
    *out_verdict = verdict;
    ...
    AGENTRT_LOG_INFO("TC3 s1_expert: verdict=%d score=%.2f opinion_len=%zu (parse_ok=%d)",
                    (int)verdict, score,
