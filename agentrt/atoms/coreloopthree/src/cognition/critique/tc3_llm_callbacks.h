 ...
 * (tc3_coordinator) 负责释放。回调内部用 AGENTRT_MALLOC 分配。
 */

#ifndef AGENTRT_TC3_LLM_CALLBACKS_H
#define AGENTRT_TC3_LLM_CALLBACKS_H
 ...
 * @param output     [out] 生成的文本（AGENTRT_MALLOC 分配，调用方释放）
 ...
 * @param out_critique   [out] 批判意见（AGENTRT_MALLOC 分配，调用方释放）
 ...
 * @param out_expert_opinion [out] 专家意见（AGENTRT_MALLOC 分配，调用方释放）
...
#endif /* AGENTRT_TC3_LLM_CALLBACKS_H */
