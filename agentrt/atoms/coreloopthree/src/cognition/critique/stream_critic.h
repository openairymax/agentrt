...
#ifndef AGENTRT_STREAM_CRITIC_H
#define AGENTRT_STREAM_CRITIC_H
...
typedef struct sc_intent_result {
    ...
    agentrt_intent_type_t original_type;
    float confidence;
...
#define SC_CONFIG_DEFAULTS                                                                   \
    {                                                                                        \
        .accept_threshold = SC_ACCEPT_QUALITY, .high_quality_threshold = SC_HIGH_QUALITY,    \
        .max_corrections = 32, .max_memory_entries = 64, .enable_intent_classify = 1,        \
        .enable_stream_validate = 1, .enable_output_correct = 0, .enable_memory_confirm = 0, \
        .calibrator_config = {                                                               \
            .decay_factor = 0.85                                                             \
        }                                                                                    \
    }
...
AGENTRT_API agentrt_error_t sc_stream_critic_create(const sc_config_t *config,
                                                    sc_stream_critic_t **out_critic);

AGENTRT_API void sc_stream_critic_destroy(sc_stream_critic_t *critic);

AGENTRT_API agentrt_error_t sc_stream_critic_reset(sc_stream_critic_t *critic);

/* ==================== Phase 0: 意图分类 ==================== */

AGENTRT_API agentrt_error_t sc_intent_classifier(sc_stream_critic_t *critic, const char *input,
                                                 size_t input_len, sc_intent_result_t *out_result);

AGENTRT_API void sc_intent_result_free(sc_intent_result_t *result);

/* ==================== Phase 1: 流式验证 ==================== */

AGENTRT_API agentrt_error_t sc_stream_validator(sc_stream_critic_t *critic, const char *content,
                                                size_t content_len,
                                                const sc_intent_result_t *intent_context,
                                                uint32_t validate_aspects,
                                                sc_validation_result_t *out_result);

AGENTRT_API void sc_validation_result_free(sc_validation_result_t *result);

/* ==================== Phase 3: 输出纠错 ==================== */

AGENTRT_API agentrt_error_t sc_output_corrector(sc_stream_critic_t *critic, const char *raw_output,
                                                size_t raw_output_len,
                                                const sc_intent_result_t *intent_context,
                                                sc_correction_result_t *out_result);

AGENTRT_API void sc_correction_result_free(sc_correction_result_t *result);

/* ==================== Phase 4: 记忆确认 ==================== */

AGENTRT_API agentrt_error_t sc_memory_confirmer(sc_stream_critic_t *critic, const char *output,
                                                size_t output_len,
                                                const sc_intent_result_t *intent_context,
                                                agentrt_memory_engine_t *memory_engine,
                                                sc_memory_result_t *out_result);

AGENTRT_API void sc_memory_result_free(sc_memory_result_t *result);

/* ==================== 便捷全管线接口 ==================== */

AGENTRT_API agentrt_error_t sc_stream_critic_pipeline(
    sc_stream_critic_t *critic, const char *input, size_t input_len, const char *raw_output,
    size_t raw_output_len, agentrt_memory_engine_t *memory_engine, char **out_final_output,
    size_t *out_final_len, float *out_final_quality);
...
AGENTRT_API agentrt_error_t sc_stream_critic_get_stats(const sc_stream_critic_t *critic,
