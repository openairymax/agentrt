...
static sc_intent_category_t map_agentrt_type_to_sc(agentrt_intent_type_t at)
{
    switch (at) {
    case AGENTRT_INTENT_QUERY:
        return SC_INTENT_QUERY;
    case AGENTRT_INTENT_COMMAND:
        return SC_INTENT_TASK;
    case AGENTRT_INTENT_EXPLANATION:
        return SC_INTENT_ANALYSIS;
    case AGENTRT_INTENT_CREATION:
        return SC_INTENT_CREATE;
    case AGENTRT_INTENT_MODIFICATION:
        return SC_INTENT_MODIFY;
    case AGENTRT_INTENT_DELETION:
        return SC_INTENT_TASK;
    case AGENTRT_INTENT_CONFIRMATION:
        return SC_INTENT_META;
    case AGENTRT_INTENT_NEGATION:
        return SC_INTENT_META;
    case AGENTRT_INTENT_GREETING:
        return SC_INTENT_QUERY;
    case AGENTRT_INTENT_FAREWELL:
        return SC_INTENT_QUERY;
    default:
...
static float tokenize_and_compare(const char *a, size_t a_len, const char *b, size_t b_len)
{
    ...
    char *sa = (char *)AGENTRT_MALLOC(total_len);
    if (!sa)
        return 0.0f;
...
agentrt_error_t sc_stream_critic_create(const sc_config_t *config, sc_stream_critic_t **out_critic)
{
    if (!out_critic) {
        AGENTRT_LOG_ERROR("sc_stream_critic_create: NULL out_critic parameter");
        return AGENTRT_EINVAL;
    }

    sc_stream_critic_t *critic =
        (sc_stream_critic_t *)AGENTRT_CALLOC(1, sizeof(sc_stream_critic_t));
    if (!critic) {
        AGENTRT_LOG_ERROR("sc_stream_critic_create: allocation failed for stream critic");
    ...
    if (!critic->calibrator) {
        AGENTRT_LOG_ERROR("sc_stream_critic_create: calibrator creation failed");
        AGENTRT_FREE(critic);
        return AGENTRT_ENOMEM;
    }

    critic->accumulated_critique_cap = 1024;
    critic->accumulated_critique = (char *)AGENTRT_CALLOC(1, critic->accumulated_critique_cap);
    if (!critic->accumulated_critique) {
        AGENTRT_LOG_ERROR("sc_stream_critic_create: accumulated_critique allocation failed (cap=%zu)", critic->accumulated_critique_cap);
...
void sc_stream_critic_destroy(sc_stream_critic_t *critic)
{
    ...
    if (critic->last_accepted_content)
        AGENTRT_FREE(critic->last_accepted_content);
    if (critic->accumulated_critique)
        AGENTRT_FREE(critic->accumulated_critique);
    AGENTRT_FREE(critic);
}

agentrt_error_t sc_stream_critic_reset(sc_stream_critic_t *critic)
{
    if (!critic) {
        AGENTRT_LOG_ERROR("sc_stream_critic_reset: NULL critic parameter");
        return AGENTRT_EINVAL;
    }
    if (critic->last_accepted_content) {
        AGENTRT_FREE(critic->last_accepted_content);
...
agentrt_error_t sc_intent_classifier(sc_stream_critic_t *critic, const char *input,
                                     size_t input_len, sc_intent_result_t *out_result)
{
    if (!critic || !input || !out_result) {
        AGENTRT_LOG_ERROR("sc_intent_classifier: NULL params (critic=%p input=%p out_result=%p)", (void *)critic, (void *)input, (void *)out_result);
        return AGENTRT_EINVAL;
    }

    __builtin_memset(out_result, 0, sizeof(sc_intent_result_t));

    agentrt_intent_classification_t cls;
    int rc = agentrt_intent_classify(input, input_len, &cls);
    if (rc != 0) {
        AGENTRT_LOG_ERROR("sc_intent_classifier: intent classification failed (rc=%d input_len=%zu)", rc, input_len);
        return AGENTRT_EUNKNOWN;
    }

    sc_intent_category_t cat = map_agentrt_type_to_sc(cls.type);
    ...
    out_result->reasoning_hint = (char *)AGENTRT_MALLOC((size_t)hl + 1);
    ...
    if (result->extracted_keywords) {
        for (size_t i = 0; i < result->keyword_count; i++) {
            if (result->extracted_keywords[i])
                AGENTRT_FREE((void *)result->extracted_keywords[i]);
        }
        AGENTRT_FREE(result->extracted_keywords);
    }
    if (result->reasoning_hint)
        AGENTRT_FREE(result->reasoning_hint);
...
agentrt_error_t sc_stream_validator(sc_stream_critic_t *critic, const char *content,
                                    ...
                                    uint32_t validate_aspects, sc_validation_result_t *out_result)
{
    if (!critic || !content || !out_result) {
        AGENTRT_LOG_ERROR("sc_stream_validator: NULL params (critic=%p content=%p out_result=%p)", (void *)critic, (void *)content, (void *)out_result);
        return AGENTRT_EINVAL;
    }
    if (content_len == 0) {
        AGENTRT_LOG_ERROR("sc_stream_validator: empty content (content_len=0)");
    ...
    if (!out_result->is_acceptable) {
        ...
        AGENTRT_LOG_WARN("sc_stream_validator: content rejected (overall_score=%.2f threshold=%.2f content_len=%zu)", out_result->overall_score, critic->config.accept_threshold, content_len);
    }
    ...
    if (out_result->is_acceptable) {
        critic->last_accepted_content = (char *)AGENTRT_MALLOC(content_len + 1);
        if (critic->last_accepted_content) {
                    ...
                    sc_correction_result_t *out_result)
{
    if (!critic || !raw_output || !out_result) {
        AGENTRT_LOG_ERROR("sc_output_corrector: NULL params (critic=%p raw_output=%p out_result=%p)", (void *)critic, (void *)raw_output, (void *)out_result);
        return AGENTRT_EINVAL;
    }

    __builtin_memset(out_result, 0, sizeof(sc_correction_result_t));

    out_result->entries = (sc_correction_entry_t *)AGENTRT_CALLOC(critic->config.max_corrections,
                                                                  sizeof(sc_correction_entry_t));
    if (!out_result->entries) {
        AGENTRT_LOG_ERROR("sc_output_corrector: entries allocation failed (max_corrections=%u)", critic->config.max_corrections);
        return AGENTRT_ENOMEM;
    }
    ...
    if (detect_truncation(raw_output, raw_output_len, &candidate) &&
        out_result->entry_count < out_result->entries_capacity) {
        AGENTRT_LOG_DEBUG("output_corrector: possible truncation detected at end");
        out_result->entries[out_result->entry_count++] = candidate;
    }

    /* 构建最终输出 — 复制原文并移除已识别的重复段 */
    size_t final_cap = raw_output_len + 256;
    char *final_buf = (char *)AGENTRT_MALLOC(final_cap);
    if (!final_buf) {
        AGENTRT_LOG_ERROR("sc_output_corrector: final buffer allocation failed (cap=%zu)", final_cap);
        AGENTRT_FREE(out_result->entries);
...
void sc_correction_result_free(sc_correction_result_t *result)
{
    ...
    if (result->final_output)
        AGENTRT_FREE(result->final_output);
    if (result->entries)
        AGENTRT_FREE(result->entries);
    __builtin_memset(result, 0, sizeof(sc_correction_result_t));
}

/* ==================== Phase 4: memory_confirmer ==================== */

agentrt_error_t sc_memory_confirmer(sc_stream_critic_t *critic, const char *output,
                                    size_t output_len, const sc_intent_result_t *intent_context,
                                    agentrt_memory_engine_t *__attribute__((unused)) memory_engine,
                                    sc_memory_result_t *out_result)
{
    if (!critic || !output || !out_result) {
        AGENTRT_LOG_ERROR("sc_memory_confirmer: NULL params (critic=%p output=%p out_result=%p)", (void *)critic, (void *)output, (void *)out_result);
        return AGENTRT_EINVAL;
    }
    ...
    out_result->entries = (sc_memory_entry_t *)AGENTRT_CALLOC(critic->config.max_memory_entries,
                                                              sizeof(sc_memory_entry_t));
    if (!out_result->entries) {
        AGENTRT_LOG_ERROR("sc_memory_confirmer: entries allocation failed (max_memory_entries=%u)", critic->config.max_memory_entries);
        return AGENTRT_ENOMEM;
    }
...
agentrt_error_t sc_stream_critic_pipeline(sc_stream_critic_t *critic, const char *input,
                                          ...
                                          float *out_final_quality)
{
    if (!critic || !input || !raw_output || !out_final_output) {
        AGENTRT_LOG_ERROR("sc_stream_critic_pipeline: NULL params (critic=%p input=%p raw_output=%p out_final_output=%p)", (void *)critic, (void *)input, (void *)raw_output, (void *)out_final_output);
        return AGENTRT_EINVAL;
    }

    /* Phase 0 */
    sc_intent_result_t intent;
    agentrt_error_t err = sc_intent_classifier(critic, input, input_len, &intent);
    if (err != AGENTRT_SUCCESS) {
        AGENTRT_LOG_ERROR("sc_stream_critic_pipeline: Phase0 intent classification failed (err=%d)", (int)err);
        return err;
    }
    ...
    if (critic->config.enable_stream_validate) {
        ...
        if (!val.is_acceptable) {
            AGENTRT_LOG_WARN("stream_critic_pipeline: Phase1 validation below "
    ...
    if (err != AGENTRT_SUCCESS) {
        AGENTRT_LOG_ERROR("sc_stream_critic_pipeline: Phase3 output correction failed (err=%d raw_output_len=%zu)", (int)err, raw_output_len);
    ...
    if (critic->config.enable_memory_confirm && memory_engine && validation_ok) {
        ...
        agentrt_error_t mem_err = sc_memory_confirmer(critic, corr.final_output ? corr.final_output : raw_output,
                            corr.final_output ? corr.final_output_len : raw_output_len, &intent,
                            memory_engine, &mem);
        if (mem_err != AGENTRT_SUCCESS) {
            AGENTRT_LOG_WARN("sc_stream_critic_pipeline: Phase4 memory confirmation failed (err=%d)", (int)mem_err);
        }
    ...
    *out_final_len = corr.final_output_len;
    ...
    if (corr.entries)
        AGENTRT_FREE(corr.entries);

    if (!corr.final_output) {
        *out_final_output = (char *)AGENTRT_MALLOC(raw_output_len + 1);
        if (*out_final_output) {
...
agentrt_error_t sc_stream_critic_get_stats(const sc_stream_critic_t *critic,
                                           sc_critic_stats_t *out_stats)
{
    if (!critic || !out_stats) {
        AGENTRT_LOG_ERROR("sc_stream_critic_get_stats: NULL params (critic=%p out_stats=%p)", (void *)critic, (void *)out_stats);
        return AGENTRT_EINVAL;
    }
