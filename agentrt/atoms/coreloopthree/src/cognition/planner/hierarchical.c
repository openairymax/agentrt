...
static agentrt_error_t hierarchical_plan_func(const agentrt_intent_t __attribute__((unused)) *
                                                  intent,
                                              void *context, agentrt_task_plan_t **out_plan)
{
    if (!context || !out_plan)
    ...
    if (count > 0) {
        plan->task_plan_nodes =
            (agentrt_task_node_t **)AGENTRT_CALLOC(count, sizeof(agentrt_task_node_t *));
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            if (node) {
                node->task_node_id = AGENTRT_STRDUP(task_names[i]);
...
static void hierarchical_destroy(agentrt_plan_strategy_t *strategy)
{
    if (!strategy)
        return;
    if (strategy->data)
        AGENTRT_FREE(strategy->data);
    AGENTRT_FREE(strategy);
}

agentrt_plan_strategy_t *
agentrt_plan_hierarchical_create(agentrt_llm_service_t __attribute__((unused)) * llm, int max_depth)
{

    hierarchical_data_t *data =
        (hierarchical_data_t *)AGENTRT_CALLOC(1, sizeof(hierarchical_data_t));
    if (!data) return NULL;
