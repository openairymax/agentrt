...
static void ml_planner_destroy(agentrt_plan_strategy_t *strategy)
{
    if (!strategy)
        return;
    ...
    if (data) {
        if (data->model) {
            if (data->model->handle) {
                AGENTRT_LOG_INFO("ML planner: model handle %p released", data->model->handle);
                data->model->handle = NULL;
            }
            AGENTRT_FREE(data->model);
        }
        if (data->model_path)
            AGENTRT_FREE(data->model_path);
        if (data->lock)
...
static void ml_planner_destroy_single_node(agentrt_task_node_t *node);
...
static agentrt_error_t ml_planner_rule_based_plan(const agentrt_intent_t *intent,
                                                  agentrt_task_plan_t **out_plan)
{
    ...
    if (rule_index >= 0) {
        ...
        if (rule_index < (int)(sizeof(MULTI_GRAIN_TABLE) / sizeof(MULTI_GRAIN_TABLE[0])) &&
            ...
            AGENTRT_LOG_INFO("ML planner: using fine-grained decomposition (%zu steps)",
                             subtask_count);
        } else {
    ...
    if (!plan) {
        AGENTRT_LOG_ERROR("ML planner: plan allocation failed");
    ...
    SAFE_MALLOC_ARRAY(plan->task_plan_nodes, subtask_count + 2, sizeof(agentrt_task_node_t *));
    if (!plan->task_plan_nodes && subtask_count > 0) {
        AGENTRT_LOG_ERROR("ML planner: task_plan_nodes allocation failed (count=%zu)", subtask_count + 2);
    ...
    for (size_t i = 0; i < subtask_count; i++) {
        ...
        if (!node) {
            AGENTRT_LOG_ERROR("ML planner: node allocation failed at step %zu", i);
            goto cleanup_nodes;
        }
        ...
        node->task_node_id = AGENTRT_STRDUP(node_id);
        if (!node->task_node_id) {
            AGENTRT_LOG_ERROR("ML planner: node id STRDUP failed at step %zu", i);
            AGENTRT_FREE(node);
            goto cleanup_nodes;
        }
        node->task_node_agent_role = AGENTRT_STRDUP(primary_role);
        if (!node->task_node_agent_role) {
            AGENTRT_LOG_ERROR("ML planner: node role STRDUP failed at step %zu", i);
            ...
            if (in_parallel_group && (int)i > my_group_start) {
                ...
                if (node->task_node_depends_on) {
                    ...
                    node->task_node_depends_on[0] =
                        AGENTRT_STRDUP(plan->task_plan_nodes[my_group_start]->task_node_id);
                    if (!node->task_node_depends_on[0]) {
                        ...
                        for (size_t d = 1; d < dep_count; d++) {
                            node->task_node_depends_on[d] =
                                AGENTRT_STRDUP(plan->task_plan_nodes[d - 1]->task_node_id);
                ...
                if (node->task_node_depends_on) {
                    ...
                    node->task_node_depends_on[0] =
                        AGENTRT_STRDUP(plan->task_plan_nodes[i - 1]->task_node_id);
                    if (!node->task_node_depends_on[0]) {
                        /* SEC-09: STRDUP 失败时清理已分配资源 */
                        AGENTRT_FREE(node->task_node_depends_on);
                        node->task_node_depends_on = NULL;
    ...
    if (needs_verify && subtask_count > 0) {
        agentrt_task_node_t *verify_node;
        SAFE_MALLOC_ARRAY(verify_node, 1, sizeof(agentrt_task_node_t));
        if (verify_node) {
            char verify_id[128];
            snprintf(verify_id, sizeof(verify_id), "%s_verify", plan_id);
            verify_node->task_node_id = AGENTRT_STRDUP(verify_id);
            verify_node->task_node_agent_role = AGENTRT_STRDUP("verifier");
            ...
            if (verify_node->task_node_depends_on) {
                ...
                verify_node->task_node_depends_on[0] = AGENTRT_STRDUP(
                    plan->task_plan_nodes[plan->task_plan_node_count - 1]->task_node_id);
                if (!verify_node->task_node_depends_on[0]) {
                    /* SEC-09: STRDUP 失败时清理已分配资源 */
                    AGENTRT_FREE(verify_node->task_node_depends_on);
                    verify_node->task_node_depends_on = NULL;
                    verify_node->task_node_depends_count = 0;
                }
            }

            agentrt_task_node_t **expanded = (agentrt_task_node_t **)AGENTRT_REALLOC(
                plan->task_plan_nodes,
                (plan->task_plan_node_count + 1) * sizeof(agentrt_task_node_t *));
            if (expanded) {
    ...
    if (complexity >= 5 && subtask_count >= 4) {
        agentrt_task_node_t *qa_node;
        SAFE_MALLOC_ARRAY(qa_node, 1, sizeof(agentrt_task_node_t));
        if (qa_node) {
            char qa_id[128];
            snprintf(qa_id, sizeof(qa_id), "%s_quality_gate", plan_id);
            qa_node->task_node_id = AGENTRT_STRDUP(qa_id);
            qa_node->task_node_agent_role = AGENTRT_STRDUP("quality_assurance");
            ...
            if (qa_node->task_node_depends_on) {
                ...
                qa_node->task_node_depends_on[0] = AGENTRT_STRDUP(
                    plan->task_plan_nodes[plan->task_plan_node_count - 1]->task_node_id);
                if (!qa_node->task_node_depends_on[0]) {
                    /* SEC-09: STRDUP 失败时清理已分配资源 */
                    AGENTRT_FREE(qa_node->task_node_depends_on);
...
cleanup_nodes:
    for (size_t n = 0; n < plan->task_plan_node_count; n++) {
        if (plan->task_plan_nodes[n]) {
            AGENTRT_FREE(plan->task_plan_nodes[n]->task_node_id);
            AGENTRT_FREE(plan->task_plan_nodes[n]->task_node_agent_role);
            if (plan->task_plan_nodes[n]->task_node_depends_on) {
                for (size_t d = 0; d < plan->task_plan_nodes[n]->task_node_depends_count; d++)
                    AGENTRT_FREE(plan->task_plan_nodes[n]->task_node_depends_on[d]);
                AGENTRT_FREE(plan->task_plan_nodes[n]->task_node_depends_on);
            }
            AGENTRT_FREE(plan->task_plan_nodes[n]);
        }
    }
    AGENTRT_FREE(plan->task_plan_nodes);
    AGENTRT_FREE(plan->task_plan_id);
    AGENTRT_FREE(plan);
    AGENTRT_LOG_ERROR("ML planner: plan construction failed, cleaned up partial plan");
    ATM_RET_ERR(AGENTRT_ENOMEM);
}
...
static agentrt_error_t ml_planner_plan(const agentrt_intent_t *intent, void *context,
                                       agentrt_task_plan_t **out_plan)
{
...
agentrt_plan_strategy_t *agentrt_plan_ml_create(const char *model_path, void *llm)
{

    agentrt_plan_strategy_t *strat;
    SAFE_MALLOC_ARRAY(strat, 1, sizeof(agentrt_plan_strategy_t));
    ...
    data->model = NULL;
    data->model_path = model_path ? AGENTRT_STRDUP(model_path) : NULL;
    ...
    if (model_path) {
        ...
        data->rule_based_active = true;
        AGENTRT_LOG_INFO("ML planner: no model path, using rule-based planning");
    }
