...
static void reactive_destroy(agentrt_plan_strategy_t *strategy)
{
    if (!strategy)
        return;
...
static agentrt_error_t reactive_plan(const agentrt_intent_t *intent, void *context,
                                     agentrt_task_plan_t **out_plan)
{
    ...
    /* 尝试LLM快速规划 */
    char *llm_plan = NULL;
    if (data && data->llm && agentrt_llm_service_is_available(data->llm)) {
        char prompt[1024];
        snprintf(prompt, sizeof(prompt),
                 "Generate a brief action plan (max 5 steps) for: %s\n"
                 "Format: one action per line, no numbering.",
                 goal_len > 500 ? "(long input)" : goal);
        agentrt_error_t llm_err = agentrt_llm_service_call(data->llm, prompt, &llm_plan);
        if (llm_err != AGENTRT_SUCCESS || !llm_plan) {
            llm_plan = NULL;
        }
    }
    ...
    agentrt_task_plan_t *plan =
        (agentrt_task_plan_t *)AGENTRT_CALLOC(1, sizeof(agentrt_task_plan_t));
    if (!plan) {
        if (llm_plan)
            AGENTRT_FREE(llm_plan);
    ...
    plan->task_plan_nodes =
        (agentrt_task_node_t **)AGENTRT_CALLOC(node_count, sizeof(agentrt_task_node_t *));
    ...
    for (size_t i = 0; i < node_count; i++) {
        agentrt_task_node_t *node =
            (agentrt_task_node_t *)AGENTRT_CALLOC(1, sizeof(agentrt_task_node_t));
        if (!node)
            goto cleanup;
        ...
        if (llm_plan && i < node_count) {
            node->task_node_agent_role = AGENTRT_STRDUP("reactive-agent");
            ...
            node->task_node_agent_role = AGENTRT_STRDUP(rule->role);
            ...
            node->task_node_agent_role = AGENTRT_STRDUP(roles[i % 3]);
            node->task_node_timeout_ms = 15000;
            node->task_node_priority = 150 - (int)i * 10;
        }

        if (i > 0) {
            node->task_node_depends_on = (char **)AGENTRT_MALLOC(sizeof(char *));
    ...
    plan->task_plan_entry_points = (char **)AGENTRT_MALLOC(sizeof(char *));
...
agentrt_plan_strategy_t *agentrt_plan_reactive_create(agentrt_llm_service_t *llm)
{
    ...
    reactive_data_t *rdata = (reactive_data_t *)AGENTRT_CALLOC(1, sizeof(reactive_data_t));
    if (!rdata) {
        AGENTRT_FREE(strat);
    ...
    rdata->llm = llm;
    rdata->lock = agentrt_mutex_create();
    if (!rdata->lock) {
        AGENTRT_FREE(rdata);
        AGENTRT_FREE(strat);
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
