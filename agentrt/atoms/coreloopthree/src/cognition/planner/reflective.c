...
#include "strategy.h"  /* P4.8.1: agentrt_plan_reflective_create 声明 + agentrt_memory_engine_t 前向声明 */
...
typedef struct {
    ...
    agentrt_memory_engine_t *memory_engine;  /* P4.8.1: 记忆引擎（用于获取历史经验） */
    int initialized;
...
static agentrt_error_t llm_build_dynamic_plan(reflective_context_t *, const agentrt_intent_t *,
                                              agentrt_task_plan_t **, int, int);
static agentrt_task_plan_t *build_fallback_plan(const agentrt_intent_t *, reflective_context_t *,
                                                int, int, uint64_t);
...
static agentrt_error_t real_s2_generate(const char *input, size_t in_len, char **output,
                                        size_t *out_len, void *user_data)
{
    ...
    if (ctx && ctx->llm && agentrt_llm_service_is_available(ctx->llm)) {
        char *response = NULL;
        agentrt_error_t err = agentrt_llm_service_call(ctx->llm, input, &response);
...
static int real_s1_verify(const char *content, size_t len, float confidence, void *user_data)
{
    ...
    if (ctx && ctx->llm && agentrt_llm_service_is_available(ctx->llm)) {
        ...
        char *response = NULL;
        agentrt_error_t err = agentrt_llm_service_call(ctx->llm, prompt, &response);
        if (err == AGENTRT_SUCCESS && response) {
...
/* P4.8.1: 移除 __attribute__((unused))，此函数现在被 agentrt_plan_reflective_create 调用 */
static agentrt_error_t reflective_plan_init(void **out_context)
{
    if (!out_context)
        ATM_RET_ERR(AGENTRT_EINVAL);
...
static void reflective_plan_cleanup(agentrt_plan_strategy_t *strategy)
{
    if (!strategy)
        return;
    ...
    if (ctx) {
        if (ctx->chain) {
            agentrt_tc_chain_stop(ctx->chain);
            agentrt_tc_chain_destroy(ctx->chain);
            ctx->chain = NULL;
        }
        if (ctx->meta) {
            agentrt_mc_destroy(ctx->meta);
            ctx->meta = NULL;
        }
        if (ctx->last_goal) {
            AGENTRT_FREE(ctx->last_goal);
            ctx->last_goal = NULL;
        }
        AGENTRT_FREE(ctx);
    }
    /* P0.20.7: 释放 strategy 本身，与 reactive_destroy 约定保持一致
     * （reactive.c L86 AGENTRT_FREE(strategy)）。
...
static agentrt_error_t reflective_plan(const agentrt_intent_t *intent, void *context,
                                       agentrt_task_plan_t **out_plan)
{
    ...
    if (!ctx) {
        agentrt_error_t init_err = reflective_plan_init((void **)&ctx);
        if (init_err != AGENTRT_SUCCESS)
            return init_err;
    }

    if (!ctx->initialized) {
        agentrt_error_t err = agentrt_tc_chain_create(
            intent->intent_goal ? (const char *)intent->intent_goal : "reflective_session", 8192,
            64, &ctx->chain);
        if (err != AGENTRT_SUCCESS)
            return err;

        err = agentrt_mc_create(&ctx->meta);
        if (err != AGENTRT_SUCCESS) {
            agentrt_tc_chain_destroy(ctx->chain);
            ctx->chain = NULL;
            return err;
        ...
        agentrt_mc_set_chain(ctx->meta, ctx->chain);
        ctx->initialized = 1;
    }
    ...
    int glen = snprintf(goal_buf, sizeof(goal_buf), "%s_flags%u",
                        ...
                        intent->intent_flags);
    ctx->last_goal = AGENTRT_STRDUP(goal_buf);

    agentrt_tc_chain_start(ctx->chain);
    ctx->session_count++;

    /* ========== Phase 0: Instruction Decomposition (S1) ========== */
    agentrt_thinking_step_t *step_decomp = NULL;
    ...
    agentrt_tc_step_create(ctx->chain, TC_STEP_DECOMPOSITION, decomp_input, (size_t)di_len, NULL, 0,
                           &step_decomp);
    ...
    if (decomp_output && decomp_out_len > 0) {
        agentrt_tc_context_window_append(ctx->chain->ctx_window, decomp_output, decomp_out_len);
    }
    agentrt_tc_step_complete(step_decomp, decomp_output ? decomp_output : "decomposition_failed",
    ...
    agentrt_tc_context_window_get_recent(ctx->chain->ctx_window, 200, &recent_ctx, &recent_ctx_len);
    agentrt_mc_evaluate_step(ctx->meta, step_decomp, recent_ctx, recent_ctx_len, &eval_decomp);
    if (recent_ctx)
        AGENTRT_FREE(recent_ctx);

    if (eval_decomp.strategy == MC_CORRECT_AUTO || eval_decomp.strategy == MC_CORRECT_RERUN) {
        agentrt_mc_apply_correction(ctx->meta, step_decomp, &eval_decomp, real_s2_generate, ctx);
    }
    if (eval_decomp.critique_text)
        AGENTRT_FREE(eval_decomp.critique_text);
    if (decomp_output)
        AGENTRT_FREE(decomp_output);

    /* ========== Phase 1: Planning (S2+S1) ========== */
    agentrt_thinking_step_t *step_plan = NULL;
    ...
    agentrt_tc_step_create(ctx->chain, TC_STEP_PLANNING, plan_input, (size_t)pi_len, deps, 1,
                           &step_plan);
    ...
    if (plan_output && plan_out_len > 0) {
        agentrt_tc_context_window_append(ctx->chain->ctx_window, plan_output, plan_out_len);
    }
    agentrt_tc_step_complete(step_plan, plan_output ? plan_output : "planning_failed",
                             plan_output ? plan_out_len : 15, 0.70f, "S2-planner");

    mc_evaluation_result_t eval_plan;
    agentrt_tc_context_window_get_recent(ctx->chain->ctx_window, 300, &recent_ctx, &recent_ctx_len);
    agentrt_mc_evaluate_step(ctx->meta, step_plan, recent_ctx, recent_ctx_len, &eval_plan);
    if (recent_ctx)
        AGENTRT_FREE(recent_ctx);

    if (eval_plan.strategy == MC_CORRECT_AUTO || eval_plan.strategy == MC_CORRECT_RERUN) {
        agentrt_mc_apply_correction(ctx->meta, step_plan, &eval_plan, real_s2_generate, ctx);
    }
    if (eval_plan.critique_text)
        AGENTRT_FREE(eval_plan.critique_text);
    if (plan_output)
        AGENTRT_FREE(plan_output);

    /* ========== Phase 2: Execution-Verification Loop ========== */
    agentrt_thinking_step_t *step_exec = NULL;
    uint32_t exec_deps[] = {step_plan->step_id};
    agentrt_tc_step_create(ctx->chain, TC_STEP_GENERATION, plan_input, (size_t)pi_len, exec_deps, 1,
                           &step_exec);
    ...
    if (exec_output && exec_out_len > 0) {
        agentrt_tc_context_window_append(ctx->chain->ctx_window, exec_output, exec_out_len);
    }
    agentrt_tc_step_complete(step_exec, exec_output ? exec_output : "execution_failed",
    ...
    agentrt_tc_step_monitor(step_exec, NULL, &mon_result);
    if (mon_result.anomaly != TC_ANOMALY_NONE && mon_result.is_critical) {
        tc_recovery_result_t rec_result;
        agentrt_tc_step_recover(ctx->chain, step_exec, &mon_result, real_s2_generate, ctx,
                                &rec_result);
        if (rec_result.recovery_log)
            AGENTRT_FREE(rec_result.recovery_log);
    }
    ...
    agentrt_thinking_step_t *step_audit = NULL;
    uint32_t audit_deps[] = {step_exec->step_id};
    agentrt_tc_step_create(ctx->chain, TC_STEP_AUDIT, goal_buf, (size_t)glen, audit_deps, 1,
                           &step_audit);

    mc_evaluation_result_t eval_audit;
    agentrt_tc_context_window_get_recent(ctx->chain->ctx_window, 400, &recent_ctx, &recent_ctx_len);
    agentrt_mc_evaluate_step(ctx->meta, step_exec, recent_ctx, recent_ctx_len, &eval_audit);
    if (recent_ctx)
        AGENTRT_FREE(recent_ctx);
    ...
    agentrt_tc_step_complete(step_audit, audit_result, (size_t)ar_len, eval_audit.overall_score,
                             "S1-auditor");
    ...
    agentrt_thinking_step_t *step_align = NULL;
    uint32_t align_deps[] = {step_audit->step_id};
    agentrt_tc_step_create(ctx->chain, TC_STEP_ALIGNMENT, goal_buf, (size_t)glen, align_deps, 1,
                           &step_align);

    mc_evaluation_result_t eval_align;
    agentrt_mc_evaluate_step(ctx->meta, step_align, intent->intent_goal, intent->intent_goal_len,
                             &eval_align);

    int aligned = eval_align.is_acceptable;
    agentrt_tc_step_complete(step_align, aligned ? "goal_aligned" : "goal_drift_detected",
                             aligned ? 12 : 18, eval_align.overall_score, "S1-alignment");

    if (eval_align.critique_text)
        AGENTRT_FREE(eval_align.critique_text);

    /* DS-005: 学习模式检测 + 自适应阈值 */
    agentrt_mc_detect_patterns(ctx->meta, NULL, NULL);
    agentrt_mc_adapt_threshold(ctx->meta);

    /* ========== Build Output Plan (LLM Dynamic Planning) ========== */
    agentrt_task_plan_t *plan = NULL;
    agentrt_error_t plan_err = llm_build_dynamic_plan(ctx, intent, &plan, audit_passed, aligned);
    if (plan_err != AGENTRT_SUCCESS || !plan) {
...
static agentrt_error_t parse_llm_plan_json(const char *json_text, llm_plan_node_t *nodes,
                                           size_t *node_count, size_t max_nodes)
{
    ...
    *node_count = 0;
    ...
    while (*p && *node_count < max_nodes) {
        ...
        AGENTRT_MEMCPY_SAFE(node->name, name_val, nlen, sizeof(node->name));
        ...
        if (role_val) {
            ...
            AGENTRT_MEMCPY_SAFE(node->role, role_val, rlen, sizeof(node->role));
        } else {
            ...
            if (dep_arr) {
                ...
                while (*dep && *dep != ']' && node->depends_on_count < 4) {
                    ...
                    if (dlen > 0 && dlen < sizeof(node->depends_on[0])) {
                        AGENTRT_MEMCPY_SAFE(node->depends_on[node->depends_on_count], dep, dlen, sizeof(node->depends_on[0]));
    ...
    return (*node_count > 0) ? AGENTRT_SUCCESS : AGENTRT_EINVAL;
}

static void reflective_destroy_single_node(agentrt_task_node_t *node)
{
    if (!node)
        return;
    ...
    if (node->task_node_depends_on) {
        for (size_t d = 0; d < node->task_node_depends_count; d++)
            AGENTRT_FREE(node->task_node_depends_on[d]);
        AGENTRT_FREE(node->task_node_depends_on);
    }
    AGENTRT_FREE(node);
}

static agentrt_error_t llm_build_dynamic_plan(reflective_context_t *ctx,
                                              const agentrt_intent_t *intent,
                                              agentrt_task_plan_t **out_plan, int audit_passed,
                                              int aligned)
{
    ...
    if (!ctx->llm || !agentrt_llm_service_is_available(ctx->llm)) {
        ATM_RET_ERR(AGENTRT_ESERVICE);
    }
    ...
    if (err != AGENTRT_SUCCESS || parsed_count == 0) {
        return err;
    }

    agentrt_task_plan_t *plan;
    SAFE_MALLOC_ARRAY(plan, 1, sizeof(agentrt_task_plan_t));
    if (!plan) {
        AGENTRT_LOG_ERROR("reflective: plan allocation failed");
        ATM_RET_ERR(AGENTRT_ENOMEM);
    }

    char plan_id[128];
    snprintf(plan_id, sizeof(plan_id), "llm_dynamic_%zu", ctx->session_count);
    plan->task_plan_id = AGENTRT_STRDUP(plan_id);
    if (!plan->task_plan_id) {
        AGENTRT_LOG_ERROR("reflective: plan id allocation failed");
        AGENTRT_FREE(plan);
        ATM_RET_ERR(AGENTRT_ENOMEM);
    }
    ...
    SAFE_MALLOC_ARRAY(plan->task_plan_nodes, total, sizeof(agentrt_task_node_t *));
    if (!plan->task_plan_nodes && total > 0) {
        AGENTRT_LOG_ERROR("reflective: task_plan_nodes allocation failed (count=%zu)", total);
    ...
    for (size_t i = 0; i < parsed_count; i++) {
        agentrt_task_node_t *node;
        SAFE_MALLOC_ARRAY(node, 1, sizeof(agentrt_task_node_t));
        if (!node) {
            AGENTRT_LOG_WARN("reflective: node allocation failed at step %zu, skipping", i);
            continue;
        }
        ...
        node->task_node_id = AGENTRT_STRDUP(nid);
        if (!node->task_node_id) {
            AGENTRT_LOG_WARN("reflective: node id STRDUP failed at step %zu, skipping", i);
            reflective_destroy_single_node(node);
            continue;
        }
        node->task_node_agent_role =
            AGENTRT_STRDUP(parsed_nodes[i].role[0] ? parsed_nodes[i].role : "worker");
        if (!node->task_node_agent_role) {
            AGENTRT_LOG_WARN("reflective: node role STRDUP failed at step %zu, skipping", i);
            reflective_destroy_single_node(node);
            continue;
        }
        ...
        if (parsed_nodes[i].depends_on_count > 0) {
            ...
            if (node->task_node_depends_on) {
                ...
                for (int d = 0; d < parsed_nodes[i].depends_on_count; d++) {
                    ...
                    node->task_node_depends_on[d] = AGENTRT_STRDUP(dep_id);
                    if (!node->task_node_depends_on[d]) {
                        /* SEC-09: STRDUP 失败时清理已分配的所有依赖项 */
                        for (int e = 0; e < d; e++)
                            AGENTRT_FREE(node->task_node_depends_on[e]);
                        AGENTRT_FREE(node->task_node_depends_on);
    ...
    if (!audit_passed) {
        agentrt_task_node_t *cn;
        SAFE_MALLOC_ARRAY(cn, 1, sizeof(agentrt_task_node_t));
        if (cn) {
            char cid[256];
            snprintf(cid, sizeof(cid), "%s_correction", plan_id);
            cn->task_node_id = AGENTRT_STRDUP(cid);
            cn->task_node_agent_role = AGENTRT_STRDUP("corrector");
    ...
    if (!aligned) {
        agentrt_task_node_t *rn;
        SAFE_MALLOC_ARRAY(rn, 1, sizeof(agentrt_task_node_t));
        if (rn) {
            char rid[256];
            snprintf(rid, sizeof(rid), "%s_realignment", plan_id);
            rn->task_node_id = AGENTRT_STRDUP(rid);
            rn->task_node_agent_role = AGENTRT_STRDUP("realigner");
            ...
            if (plan->task_plan_node_count > 2) {
                ...
                if (rn->task_node_depends_on) {
                    ...
                    rn->task_node_depends_on[0] = AGENTRT_STRDUP(
                        plan->task_plan_nodes[plan->task_plan_node_count - 1]->task_node_id);
                    if (!rn->task_node_depends_on[0]) {
                        AGENTRT_FREE(rn->task_node_depends_on);
    ...
    if (plan->task_plan_entry_points && plan->task_plan_node_count > 0) {
        ...
        plan->task_plan_entry_points[0] = AGENTRT_STRDUP(plan->task_plan_nodes[0]->task_node_id);
    }
...
static agentrt_task_plan_t *build_fallback_plan(const agentrt_intent_t *intent,
                                                ...
                                                int aligned, uint64_t session_count)
{
    ...
    plan->task_plan_id = AGENTRT_STRDUP(plan_id);
    if (!plan->task_plan_id) {
        AGENTRT_LOG_ERROR("reflective: fallback plan id allocation failed");
        AGENTRT_FREE(plan);
        return NULL;
    }

    size_t nc = 5 + (audit_passed ? 0 : 1) + (aligned ? 0 : 1);
    SAFE_MALLOC_ARRAY(plan->task_plan_nodes, nc, sizeof(agentrt_task_node_t *));
    ...
    for (int s = 0; s < 5; s++) {
        ...
        SAFE_MALLOC_ARRAY(node, 1, sizeof(agentrt_task_node_t));
        if (!node) {
            AGENTRT_LOG_WARN("reflective: fallback node allocation failed at step %d", s);
            break;
        }
        ...
        node->task_node_id = AGENTRT_STRDUP(nid);
        if (!node->task_node_id) {
            AGENTRT_LOG_WARN("reflective: fallback node id STRDUP failed at step %d", s);
            AGENTRT_FREE(node);
            break;
        }
        node->task_node_agent_role = AGENTRT_STRDUP(defaults[s].role);
        if (!node->task_node_agent_role) {
            AGENTRT_LOG_WARN("reflective: fallback node role STRDUP failed at step %d", s);
            AGENTRT_FREE(node->task_node_id);
            AGENTRT_FREE(node);
            break;
        }
        ...
        if (s > 0) {
            ...
            if (node->task_node_depends_on) {
                ...
                node->task_node_depends_on[0] =
                    AGENTRT_STRDUP(plan->task_plan_nodes[s - 1]->task_node_id);
            }
        }
        plan->task_plan_nodes[plan->task_plan_node_count++] = node;
    }
...
/* P4.8.1 (ACC-DT32): 实现 agentrt_plan_reflective_create 工厂函数，
 * 将 reflective 规划策略接入主流程，消除死代码。
 * 原 g_reflective_strategy 全局变量未被任何代码引用，属于死代码，已删除。
 *
 * 使用方式（参照 agentrt_plan_reactive_create）：
 *   agentrt_plan_strategy_t *strat = agentrt_plan_reflective_create(llm, memory_engine);
 *   agentrt_cognition_set_plan_strategy(cognition, strat);
 *
 * 参数:
...
{
    agentrt_plan_strategy_t *strat =
        (agentrt_plan_strategy_t *)AGENTRT_CALLOC(1, sizeof(agentrt_plan_strategy_t));
    if (!strat)
        return NULL;

    reflective_context_t *ctx = NULL;
    if (reflective_plan_init((void **)&ctx) != AGENTRT_SUCCESS) {
        AGENTRT_FREE(strat);
        AGENTRT_LOG_ERROR("reflective: reflective_plan_init failed");
        return NULL;
    }
    ...
    AGENTRT_LOG_INFO("reflective: strategy created (llm=%p, memory_engine=%p)",
