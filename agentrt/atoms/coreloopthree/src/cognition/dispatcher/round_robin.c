 ...
 * @brief 轮询调度策略内部结构
 */
struct agentrt_round_robin_dispatch {
    void *registry_ctx;
 ...
 * @brief 销毁轮询调度策略实例
 */
static void rr_destroy(agentrt_dispatching_strategy_t *strategy)
{
    if (!strategy)
        return;
    struct agentrt_round_robin_dispatch *rr = (struct agentrt_round_robin_dispatch *)strategy->data;
    if (rr) {
        agentrt_mutex_free(rr->lock);
        AGENTRT_FREE(rr);
    }
 ...
 * @param count [in] 候选数量
 * @param context [in] 策略上下文（agentrt_round_robin_dispatch*）
 * @param out_agent_id [out] 选中的Agent ID
 * @return AGENTRT_SUCCESS 成功
 */
static agentrt_error_t rr_select(const agentrt_task_node_t __attribute__((unused)) * task,
                                 const void **candidates, size_t count, void *context,
                                 char **out_agent_id)
{

    struct agentrt_round_robin_dispatch *rr = (struct agentrt_round_robin_dispatch *)context;
    if (!rr || !out_agent_id)
    ...
    size_t next_index;
    agentrt_mutex_lock(rr->lock);
    next_index = (rr->last_index + 1) % agent_count;
    rr->last_index = next_index;
    agentrt_mutex_unlock(rr->lock);

    if (agents[next_index]) {
        *out_agent_id = AGENTRT_STRDUP(agents[next_index]->agent_id);
                                       ...
                                       agent_registry_get_agents_func get_agents_func)
{
    ...
    struct agentrt_round_robin_dispatch *rr =
        (struct agentrt_round_robin_dispatch *)AGENTRT_CALLOC(1, sizeof(*rr));
    if (!rr) {
