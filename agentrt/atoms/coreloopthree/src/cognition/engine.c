...
struct agentrt_cognition_engine {
    agentrt_plan_strategy_t *plan_strat;
    agentrt_plan_strategy_t *fallback_plan_strat;
    agentrt_coordinator_strategy_t *coord_strat;
    agentrt_dispatching_strategy_t *disp_strat;
    void *context;
    void (*context_destroy)(void *);
    agentrt_mutex_t *lock;
    uint32_t stats_processed;
    uint64_t stats_total_time_ns;
    agentrt_cognition_config_t manager;
    agentrt_feedback_callback_t feedback_cb;
    ...
    agentrt_task_plan_t *current_plan; /**< Currently processing plan (borrowed, for checkpoint) */

    /* C-L12: MemoryRovol → CoreLoopThree — memory provider from bridge */
    agentrt_memory_provider_t *memory_provider;
...
static void trigger_feedback(agentrt_cognition_engine_t *engine, int level, const char *event,
                             const char *data)
{
...
static int cognition_provider_write(agentrt_cognition_engine_t *engine, const char *phase,
                                    const char *content, size_t content_len)
{
     ...
     * 其他决策 → 正常写入（仅记录/审计）。 */
    {
        ...
        if (hd == HOOK_DECISION_ABORT) {
            AGENTRT_LOG_INFO("C-L09: ON_MEMORY_EVOLVE hook ABORT — skipping memory write (phase=%s)",
                             phase ? phase : "unknown");
            return 0;
        }
        if (hd == HOOK_DECISION_RETRY) {
            AGENTRT_LOG_DEBUG("C-L09: ON_MEMORY_EVOLVE hook RETRY — degraded to CONTINUE (P4)");
        }
    }
    ...
    char *record_id = NULL;
    agentrt_error_t err = engine->memory_provider->write_raw(
        engine->memory_provider, content, content_len, metadata, &record_id);
    if (err == AGENTRT_SUCCESS && record_id) {
        AGENTRT_LOG_DEBUG("C-L12: Provider write phase=%s record_id=%s", phase, record_id);
...
agentrt_error_t agentrt_cognition_create_take(agentrt_plan_strategy_t *plan_strategy,
...
{
    return agentrt_cognition_create_ex_take(NULL, plan_strategy, coord_strategy, disp_strategy,
                                            out_engine);
}

/* _take: caller transfers ownership */
agentrt_error_t agentrt_cognition_create_ex_take(const agentrt_cognition_config_t *manager,
                                                 ...
                                                 agentrt_cognition_engine_t **out_engine)
{
    ...
    agentrt_cognition_engine_t *engine =
        (agentrt_cognition_engine_t *)AGENTRT_CALLOC(1, sizeof(agentrt_cognition_engine_t));
    if (!engine) {
        AGENTRT_LOG_ERROR("Failed to allocate cognition engine");
    ...
    agentrt_error_t ds_err = agentrt_mc_create(&engine->meta);
    if (ds_err != AGENTRT_SUCCESS) {
        AGENTRT_LOG_WARN("Metacognition init failed: err=%d, dual-thinking disabled", (int)ds_err);
    ...
    agentrt_error_t sc_err = sc_stream_critic_create(&sc_cfg, &engine->stream_critic);
    if (sc_err != AGENTRT_SUCCESS) {
        AGENTRT_LOG_WARN("Stream critic init failed: err=%d, proceeding without critic",
                         (int)sc_err);
    ...
    /* C-L01: Auto-load agentrt.yaml global config if not already loaded */
    if (agentrt_config_get_global() == NULL) {
        int cfg_ret = agentrt_config_init(NULL);
        if (cfg_ret == 0) {
            const agentrt_yaml_config_t *yaml_cfg = agentrt_config_get_global();
            if (yaml_cfg) {
                ...
                AGENTRT_LOG_INFO("C-L01: Config loaded from agentrt.yaml"
            ...
            AGENTRT_LOG_WARN("C-L01: Failed to load agentrt.yaml, using defaults");
        }
    }
    ...
    *out_engine = engine;
    ...
    if (agentrt_hook_init() != 0) {
        AGENTRT_LOG_WARN("C-L09: hook_init failed — hooks disabled (non-fatal)");
    } else {
        AGENTRT_LOG_INFO("C-L09: hook system initialized for cognition engine");
    }
...
void agentrt_cognition_destroy(agentrt_cognition_engine_t *engine)
{
    ...
    /* P3.18 (ACC-DT26): 销毁 hook 系统 — 与 create 中的 agentrt_hook_init 配对。
     * hook_shutdown 幂等，重复调用安全。 */
    /* P0.20.1: 先注销内置 handler，再销毁 hook 系统（顺序正确避免悬挂指针）。 */
    agentrt_hook_unregister_builtin_handlers();
    agentrt_hook_shutdown();

    if (engine->chain) {
        agentrt_tc_chain_stop(engine->chain);
        agentrt_tc_chain_destroy(engine->chain);
    }
    if (engine->meta) {
        agentrt_mc_destroy(engine->meta);
    }
...
void agentrt_cognition_set_fallback_plan(agentrt_cognition_engine_t *engine,
...
void agentrt_cognition_set_plan_strategy(agentrt_cognition_engine_t *engine,
                                          agentrt_plan_strategy_t *strategy)
{
    if (!engine)
        return;
    agentrt_mutex_lock(engine->lock);
    ...
    agentrt_mutex_unlock(engine->lock);
    if (strategy) {
        AGENTRT_LOG_INFO("P1.14: plan_strategy injected into cognition engine (plan=%p destroy=%p)",
                         (void *)strategy->plan, (void *)strategy->destroy);
    } else {
        AGENTRT_LOG_WARN("P1.14: plan_strategy detached from cognition engine");
    }
}

/* P1.14: 协同策略 setter — 替换旧策略（TRANSFER ownership） */
void agentrt_cognition_set_coord_strategy(agentrt_cognition_engine_t *engine,
                                           agentrt_coordinator_strategy_t *strategy)
{
    ...
    agentrt_mutex_unlock(engine->lock);
    if (strategy) {
        AGENTRT_LOG_INFO("P1.14: coord_strategy injected into cognition engine (coordinate=%p)",
                         (void *)strategy->coordinate);
    } else {
        AGENTRT_LOG_WARN("P1.14: coord_strategy detached from cognition engine");
    }
}

/* P1.14: 调度策略 setter — 替换旧策略（TRANSFER ownership） */
void agentrt_cognition_set_disp_strategy(agentrt_cognition_engine_t *engine,
                                          agentrt_dispatching_strategy_t *strategy)
{
    ...
    agentrt_mutex_unlock(engine->lock);
    if (strategy) {
        AGENTRT_LOG_INFO("P1.14: disp_strategy injected into cognition engine (dispatch=%p)",
                         (void *)strategy->dispatch);
    } else {
        AGENTRT_LOG_WARN("P1.14: disp_strategy detached from cognition engine");
    }
}

/* _take: caller transfers ownership */
void agentrt_cognition_set_context_take(agentrt_cognition_engine_t *engine, void *context,
                                        void (*destroy)(void *))
{
    if (!engine)
        return;
...
void agentrt_cognition_set_memory(agentrt_cognition_engine_t *engine,
                                  agentrt_memory_engine_t *memory)
{
    if (!engine)
        return;
    ...
    if (engine->chain) {
        agentrt_tc_chain_set_memory(engine->chain, memory);
    }
    agentrt_mutex_unlock(engine->lock);
}

/* C-L12: MemoryRovol → CoreLoopThree */
void agentrt_cognition_set_memory_provider(agentrt_cognition_engine_t *engine,
                                           agentrt_memory_provider_t *provider)
{
    ...
    agentrt_mutex_unlock(engine->lock);
    if (provider) {
        AGENTRT_LOG_INFO("C-L12: Memory provider attached to cognition engine (%s v%s)",
                         provider->name ? provider->name : "unknown",
                         provider->version ? provider->version : "?");
    } else {
        AGENTRT_LOG_INFO("C-L12: Memory provider detached from cognition engine");
    }
}

/* C-L02: llm_d → CoreLoopThree */
void agentrt_cognition_set_llm_service(agentrt_cognition_engine_t *engine,
                                        llm_service_t *llm_svc)
{
    if (!engine)
        return;
    ...
    agentrt_mutex_unlock(engine->lock);
    if (llm_svc) {
        AGENTRT_LOG_INFO("C-L02: LLM service attached to cognition engine (direct)");
    } else {
        AGENTRT_LOG_INFO("C-L02: LLM service detached from cognition engine");
    }
}

/* C-L02: llm_d → CoreLoopThree — IPC adapter (P1.2.1 preferred path) */
void agentrt_cognition_set_llm_adapter(agentrt_cognition_engine_t *engine,
                                        llm_svc_adapter_t *adapter)
{
    ...
    agentrt_mutex_unlock(engine->lock);
    if (adapter) {
        AGENTRT_LOG_INFO("C-L02: LLM IPC adapter attached to cognition engine"
                         " (requests=%llu errors=%llu)",
                         (unsigned long long)0, (unsigned long long)0);
    } else {
        AGENTRT_LOG_INFO("C-L02: LLM IPC adapter detached from cognition engine");
    }
}

/* C-L02: Streaming LLM callback support (P1.2.2 async callback) */
void agentrt_cognition_set_llm_streaming(agentrt_cognition_engine_t *engine,
                                         int enabled,
                                         ...
                                         void *user_data)
{
    ...
    agentrt_mutex_unlock(engine->lock);
    if (enabled) {
        AGENTRT_LOG_INFO("C-L02: Streaming LLM mode enabled (callback=%p)", (void *)(uintptr_t)callback);
    } else {
        AGENTRT_LOG_INFO("C-L02: Streaming LLM mode disabled");
    }
}

/* C-L04: tool_d → CoreLoopThree */
void agentrt_cognition_set_tool_service(agentrt_cognition_engine_t *engine,
                                         tool_service_t *tool_svc)
{
    if (!engine)
        return;
    ...
    agentrt_mutex_unlock(engine->lock);
    if (tool_svc) {
        AGENTRT_LOG_INFO("C-L04: Tool service attached to cognition engine (direct)");
    } else {
        AGENTRT_LOG_INFO("C-L04: Tool service detached from cognition engine");
    }
}

/* C-L04: tool_d → CoreLoopThree — IPC adapter (P1.3.1 preferred path) */
void agentrt_cognition_set_tool_adapter(agentrt_cognition_engine_t *engine,
                                         tool_svc_adapter_t *adapter)
{
    ...
    agentrt_mutex_unlock(engine->lock);
    if (adapter) {
        AGENTRT_LOG_INFO("C-L04: Tool IPC adapter attached to cognition engine");
    } else {
        AGENTRT_LOG_INFO("C-L04: Tool IPC adapter detached from cognition engine");
    }
}

tool_service_t *agentrt_cognition_get_tool_service(agentrt_cognition_engine_t *engine)
{
    if (!engine)
        return NULL;
...
void agentrt_cognition_enable_checkpoint(agentrt_cognition_engine_t *engine,
                                          int enable, const char *session_id)
{
    ...
    if (session_id && enable) {
        AGENTRT_STRNCPY_TERM(engine->checkpoint_session_id, session_id,
    ...
    agentrt_mutex_unlock(engine->lock);
    AGENTRT_LOG_INFO("C-L07: Checkpoint auto-save %s (session=%s)",
                     enable ? "enabled" : "disabled",
                     enable && session_id ? session_id : "N/A");
}

int agentrt_cognition_save_checkpoint(agentrt_cognition_engine_t *engine,
                                       uint64_t sequence_num, const char *phase_name)
{
    ...
    AGENTRT_STRNCPY_TERM(session_id, engine->checkpoint_session_id, sizeof(session_id));
    agentrt_mutex_unlock(engine->lock);
    ...
    if (sj_len < 0 || (size_t)sj_len >= sizeof(state_json)) {
        AGENTRT_LOG_WARN("C-L07: Checkpoint state JSON truncated");
    }
    ...
    if (plan && plan->task_plan_nodes && plan->task_plan_node_count > 0) {
        ...
        pending_nodes = (char **)AGENTRT_CALLOC(pending_count, sizeof(char *));
        if (pending_nodes) {
    ...
    agentrt_error_t cp_err =
        agentrt_checkpoint_create(plan_id, session_id, sequence_num, state_json,
    ...
    if (cp_err != AGENTRT_SUCCESS || !cp) {
        AGENTRT_LOG_WARN("C-L07: Failed to create checkpoint: err=%d phase=%s seq=%llu",
    ...
    agentrt_error_t save_err = agentrt_checkpoint_save(cp);
    agentrt_checkpoint_destroy(cp);

    if (save_err == AGENTRT_SUCCESS) {
        AGENTRT_LOG_INFO("C-L07: Checkpoint saved: phase=%s seq=%llu plan=%s",
                         phase_name, (unsigned long long)sequence_num, plan_id);
        return 0;
    } else {
        AGENTRT_LOG_WARN("C-L07: Failed to save checkpoint: err=%d phase=%s",
 ...
 *
 * 构造 hook_context_t 并调用 agentrt_hook_trigger，返回聚合决策。
                                           ...
                                           size_t input_data_len)
{
    ...
    return agentrt_hook_trigger(&ctx);
}

agentrt_error_t agentrt_cognition_process(agentrt_cognition_engine_t *engine, const char *input,
                                          size_t input_len, agentrt_task_plan_t **out_plan)
{

    if (!engine || !input || !out_plan) {
        AGENTRT_LOG_ERROR("Invalid parameters to cognition_process: engine=%p input=%p out_plan=%p",
    ...
    agentrt_task_plan_t *plan = NULL;
    agentrt_error_t err = AGENTRT_EUNKNOWN;
     ...
     * ABORT → 直接进入失败路径；CONTINUE/MODIFY/RETRY → 正常继续。 */
    {
        hook_decision_t hd = cognition_fire_hook(HOOK_TYPE_PRE_EXEC, "cognition_process",
                                                  input, input_len);
        if (hd == HOOK_DECISION_ABORT) {
            AGENTRT_LOG_WARN("C-L09: PRE_EXEC hook ABORT — skipping cognition process");
            goto process_fail;
        }
        if (hd == HOOK_DECISION_RETRY) {
            AGENTRT_LOG_DEBUG("C-L09: PRE_EXEC hook RETRY — degraded to CONTINUE (P4 enhancement)");
        }
    }
     ...
     * agentrt_tc_context_window_prepopulate → agentrt_memory_query → provider->query）。
     *
    ...
    if (engine->enable_dual_thinking && engine->meta) {
        ...
        agentrt_error_t tc_err = agentrt_tc_chain_create(input, engine->chain_max_tokens,
                                                         engine->chain_wm_capacity, &engine->chain);
        if (tc_err == AGENTRT_SUCCESS) {
            agentrt_tc_chain_start(engine->chain);
            agentrt_mc_set_chain(engine->meta, engine->chain);

            if (engine->memory) {
                agentrt_tc_chain_set_memory(engine->chain, engine->memory);
                agentrt_tc_context_window_prepopulate(engine->chain, input, input_len, 5);
            }

            agentrt_thinking_step_t *decomp_step = NULL;
            agentrt_tc_step_create(engine->chain, TC_STEP_DECOMPOSITION, input, input_len, NULL, 0,
                                   &decomp_step);
            ...
            if (decomp_llm_adapter || decomp_llm_svc) {
                ...
                if (decomp_ret == 0 && decomp_resp && decomp_resp->choices &&
                    ...
                    decomp_output = AGENTRT_STRDUP(decomp_resp->choices[0].content);
                    if (decomp_output) {
                        decomp_output_len = strlen(decomp_output);
                        decomp_confidence = 0.85f;
                    }
                    AGENTRT_LOG_INFO("Phase 0: LLM decomposition succeeded (len=%zu)",
                                     decomp_output_len);
                } else {
                    AGENTRT_LOG_WARN("Phase 0: LLM decomposition failed (ret=%d), using fallback",
                                     decomp_ret);
                }
                if (decomp_resp) {
            ...
            if (!decomp_output) {
                ...
                decomp_output = (char *)AGENTRT_MALLOC(fb_sz);
                if (decomp_output) {
            ...
            if (decomp_step && decomp_output) {
                agentrt_tc_step_complete(decomp_step, decomp_output, decomp_output_len,
                                         decomp_confidence, decomp_agent);
                /* 存入 working memory 供后续 Phase 1 规划访问 */
                if (engine->chain->working_mem) {
                    agentrt_tc_working_memory_store(engine->chain->working_mem,
            ...
            size_t hint_len = 0;
            int preempt = agentrt_mc_preemptive_check(engine->meta, TC_STEP_PLANNING, input,
                                                      input_len, &preemptive_hint, &hint_len);
            if (preempt == 1 && preemptive_hint) {
                if (engine->chain->working_mem) {
                    agentrt_tc_working_memory_store(engine->chain->working_mem, "preemptive_hint",
                                                    preemptive_hint, hint_len + 1, "text/plain", 1);
                }
                AGENTRT_FREE(preemptive_hint);
                preemptive_hint = NULL;
            }
        } else {
            AGENTRT_LOG_WARN("Thinking chain creation failed: err=%d", (int)tc_err);
        }
    ...
    if (engine->checkpoint_enabled) {
        agentrt_cognition_save_checkpoint(engine, engine->checkpoint_last_seq++, "decomposition");
    }

    /* ========== Phase 1: Planning (S2 + S1 pre-validation) ========== */
    plan = NULL;                /* P3.18 fix: 重置 — 已在函数顶部声明 */
    err = AGENTRT_EUNKNOWN;     /* P3.18 fix: 重置 — 已在函数顶部声明 */

    agentrt_plan_strategy_t *plan_strat = NULL;
    agentrt_plan_strategy_t *fallback_strat = NULL;
    agentrt_mutex_lock(engine->lock);
    ...
    if (err == AGENTRT_SUCCESS && plan && engine->enable_dual_thinking && engine->meta &&
        engine->chain) {
        agentrt_thinking_step_t *plan_step = NULL;
        agentrt_tc_step_create(engine->chain, TC_STEP_PLANNING, input, input_len, NULL, 0,
                               &plan_step);
        if (plan_step) {
            char plan_desc[256];
            int pd_len =
            ...
            agentrt_tc_step_complete(plan_step, plan_desc, (size_t)pd_len, 0.75f, "S2-planner");

            mc_evaluation_result_t eval;
            agentrt_mc_evaluate_step(engine->meta, plan_step, NULL, 0, &eval);
            if (!eval.is_acceptable && eval.strategy != MC_CORRECT_NONE) {
                AGENTRT_LOG_WARN("Plan S1 pre-validation failed: score=%.2f strategy=%d",
                                 eval.overall_score, eval.strategy);
                engine->dual_think_corrections++;
            }
            if (eval.critique_text) {
                AGENTRT_FREE(eval.critique_text);
    ...
    if (err != AGENTRT_SUCCESS) {
        AGENTRT_LOG_WARN("Primary planning failed: err=%d, trying fallback", (int)err);
        char err_buf[256];
    ...
    if (plan && !plan->task_plan_id) {
        char id_buf[64];
        agentrt_generate_plan_id(id_buf, sizeof(id_buf));
        plan->task_plan_id = AGENTRT_STRDUP(id_buf);
    ...
    if (engine->checkpoint_enabled) {
        agentrt_cognition_save_checkpoint(engine, engine->checkpoint_last_seq++, "planning");
    }
    ...
    if (engine->enable_dual_thinking && engine->chain && engine->meta && plan) {
        ...
        int has_critical = 0;
        agentrt_tc_chain_health_check(engine->chain, &anomaly_count, &has_critical);
        if (has_critical) {
            AGENTRT_LOG_WARN("Chain health check: %zu anomalies, critical detected", anomaly_count);
            trigger_feedback(engine, 1, "anomaly_detected", "{\"anomalies\":1,\"critical\":true}");
        }

        agentrt_thinking_step_t *gen_step = NULL;
        agentrt_tc_step_create(engine->chain, TC_STEP_GENERATION, input, input_len, NULL, 0,
                               &gen_step);
        ...
        if (llm_adapter || llm_svc) {
            ...
            int llm_ret = -1;
            AGENTRT_LOG_INFO("C-L02: Phase 2 LLM call started (max_tokens=%u)",
            ...
            int skip_llm_call = 0;
            {
                ...
                if (hd == HOOK_DECISION_ABORT || hd == HOOK_DECISION_SKIP) {
                    AGENTRT_LOG_INFO("C-L09: PRE_LLM hook %s — skipping LLM call",
                                     hd == HOOK_DECISION_ABORT ? "ABORT" : "SKIP");
                    skip_llm_call = 1;
                } else if (hd == HOOK_DECISION_MODIFY) {
                    AGENTRT_LOG_DEBUG("C-L09: PRE_LLM hook MODIFY — degraded to CONTINUE (P4)");
                }
            }

            /* P1.2.1: Prefer IPC adapter path over direct LLM service */
            if (!skip_llm_call && llm_adapter) {
                AGENTRT_LOG_DEBUG("C-L02: Using IPC adapter for LLM request"
                                  " (streaming=%s)",
                ...
                if (use_streaming && stream_cb) {
                    ...
                    AGENTRT_LOG_DEBUG("C-L02: IPC streaming complete: ret=%d", llm_ret);
                } else {
                    /* P1.2.1: Sync via IPC adapter */
                    llm_ret = llm_svc_adapter_complete(
                        llm_adapter, &llm_cfg, &llm_resp);
                    AGENTRT_LOG_DEBUG("C-L02: IPC sync complete: ret=%d", llm_ret);
                }
            } else if (!skip_llm_call && llm_svc) {
                AGENTRT_LOG_DEBUG("C-L02: Using direct LLM service (no IPC adapter)"
                                  " streaming=%s",
                                  (use_streaming && stream_cb) ? "on" : "off");
            ...
            if (llm_ret == 0 && llm_resp && llm_resp->choices && llm_resp->choice_count > 0) {
                ...
                AGENTRT_LOG_WARN("C-L02: LLM service complete failed: ret=%d", llm_ret);
            }
             ...
             *          路径不释放该指针）；其他决策 → 正常继续。 */
            {
                ...
                if (hd == HOOK_DECISION_ABORT) {
                    AGENTRT_LOG_WARN("C-L09: POST_LLM hook ABORT — terminating phase");
                    if (llm_response_text) {
                        AGENTRT_FREE(llm_response_text);
        ...
        if (llm_adapter || llm_svc) {
            ...
            AGENTRT_LOG_INFO("C-L02: ThinkDual activated (LLM-driven t2/t1-f/t1-p "
            ...
            AGENTRT_LOG_INFO("C-L02: ThinkDual inactive (no LLM available, "
                            "using direct fallback)");
        }

        tc3_coordinator_t *tc3 = NULL;
        agentrt_error_t tc3_err =
            tc3_coordinator_create(&tc3_cfg, engine->chain, engine->meta, &tc3);

        if (tc3_err == AGENTRT_SUCCESS && tc3) {
            ...
            if (tc3_err == AGENTRT_SUCCESS && phase2_output) {
                if (gen_step) {
                    agentrt_tc_step_complete(gen_step, phase2_output, phase2_output_len, 0.8f,
                                             llm_svc ? "t2-llm+streaming" : "t2-streaming");
                }
                if (engine->chain->ctx_window) {
                    agentrt_tc_context_window_append(engine->chain->ctx_window, phase2_output,
                                                     phase2_output_len);
                }
                ...
                AGENTRT_LOG_INFO("C-L02: ThinkDual tc3 success (phase2_output_len=%zu units=%u "
                ...
                AGENTRT_FREE(phase2_output);
                phase2_output = NULL;
            } else if (gen_step) {
                AGENTRT_LOG_WARN("C-L02: ThinkDual tc3 failed (err=%d), falling back to LLM output",
                                (int)tc3_err);
                /* Triple coordinator failed — fall back to LLM output if available */
                if (llm_response_text && llm_response_len > 0) {
                    agentrt_tc_step_complete(gen_step, llm_response_text, llm_response_len,
                                             0.6f, "t2-llm-fallback");
                    if (engine->chain->ctx_window) {
                        agentrt_tc_context_window_append(engine->chain->ctx_window,
                                                         llm_response_text, llm_response_len);
                    }
                } else {
                    agentrt_tc_step_complete(gen_step, "streaming_loop_failed", 21, 0.3f,
                                             "t2-failed");
                }
            }

            if (engine->memory && engine->chain && gen_step) {
                agentrt_tc_step_write_to_memory(engine->chain, gen_step);
            }
            ...
            if (llm_response_text && llm_response_len > 0) {
                AGENTRT_LOG_INFO("C-L02: Using LLM output directly (tc3 unavailable)");
                if (gen_step) {
                    agentrt_tc_step_complete(gen_step, llm_response_text, llm_response_len,
                                             0.55f, "t2-llm-direct");
                }
                ...
                AGENTRT_LOG_WARN("Triple coordinator creation failed, falling back to basic loop");
                if (gen_step) {
                    agentrt_tc_step_complete(gen_step, input, input_len, 0.5f, "t2-fallback");
                }
            }
        }
        ...
        if (gen_step) {
            ...
            agentrt_tc_step_monitor(gen_step, NULL, &mon_result);
            ...
            if (mon_result.description) {
                AGENTRT_FREE(mon_result.description);
    ...
    if (engine->checkpoint_enabled) {
        agentrt_cognition_save_checkpoint(engine, engine->checkpoint_last_seq++, "execution");
    }
    ...
    /* ── C-L04: tool_d → CoreLoopThree — external tool execution ── */
    {
        ...
        if ((tool_adapter || tool_svc) && plan && plan->task_plan_nodes
            ...
            AGENTRT_LOG_DEBUG("C-L04: Dispatching %zu tool tasks to tool_d"
                              " (adapter=%s)",
            ...
            for (size_t i = 0; i < plan->task_plan_node_count; i++) {
                 ...
                 * ABORT/SKIP → 跳过该工具；MODIFY → 修改 params_json；CONTINUE/RETRY → 继续。 */
                {
                    ...
                    if (hd == HOOK_DECISION_ABORT || hd == HOOK_DECISION_SKIP) {
                        AGENTRT_LOG_INFO("C-L09: PRE_TOOL hook %s — skipping tool '%s'",
                                         hd == HOOK_DECISION_ABORT ? "ABORT" : "SKIP",
                                         node->task_node_handler_name);
                        continue;
                    }
                    if (hd == HOOK_DECISION_RETRY) {
                        AGENTRT_LOG_DEBUG("C-L09: PRE_TOOL hook RETRY — degraded to CONTINUE (P4)");
                    }
                }
                ...
                if (tool_adapter) {
                    ...
                    AGENTRT_LOG_DEBUG("C-L04: IPC tool '%s' execute: ret=%d",
                                      node->task_node_handler_name, tool_err);
                } else if (tool_svc) {
                    tool_err = tool_service_execute(tool_svc, &tool_req, &result);
                    AGENTRT_LOG_DEBUG("C-L04: Direct tool '%s' execute: ret=%d",
                                      node->task_node_handler_name, tool_err);
                }
                 ...
                 * ABORT → 跳出工具循环；其他决策 → 正常继续（仅记录/审计）。 */
                {
                    ...
                    if (hd == HOOK_DECISION_ABORT) {
                        AGENTRT_LOG_WARN("C-L09: POST_TOOL hook ABORT — breaking tool loop at '%s'",
                                         node->task_node_handler_name);
                ...
                if (tool_err == 0 && result && result->success == 0) {
                    AGENTRT_LOG_DEBUG("C-L04: Tool '%s' executed successfully"
                                      " (duration=%llums)",
                                      node->task_node_handler_name,
                    ...
                    if (result->output) {
                        ...
                        if (node->task_node_output) {
                            AGENTRT_FREE(node->task_node_output);
                        }
                        node->task_node_output = AGENTRT_STRDUP(result->output);
                        /* Store tool result in working memory */
                        if (engine->chain && engine->chain->working_mem) {
                            agentrt_tc_working_memory_store(
                    ...
                    AGENTRT_LOG_WARN("C-L04: Tool '%s' execution failed: ret=%d err=%s",
                                     node->task_node_handler_name, tool_err,
    ...
    if (engine->enable_dual_thinking && engine->meta && engine->chain && plan) {
        agentrt_thinking_step_t *audit_step = NULL;
        agentrt_tc_step_create(engine->chain, TC_STEP_AUDIT, input, input_len, NULL, 0,
                               &audit_step);
        if (audit_step) {
            char audit_desc[256];
            int ad_len = snprintf(
            ...
            agentrt_mc_evaluate_step(engine->meta, audit_step, input, input_len, &eval_audit);

            agentrt_tc_step_complete(audit_step, audit_desc, (size_t)ad_len,
                                     eval_audit.overall_score, "S1-auditor");

            if (engine->memory && engine->chain && audit_step) {
                agentrt_tc_metacognition_inform_memory(engine->chain, &eval_audit, audit_step);
                agentrt_tc_step_write_to_memory(engine->chain, audit_step);
            }
            ...
            if (eval_audit.critique_text) {
                AGENTRT_FREE(eval_audit.critique_text);
    ...
    if (engine->checkpoint_enabled) {
        agentrt_cognition_save_checkpoint(engine, engine->checkpoint_last_seq++, "audit");
    }

    /* ========== Phase 4: Enhanced Goal Alignment Check (P2-B05) ========== */
    if (engine->enable_dual_thinking && engine->meta && engine->chain) {
        agentrt_thinking_step_t *align_step = NULL;
        agentrt_tc_step_create(engine->chain, TC_STEP_ALIGNMENT, input, input_len, NULL, 0,
                               &align_step);
        if (align_step) {
            mc_evaluation_result_t align_eval;
            agentrt_mc_evaluate_step(engine->meta, align_step, input, input_len, &align_eval);
            ...
            if (engine->chain->ctx_window) {
                ...
                size_t recent_len = 0;
                agentrt_tc_context_window_get_recent(engine->chain->ctx_window, 300, &recent_ctx,
                                                     &recent_len);
            ...
            agentrt_tc_step_complete(align_step, aligned ? "goal_aligned" : "goal_drift_detected",
                                     aligned ? 12 : 18, composite, "S1-alignment");

            if (!aligned || strcmp(severity, "ok") != 0) {
                AGENTRT_LOG_WARN("Goal alignment: %s (score=%.2f trend=%.3f severity=%s)",
                ...
                if (trigger_replan) {
                    ...
                    if (plan_strat && plan_strat->plan) {
                        agentrt_task_plan_t *replan = NULL;
                        agentrt_error_t replan_err =
                            plan_strat->plan(&intent, plan_strat->data, &replan);
                        if (replan_err == AGENTRT_SUCCESS && replan) {
                            AGENTRT_LOG_WARN("Phase 4: goal drift triggered replan "
                            ...
                            if (replan->task_plan_id) {
                                AGENTRT_FREE(replan->task_plan_id);
                            }
                            char rp_id[64];
                            int rp_n = snprintf(rp_id, sizeof(rp_id),
                                                "replan_%016" PRIx64,
                                                (uint64_t)agentrt_time_monotonic_ns());
                            if (rp_n > 0 && (size_t)rp_n < sizeof(rp_id)) {
                                replan->task_plan_id = AGENTRT_STRDUP(rp_id);
                            ...
                            AGENTRT_LOG_ERROR("Phase 4: replan failed (err=%d), "
                                              "retaining old plan",
                                              (int)replan_err);
                        }
                    } else {
                        AGENTRT_LOG_WARN("Phase 4: goal drift detected but no plan_strategy "
            ...
            if (align_eval.critique_text) {
                AGENTRT_FREE(align_eval.critique_text);
                align_eval.critique_text = NULL;
            }

            if (engine->memory && engine->chain && align_step) {
                agentrt_tc_metacognition_inform_memory(engine->chain, &align_eval, align_step);
            }
            ...
            agentrt_mc_detect_patterns(engine->meta, NULL, NULL);
            agentrt_mc_adapt_threshold(engine->meta);
        }
    }

    /* C-L07: Checkpoint after Phase 4 — alignment */
    if (engine->checkpoint_enabled) {
        agentrt_cognition_save_checkpoint(engine, engine->checkpoint_last_seq++, "alignment");
    }
    ...
    if (engine->stream_critic && plan) {
        ...
        if (engine->chain && engine->chain->ctx_window) {
            agentrt_tc_context_window_get_recent(engine->chain->ctx_window, 2000, &pipeline_output,
                                                 &pipeline_output_len);
        }
        ...
        if (!pipeline_output || pipeline_output_len == 0) {
            ...
            if (pipeline_output) {
                AGENTRT_FREE(pipeline_output);
                pipeline_output = NULL;
            }
            pipeline_output = AGENTRT_STRDUP(input);
        ...
        agentrt_error_t scp_err = sc_stream_critic_pipeline(
        ...
        if (scp_err == AGENTRT_SUCCESS && final_output) {
            char scp_fb[384];
            ...
            AGENTRT_FREE(final_output);
            final_output = NULL;
        } else if (scp_err != AGENTRT_SUCCESS) {
            AGENTRT_LOG_WARN("Stream critic pipeline failed: err=%d", (int)scp_err);
        }
    ...
    for (size_t i = 0; i < plan->task_plan_node_count; i++) {
        ...
        if (node) {
            if (node->task_node_id)
                AGENTRT_FREE(node->task_node_id);
            if (node->task_node_agent_role)
                AGENTRT_FREE(node->task_node_agent_role);
            /* P2.9: 释放 handler_name、goal、output（原先遗漏导致潜在泄漏） */
            if (node->task_node_handler_name)
                AGENTRT_FREE(node->task_node_handler_name);
            if (node->task_node_goal)
                AGENTRT_FREE(node->task_node_goal);
            if (node->task_node_output)
                AGENTRT_FREE(node->task_node_output);
            /* task_node_input 由策略管理，不由 plan_free 释放 */
            if (node->task_node_depends_on) {
                for (size_t j = 0; j < node->task_node_depends_count; j++) {
                    AGENTRT_FREE(node->task_node_depends_on[j]);
                }
                AGENTRT_FREE(node->task_node_depends_on);
            }
            AGENTRT_FREE(node);
        }
    }
    AGENTRT_FREE(plan->task_plan_nodes);
    /* P1.14: 释放 entry_points 数组中的每个字符串（reactive/hierarchical 规划器
     * 通过 AGENTRT_STRDUP 分配 entry_points[i]，原先只释放数组本身导致泄漏） */
    if (plan->task_plan_entry_points) {
        for (size_t e = 0; e < plan->task_plan_entry_count; e++) {
            if (plan->task_plan_entry_points[e])
                AGENTRT_FREE(plan->task_plan_entry_points[e]);
        }
        AGENTRT_FREE(plan->task_plan_entry_points);
    }
    if (plan->task_plan_id)
        AGENTRT_FREE(plan->task_plan_id);
    AGENTRT_FREE(plan);
}

agentrt_error_t agentrt_cognition_stats(agentrt_cognition_engine_t *engine, char **out_stats,
                                        size_t *out_len)
{
...
agentrt_error_t agentrt_cognition_health_check(agentrt_cognition_engine_t *engine, char **out_json)
{
