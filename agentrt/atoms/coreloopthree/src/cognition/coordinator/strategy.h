...
#ifndef AGENTRT_COORDINATOR_STRATEGIES_H
#define AGENTRT_COORDINATOR_STRATEGIES_H

#include "cognition.h"

/* 前向声明：LLM 服务类型（定义在 agentrt/daemons/llm_d/include/llm_service.h） */
struct llm_service;
typedef struct llm_service agentrt_llm_service_t;
 ...
 * 此结构为内部使用，对外隐藏实现细节。
 */
typedef struct agentrt_coordinator_base {
    void *data; /**< 私有数据指针（指向具体实现的私有数据） */
    struct agentrt_llm_service *llm; /**< LLM服务客户端 */
     ...
     * @return 错误码
     */
    agentrt_error_t (*coordinate)(struct agentrt_coordinator_base *base,
     ...
     * @param base 基础结构指针
     */
    void (*destroy)(struct agentrt_coordinator_base *base);
} agentrt_coordinator_base_t;
...
#endif /* AGENTRT_COORDINATOR_STRATEGIES_H */
