...
static void weighted_destroy(agentrt_coordinator_base_t *base)
{
    if (!base)
        return;
    ...
    if (data) {
        for (size_t i = 0; i < data->model_count; i++) {
            if (data->model_names[i])
                AGENTRT_FREE(data->model_names[i]);
        }
        AGENTRT_FREE(data->model_names);
        AGENTRT_FREE(data->weights);
        if (data->lock)
...
weighted_coordinate(agentrt_coordinator_base_t *base,
                    ...
                    const char **inputs, size_t input_count, char **out_result)
{
    ...
    if (!data || !inputs || input_count == 0) {
        *out_result = AGENTRT_STRDUP("invalid_input");
        return AGENTRT_EINVAL;
    }

    size_t count = input_count < data->model_count ? input_count : data->model_count;
    if (count == 0) {
        *out_result = AGENTRT_STRDUP("no_models");
        return AGENTRT_EINVAL;
    }
    ...
    if (count > 2 && consistency_ratio < 0.5f) {
        AGENTRT_LOG_WARN("weighted_coordinate: high-weight model output conflicts with "
    ...
    char *result = (char *)AGENTRT_MALLOC(out_sz);
    if (!result)
    ...
    if (written <= 0 || (size_t)written >= out_sz) {
        /* JSON 封装失败 — 回退为纯文本输出 */
        AGENTRT_FREE(result);
        *out_result = AGENTRT_STRDUP(primary);
        if (!*out_result)
            return AGENTRT_ENOMEM;
        return AGENTRT_SUCCESS;
    }

    AGENTRT_LOG_DEBUG("weighted_coordinate: selected model[%zu] (weight=%.2f consistency=%.0f%%)",
                      best_index, (double)max_norm_weight,
                      (double)(consistency_ratio * 100.0f));
 ...
 * @brief 创建加权融合协调器
 */
agentrt_error_t agentrt_coordinator_weighted_create(const char **model_names, const float *weights,
                                                    size_t model_count,
                                                    agentrt_coordinator_base_t **out_base)
{
    ...
    // 复制模型名称
    data->model_names = (char **)AGENTRT_CALLOC(model_count, sizeof(char *));
    if (!data->model_names) {
        agentrt_mutex_free(data->lock);
        AGENTRT_FREE(data);
        AGENTRT_FREE(base);
    ...
    for (size_t i = 0; i < model_count; i++) {
        data->model_names[i] = AGENTRT_STRDUP(model_names[i]);
        if (!data->model_names[i]) {
            for (size_t j = 0; j < i; j++) {
                AGENTRT_FREE(data->model_names[j]);
            }
            AGENTRT_FREE(data->model_names);
            agentrt_mutex_free(data->lock);
            AGENTRT_FREE(data);
    ...
    // 复制权重
    data->weights = (float *)AGENTRT_CALLOC(model_count, sizeof(float));
    if (!data->weights) {
        for (size_t i = 0; i < model_count; i++) {
            AGENTRT_FREE(data->model_names[i]);
        }
