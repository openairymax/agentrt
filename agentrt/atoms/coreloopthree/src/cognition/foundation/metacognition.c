...
agentrt_error_t agentrt_mc_create(agentrt_metacognition_t **out_mc)
{
    if (!out_mc) {
        AGENTRT_LOG_ERROR("agentrt_mc_create: NULL out_mc parameter");
        return AGENTRT_EINVAL;
    }

    agentrt_metacognition_t *mc =
        (agentrt_metacognition_t *)AGENTRT_CALLOC(1, sizeof(agentrt_metacognition_t));
    if (!mc) {
        AGENTRT_LOG_ERROR("agentrt_mc_create: allocation failed for metacognition");
    ...
    mc->records = (mc_evaluation_record_t *)AGENTRT_CALLOC(mc->record_capacity,
                                                           sizeof(mc_evaluation_record_t));
    if (!mc->records) {
        AGENTRT_LOG_ERROR("agentrt_mc_create: records allocation failed (capacity=%zu)", mc->record_capacity);
        AGENTRT_FREE(mc);
        return AGENTRT_ENOMEM;
    }
    mc->record_count = 0;
...
void agentrt_mc_destroy(agentrt_metacognition_t *mc)
{
    if (!mc)
        return;
    for (size_t i = 0; i < mc->record_count; i++) {
        if (mc->records[i].result.critique_text)
            AGENTRT_FREE((void *)mc->records[i].result.critique_text);
    }
    AGENTRT_FREE(mc->records);
    AGENTRT_FREE(mc);
}

void agentrt_mc_set_chain(agentrt_metacognition_t *mc, agentrt_thinking_chain_t *chain)
{
    if (!mc)
        return;
    mc->chain = chain;
...
agentrt_error_t agentrt_mc_evaluate_step(agentrt_metacognition_t *mc, agentrt_thinking_step_t *step,
                                         const char *context, size_t context_len,
                                         mc_evaluation_result_t *out_result)
{

    if (!mc || !step || !step->content || !out_result) {
        AGENTRT_LOG_ERROR("agentrt_mc_evaluate_step: NULL/invalid params (mc=%p step=%p content=%p out_result=%p)", (void *)mc, (void *)step, step ? (void *)step->content : NULL, (void *)out_result);
        return AGENTRT_EINVAL;
    }
    ...
    /* 应用置信度校准 */
    out_result->calibrated_confidence = agentrt_mc_calibrate_confidence(mc, step->confidence);
    ...
    out_result->critique_text = (char *)AGENTRT_MALLOC(pos + 1);
...
agentrt_error_t agentrt_mc_evaluate_quick(agentrt_metacognition_t *mc,
                                          agentrt_thinking_step_t *step, float *out_score,
                                          int *out_acceptable)
{

    if (!mc || !step || !out_score || !out_acceptable) {
        AGENTRT_LOG_ERROR("agentrt_mc_evaluate_quick: NULL params (mc=%p step=%p out_score=%p out_acceptable=%p)", (void *)mc, (void *)step, (void *)out_score, (void *)out_acceptable);
        return AGENTRT_EINVAL;
    }

    mc_evaluation_result_t result;
    agentrt_error_t err = agentrt_mc_evaluate_step(mc, step, NULL, 0, &result);
...
agentrt_error_t agentrt_mc_apply_correction(
    agentrt_metacognition_t *mc, agentrt_thinking_step_t *step, const mc_evaluation_result_t *eval,
    agentrt_error_t (*corrector_fn)(const char *, size_t, char **, size_t *, void *),
    void *user_data)
{

    if (!mc || !step || !eval) {
        AGENTRT_LOG_ERROR("agentrt_mc_apply_correction: NULL params (mc=%p step=%p eval=%p)", (void *)mc, (void *)step, (void *)eval);
        return AGENTRT_EINVAL;
    }
    ...
    case MC_CORRECT_AUTO: {
        if (!corrector_fn) {
            AGENTRT_LOG_ERROR("agentrt_mc_apply_correction: AUTO correction requested but corrector_fn is NULL (step_id=%u)", step->step_id);
            return AGENTRT_EINVAL;
        }
        char *corrected = NULL;
        size_t corr_len = 0;
        ...
        if (err == AGENTRT_SUCCESS && corrected && corr_len > 0) {
            agentrt_tc_step_correct(step, corrected, corr_len);
    ...
    case MC_CORRECT_RERUN: {
        mc->total_corrections++;
        if (!corrector_fn) {
            AGENTRT_LOG_ERROR("agentrt_mc_apply_correction: RERUN correction requested but corrector_fn is NULL (step_id=%u)", step->step_id);
        ...
        agentrt_error_t last_err = AGENTRT_EPERM;
        for (int attempt = 0; attempt < max_retries; attempt++) {
            ...
            if (attempt > 0 && eval->critique_text) {
                ...
                enhanced_input = (char *)AGENTRT_CALLOC(1, enhanced_len);
                if (enhanced_input) {
            ...
            if (enhanced_input)
                AGENTRT_FREE(enhanced_input);

            if (last_err == AGENTRT_SUCCESS && corrected && corr_len > 0) {
                agentrt_tc_step_correct(step, corrected, corr_len);
                AGENTRT_FREE(corrected);
                if (mc->chain && mc->chain->on_correction) {
        ...
        step->status = TC_STATUS_FAILED;
        mc->total_rejections++;
        AGENTRT_LOG_ERROR("agentrt_mc_apply_correction: RERUN all retries exhausted (step_id=%u last_err=%d)", step->step_id, (int)last_err);
        return last_err;
    }
    ...
    default:
        AGENTRT_LOG_ERROR("agentrt_mc_apply_correction: unknown correction strategy (strategy=%d)", (int)eval->strategy);
        return AGENTRT_EINVAL;
    }
}

int agentrt_mc_should_self_correct(agentrt_metacognition_t *mc, tc_step_type_t step_type)
{
    if (!mc || mc->record_count < 3) {
        if (!mc)
            AGENTRT_LOG_WARN("agentrt_mc_should_self_correct: NULL mc parameter");
        return 0;
    }
...
float agentrt_mc_calibrate_confidence(agentrt_metacognition_t *mc, float raw_confidence)
{
    if (!mc) {
        AGENTRT_LOG_WARN("agentrt_mc_calibrate_confidence: NULL mc parameter, returning raw value");
...
agentrt_error_t agentrt_mc_feedback(agentrt_metacognition_t *mc, float predicted_confidence,
                                    int was_correct)
{

    if (!mc) {
        AGENTRT_LOG_ERROR("agentrt_mc_feedback: NULL mc parameter");
        return AGENTRT_EINVAL;
    }
    if (!mc->enable_confidence_calibration)
        return AGENTRT_SUCCESS;
...
agentrt_error_t agentrt_mc_stats(agentrt_metacognition_t *mc, char **out_json)
{
    if (!mc || !out_json) {
        AGENTRT_LOG_ERROR("agentrt_mc_stats: NULL params (mc=%p out_json=%p)", (void *)mc, (void *)out_json);
        return AGENTRT_EINVAL;
    }
    ...
    if (written < 0 || (size_t)written >= sizeof(buf)) {
        AGENTRT_LOG_ERROR("agentrt_mc_stats: stats JSON truncation or encoding error (written=%d buf_size=%zu)", written, sizeof(buf));
        return AGENTRT_ERANGE;
    }

    char *result = AGENTRT_STRDUP(buf);
    if (!result) {
        AGENTRT_LOG_ERROR("agentrt_mc_stats: result allocation failed");
        return AGENTRT_ENOMEM;
    }
    *out_json = result;
    return AGENTRT_SUCCESS;
...
agentrt_error_t agentrt_mc_get_history(agentrt_metacognition_t *mc, size_t count,
                                       mc_evaluation_record_t **out_records, size_t *out_count)
{

    if (!mc || !out_records || !out_count) {
        AGENTRT_LOG_ERROR("agentrt_mc_get_history: NULL params (mc=%p out_records=%p out_count=%p)", (void *)mc, (void *)out_records, (void *)out_count);
        return AGENTRT_EINVAL;
    }
...
void agentrt_mc_reset(agentrt_metacognition_t *mc)
{
    if (!mc)
        return;
    for (size_t i = 0; i < mc->record_count; i++) {
...
static size_t find_or_create_pattern(agentrt_metacognition_t *mc, const char *key)
{
...
agentrt_error_t agentrt_mc_detect_patterns(agentrt_metacognition_t *mc,
                                           mc_error_pattern_t **out_patterns, size_t *out_count)
{
    if (!mc || !out_patterns || !out_count) {
        AGENTRT_LOG_ERROR("agentrt_mc_detect_patterns: NULL params (mc=%p out_patterns=%p out_count=%p)", (void *)mc, (void *)out_patterns, (void *)out_count);
        return AGENTRT_EINVAL;
    }
...
agentrt_error_t agentrt_mc_learn_best_strategy(agentrt_metacognition_t *mc, const char *pattern_key,
                                               mc_correction_strategy_t *out_strategy)
{
    if (!mc || !pattern_key || !out_strategy) {
        AGENTRT_LOG_ERROR("agentrt_mc_learn_best_strategy: NULL params (mc=%p pattern_key=%p out_strategy=%p)", (void *)mc, (void *)pattern_key, (void *)out_strategy);
        return AGENTRT_EINVAL;
    }
...
int agentrt_mc_preemptive_check(agentrt_metacognition_t *mc, tc_step_type_t step_type,
                                const char *input, size_t input_len, char **out_preemptive_hint,
                                size_t *out_hint_len)
{
    if (!mc || !input || !out_preemptive_hint || !out_hint_len) {
        AGENTRT_LOG_ERROR("agentrt_mc_preemptive_check: NULL params (mc=%p input=%p out_preemptive_hint=%p out_hint_len=%p)", (void *)mc, (void *)input, (void *)out_preemptive_hint, (void *)out_hint_len);
        return AGENTRT_EINVAL;
    }
    ...
    *out_hint_len = 0;
    ...
    for (size_t i = 0; i < mc->pattern_count; i++) {
        ...
        char *hint = (char *)AGENTRT_MALLOC(hlen);
        if (!hint)
            return AGENTRT_EINVAL;
        ...
        *out_hint_len = (size_t)written;
        ...
        AGENTRT_LOG_INFO("MC preemptive: matched pattern '%s' (rate=%.2f)", pat->pattern_key,
                         pat->failure_rate);
        return 1;
    }
...
agentrt_error_t agentrt_mc_record_strategy_result(agentrt_metacognition_t *mc,
                                                  const char *pattern_key,
                                                  mc_correction_strategy_t strategy, int success)
{
    if (!mc || !pattern_key) {
        AGENTRT_LOG_ERROR("agentrt_mc_record_strategy_result: NULL params (mc=%p pattern_key=%p)", (void *)mc, (void *)pattern_key);
        return AGENTRT_EINVAL;
    }

    size_t pidx = find_or_create_pattern(mc, pattern_key);
    if (pidx >= MC_MAX_PATTERNS) {
        AGENTRT_LOG_ERROR("agentrt_mc_record_strategy_result: pattern storage full, cannot record (pattern_key=%s pattern_count=%zu)", pattern_key, mc->pattern_count);
        return AGENTRT_ENOMEM;
    }
...
float agentrt_mc_adapt_threshold(agentrt_metacognition_t *mc)
{
    if (!mc) {
        AGENTRT_LOG_WARN("agentrt_mc_adapt_threshold: NULL mc parameter, returning default threshold");
        return 0.7f;
    }
