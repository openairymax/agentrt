...
#ifndef AGENTRT_THINKING_CHAIN_H
#define AGENTRT_THINKING_CHAIN_H
...
/* 前向声明 */
typedef struct agentrt_thinking_chain agentrt_thinking_chain_t;
typedef struct agentrt_thinking_step agentrt_thinking_step_t;
typedef struct agentrt_working_memory agentrt_working_memory_t;
typedef struct agentrt_context_window agentrt_context_window_t;
...
struct agentrt_thinking_step {
    ...
    /* 内部引用（用于回调通知） */
    struct agentrt_thinking_chain *chain_ref; /**< 所属思考链路（内部使用） */
};
 ...
 * 整合为统一的Thinkdual推理管线。
 */
struct agentrt_thinking_chain {
    uint64_t session_id;                   /**< 会话唯一ID */
    char *session_goal;                    /**< 会话目标描述 */
    agentrt_context_window_t *ctx_window;  /**< Context Window */
    agentrt_working_memory_t *working_mem; /**< Working Memory */
    agentrt_memory_engine_t *memory;       /**< MemoryRovol 引用 (P2-B03) */

    /* 思考步骤DAG */
    agentrt_thinking_step_t *steps; /**< 步骤数组 */
    ...
    /* 回调函数 */
    void (*on_step_completed)(agentrt_thinking_step_t *step, void *user_data);
    void (*on_correction)(agentrt_thinking_step_t *step, const char *critique, void *user_data);
 ...
 */
AGENTRT_API agentrt_error_t agentrt_tc_context_window_create(size_t max_tokens,
                                                             agentrt_context_window_t **out_window);

/**
 * @brief 销毁Context Window
 */
AGENTRT_API void agentrt_tc_context_window_destroy(agentrt_context_window_t *window);

/**
 * @brief 向Context Window追加内容
 * @return 追加后的总token数估算，或错误码(<0时)
 */
AGENTRT_API ssize_t agentrt_tc_context_window_append(agentrt_context_window_t *window,
 ...
 * @param out_len 输出长度
 */
AGENTRT_API agentrt_error_t agentrt_tc_context_window_get_recent(agentrt_context_window_t *window,
                                                                 size_t token_count,
 ...
 * @return 1=有空间, 0=空间不足
 */
AGENTRT_API int agentrt_tc_context_window_has_space(agentrt_context_window_t *window,
                                                    size_t needed_tokens);

/**
 * @brief 获取Context Window统计信息
 */
AGENTRT_API agentrt_error_t agentrt_tc_context_window_stats(agentrt_context_window_t *window,
                                                            char **out_json);
 ...
 */
AGENTRT_API agentrt_error_t agentrt_tc_working_memory_create(size_t capacity,
                                                             agentrt_working_memory_t **out_mem);

/**
 * @brief 销毁Working Memory
 */
AGENTRT_API void agentrt_tc_working_memory_destroy(agentrt_working_memory_t *mem);
 ...
 */
AGENTRT_API agentrt_error_t agentrt_tc_working_memory_store(agentrt_working_memory_t *mem,
 ...
 * @param out_size 输出大小
 * @return AGENTRT_SUCCESS 或 AGENTRT_ENOTFOUND
 */
AGENTRT_API agentrt_error_t agentrt_tc_working_memory_retrieve(agentrt_working_memory_t *mem,
 ...
 */
AGENTRT_API agentrt_error_t agentrt_tc_working_memory_remove(agentrt_working_memory_t *mem,
                                                             const char *key);

/**
 * @brief 清空Working Memory所有非锁定条目
 */
AGENTRT_API void agentrt_tc_working_memory_clear_unpinned(agentrt_working_memory_t *mem);
 ...
 */
AGENTRT_API agentrt_error_t agentrt_tc_step_create(agentrt_thinking_chain_t *chain,
 ...
 * @param role 执行角色标识
 */
AGENTRT_API agentrt_error_t agentrt_tc_step_complete(agentrt_thinking_step_t *step,
 ...
 */
AGENTRT_API agentrt_error_t agentrt_tc_step_verify(agentrt_thinking_step_t *step, int *is_valid,
 ...
 */
AGENTRT_API agentrt_error_t agentrt_tc_step_correct(agentrt_thinking_step_t *step,
 ...
 */
AGENTRT_API int agentrt_tc_step_is_ready(const agentrt_thinking_step_t *step,
 ...
 */
AGENTRT_API agentrt_error_t agentrt_tc_chain_create(const char *goal, size_t max_tokens,
                                                    size_t wm_capacity,
 ...
 * @brief 销毁思考链路及其所有子组件
 */
AGENTRT_API void agentrt_tc_chain_destroy(agentrt_thinking_chain_t *chain);

/**
 * @brief 启动思考链路（标记活跃，初始化统计）
 */
AGENTRT_API agentrt_error_t agentrt_tc_chain_start(agentrt_thinking_chain_t *chain);

/**
 * @brief 停止思考链路
 */
AGENTRT_API void agentrt_tc_chain_stop(agentrt_thinking_chain_t *chain);

/**
 * @brief 获取下一个可执行的思考步骤
 * @return AGENTRT_SUCCESS且有步骤, AGENTRT_ENOENT(无可执行步骤), 或其他错误
 */
AGENTRT_API agentrt_error_t agentrt_tc_chain_next_ready_step(agentrt_thinking_chain_t *chain,
                                                             agentrt_thinking_step_t **out_step);

/**
 * @brief 获取思考链路的完整执行统计（JSON格式）
 */
AGENTRT_API agentrt_error_t agentrt_tc_chain_stats(agentrt_thinking_chain_t *chain, char **out_json,
                                                   size_t *out_len);

/**
 * @brief 设置步骤完成回调
 */
AGENTRT_API void agentrt_tc_chain_set_step_callback(
...
AGENTRT_API void agentrt_tc_chain_set_memory(agentrt_thinking_chain_t *chain,
                                             agentrt_memory_engine_t *memory);

AGENTRT_API agentrt_error_t agentrt_tc_context_window_prepopulate(agentrt_thinking_chain_t *chain,
                                                                  const char *query_text,
                                                                  size_t query_len, uint32_t limit);

AGENTRT_API agentrt_error_t
agentrt_tc_working_memory_sync_to_persistent(agentrt_thinking_chain_t *chain, float min_importance);

AGENTRT_API agentrt_error_t agentrt_tc_step_write_to_memory(agentrt_thinking_chain_t *chain,
                                                            agentrt_thinking_step_t *step);

AGENTRT_API agentrt_error_t agentrt_tc_metacognition_inform_memory(agentrt_thinking_chain_t *chain,
                                                                   const void *eval,
                                                                   agentrt_thinking_step_t *step);
 ...
 * @return AGENTRT_SUCCESS 或错误码
 */
AGENTRT_API agentrt_error_t agentrt_tc_step_monitor(const agentrt_thinking_step_t *step,
 ...
 * @return AGENTRT_SUCCESS 或错误码
 */
AGENTRT_API agentrt_error_t agentrt_tc_chain_health_check(const agentrt_thinking_chain_t *chain,
 ...
 * @return AGENTRT_SUCCESS 或错误码
 */
AGENTRT_API agentrt_error_t agentrt_tc_step_recover(
    agentrt_thinking_chain_t *chain, agentrt_thinking_step_t *failed_step,
 ...
 */
AGENTRT_API uint32_t agentrt_tc_chain_checkpoint(agentrt_thinking_chain_t *chain);
 ...
 * @return 成功删除的步骤数
 */
AGENTRT_API size_t agentrt_tc_chain_rollback(agentrt_thinking_chain_t *chain,
...
AGENTRT_API agentrt_error_t agentrt_tc_set_attention_config(agentrt_thinking_chain_t *chain,
                                                            const tc_attention_config_t *config);

AGENTRT_API agentrt_error_t agentrt_tc_allocate_attention(agentrt_thinking_chain_t *chain,
                                                          agentrt_thinking_step_t *step,
                                                          tc_allocation_result_t *out_alloc);

AGENTRT_API agentrt_error_t agentrt_tc_adjust_dynamic_budget(agentrt_thinking_chain_t *chain,
                                                             tc_step_type_t step_type,
                                                             float performance_score);

AGENTRT_API float agentrt_tc_compute_priority(const agentrt_thinking_step_t *step,
                                              const agentrt_thinking_chain_t *chain);

AGENTRT_API agentrt_error_t agentrt_tc_wm_set_priority(agentrt_working_memory_t *wm,
...
#endif /* AGENTRT_THINKING_CHAIN_H */
