...
{
    if (!src) return NULL;
    return AGENTRT_STRDUP(src);
}

cog_evolution_t *cog_evolution_create(cog_level_t initial_level)
{
    cog_evolution_t *evo = (cog_evolution_t *)AGENTRT_CALLOC(1, sizeof(cog_evolution_t));
    if (!evo) return NULL;
    ...
    evo->experiences =
        (cog_experience_t *)AGENTRT_CALLOC(evo->experience_capacity, sizeof(cog_experience_t));
    if (!evo->experiences) {
        AGENTRT_FREE(evo);
    ...
    evo->strategies = (cog_strategy_t *)AGENTRT_CALLOC(evo->strategy_capacity, sizeof(cog_strategy_t));
    if (!evo->strategies) {
        AGENTRT_FREE(evo->experiences);
        AGENTRT_FREE(evo);
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
    }

    evo->pattern_capacity = 256;
    evo->patterns = (cog_pattern_t *)AGENTRT_CALLOC(evo->pattern_capacity, sizeof(cog_pattern_t));
    if (!evo->patterns) {
        AGENTRT_FREE(evo->strategies);
        AGENTRT_FREE(evo->experiences);
        AGENTRT_FREE(evo);
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
    }
    ...
    evo->knowledge = (cog_knowledge_t *)AGENTRT_CALLOC(evo->knowledge_capacity, sizeof(cog_knowledge_t));
    if (!evo->knowledge) {
        AGENTRT_FREE(evo->patterns);
        AGENTRT_FREE(evo->strategies);
        AGENTRT_FREE(evo->experiences);
        AGENTRT_FREE(evo);
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
...
static void free_experience(cog_experience_t *e)
{
    if (!e)
        return;
    if (e->context_json)
        AGENTRT_FREE(e->context_json);
    if (e->action_json)
        AGENTRT_FREE(e->action_json);
    if (e->outcome_json)
        AGENTRT_FREE(e->outcome_json);
    e->context_json = NULL;
    e->action_json = NULL;
    e->outcome_json = NULL;
}
...
static void free_strategy(cog_strategy_t *s)
{
    if (!s)
        return;
    if (s->condition_json)
        AGENTRT_FREE(s->condition_json);
    if (s->action_json)
        AGENTRT_FREE(s->action_json);
...
static void free_pattern(cog_pattern_t *p)
{
    if (!p)
        return;
    if (p->trigger_json)
        AGENTRT_FREE(p->trigger_json);
    if (p->response_json)
        AGENTRT_FREE(p->response_json);
    p->trigger_json = NULL;
...
static void free_knowledge(cog_knowledge_t *k)
{
    if (!k)
        return;
    if (k->knowledge_json)
        AGENTRT_FREE(k->knowledge_json);
    if (k->adaptation_json)
        AGENTRT_FREE(k->adaptation_json);
...
int cog_evolution_record_experience(cog_evolution_t *evo, const cog_experience_t *experience)
{
    ...
    if (evo->experience_count >= evo->experience_capacity) {
        ...
        cog_experience_t *new_arr =
            (cog_experience_t *)AGENTRT_REALLOC(evo->experiences, new_cap * sizeof(cog_experience_t));
        if (!new_arr)
            COG_RET_ERR(AGENTRT_ERR_OVERFLOW);
        ...
        if (!found) {
            if (evo->pattern_count >= evo->pattern_capacity) {
                ...
                cog_pattern_t *new_arr =
                    (cog_pattern_t *)AGENTRT_REALLOC(evo->patterns, new_cap * sizeof(cog_pattern_t));
                if (!new_arr)
                    break;
        ...
        if (!found) {
            if (evo->strategy_count >= evo->strategy_capacity) {
                ...
                cog_strategy_t *new_arr =
                    (cog_strategy_t *)AGENTRT_REALLOC(evo->strategies, new_cap * sizeof(cog_strategy_t));
                if (!new_arr)
                    break;
        ...
        for (size_t i = 0; i < evo->strategy_count; i++) {
            ...
            if (strcmp(s->domain, source_domain) == 0 && s->fitness > 0.5) {
                if (evo->knowledge_count >= evo->knowledge_capacity) {
                    ...
                    cog_knowledge_t *new_arr = (cog_knowledge_t *)AGENTRT_REALLOC(
...
AGENTRT_STRNCPY_TERM(k->source_domain, source_domain, sizeof(k->source_domain));
AGENTRT_STRNCPY_TERM(k->target_domain, target_domain, sizeof(k->target_domain));
