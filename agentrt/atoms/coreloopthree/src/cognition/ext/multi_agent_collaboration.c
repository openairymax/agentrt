...
{
    snprintf(buf, buf_size, "%s_%lu_%u", prefix, (unsigned long)(agentrt_time_ns() / 1000000ULL),
             (unsigned int)((agentrt_time_ns() ^ (size_t)buf) % 100000));
}
...
{
    mac_framework_t *fw = (mac_framework_t *)AGENTRT_CALLOC(1, sizeof(mac_framework_t));
    if (!fw) return NULL;
    fw->default_mode = default_mode;
    fw->lock_init = 0;
    __builtin_memset(fw->agent_hash, 0, sizeof(fw->agent_hash));
    __builtin_memset(fw->group_hash, 0, sizeof(fw->group_hash));
    __builtin_memset(fw->task_hash, 0, sizeof(fw->task_hash));
    __builtin_memset(fw->consensus_hash, 0, sizeof(fw->consensus_hash));
    return fw;
}
...
void mac_framework_destroy(mac_framework_t *fw)
{
    ...
    for (size_t i = 0; i < fw->agent_count; i++) {
        AGENTRT_FREE(fw->agents[i].capabilities_json);
    }
    for (size_t i = 0; i < fw->group_count; i++) {
        for (size_t m = 0; m < fw->groups[i].member_count; m++) {
            AGENTRT_FREE(fw->groups[i].members[m].capabilities_json);
        }
        AGENTRT_FREE(fw->groups[i].members);
        AGENTRT_FREE(fw->groups[i].shared_context_json);
    }
    for (size_t i = 0; i < fw->task_count; i++) {
        AGENTRT_FREE(fw->tasks[i].input_json);
        AGENTRT_FREE(fw->tasks[i].output_json);
    }
    for (size_t i = 0; i < fw->consensus_count; i++) {
        AGENTRT_FREE(fw->consensuses[i].proposal_json);
        for (size_t j = 0; j < fw->consensuses[i].vote_count; j++) {
            AGENTRT_FREE(fw->consensuses[i].votes[j]);
            AGENTRT_FREE(fw->consensuses[i].voter_ids[j]);
        }
        AGENTRT_FREE(fw->consensuses[i].votes);
        AGENTRT_FREE(fw->consensuses[i].voter_ids);
        AGENTRT_FREE(fw->consensuses[i].result_json);
    }
    if (fw->lock_init)
        agentrt_mutex_destroy(&fw->lock);
    AGENTRT_FREE(fw);
}
...
int mac_framework_register_agent(mac_framework_t *fw, const mac_agent_info_t *agent)
{
    if (!fw || !agent)
        MAC_RET_ERR(AGENTRT_EINVAL);
    ensure_lock(fw);
    agentrt_mutex_lock(&fw->lock);

    if (fw->agent_count >= MAC_MAX_AGENTS) {
        agentrt_mutex_unlock(&fw->lock);
        MAC_RET_ERR(AGENTRT_ERR_OVERFLOW);
    }
    if (mac_hash_find_agent(fw, agent->id) >= 0) {
        agentrt_mutex_unlock(&fw->lock);
        MAC_RET_ERR(AGENTRT_ERR_NULL_POINTER);
    }
    mac_agent_info_t *slot = &fw->agents[fw->agent_count];
    __builtin_memcpy(slot, agent, sizeof(mac_agent_info_t));
    slot->capabilities_json = agent->capabilities_json ? AGENTRT_STRDUP(agent->capabilities_json) : NULL;
...
int mac_framework_unregister_agent(mac_framework_t *fw, const char *agent_id)
{
    ...
    if (idx < 0) {
        agentrt_mutex_unlock(&fw->lock);
        MAC_RET_ERR(AGENTRT_ERR_NOT_FOUND);
    }

    AGENTRT_FREE(fw->agents[idx].capabilities_json);
    ...
    __builtin_memset(&fw->agents[fw->agent_count - 1], 0, sizeof(mac_agent_info_t));
    fw->agent_count--;
    agentrt_mutex_unlock(&fw->lock);
    return 0;
}
                               ...
                               const char **agent_ids, size_t agent_count, char **group_id)
{
    ...
    __builtin_memset(group, 0, sizeof(mac_group_t));

    generate_id(group->id, sizeof(group->id), "grp");
AGENTRT_STRNCPY_TERM(group->name, name, sizeof(group->name));
    group->name[sizeof(group->name) - 1] = '\0';
    group->mode = mode;
    group->created_at = agentrt_time_ms();

    if (agent_count > 0 && agent_ids) {
        size_t valid_count = 0;
        mac_agent_info_t *members =
            (mac_agent_info_t *)AGENTRT_CALLOC(agent_count, sizeof(mac_agent_info_t));
        if (!members) {
            agentrt_mutex_unlock(&fw->lock);
            MAC_RET_ERR(AGENTRT_ERR_OUT_OF_MEMORY);
        }

        for (size_t i = 0; i < agent_count; i++) {
            ssize_t aidx = mac_hash_find_agent(fw, agent_ids[i]);
            if (aidx >= 0) {
                __builtin_memcpy(&members[valid_count], &fw->agents[aidx], sizeof(mac_agent_info_t));
                if (members[valid_count].capabilities_json)
                    members[valid_count].capabilities_json =
                        AGENTRT_STRDUP(members[valid_count].capabilities_json);
                if (valid_count == 0) {
AGENTRT_STRNCPY_TERM(group->leader_id, fw->agents[aidx].id, sizeof(group->leader_id));
    ...
    if (group_id)
        *group_id = AGENTRT_STRDUP(group->id);
...
int mac_framework_disband_group(mac_framework_t *fw, const char *group_id)
{
    ...
    for (size_t m = 0; m < fw->groups[idx].member_count; m++) {
        AGENTRT_FREE(fw->groups[idx].members[m].capabilities_json);
    }
    AGENTRT_FREE(fw->groups[idx].members);
    AGENTRT_FREE(fw->groups[idx].shared_context_json);
    ...
    __builtin_memset(&fw->groups[fw->group_count - 1], 0, sizeof(mac_group_t));
    fw->group_count--;
    agentrt_mutex_unlock(&fw->lock);
    return 0;
}
                                ...
                                const mac_collab_task_t *task, char **assigned_agent_id)
{
    ...
    __builtin_memcpy(slot, task, sizeof(mac_collab_task_t));
    slot->input_json = task->input_json ? AGENTRT_STRDUP(task->input_json) : NULL;
    ...
    slot->created_at = agentrt_time_ms();
    if (group_id) {
AGENTRT_STRNCPY_TERM(slot->group_id, group_id, sizeof(slot->group_id));
    ...
    if (assigned_agent_id)
        *assigned_agent_id = AGENTRT_STRDUP(agent->id);
    agentrt_mutex_unlock(&fw->lock);
    return 0;
}
                                  ...
                                  char ***results, size_t *result_count)
{
    ...
    char **out = (char **)AGENTRT_CALLOC(capacity, sizeof(char *));
    if (!out) {
        agentrt_mutex_unlock(&fw->lock);
        MAC_RET_ERR(AGENTRT_ERR_OVERFLOW);
    }
    ...
    for (size_t i = 0; i < fw->task_count; i++) {
        ...
        if (count >= capacity) {
            capacity *= 2;
            char **new_out = (char **)AGENTRT_REALLOC(out, capacity * sizeof(char *));
            if (!new_out) {
        ...
        out[count] = t->output_json ? AGENTRT_STRDUP(t->output_json) : AGENTRT_STRDUP("{}");
        if (!out[count]) {
            for (size_t j = 0; j < count; j++)
                AGENTRT_FREE(out[j]);
            AGENTRT_FREE(out);
                                  ...
                                  char **consensus_id)
{
    ...
    __builtin_memset(c, 0, sizeof(mac_consensus_t));

    generate_id(c->id, sizeof(c->id), "cns");
    if (group_id) {
AGENTRT_STRNCPY_TERM(c->group_id, group_id, sizeof(c->group_id));
        c->group_id[sizeof(c->group_id) - 1] = '\0';
    }
    c->strategy = strategy;
    c->proposal_json = AGENTRT_STRDUP(proposal_json);
    c->votes = NULL;
    c->voter_ids = NULL;
    c->vote_count = 0;
    c->result_json = NULL;
    ...
    if (consensus_id)
        *consensus_id = AGENTRT_STRDUP(c->id);
                       ...
                       const char *vote_json)
{
    ...
    char **new_votes = (char **)AGENTRT_REALLOC(c->votes, new_count * sizeof(char *));
    if (!new_votes) {
        agentrt_mutex_unlock(&fw->lock);
        MAC_RET_ERR(AGENTRT_ERR_OUT_OF_MEMORY);
    }
    c->votes = new_votes;
    char **new_voter_ids = (char **)AGENTRT_REALLOC(c->voter_ids, new_count * sizeof(char *));
    if (!new_voter_ids) {
        agentrt_mutex_unlock(&fw->lock);
        MAC_RET_ERR(AGENTRT_ERR_OUT_OF_MEMORY);
    }
    c->voter_ids = new_voter_ids;
    c->votes[c->vote_count] = AGENTRT_STRDUP(vote_json);
    c->voter_ids[c->vote_count] = AGENTRT_STRDUP(agent_id);
                                    ...
                                    char **result_json)
{
    ...
    if (c->resolved) {
        *result_json = c->result_json ? AGENTRT_STRDUP(c->result_json) : AGENTRT_STRDUP("{}");
        agentrt_mutex_unlock(&fw->lock);
        return 0;
    }
    ...
    c->resolved = true;
    if (approved) {
        c->result_json = AGENTRT_STRDUP(c->proposal_json ? c->proposal_json : "{}");
    } else {
        c->result_json = AGENTRT_STRDUP("{\"rejected\":true,\"reason\":\"consensus_not_reached\"}");
    }
    *result_json = AGENTRT_STRDUP(c->result_json);
                                        ...
                                        size_t count, size_t *registered_count)
{
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        mac_agent_info_t *slot = &fw->agents[fw->agent_count];
        __builtin_memcpy(slot, &agents[i], sizeof(mac_agent_info_t));
        slot->capabilities_json =
            agents[i].capabilities_json ? AGENTRT_STRDUP(agents[i].capabilities_json) : NULL;
        slot->current_tasks = 0;
        slot->available = true;
