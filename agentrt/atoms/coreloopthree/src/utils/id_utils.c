...
__attribute__((used)) void agentrt_generate_task_id(const char *prefix, char *buf, size_t len)
{
    if (!buf || len == 0)
        return;
...
__attribute__((used)) void agentrt_generate_plan_id(char *buf, size_t len)
{
    if (!buf || len == 0)
        return;
...
__attribute__((used)) void agentrt_generate_record_id(char *buf, size_t len)
{
    if (!buf || len == 0)
        return;
...
__attribute__((used)) void agentrt_generate_session_id(char *buf, size_t len)
{
    if (!buf || len == 0)
        return;
...
__attribute__((used)) agentrt_error_t agentrt_generate_uuid(char *buf)
{
    if (!buf)
...
AGENTRT_STRNCPY_TERM(buf, (char *)str, 37);
    RpcStringFreeA(&str);
#else
    uuid_t uuid;
    uuid_generate(uuid);
