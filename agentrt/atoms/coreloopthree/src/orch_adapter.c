...
#define DEFAULT_CHECKPOINT_PATH         AGENTRT_DATA_DIR "/checkpoints"
...
struct orch_adapter_s {
    ...
    /* 实例池 */
    agentrt_core_loop_t **loop_instances; /* CoreLoopThree 实例池 */
 ...
 */
static agentrt_core_loop_t *acquire_loop_instance(orch_adapter_t *adapter)
{
    if (!adapter) return NULL;

    /* 如果实例池未满，创建新实例 */
    if (adapter->instance_count < adapter->max_instances) {
        agentrt_loop_config_t loop_cfg;
        ...
        agentrt_error_t ret = agentrt_loop_create(&loop_cfg, &loop);
        if (ret == AGENTRT_SUCCESS && loop) {
...
static char *orch_result_to_output(const orch_result_t *result)
{
    ...
    if (result->output) {
        return AGENTRT_STRDUP(result->output);
    }
                                    ...
                                    const orch_adapter_config_t *config)
{
    ...
    orch_adapter_t *adapter =
        (orch_adapter_t *)AGENTRT_CALLOC(1, sizeof(orch_adapter_t));
    if (!adapter) {
        AGENTRT_LOG_ERROR("C-L06: orch_adapter_create: OOM");
        return NULL;
    }
    ...
    /* 分配实例池 */
    adapter->loop_instances = (agentrt_core_loop_t **)AGENTRT_CALLOC(
        adapter->max_instances, sizeof(agentrt_core_loop_t *));
    if (!adapter->loop_instances) {
        AGENTRT_LOG_ERROR("C-L06: Failed to allocate instance pool");
    ...
    if (adapter->config.enable_checkpoint) {
        ...
        if (!adapter->checkpoint) {
            AGENTRT_LOG_WARN("C-L06: Checkpoint adapter init failed, "
                             "proceeding without checkpoint");
        }
    }

    AGENTRT_LOG_INFO("C-L06: Orchestrator adapter created "
...
void orch_adapter_destroy(orch_adapter_t *adapter)
{
    ...
    AGENTRT_LOG_INFO("C-L06: Orchestrator adapter destroyed "
    ...
    for (uint32_t i = 0; i < adapter->instance_count; i++) {
        if (adapter->loop_instances[i]) {
            agentrt_loop_destroy(adapter->loop_instances[i]);
            adapter->loop_instances[i] = NULL;
        }
    }

    if (adapter->loop_instances) AGENTRT_FREE(adapter->loop_instances);
                                 ...
                                 void *llm_svc_adapter)
{
     ...
     * agentrt_cognition_set_llm_adapter() 注入到认知引擎（IPC 路径，
    ...
    AGENTRT_LOG_INFO("C-L06: LLM service injected into orchestrator");
    return 0;
}
                                  ...
                                  void *tool_svc_adapter)
{
     ...
     * agentrt_cognition_set_tool_adapter() 注入到认知引擎（IPC 路径，
    ...
    AGENTRT_LOG_INFO("C-L06: Tool service injected into orchestrator");
    return 0;
}
                                  ...
                                  size_t *out_count)
{
    ...
    *out_count = 0;
    ...
    /* 获取 CoreLoopThree 实例 */
    agentrt_core_loop_t *loop = acquire_loop_instance(adapter);
    if (!loop) {
        AGENTRT_LOG_ERROR("C-L06: No available CoreLoopThree instance");
    ...
    if (ret != 0) {
        AGENTRT_LOG_ERROR("C-L06: Pipeline execution failed: ret=%d", ret);
        adapter->total_errors++;
        return ret;
    }
    ...
    AGENTRT_LOG_INFO("C-L06: Pipeline executed (%zu results, %llums)",
                               ...
                               orch_result_t **out_result)
{
    ...
    *out_result = NULL;
    ...
    if (ret != 0) {
        AGENTRT_LOG_ERROR("C-L06: Phase execution failed: phase=%d ret=%d",
                          phase, ret);
        adapter->total_errors++;
        return ret;
    }
    ...
    AGENTRT_LOG_INFO("C-L06: Phase %d executed successfully", phase);
    return 0;
}
...
{
    if (!adapter) {
        AGENTRT_LOG_WARN("C-L06: ORCH-ADAPTER-STATS unavailable");
        return;
    }
    ...
    AGENTRT_LOG_INFO("C-L06: ORCH-ADAPTER-STATS "
                     "pipelines=%llu steps=%llu errors=%llu "
