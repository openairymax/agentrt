...
{
    orchestrator_t *orch = (orchestrator_t *)AGENTRT_CALLOC(1, sizeof(orchestrator_t));
    if (!orch) {
    ...
    if (orch->config.timeout_ms == 0)
        orch->config.timeout_ms = AGENTRT_DEFAULT_TIMEOUT_MS * 2;
    if (orch->config.max_retries == 0)
        orch->config.max_retries = AGENTRT_DEFAULT_MAX_RETRIES;
    ...
    if (orch->config.enable_thinking_chain) {
        ...
        if (active) {
            ...
            agentrt_error_t mem_err = agentrt_builtin_memory_provider_init(NULL);
            if (mem_err != AGENTRT_SUCCESS) {
                SVC_LOG_WARN("orchestrator: memory init failed (err=%d), continuing without memory",
                             mem_err);
                orch->memory = NULL;
            } else {
                orch->memory = agentrt_memory_provider_get_active();
    ...
    if (orch->config.enable_metacognition) {
        agentrt_error_t cog_err = agentrt_cognition_create_take(NULL, NULL, NULL, &orch->cognition);
        if (cog_err != AGENTRT_SUCCESS) {
            SVC_LOG_WARN(
    ...
    if (orch->config.enable_critique_loop) {
        agentrt_error_t mc_err = agentrt_mc_create(&orch->metacognition);
        if (mc_err != AGENTRT_SUCCESS) {
    ...
    if (orch->cognition) {
        agentrt_cognition_set_context_take(orch->cognition,
...
void orchestrator_destroy(orchestrator_t *orch)
{
    ...
    for (uint32_t i = 0; i < orch->task_count; i++) {
        AGENTRT_FREE(orch->tasks[i].input);
        AGENTRT_FREE(orch->tasks[i].output);
    }
    ...
    if (orch->cognition)
        agentrt_cognition_destroy(orch->cognition);
    if (orch->metacognition)
        agentrt_mc_destroy(orch->metacognition);
    if (orch->calibrator)
...
orch_pipeline_t *orchestrator_pipeline_create(orchestrator_t *orch, const char *name)
{
    ...
    orch_pipeline_t *p = (orch_pipeline_t *)AGENTRT_CALLOC(1, sizeof(orch_pipeline_t));
    if (!p) {
    ...
    if (name)
AGENTRT_STRNCPY_TERM(p->name, name, sizeof(p->name));
    else
...
int orchestrator_pipeline_add_step(orch_pipeline_t *pipeline, const orch_pipeline_step_t *step)
{
    ...
    if (pipeline->step_count >= ORCH_MAX_STEPS) {
        ...
        AGENTRT_ERROR(AGENTRT_ERR_OVERFLOW, "pipeline step limit exceeded");
                                         ...
                                         const char *input)
{
    ...
    t->input = input ? AGENTRT_STRDUP(input) : AGENTRT_STRDUP("");
...
static agentrt_mutex_t g_orch_bus_mutex;
...
static void ensure_orch_bus_mutex(void)
{
    ...
    if (atomic_compare_exchange_strong_explicit(&g_orch_bus_mutex_initialized, &expected, 1,
        ...
        agentrt_mutex_init(&g_orch_bus_mutex);
    }
}

static char *memory_query_context(agentrt_memory_provider_t *mem, const char *query, uint32_t limit)
{
    if (!mem || !query) {
    ...
    float *scores = NULL;
    size_t count = 0;
    agentrt_error_t err = mem->query(mem, query, limit, &ids, &scores, &count);
    if (err != AGENTRT_SUCCESS || count == 0) {
        if (ids) {
    ...
    char *context = (char *)AGENTRT_MALLOC(buf_sz);
    if (!context) {
        for (size_t i = 0; i < count; i++)
            AGENTRT_FREE(ids[i]);
        AGENTRT_FREE(ids);
    ...
    for (size_t i = 0; i < count && pos < buf_sz - 2; i++) {
        void *data = NULL;
        size_t data_len = 0;
        if (mem->get_raw(mem, ids[i], &data, &data_len) == AGENTRT_SUCCESS && data) {
            if (i > 0)
...
static void memory_write_step(agentrt_memory_provider_t *mem, const char *phase_name,
...
static void memory_inform_evaluation(agentrt_memory_provider_t *mem, const char *phase, float score,
                                     bool passed)
{
...
static void memory_sync_persistent(agentrt_memory_provider_t *mem)
{
    if (!mem || !mem->evolve)
        return;
    mem->evolve(mem, 0);
...
static char *memory_retrieve_for_generation(agentrt_memory_provider_t *mem, const char *topic)
{
    ...
    float *scores = NULL;
    size_t count = 0;
    agentrt_error_t err = mem->retrieve(mem, topic, 5, &ids, &scores, &count);
    if (err != AGENTRT_SUCCESS || count == 0 || !ids) {
        if (ids) {
            for (size_t i = 0; i < count; i++)
                AGENTRT_FREE(ids[i]);
            AGENTRT_FREE(ids);
...
static char *call_llm_service(const char *prompt, const char *system_role)
{
    ...
    agentrt_mutex_lock(&g_orch_bus_mutex);
    if (!g_orch_bus) {
        g_orch_bus = ipc_service_bus_create("orchestrator_bus", NULL);
        if (!g_orch_bus) {
            agentrt_mutex_unlock(&g_orch_bus_mutex);
...
AGENTRT_STRNCPY_TERM(request.header.target, "llm_d", sizeof(request.header.target));
    ...
    agentrt_error_t err = ipc_service_bus_request(bus, "llm_d", &request, &response, 30000);
    if (err != AGENTRT_SUCCESS || !response.payload) {
        SVC_LOG_WARN("orchestrator: LLM call failed (err=%d), using fallback", err);
        if (response.payload) {
            AGENTRT_FREE(response.payload);
            response.payload = NULL;
        }
        agentrt_error_push_ex(AGENTRT_ERR_LLM_PROVIDER_FAIL, __FILE__, __LINE__, __func__,
                              "LLM service call failed (err=%d)", err);
        return NULL;
    }

    char *result = AGENTRT_STRDUP((const char *)response.payload);
    AGENTRT_FREE(response.payload);
    return result;
}
...
static char *build_decomposition_prompt(const char *input)
{
    ...
    char *prompt = (char *)AGENTRT_MALLOC(len);
    if (!prompt) {
...
static agentrt_mutex_t g_align_mutex;
                                ...
                                orch_result_t **out_result)
{
    ...
    if (!task) {
        SVC_LOG_ERROR("execute_single_phase: failed to create task entry for phase=%s",
                      phase_name(phase));
        AGENTRT_ERROR(AGENTRT_ERR_UNKNOWN, "failed to create task entry");
    }
    ...
    if (orch->breaker && !cb_allow_request(orch->breaker)) {
        ...
        task->error_code = -2;
        agentrt_error_push_ex(AGENTRT_ERR_SVC_BUSY, __FILE__, __LINE__, __func__,
    ...
    for (uint32_t attempt = 0; attempt < max_attempts; attempt++) {
        ...
        case ORCH_PHASE_DECOMPOSITION: {
            ...
            agentrt_task_plan_t *cog_plan = NULL;
            bool cognition_produced_plan = false;
            if (orch->cognition) {
                agentrt_error_t cog_err = agentrt_cognition_process(
                    orch->cognition, input ? input : "", strlen(input ? input : ""), &cog_plan);
                if (cog_err == AGENTRT_SUCCESS && cog_plan && cog_plan->task_plan_node_count > 0 &&
            ...
            if (cognition_produced_plan && cog_plan) {
                ...
                char *plan_json = (char *)AGENTRT_MALLOC(buf_sz);
                if (plan_json) {
                    size_t pos = snprintf(
                        plan_json, buf_sz,
                    ...
                    for (size_t n = 0; n < cog_plan->task_plan_node_count && pos < buf_sz - 2;
                         n++) {
                        agentrt_task_node_t *node = cog_plan->task_plan_nodes[n];
                        if (!node)
                            continue;
                        if (n > 0)
                    ...
                    SVC_LOG_ERROR("execute_single_phase: decomposition plan_json allocation failed (size=%zu)",
                                  buf_sz);
                    task->output = AGENTRT_STRDUP("{\"error\":\"memory_allocation_failed\","
                                                  "\"code\":-2,\"phase\":\"decomposition\"}");
                }
                agentrt_task_plan_free(cog_plan);
                cog_plan = NULL;
            } else {
                if (cog_plan) {
                    agentrt_task_plan_free(cog_plan);
        ...
        case ORCH_PHASE_PLANNING: {
            ...
            if (llm_result) {
                ...
                task->output =
                    AGENTRT_STRDUP("{\"error\":\"planning_failed\","
                                   "\"code\":-1,"
                                   "\"reason\":\"llm_service_unavailable\","
                                   "\"phase\":\"planning\","
        ...
        case ORCH_PHASE_GENERATION: {
            ...
            agentrt_task_plan_t *gen_plan = NULL;
            bool gen_cog_has_output = false;
            if (orch->cognition) {
                agentrt_error_t gerr = agentrt_cognition_process(
                    orch->cognition, input ? input : "", strlen(input ? input : ""), &gen_plan);
                if (gerr == AGENTRT_SUCCESS && gen_plan && gen_plan->task_plan_nodes) {
                ...
                if (loop_err == AGENTRT_SUCCESS && task_id) {
                    ...
                    if (wait_err == AGENTRT_SUCCESS && loop_result) {
                        ...
                        SVC_LOG_WARN("C-L06: CoreLoopThree wait failed (err=%d), falling back to LLM",
                                     wait_err);
                        if (loop_result) AGENTRT_FREE(loop_result);
                        loop_result = NULL;
                        /* 回退到 LLM */
...
generation_llm_fallback: {
            ...
            if (!t2_output) {
                if (gen_cog_has_output && gen_plan) {
                    ...
                    char *cog_fallback = (char *)AGENTRT_MALLOC(cbuf_sz);
                    if (cog_fallback) {
                        size_t cp =
                        ...
                        t2_output =
                            AGENTRT_STRDUP("{\"generated\":null,\"error\":\"llm_unavailable_and_"
                                           "alloc_failed\",\"mode\":\"t2_no_llm\"}");
                    }
                } else {
                    t2_output = AGENTRT_STRDUP("{\"generated\":null,\"error\":\"llm_unavailable\","
                                               "\"mode\":\"t2_no_llm\"}");
                }
            }
            ...
            for (int round = 0; round < ORCH_VERIFY_MAX_ROUNDS; round++) {
                ...
                if (corrected) {
                    AGENTRT_FREE(t2_output);
                    t2_output = corrected;
                }
            }

            if (gen_plan) {
                agentrt_task_plan_free(gen_plan);
                gen_plan = NULL;
            }

            size_t meta_sz = strlen(t2_output) + 256;
            char *meta_buf = (char *)AGENTRT_MALLOC(meta_sz);
            if (meta_buf) {
...
generation_skip_llm:
        ...
        case ORCH_PHASE_CRITIQUE: {
            ...
            char *current_output = AGENTRT_STRDUP(gen_output);
            ...
            for (uint32_t round = 0; round < max_critique_rounds; round++) {
                ...
                agentrt_thinking_step_t eval_step;
                ...
                agentrt_error_t mc_err =
                    agentrt_mc_evaluate_step(orch->metacognition, &eval_step, original_input,
                ...
                if (mc_err != AGENTRT_SUCCESS) {
                    ...
                    if (mc_result.critique_text)
                        AGENTRT_FREE(mc_result.critique_text);
                    break;
                }
                ...
                if (final_critique_text)
                    AGENTRT_FREE(final_critique_text);
                final_critique_text =
                    mc_result.critique_text ? AGENTRT_STRDUP(mc_result.critique_text) : NULL;
                if (mc_result.critique_text) {
                    AGENTRT_FREE(mc_result.critique_text);
                ...
                if (round < max_critique_rounds - 1) {
                    ...
                    char *corr_input = (char *)AGENTRT_MALLOC(corr_input_sz);
                    if (!corr_input)
                        break;
                ...
                if (mc_result.critique_text) {
                    agentrt_mc_feedback(orch->metacognition, round_score,
            ...
            if (orch->metacognition && total_critique_rounds > 1) {
                ...
                size_t pat_count = 0;
                agentrt_mc_detect_patterns(orch->metacognition, &patterns, &pat_count);
                if (pat_count > 0) {
                    SVC_LOG_INFO(
            ...
            char *cbuf = (char *)AGENTRT_MALLOC(cbuf_sz);
            if (cbuf) {
                ...
                task->output =
                    AGENTRT_STRDUP("{\"phase\":\"critique\",\"error\":\"alloc_failed\"}");
            }
        ...
        case ORCH_PHASE_VERIFICATION: {
            ...
            if (!content)
                content = AGENTRT_STRDUP(input ? input : "");
            ...
            task->output = AGENTRT_STRDUP(vbuf);
            task->output_len =
        ...
        case ORCH_PHASE_AUDIT: {
            ...
            char *abuf = (char *)AGENTRT_MALLOC(abuf_sz);
            if (abuf) {
                ...
                task->output = AGENTRT_STRDUP(audit_passed ? "{\"audit_passed\":true}"
                                                           : "{\"audit_passed\":false}");
            }
            task->output_len = strlen(task->output);
            if (audit_passed)
            ...
            AGENTRT_FREE(audit_result);
            audit_result = NULL;
            AGENTRT_FREE(severity);
        } break;
        ...
        case ORCH_PHASE_ALIGNMENT: {
...

            {
                ...
                if (atomic_compare_exchange_strong_explicit(&g_align_mutex_initialized, &expected,
                    ...
                    agentrt_mutex_init(&g_align_mutex);
                }
            }

            agentrt_mutex_lock(&g_align_mutex);
            ...
            agentrt_mutex_unlock(&g_align_mutex);
            ...
            task->output = AGENTRT_STRDUP(albuf);
            task->output_len =
        ...
        default:
            ...
            task->status = ORCH_TASK_FAILED;
            task->error_code = AGENTRT_ERR_UNKNOWN;
            ret = AGENTRT_ERR_UNKNOWN;
            break;
        }
    ...
    if (out_result) {
        orch_result_t *r = (orch_result_t *)AGENTRT_CALLOC(1, sizeof(orch_result_t));
        if (r) {
            r->task_id = AGENTRT_STRDUP(task->id);
            r->output = task->output ? AGENTRT_STRDUP(task->output) : AGENTRT_STRDUP("");
            r->output_len = task->output_len;
            r->status = task->status;
            r->error_code = task->error_code;
            r->duration_ms = task->duration_ms;
            r->thinking_chain_id = AGENTRT_STRDUP(task->thinking_chain_id);
            *out_result = r;
        } else {
                         ...
                         size_t *out_count)
{
        ...
        if (orch->memory->retrieve) {
            ...
            if (vol_ids) {
                for (size_t v = 0; v < vol_count; v++)
                    AGENTRT_FREE(vol_ids[v]);
                AGENTRT_FREE(vol_ids);
                vol_ids = NULL;
            }
            if (vol_scores) {
                AGENTRT_FREE(vol_scores);
                vol_scores = NULL;
            }
        }
    }
    ...
    orch_result_t *results = (orch_result_t *)AGENTRT_CALLOC(phase_count, sizeof(orch_result_t));
    if (!results) {
                                  ...
                                  const char *input, orch_result_t **out_results, size_t *out_count)
{
    ...
    orch_result_t *results =
        (orch_result_t *)AGENTRT_CALLOC(pipeline->step_count, sizeof(orch_result_t));
    if (!results) {
    ...
    for (uint32_t i = 0; i < pipeline->step_count; i++) {
        ...
        if (step->condition_fn && !step->condition_fn(current_input, step->condition_data)) {
            results[completed].task_id = AGENTRT_STRDUP("skipped");
            results[completed].status = ORCH_TASK_CANCELLED;
            results[completed].output = AGENTRT_STRDUP("");
            completed++;
            continue;
        }
...
void orchestrator_set_core_loop(orchestrator_t *orch, void *core_loop)
{
    if (!orch)
        return;
    orch->core_loop = (agentrt_core_loop_t *)core_loop;
    if (orch->core_loop) {
...
void orchestrator_set_cognition_llm_service(orchestrator_t *orch, llm_service_t *llm_svc)
{
    if (!orch)
        return;
    if (orch->cognition) {
        agentrt_cognition_set_llm_service(orch->cognition, llm_svc);
...
void orchestrator_set_cognition_tool_service(orchestrator_t *orch, tool_service_t *tool_svc)
{
    if (!orch)
        return;
    if (orch->cognition) {
        agentrt_cognition_set_tool_service(orch->cognition, tool_svc);
...
orch_result_t *orchestrator_get_result(orchestrator_t *orch, const char *task_id)
{
    ...
    for (uint32_t i = 0; i < orch->task_count; i++) {
        if (strcmp(orch->tasks[i].id, task_id) == 0) {
            ...
            r->task_id = AGENTRT_STRDUP(orch->tasks[i].id);
            r->output =
                orch->tasks[i].output ? AGENTRT_STRDUP(orch->tasks[i].output) : AGENTRT_STRDUP("");
            ...
            r->thinking_chain_id = AGENTRT_STRDUP(orch->tasks[i].thinking_chain_id);
            return r;
        }
    }
    return NULL;
...
void orchestrator_result_free(orch_result_t *result)
{
    ...
    result->task_id = NULL;
    AGENTRT_FREE(result->output);
    result->output = NULL;
    AGENTRT_FREE(result->thinking_chain_id);
...
int orchestrator_cancel(orchestrator_t *orch, const char *task_id)
{
    ...
    for (uint32_t i = 0; i < orch->task_count; i++) {
        if (strcmp(orch->tasks[i].id, task_id) == 0) {
            ...
            AGENTRT_ERROR(AGENTRT_ERR_UNKNOWN, "task not in cancellable state");
        }
    }
    SVC_LOG_WARN("orchestrator_cancel: task '%s' not found", task_id);
    AGENTRT_ERROR(AGENTRT_ERR_NOT_FOUND, "task not found");
}
