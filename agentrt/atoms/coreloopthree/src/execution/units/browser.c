...
{
    return (uint32_t)(agentrt_time_ms() & 0xFFFFFFFF);
}
...
static int browser_mgr_init(void)
{
    ...
    agentrt_mutex_init(&g_browser_mgr.pool_lock);
    agentrt_mutex_init(&g_browser_mgr.browser_lock);
    return 0;
}
...
static void browser_mgr_shutdown(void)
{
    ...
    agentrt_mutex_lock(&g_browser_mgr.browser_lock);
    ...
    agentrt_mutex_unlock(&g_browser_mgr.browser_lock);

    agentrt_mutex_destroy(&g_browser_mgr.pool_lock);
    agentrt_mutex_destroy(&g_browser_mgr.browser_lock);
    atomic_store_explicit(&g_browser_mgr_initialized, 0, memory_order_seq_cst);
}

int agentrt_browser_launch(const char *browser_path, int port, int headless,
...
int agentrt_browser_attach(const char *debugging_url)
{
    browser_mgr_init();
...
int agentrt_browser_close(void)
{
    if (!atomic_load_explicit(&g_browser_mgr_initialized, memory_order_acquire))
        ATM_RET_ERR(AGENTRT_EINVAL);
    ...
    agentrt_mutex_lock(&g_browser_mgr.pool_lock);
    for (size_t i = 0; i < g_browser_mgr.connection_count; i++) {
        if (g_browser_mgr.connections[i].socket_fd > 0) {
#ifdef _WIN32
            closesocket(g_browser_mgr.connections[i].socket_fd);
    ...
    agentrt_mutex_unlock(&g_browser_mgr.pool_lock);
...
int agentrt_browser_get_state(void)
{
    if (!atomic_load_explicit(&g_browser_mgr_initialized, memory_order_acquire))
...
int agentrt_browser_create_context(const char *agent_id, char *out_context_id, size_t ctx_size)
{
...
int agentrt_browser_destroy_context(const char *context_id)
{
...
int agentrt_browser_get_context_count(void)
{
    if (!atomic_load_explicit(&g_browser_mgr_initialized, memory_order_acquire))
        return 0;
...
static int ws_send_frame(int fd, const char *payload, size_t payload_len)
{
    ...
    uint8_t *masked = (uint8_t *)AGENTRT_MALLOC(payload_len);
    if (!masked)
        ATM_RET_ERR(AGENTRT_EINVAL);
...
static int ws_recv_frame(int fd, char **out_payload, size_t *out_len, uint32_t timeout_ms)
{
    ...
    char *payload = (char *)AGENTRT_MALLOC((size_t)payload_len + 1);
    if (!payload)
        ATM_RET_ERR(AGENTRT_EINVAL);
...
static char *js_escape(const char *src, size_t src_len)
{
    ...
    char *dst = (char *)AGENTRT_MALLOC(est);
    if (!dst) return NULL;
    ...
    for (size_t i = 0; i < src_len; i++) {
        ...
        if (ch == '\\') {
            if (dp + 2 >= est) {
                est *= 2;
                char *n = (char *)AGENTRT_MALLOC(est);
                if (!n) {
                    AGENTRT_FREE(dst);
                    dst = NULL;
...
static agentrt_error_t browser_execute(agentrt_execution_unit_t *unit, const void *input,
                                       void **out_output)
{
    ...
    if (strstr(cmd, "navigate") != NULL) {
        ...
        if (!url_start) {
            ret = AGENTRT_EINVAL;
            result_json = AGENTRT_STRDUP("{\"error\":\"no_url_provided\",\"status\":\"failed\"}");
            if (!result_json) {
                ret = AGENTRT_ENOMEM;
                goto cleanup;
            }
        ...
        if (!is_safe_url(url_start)) {
            ret = AGENTRT_EPERM;
            result_json = AGENTRT_STRDUP("{\"error\":\"unsafe_url\",\"status\":\"denied\"}");
            if (!result_json) {
                ret = AGENTRT_ENOMEM;
                goto cleanup;
            }
                ...
                if (ws_send_frame(conn->socket_fd, cdp_json, strlen(cdp_json)) == 0) {
                    ...
                    if (ws_recv_frame(conn->socket_fd, &resp, NULL, 5000) == 0 && resp) {
                        if (strstr(resp, "\"result\"")) {
                            ...
                            result_json = (char *)AGENTRT_MALLOC(buf_size);
                            if (result_json) {
        ...
        *out_output = result_json;
        ...
        if (!script) {
            ret = AGENTRT_EINVAL;
            *out_output = AGENTRT_STRDUP("{\"error\":\"missing_script\",\"status\":\"failed\"}");
            if (!*out_output)
                ret = AGENTRT_ENOMEM;
            goto cleanup;
        }
        ...
        *out_output = result_json;
        ...
        if (selector) {
            ...
            sel_copy = (char *)AGENTRT_MALLOC(sel_len + 1);
            if (!sel_copy) {
                ret = AGENTRT_ENOMEM;
                goto cleanup;
            }
        ...
        if (!cdp_ok || !result_json) {
            if (sel_copy) {
                ...
                result_json = AGENTRT_STRDUP("{\"status\":\"clicked\",\"selector\":\"unknown\","
                                             "\"simulated\":true}");
                if (!result_json) {
                    AGENTRT_FREE(sel_copy);
                    sel_copy = NULL;
        ...
        *out_output = result_json;
        ...
        if (has_cdp) {
            ...
            if (wait_selector) {
                ...
                char *sel_copy = (char *)AGENTRT_MALLOC(sel_len + 1);
                if (!sel_copy) {
                    ret = AGENTRT_ENOMEM;
                    goto cleanup;
                }
    ...
    ret = AGENTRT_EPROTONOSUPPORT;
    *out_output = AGENTRT_STRDUP("{\"error\":\"unsupported_command\",\"status\":\"failed\"}");
    if (!*out_output)
        ret = AGENTRT_ENOMEM;
...
agentrt_error_t agentrt_browser_fill_form(void *conn_ptr, const char *selector, size_t selector_len,
                                          ...
                                          size_t *out_output_len)
{
    ...
    *out_output_len = 0;
    ...
    if (!conn || conn->socket_fd < 0) {
        ...
        char *result = (char *)AGENTRT_MALLOC(buf_size);
        if (!result)
    ...
    if (used_cdp) {
        char *desc_buf = (char *)AGENTRT_MALLOC(384);
        if (desc_buf) {
        ...
        final_result = (char *)AGENTRT_MALLOC(384);
        if (final_result) {
...
agentrt_error_t agentrt_browser_wait_for_element(void *conn_ptr, const char *selector,
    ...
    *out_output_len = 0;
    ...
    if (selector && selector_len > 0 && !is_delay) {
        ...
        if (escaped_sel) {
            ...
            char *result = (char *)AGENTRT_MALLOC(384);
            if (result) {
                snprintf(result, 383,
                         "{\"status\":\"waited\",\"selector\":\"%.*s\","
            ...
            return result ? AGENTRT_SUCCESS : AGENTRT_ENOMEM;
        }
...
static void browser_destroy(agentrt_execution_unit_t *unit)
{
    if (!unit)
        return;
...
agentrt_execution_unit_t *agentrt_browser_unit_create(void)
{
    ...
    browser_unit_data_t *data = (browser_unit_data_t *)AGENTRT_MALLOC(sizeof(browser_unit_data_t));
    if (!data) {
        AGENTRT_FREE(unit);
        unit = NULL;
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
...
int agentrt_browser_unit_set_agent(agentrt_execution_unit_t *unit, const char *agent_id)
{
    if (!unit || !unit->execution_unit_data || !agent_id)
        ATM_RET_ERR(AGENTRT_EINVAL);
    browser_unit_data_t *data = (browser_unit_data_t *)unit->execution_unit_data;
AGENTRT_STRNCPY_TERM(data->agent_id, agent_id, sizeof(data->agent_id));
