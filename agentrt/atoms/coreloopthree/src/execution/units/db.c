...
static agentrt_error_t db_execute(agentrt_execution_unit_t *unit, const void *input,
                                  void **out_output)
{
    ...
    if (!data->db) {
        *out_output = AGENTRT_STRDUP("{\"error\":\"no_database_connection\"}");
    ...
    if (rc != SQLITE_OK) {
        ...
        char *err_result = (char *)AGENTRT_MALLOC(buf_size);
        if (err_result) {
            snprintf(err_result, buf_size, "{\"error\":\"prepare_failed\",\"detail\":\"%s\"}",
                     sqlite3_errmsg(data->db));
        }
        *out_output = err_result;
        return *out_output ? AGENTRT_EIO : AGENTRT_ENOMEM;
    }

    size_t result_cap = 4096;
    char *result = (char *)AGENTRT_MALLOC(result_cap);
    if (!result) {
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        if (pos + 256 > result_cap) {
            result_cap *= 2;
            char *new_result = (char *)AGENTRT_REALLOC(result, result_cap);
            if (!new_result) {
                AGENTRT_FREE(result);
                sqlite3_finalize(stmt);
                ATM_RET_ERR(AGENTRT_ENOMEM);
...
static void db_destroy(agentrt_execution_unit_t *unit)
{
    if (!unit)
        return;
    db_unit_data_t *data = (db_unit_data_t *)unit->execution_unit_data;
    if (data) {
        ...
        if (data->connection_string)
            AGENTRT_FREE(data->connection_string);
...
agentrt_execution_unit_t *agentrt_db_unit_create(const char *connection_string)
{
    ...
    db_unit_data_t *data = (db_unit_data_t *)AGENTRT_MALLOC(sizeof(db_unit_data_t));
    if (!data) {
        AGENTRT_FREE(unit);
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
    }

    data->connection_string = connection_string ? AGENTRT_STRDUP(connection_string) : NULL;
