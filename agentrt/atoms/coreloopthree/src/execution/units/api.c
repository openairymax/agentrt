 ...
 * - 支持自定义请求体和超时
 * - libcurl 不可用时返回 AGENTRT_ENOTSUP（非桩函数）
 */
                     ...
                     size_t path_size)
{
    ...
    if (slash) {
        AGENTRT_STRNCPY_TERM(path, slash, path_size);
    } else {
        AGENTRT_STRNCPY_TERM(path, "/", path_size);
    }
 ...
 */
static agentrt_error_t http_execute_curl(const char *url, const char *method,
                                          ...
                                          long timeout_ms, http_response_buffer_t *out_buf)
{
    ...
    /* 初始化响应缓冲区 */
    out_buf->data = (char *)AGENTRT_MALLOC(8192);
    if (!out_buf->data) {
...
static agentrt_error_t api_execute(agentrt_execution_unit_t *unit, const void *input,
                                   void **out_output)
{
    ...
    if (parsed_fields < 2)
    {
        ...
        AGENTRT_STRNCPY_TERM(method, "GET", sizeof(method));
    ...
    for (size_t i = 0; i < data->cached_count; i++) {
        if (data->cached_responses[i] && strstr(data->cached_responses[i], url)) {
            *out_output = AGENTRT_STRDUP(data->cached_responses[i]);
            agentrt_mutex_unlock(data->lock);
            return *out_output ? AGENTRT_SUCCESS : AGENTRT_ENOMEM;
        }
    }
    agentrt_mutex_unlock(data->lock);
    ...
    agentrt_error_t err = http_execute_curl(url, method,
    ...
    if (data->cached_count < data->cache_capacity) {
        ...
        char *cache_entry = (char *)AGENTRT_MALLOC(entry_size);
        if (cache_entry) {
    ...
    *out_output = response.data ? AGENTRT_STRDUP(response.data) : AGENTRT_STRDUP("");
    http_response_free(&response);
...
static void api_destroy(agentrt_execution_unit_t *unit)
{
    if (!unit)
        return;
    ...
    if (data) {
        if (data->id)
            AGENTRT_FREE(data->id);
        if (data->description)
            AGENTRT_FREE(data->description);

        if (data->cached_responses) {
            for (size_t i = 0; i < data->cached_count; i++) {
                if (data->cached_responses[i]) {
                    AGENTRT_FREE(data->cached_responses[i]);
                }
            }
            AGENTRT_FREE(data->cached_responses);
        }
...
static const char *api_get_metadata(agentrt_execution_unit_t *unit)
{
...
agentrt_error_t agentrt_unit_api_create(const api_config_t *config,
                                        agentrt_execution_unit_t **out_unit)
{
    if (!config || !out_unit) {
        ATM_RET_ERR(AGENTRT_EINVAL);
    }
    ...
    api_unit_data_t *data = (api_unit_data_t *)AGENTRT_CALLOC(1, sizeof(api_unit_data_t));
    if (!data) {
        AGENTRT_FREE(unit);
        ATM_RET_ERR(AGENTRT_ENOMEM);
    }

    data->id = AGENTRT_STRDUP("api");
    data->description = AGENTRT_STRDUP("Execute HTTP/HTTPS API calls via libcurl");
    ...
    data->cached_responses = (char **)AGENTRT_CALLOC(10, sizeof(char *));
    if (!data->cached_responses) {
        AGENTRT_FREE(data->id);
        AGENTRT_FREE(data->description);
        AGENTRT_FREE(data);
