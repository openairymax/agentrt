...
static char *build_tool_execute_json(const tool_execute_request_t *req)
{
    ...
    char *json = (char *)AGENTRT_MALLOC(65536);
    if (!json) return NULL;
...
static tool_result_t *parse_tool_result_json(const char *json)
{
    ...
    tool_result_t *result =
        (tool_result_t *)AGENTRT_CALLOC(1, sizeof(tool_result_t));
    ...
    if (output_start) {
        ...
        if (output_start) {
            ...
            if (len > 0) {
                result->output = (char *)AGENTRT_MALLOC(len + 1);
                if (result->output) {
                    AGENTRT_MEMCPY(result->output, output_start, len);
                    result->output[len] = '\0';
                }
            }
        }
    ...
    if (error_start) {
        ...
        if (error_start) {
            ...
            if (len > 0) {
                result->error = (char *)AGENTRT_MALLOC(len + 1);
                if (result->error) {
                    AGENTRT_MEMCPY(result->error, error_start, len);
                    result->error[len] = '\0';
                }
            }
        }
...
tool_svc_adapter_t *tool_svc_adapter_create(const tool_svc_adapter_config_t *config)
{
    tool_svc_adapter_t *adapter =
        (tool_svc_adapter_t *)AGENTRT_CALLOC(1, sizeof(tool_svc_adapter_t));
    if (!adapter) {
        AGENTRT_LOG_ERROR("C-L04: Failed to allocate tool adapter");
        return NULL;
    }
    ...
    if (!adapter->bsd) {
        AGENTRT_LOG_WARN("C-L04: SD bootstrap failed for '%s', "
    ...
    if (!adapter->bipc) {
        AGENTRT_LOG_ERROR("C-L04: IPC bootstrap failed for '%s'", ch_name);
        if (adapter->bsd) {
    ...
    if (adapter->enable_approval) {
        ...
        if (adapter->approval_ctx) {
            AGENTRT_LOG_INFO("C-L04: Tool approval enabled (C-L05 integrated)");
        } else {
            AGENTRT_LOG_WARN("C-L04: Tool approval init failed, "
                             "proceeding without approval");
        }
    }

    /* tool_service_t 由 tool_d daemon 内部构造和管理（见 daemons/tool_d/src/service.h，
     ...
     * 始终返回 NULL；认知引擎应通过 agentrt_cognition_set_tool_adapter() 注入 adapter。
     * 旧实现在此 AGENTRT_MALLOC(1) 分配 1 字节占位符桩并作为 tool_service_t 返回，
    ...
    AGENTRT_LOG_INFO("C-L04: Tool adapter created (service=%s, channel=%s, "
...
void tool_svc_adapter_destroy(tool_svc_adapter_t *adapter)
{
    ...
    AGENTRT_LOG_INFO("C-L04: Tool adapter destroyed (executions=%llu errors=%llu)",
...
tool_service_t *tool_svc_adapter_get_service(tool_svc_adapter_t *adapter)
{
     ...
     * 见 daemons/tool_d/src/service.h），由 daemon 构造和管理，coreloopthree 无法
     * 真实构造。本函数保留为接口兼容，始终返回 NULL。认知引擎应通过
     * agentrt_cognition_set_tool_adapter() 注入 IPC adapter，由 adapter 通过
     * IPC 调用 tool_d，而非直接持有 tool_service_t 句柄。
     * 旧实现返回 1 字节 AGENTRT_MALLOC(1) 占位符桩，若被 tool_service_execute()
     * 解引用会导致堆越界读——已移除。 */
    (void)adapter;
    return NULL;
}
                             ...
                             tool_result_t **out_result)
{
    ...
    *out_result = NULL;
    ...
    if (!adapter->connected || !adapter->bipc) {
        AGENTRT_LOG_WARN("C-L04: Adapter not connected, cannot execute tool");
    ...
    if (adapter->enable_approval && adapter->approval_ctx) {
        ...
        if (approval_ret != 0 || detail.decision == TOOL_APPROVAL_DENIED) {
            AGENTRT_LOG_WARN("C-L05: Tool '%s' denied by approval: %s",
                             req->tool_id ? req->tool_id : "?",
                             detail.reason[0] ? detail.reason : "policy");
            adapter->total_errors++;

            /* 返回审批拒绝的结果 */
            *out_result = (tool_result_t *)AGENTRT_CALLOC(1, sizeof(tool_result_t));
            if (*out_result) {
                (*out_result)->success = 1;
                (*out_result)->exit_code = 1;
                (*out_result)->error = AGENTRT_STRDUP(
        ...
        if (detail.decision == TOOL_APPROVAL_SANITIZED) {
            AGENTRT_LOG_INFO("C-L05: Tool '%s' params sanitized by approval",
                             req->tool_id ? req->tool_id : "?");
        }
    }
    ...
    if (adapter->bsd) {
        ...
        if (sdh) {
            ...
            if (sd_ret != 0) {
                AGENTRT_LOG_WARN("C-L04: tool_d '%s' not found in SD",
    ...
    if (!request_json) {
        AGENTRT_LOG_ERROR("C-L04: Failed to build execute request JSON");
        adapter->total_errors++;
        return -1;
    }
    ...
    if (ipc_ret != 0) {
        AGENTRT_LOG_ERROR("C-L04: IPC tool execute failed: ret=%d", ipc_ret);
        adapter->total_errors++;
        return -1;
    }
    ...
    if (response_msg.payload) {
        AGENTRT_FREE(response_msg.payload);
    }

    if (!*out_result) {
        AGENTRT_LOG_WARN("C-L04: Failed to parse tool result");
        adapter->total_errors++;
        return -1;
    }
                              ...
                              const tool_metadata_t *meta)
{
    ...
    if (ipc_ret != 0) {
        AGENTRT_LOG_ERROR("C-L04: Tool register IPC failed: ret=%d", ipc_ret);
        return -1;
    }

    AGENTRT_LOG_INFO("C-L04: Tool '%s' registered via IPC", meta->name);
    return 0;
}
...
int tool_svc_adapter_list(tool_svc_adapter_t *adapter, char **out_json)
{
    ...
    *out_json = NULL;
    ...
    if (ipc_ret == 0 && response_msg.payload && response_msg.payload_size > 0) {
        *out_json = (char *)AGENTRT_MALLOC(response_msg.payload_size + 1);
        if (*out_json) {
            AGENTRT_MEMCPY(*out_json, response_msg.payload, response_msg.payload_size);
