...
static void free_record_copy(agentrt_memory_record_t *rec)
{
    if (!rec)
        return;
    if (rec->memory_record_id)
        AGENTRT_FREE(rec->memory_record_id);
    if (rec->memory_record_source_agent)
        AGENTRT_FREE(rec->memory_record_source_agent);
    if (rec->memory_record_trace_id)
        AGENTRT_FREE(rec->memory_record_trace_id);
    if (rec->memory_record_data)
        AGENTRT_FREE(rec->memory_record_data);
    AGENTRT_FREE(rec);
}

static agentrt_memory_record_t *deep_copy_record(const agentrt_memory_record_t *record)
{
    agentrt_memory_record_t *copy =
        (agentrt_memory_record_t *)AGENTRT_CALLOC(1, sizeof(agentrt_memory_record_t));
    if (!copy) return NULL;

    if (record->memory_record_id) {
        copy->memory_record_id = AGENTRT_STRDUP(record->memory_record_id);
        if (!copy->memory_record_id)
            goto fail;
    }
    if (record->memory_record_source_agent) {
        copy->memory_record_source_agent = AGENTRT_STRDUP(record->memory_record_source_agent);
        if (!copy->memory_record_source_agent)
            goto fail;
    }
    if (record->memory_record_trace_id) {
        copy->memory_record_trace_id = AGENTRT_STRDUP(record->memory_record_trace_id);
        if (!copy->memory_record_trace_id)
            goto fail;
    }
    if (record->memory_record_data && record->memory_record_data_len > 0) {
        copy->memory_record_data = AGENTRT_MALLOC(record->memory_record_data_len);
...
static void *async_write_thread(void *arg)
{
    ...
    char *record_id = NULL;
    int err = agentrt_memory_write(req->engine, req->record, &record_id);
...
int agentrt_memory_write_async(agentrt_memory_engine_t *engine,
                               ...
                               void *userdata)
{
    ...
    async_write_req_t *req = (async_write_req_t *)AGENTRT_CALLOC(1, sizeof(async_write_req_t));
    if (!req)
        ATM_RET_ERR(AGENTRT_ENOMEM);

    agentrt_memory_record_t *rec_copy = deep_copy_record(record);
    if (!rec_copy) {
        AGENTRT_FREE(req);
        ATM_RET_ERR(AGENTRT_ENOMEM);
    }
    ...
    int rc = agentrt_platform_thread_create(&thread, async_write_thread, req);
    if (rc == 0) {
