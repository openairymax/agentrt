 ...
 *
 * 实现 agentrt_memory_provider_t 函数指针表，
 * 将 MemoryRovol API 封装为 AgentRT 标准内存提供商接口。
 *
 * 数据流：
 *   CoreLoopThree cognition → agentrt_memory_provider_t::write_raw()
 *     → memoryrovol_bridge → agentrt_memoryrov_write_raw()
 *     → MemoryRovol L1 存储 → L2 特征提取 → L3 结构绑定 → L4 模式识别
 *
 *   CoreLoopThree retrieval → agentrt_memory_provider_t::query()
 *     → memoryrovol_bridge → agentrt_memoryrov_query()
 *     → FAISS 向量检索 → 返回语义相似结果
 */
...
/* 跨平台路径常量（AGENTRT_DATA_DIR / AGENTRT_CONFIG_DIR） */
#include "platform.h"
...
#define DEFAULT_STORAGE_PATH     AGENTRT_DATA_DIR "/memoryrovol"
#define DEFAULT_CONFIG_PATH      AGENTRT_CONFIG_DIR "/memoryrovol.yaml"
...
struct memoryrovol_bridge_s {
    ...
    agentrt_memoryrovol_handle_t *rov_handle;  /* MemoryRovol 引擎句柄 */
    agentrt_memory_provider_t provider;         /* 填充的 provider 接口 */
    agentrt_memory_provider_t *builtin_provider;/* 内置提供商（混合模式） */
...
static void fill_capabilities(agentrt_memory_capabilities_t *caps,
...
static agentrt_error_t bridge_init(struct agentrt_memory_provider *provider,
                                   const char *config_path)
{
    memoryrovol_bridge_t *bridge =
        (memoryrovol_bridge_t *)provider->impl;
    if (!bridge) return AGENTRT_ERR_INVALID_PARAM;

    agentrt_memoryrov_config_t rov_cfg;
    __builtin_memset(&rov_cfg, 0, sizeof(rov_cfg));

    agentrt_error_t ret = agentrt_memoryrov_init(
        &rov_cfg, &bridge->rov_handle);
    if (ret != AGENTRT_SUCCESS) {
        AGENTRT_LOG_ERROR("C-L12: MemoryRovol init failed: ret=%d", ret);
        return ret;
    }

    AGENTRT_LOG_INFO("C-L12: MemoryRovol engine initialized");
    return AGENTRT_SUCCESS;
}

static void bridge_destroy(struct agentrt_memory_provider *provider)
{
    memoryrovol_bridge_t *bridge =
        (memoryrovol_bridge_t *)provider->impl;
    if (!bridge) return;
    ...
    if (bridge->rov_handle) {
        agentrt_memoryrov_cleanup(bridge->rov_handle);
        bridge->rov_handle = NULL;
    }
}

static agentrt_error_t bridge_write_raw(struct agentrt_memory_provider *provider,
                                        ...
                                        char **out_record_id)
{
    memoryrovol_bridge_t *bridge =
        (memoryrovol_bridge_t *)provider->impl;
    if (!bridge || !bridge->rov_handle) return AGENTRT_ERR_INVALID_STATE;

    uint64_t start_us = agentrt_time_ns() / 1000;

    agentrt_error_t ret = agentrt_memoryrov_write_raw(
    ...
    if (ret == AGENTRT_SUCCESS) {
        ...
        AGENTRT_LOG_DEBUG("C-L12: WRITE-OK id=%s len=%zu latency=%lluus "
        ...
        bridge->total_errors++;
        AGENTRT_LOG_WARN("C-L12: WRITE-FAIL len=%zu ret=%d latency=%lluus",
                         len, ret, (unsigned long long)latency_us);
    }
    return ret;
}

static agentrt_error_t bridge_get_raw(struct agentrt_memory_provider *provider,
                                      ...
                                      void **out_data, size_t *out_len)
{
    ...
    agentrt_error_t ret = agentrt_memoryrov_get_raw(
    ...
    if (ret == AGENTRT_SUCCESS) {
        ...
        AGENTRT_LOG_DEBUG("C-L12: READ-OK id=%s len=%zu latency=%lluus "
        ...
        bridge->total_errors++;
        AGENTRT_LOG_DEBUG("C-L12: READ-FAIL id=%s ret=%d latency=%lluus",
...
static agentrt_error_t bridge_delete_raw(struct agentrt_memory_provider *provider,
                                         const char *record_id)
{
    memoryrovol_bridge_t *bridge =
        (memoryrovol_bridge_t *)provider->impl;
    ...
    agentrt_error_t ret = agentrt_memoryrov_delete_raw(bridge->rov_handle, record_id);
    if (ret == AGENTRT_SUCCESS) {
        bridge->total_deletes++;
        AGENTRT_LOG_DEBUG("C-L12: DELETE-OK id=%s (total_deletes=%llu)",
                          record_id ? record_id : "?",
                          (unsigned long long)bridge->total_deletes);
    } else {
        bridge->total_errors++;
        AGENTRT_LOG_WARN("C-L12: DELETE-FAIL id=%s ret=%d",
                         record_id ? record_id : "?", ret);
    }
    return ret;
}

static agentrt_error_t bridge_query(struct agentrt_memory_provider *provider,
                                    ...
                                    size_t *out_count)
{
    ...
    agentrt_error_t ret = agentrt_memoryrov_query(
    ...
    if (ret == AGENTRT_SUCCESS) {
        ...
        AGENTRT_LOG_DEBUG("C-L12: QUERY-OK "
        ...
        bridge->total_errors++;
        AGENTRT_LOG_WARN("C-L12: QUERY-FAIL q=\"%.64s\" limit=%u ret=%d latency=%lluus",
...
static agentrt_error_t bridge_retrieve(struct agentrt_memory_provider *provider,
                                       const char *query_text, uint32_t limit,
                                       char ***out_record_ids, float **out_scores,
                                       size_t *out_count)
{
...
static agentrt_error_t bridge_evolve(struct agentrt_memory_provider *provider,
                                     int force)
{
    memoryrovol_bridge_t *bridge =
        (memoryrovol_bridge_t *)provider->impl;
    ...
    AGENTRT_LOG_INFO("C-L12: EVOLVE force=%d (total_evolves=%llu)",
                     force, (unsigned long long)bridge->total_evolves);

    agentrt_error_t ret = agentrt_memoryrov_evolve(bridge->rov_handle, force);
    if (ret == AGENTRT_SUCCESS) {
        bridge->total_evolves++;
    } else {
        bridge->total_errors++;
        AGENTRT_LOG_WARN("C-L12: EVOLVE-FAIL force=%d ret=%d", force, ret);
    }
    return ret;
}

static agentrt_error_t bridge_forget(struct agentrt_memory_provider *provider)
{
    memoryrovol_bridge_t *bridge =
        (memoryrovol_bridge_t *)provider->impl;
    if (!bridge || !bridge->rov_handle) return AGENTRT_ERR_INVALID_STATE;

    AGENTRT_LOG_INFO("C-L12: FORGET (total_forgets=%llu)",
                     (unsigned long long)bridge->total_forgets);

    agentrt_error_t ret = agentrt_memoryrov_forget(bridge->rov_handle);
    if (ret == AGENTRT_SUCCESS) {
        bridge->total_forgets++;
    } else {
        bridge->total_errors++;
        AGENTRT_LOG_WARN("C-L12: FORGET-FAIL ret=%d", ret);
    }
    return ret;
}

static agentrt_error_t bridge_stats(struct agentrt_memory_provider *provider,
                                    agentrt_memory_stats_t *out_stats)
{
    memoryrovol_bridge_t *bridge =
        (memoryrovol_bridge_t *)provider->impl;
    if (!bridge || !out_stats) return AGENTRT_ERR_INVALID_PARAM;

    __builtin_memset(out_stats, 0, sizeof(*out_stats));

    char *stats_json = NULL;
    agentrt_error_t ret = agentrt_memoryrov_stats(
        bridge->rov_handle, &stats_json);
    if (ret != AGENTRT_SUCCESS) return ret;
    ...
    if (stats_json) AGENTRT_FREE(stats_json);
    return AGENTRT_SUCCESS;
}

static agentrt_error_t bridge_mount(struct agentrt_memory_provider *provider,
                                    const char *record_id, const char *context)
{
    ...
    agentrt_error_t ret = agentrt_memoryrov_mount(bridge->rov_handle, record_id, context);
    if (ret == AGENTRT_SUCCESS) {
        bridge->total_mounts++;
        AGENTRT_LOG_DEBUG("C-L12: MOUNT-OK id=%s ctx=%.32s (total_mounts=%llu)",
                          record_id ? record_id : "?",
                          context ? context : "(none)",
                          (unsigned long long)bridge->total_mounts);
    } else {
        bridge->total_errors++;
        AGENTRT_LOG_WARN("C-L12: MOUNT-FAIL id=%s ret=%d",
                         record_id ? record_id : "?", ret);
    }
    return ret;
}

static agentrt_error_t bridge_health_check(struct agentrt_memory_provider *provider,
                                           char **out_json)
{
    memoryrovol_bridge_t *bridge =
        (memoryrovol_bridge_t *)provider->impl;
    if (!bridge || !out_json) return AGENTRT_ERR_INVALID_PARAM;
    ...
    *out_json = (char *)AGENTRT_MALLOC(needed + 1);
    if (!*out_json) return AGENTRT_ERR_OUT_OF_MEMORY;
...
static agentrt_error_t bridge_add_memory(struct agentrt_memory_provider *provider,
                                         const char *content, size_t content_len)
{
    ...
    return agentrt_memoryrov_add_memory(
        bridge->rov_handle, content, content_len);
}

static agentrt_error_t bridge_sync_push(struct agentrt_memory_provider *provider,
                                        const char *record_id)
{
    /* 混合模式：将内置提供商的数据推送到 MemoryRovol */
    memoryrovol_bridge_t *bridge =
        (memoryrovol_bridge_t *)provider->impl;
    if (!bridge) return AGENTRT_ERR_INVALID_STATE;

    if (!bridge->builtin_provider) return AGENTRT_ERR_NOT_SUPPORTED;

    /* 从内置提供商读取数据 */
    void *data = NULL;
    size_t len = 0;
    agentrt_error_t ret = bridge->builtin_provider->get_raw(
        bridge->builtin_provider, record_id, &data, &len);
    if (ret != AGENTRT_SUCCESS) return ret;

    /* 写入 MemoryRovol */
    ret = agentrt_memoryrov_write_raw(
...
static agentrt_error_t bridge_sync_pull(struct agentrt_memory_provider *provider,
...
static int bridge_has_active_sync(struct agentrt_memory_provider *provider)
{
    memoryrovol_bridge_t *bridge =
        (memoryrovol_bridge_t *)provider->impl;
    ...
    const memoryrovol_bridge_config_t *config)
{
    ...
    if (!bridge) {
        AGENTRT_LOG_ERROR("C-L12: memoryrovol_bridge_create: OOM");
        return NULL;
    }
    ...
    /* 填充 agentrt_memory_provider_t 函数指针表 */
    agentrt_memory_provider_t *p = &bridge->provider;
    ...
    agentrt_error_t ret = agentrt_memoryrov_init(
        &rov_cfg, &bridge->rov_handle);
    if (ret != AGENTRT_SUCCESS) {
        AGENTRT_LOG_WARN("C-L12: MemoryRovol init failed (ret=%d), "
    ...
    AGENTRT_LOG_INFO("C-L12: MemoryRovol bridge created "
...
void memoryrovol_bridge_destroy(memoryrovol_bridge_t *bridge)
{
    ...
    AGENTRT_LOG_INFO("C-L12: MemoryRovol bridge destroyed "
                     "(reads=%llu writes=%llu queries=%llu errors=%llu)",
                     (unsigned long long)bridge->total_reads,
                     (unsigned long long)bridge->total_writes,
                     (unsigned long long)bridge->total_queries,
                                   ...
                                   const char *mode)
{
    ...
    if (strcmp(mode, "builtin") == 0) {
        ...
        if (!bridge->builtin_provider) {
            bridge->builtin_provider = agentrt_builtin_provider_create();
        }
        if (bridge->builtin_provider) {
            agentrt_memory_provider_set_active(bridge->builtin_provider);
        }
        safe_strcpy(bridge->current_mode, "builtin",
                    sizeof(bridge->current_mode));
    } else if (strcmp(mode, "memoryrovol") == 0) {
        agentrt_memory_provider_set_active(&bridge->provider);
        safe_strcpy(bridge->current_mode, "memoryrovol",
                    sizeof(bridge->current_mode));
        ...
        AGENTRT_LOG_ERROR("C-L12: Unknown mode '%s'", mode);
        return -1;
    }

    AGENTRT_LOG_INFO("C-L12: Switched to mode '%s'", bridge->current_mode);
    return 0;
}
...
int memoryrovol_bridge_start_sync(memoryrovol_bridge_t *bridge)
{
    ...
    if (strcmp(bridge->current_mode, "hybrid") != 0) {
        AGENTRT_LOG_WARN("C-L12: Sync only available in hybrid mode "
                         "(current=%s)", bridge->current_mode);
        return -1;
    }

    bridge->sync_active = true;
    AGENTRT_LOG_INFO("C-L12: Sync started");
    return 0;
}
...
void memoryrovol_bridge_stop_sync(memoryrovol_bridge_t *bridge)
{
    if (!bridge) return;
    bridge->sync_active = false;
    AGENTRT_LOG_INFO("C-L12: Sync stopped");
}
                                 ...
                                 agentrt_memory_stats_t *out_stats)
{
    ...
    if (bridge->rov_handle) {
        char *stats_json = NULL;
        agentrt_memoryrov_stats(bridge->rov_handle, &stats_json);
        if (stats_json) AGENTRT_FREE(stats_json);
    }
...
void memoryrovol_bridge_dump_stats(memoryrovol_bridge_t *bridge)
{
    if (!bridge || !bridge->initialized) {
        AGENTRT_LOG_WARN("C-L12: BRIDGE-STATS unavailable");
        return;
    }
    ...
    AGENTRT_LOG_INFO("C-L12: BRIDGE-STATS mode=%s "
