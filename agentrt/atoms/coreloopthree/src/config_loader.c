...
/* Cross-platform abstraction layer (provides agentrt_thread_t,
 * agentrt_platform_thread_create/join — works on Linux, macOS, Windows) */
#include "platform.h"
...
#define AGENTRT_MAX_CONFIG_SIZE (4 * 1024 * 1024)
...
agentrt_error_t agentrt_config_load(const char *path, char **out_json)
{
    ...
    if (!file) {
        AGENTRT_LOG_ERROR("Failed to open manager file: %s", path);
    ...
    if (size < 0 || size > AGENTRT_MAX_CONFIG_SIZE) {
        fclose(file);
        return (size < 0) ? AGENTRT_EIO : AGENTRT_EINVAL;
    }

    char *buffer = (char *)AGENTRT_MALLOC((size_t)size + 1);
    if (!buffer) {
        fclose(file);
...
#define AGENTRT_DEFAULT_CONFIG_PATH  "./configs/agentrt.yaml"
#define AGENTRT_CONFIG_WATCH_INTERVAL_MS 1000
#define AGENTRT_MAX_RELOAD_CALLBACKS 32

/* ── 全局配置实例 ── */
static agentrt_yaml_config_t g_global_config;
static bool g_config_loaded = false;
static char g_config_path[256] = AGENTRT_DEFAULT_CONFIG_PATH;

/* ── 热重载回调 ── */
static agentrt_config_reload_cb_t g_reload_callbacks[AGENTRT_MAX_RELOAD_CALLBACKS];
static void *g_reload_userdata[AGENTRT_MAX_RELOAD_CALLBACKS];
static size_t g_reload_cb_count = 0;

/* ── 监听线程 ── */
static agentrt_thread_t g_watch_thread;
                             ...
                             agentrt_yaml_config_t *config)
{
    ...
    const char *path = yaml_path ? yaml_path : AGENTRT_DEFAULT_CONFIG_PATH;

    AGENTRT_LOG_INFO("ConfigLoader: loading YAML config from %s", path);

    /* 保存路径用于热重载 */
    safe_strcpy(g_config_path, sizeof(g_config_path), path);

    int ret = agentrt_yaml_load(path, config);
    if (ret != 0) {
        /* 加载失败，使用默认配置 */
        AGENTRT_LOG_WARN("ConfigLoader: YAML load failed for %s, using defaults", path);
        agentrt_yaml_config_defaults(config);
        agentrt_yaml_resolve_platform_paths(config);
        return 0; /* 返回成功但使用默认值 */
    }

    AGENTRT_LOG_INFO("ConfigLoader: YAML loaded successfully from %s", path);

    /* 平台路径映射（P1.12.4） */
    agentrt_yaml_resolve_platform_paths(config);

    /* 验证配置 */
    if (agentrt_yaml_validate(config) != 0) {
        agentrt_yaml_config_defaults(config);
        agentrt_yaml_resolve_platform_paths(config);
    }
                             ...
                             void *user_data)
{
    ...
    if (g_reload_cb_count >= AGENTRT_MAX_RELOAD_CALLBACKS) return -1;
...
static void config_reload_notify(const agentrt_yaml_config_t *old_config,
...
static void *config_watch_thread(void *arg)
{
    ...
    if (g_inotify_fd < 0) {
        AGENTRT_LOG_ERROR("C-L01: ConfigLoader: WATCH-FAIL — inotify_init1 failed "
                          "errno=%d (%s) "
    ...
    if (wd < 0) {
        AGENTRT_LOG_ERROR("C-L01: ConfigLoader: WATCH-FAIL — inotify_add_watch failed "
                          "path=%s errno=%d (%s) inotify_fd=%d "
                          "STACK: inotify_add_watch() → config_watch_thread() → "
                          "g_watch_running=%d",
                          g_config_path, errno, strerror(errno), g_inotify_fd,
                          (int)g_watch_running);
        AGENTRT_LOG_ERROR("C-L01: ConfigLoader: WATCH-FAIL — check if file exists "
    ...
    AGENTRT_LOG_INFO("C-L01: ConfigLoader: WATCH-STARTED path=%s inotify_fd=%d wd=%d "
    ...
    while (g_watch_running) {
        ...
        if (len < 0) {
            ...
            AGENTRT_LOG_WARN("C-L01: ConfigLoader: WATCH — read error (errno=%d)", errno);
            break;
        }

        if (len == 0) {
            /* EOF — 不应在 inotify 上发生 */
            AGENTRT_LOG_WARN("C-L01: ConfigLoader: WATCH — unexpected EOF");
            break;
        }
        ...
        while (offset < len) {
            ...
            if (event->mask & (IN_MODIFY | IN_CLOSE_WRITE)) {
                AGENTRT_LOG_INFO("C-L01: ConfigLoader: WATCH — file modified "
                ...
                AGENTRT_LOG_WARN("C-L01: ConfigLoader: WATCH — file deleted/moved "
                ...
                AGENTRT_LOG_INFO("C-L01: ConfigLoader: WATCH — removed old watch "
                ...
                if (wd < 0) {
                    ...
                    AGENTRT_LOG_ERROR("C-L01: ConfigLoader: WATCH-RE-ADD-FAIL "
                ...
                AGENTRT_LOG_INFO("C-L01: ConfigLoader: WATCH — watch re-added successfully "
        ...
        if (reload_triggered) {
            ...
            agentrt_yaml_config_t new_config;
            AGENTRT_MEMSET(&new_config, 0, sizeof(new_config));

            /* 分步执行以捕获详细错误码 */
            int load_rc = agentrt_yaml_load(g_config_path, &new_config);
            if (load_rc != 0) {
                /* 获取文件状态信息用于诊断 */
                struct stat file_stat;
                int stat_rc = stat(g_config_path, &file_stat);
                ...
                AGENTRT_LOG_ERROR("C-L01: ConfigLoader: WATCH-RELOAD-FAIL — "
                                  "YAML load failed "
                                  "path=%s rc=%d "
                                  "errno=%d (%s) "
                                  "file_stat=(rc=%d, err=%s, size=%ld, mode=%o, uid=%d, gid=%d) "
                                  "STACK: agentrt_yaml_load() → config_watch_thread() → "
                                  ...
                                  "[3] agentrt_yaml_load() returned rc=%d "
                                  ...
                                  load_rc);
                AGENTRT_LOG_WARN("C-L01: ConfigLoader: WATCH-RELOAD-FAIL — "
            ...
            int val_rc = agentrt_yaml_validate(&new_config);
            if (val_rc != 0) {
                /* 记录验证失败详情，同时记录文件状态 */
                struct stat file_stat;
                int stat_rc = stat(g_config_path, &file_stat);
                ...
                AGENTRT_LOG_ERROR("C-L01: ConfigLoader: WATCH-RELOAD-FAIL — "
                                  ...
                                  "errno=%d (%s) "
                                  "file_stat=(rc=%d, err=%s, size=%ld, mode=%o, uid=%d, gid=%d) "
                                  "STACK: agentrt_yaml_validate() → config_watch_thread() → "
                                  "IN_MODIFY/IN_CLOSE_WRITE handler → "
                                  "g_watch_running=%d inotify_fd=%d wd=%d "
                                  "TRACEBACK: [1] inotify event received (mask=IN_MODIFY|IN_CLOSE_WRITE) "
                                  "[2] agentrt_yaml_load() succeeded "
                                  "[3] agentrt_yaml_validate() returned rc=%d "
                                  "[4] ERROR: config NOT reloaded, keeping previous config",
                ...
                AGENTRT_LOG_WARN("C-L01: ConfigLoader: WATCH-RELOAD-FAIL — "
                                 ...
                                 "Check agentrt.yaml schema at %s",
                                 val_rc, g_config_path);
                continue;
            }

            agentrt_yaml_env_override(&new_config);

            agentrt_yaml_config_t old_config = g_global_config;
            g_global_config = new_config;

            config_reload_notify(&old_config, &new_config);

            AGENTRT_LOG_INFO("C-L01: ConfigLoader: WATCH-RELOAD — config reloaded "
    ...
    AGENTRT_LOG_INFO("C-L01: ConfigLoader: WATCH-STOPPED path=%s", g_config_path);
    return NULL;
}
...
{
    if (g_watch_running) {
        AGENTRT_LOG_WARN("C-L01: ConfigLoader: WATCH-START — already running");
        return -1;
    }
    ...
    AGENTRT_LOG_INFO("C-L01: ConfigLoader: WATCH-START path=%s interval_ms=%u "
                     "(P3.15: inotify-based, interval_ms ignored)",
                     g_config_path, interval_ms);

#ifdef __linux__
    if (agentrt_platform_thread_create(&g_watch_thread, config_watch_thread, NULL) != 0) {
        AGENTRT_LOG_ERROR("C-L01: ConfigLoader: WATCH-FAIL — agentrt_platform_thread_create failed");
        g_watch_running = false;
        return -1;
    }
#else
    /* macOS/Windows: inotify 不可用 — 优雅降级（返回成功但不实际监视文件）。
     * 手动热重载仍可通过 agentrt_config_reload() 触发。 */
    AGENTRT_LOG_WARN("C-L01: ConfigLoader: WATCH-START — inotify not supported on this "
                     "platform, config file watching disabled (graceful degradation). "
                     "Manual reload via agentrt_config_reload() is still available. path=%s",
                     g_config_path);
    g_watch_running = false;
#endif
...
{
    if (!g_watch_running) {
        AGENTRT_LOG_DEBUG("C-L01: ConfigLoader: WATCH-STOP — not running");
        return;
    }

    AGENTRT_LOG_INFO("C-L01: ConfigLoader: WATCH-STOP — stopping watch thread");
    ...
    /* P3.15: 等待监听线程退出 */
    int join_rc = agentrt_platform_thread_join(g_watch_thread, NULL);
    if (join_rc != 0) {
        AGENTRT_LOG_WARN("C-L01: ConfigLoader: WATCH-STOP — agentrt_platform_thread_join failed (rc=%d)",
                         join_rc);
    } else {
        AGENTRT_LOG_INFO("C-L01: ConfigLoader: WATCH-STOPPED — thread joined successfully");
    }
}
...
int agentrt_config_reload(const char *yaml_path)
{
    ...
    AGENTRT_LOG_INFO("C-L01: ConfigLoader: RELOAD-TRIGGERED path=%s g_config_loaded=%d",
                     path, (int)g_config_loaded);

    agentrt_yaml_config_t new_config;
    AGENTRT_MEMSET(&new_config, 0, sizeof(new_config));

    int load_rc = agentrt_yaml_load(path, &new_config);
    if (load_rc != 0) {
        struct stat file_stat;
        int stat_rc = stat(path, &file_stat);
        const char *stat_err = (stat_rc != 0) ? strerror(errno) : "OK";

        AGENTRT_LOG_ERROR("C-L01: ConfigLoader: RELOAD-FAIL — YAML load failed "
                          "path=%s rc=%d errno=%d (%s) "
                          "file_stat=(rc=%d, err=%s, size=%ld, mode=%o, uid=%d, gid=%d) "
                          "STACK: agentrt_yaml_load() → agentrt_config_reload() → caller "
                          "TRACEBACK: [1] agentrt_config_reload(%s) called "
                          "[2] agentrt_yaml_load() returned rc=%d "
    ...
    if (val_rc != 0) {
        ...
        AGENTRT_LOG_ERROR("C-L01: ConfigLoader: RELOAD-FAIL — validation failed "
                          "path=%s val_rc=%d errno=%d (%s) "
                          "file_stat=(rc=%d, err=%s, size=%ld, mode=%o, uid=%d, gid=%d) "
                          "STACK: agentrt_yaml_validate() → agentrt_config_reload() → caller "
                          "TRACEBACK: [1] agentrt_config_reload(%s) called "
                          "[2] agentrt_yaml_load() succeeded "
                          "[3] agentrt_yaml_validate() returned rc=%d "
    ...
    config_reload_notify(&old_config, &new_config);
    AGENTRT_LOG_INFO("C-L01: ConfigLoader: RELOAD-OK path=%s version=%s",
...
/**
 * @brief CoreLoopThree 配置模块初始化（在 agentrt_init() 之后调用）
 * @return 0 成功，非0失败
 */
int agentrt_config_init(const char *yaml_path)
{
    ...
    AGENTRT_LOG_INFO("ConfigLoader: init START (path=%s)", path);

    agentrt_yaml_config_defaults(&g_global_config);

    if (agentrt_yaml_load(path, &g_global_config) == 0) {
        g_config_loaded = true;
        AGENTRT_LOG_INFO("ConfigLoader: init OK (config loaded from %s)", path);
    } else {
        /* 文件不存在时使用默认配置 */
        g_config_loaded = true;
        AGENTRT_LOG_INFO("ConfigLoader: init OK (using defaults, %s not found)", path);
    }

    /* P1.1.3: 应用环境变量覆盖（C-L01） */
    agentrt_yaml_env_override(&g_global_config);

    /* 平台路径映射（P1.12.4） */
    agentrt_yaml_resolve_platform_paths(&g_global_config);
