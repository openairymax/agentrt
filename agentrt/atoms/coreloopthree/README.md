...
CoreLoopThree 是 AgentRT 的核心运行时引擎，采用创新的**"三循环"架构**实现智能体的感知-思考-行动闭环。该设计受心理学"双系统理论"（System 1 / System 2）启发，将快速直觉思维与慢速理性思维融入同一架构，并通过学习循环实现持续进化。
...
| 接口 | 功能 | 线程安全 |
|------|------|----------|
| `agentrt_loop_create()` | 创建核心循环（可指定配置） | 否 |
| `agentrt_loop_destroy()` | 销毁核心循环及所有内部资源 | 否 |
| `agentrt_loop_run()` | 启动核心循环（阻塞，直到停止） | 否 |
| `agentrt_loop_stop()` | 停止核心循环 | 是 |
| `agentrt_loop_submit()` | 提交用户任务（自然语言输入） | 是 |
| `agentrt_loop_wait()` | 等待任务完成并获取结果 | 是 |
| `agentrt_loop_get_engines()` | 获取内部引擎指针（用于扩展） | 否 |
| `agentrt_loop_submit_persistent()` | 提交持久化长任务（带检查点） | 是 |
| `agentrt_loop_restore_task()` | 从检查点恢复长任务 | 是 |
| `agentrt_loop_list_checkpoints()` | 列出可恢复的检查点任务 | 是 |

**配置结构** (`agentrt_loop_config_t`):
...
| 接口 | 功能 |
|------|------|
| `agentrt_cognition_create_take()` | 创建认知引擎（指定策略，获取策略所有权） |
| `agentrt_cognition_create_ex_take()` | 创建认知引擎（带配置，获取策略所有权） |
| `agentrt_cognition_destroy()` | 销毁认知引擎 |
| `agentrt_cognition_process()` | 处理用户输入，生成任务计划（DAG） |
| `agentrt_cognition_set_fallback_plan()` | 设置回退规划策略 |
| `agentrt_cognition_set_context_take()` | 设置全局上下文（获取上下文所有权） |
| `agentrt_cognition_set_memory()` | 绑定记忆引擎 |
| `agentrt_cognition_stats()` | 获取统计信息 |
| `agentrt_cognition_health_check()` | 健康检查 |

**策略接口**（可插拔）:

| 策略类型 | 结构体 | 说明 |
|----------|--------|------|
| 规划策略 | `agentrt_plan_strategy_t` | 根据意图生成任务计划（reactive/reflective/hierarchical/ml） |
| 协同策略 | `agentrt_coordinator_strategy_t` | 协调多个模型输出（majority/weighted/arbiter/dual_model） |
| 调度策略 | `agentrt_dispatching_strategy_t` | 从候选 Agent 中选择最合适的（round_robin/priority/weighted/ml） |

**意图解析器**:

| 接口 | 功能 |
|------|------|
| `agentrt_intent_parser_create()` | 创建意图解析器 |
| `agentrt_intent_parser_parse()` | 解析用户输入，提取意图 |
| `agentrt_intent_parser_add_rule()` | 添加自定义意图规则 |
| `agentrt_intent_free()` | 释放意图结构 |

### 执行引擎接口 (execution.h)

| 接口 | 功能 | 线程安全 |
|------|------|----------|
| `agentrt_execution_create()` | 创建执行引擎（指定最大并发数） | 否 |
| `agentrt_execution_destroy()` | 销毁执行引擎 | 否 |
| `agentrt_execution_register_unit()` | 注册执行单元 | 否 |
| `agentrt_execution_unregister_unit()` | 注销执行单元 | 否 |
| `agentrt_execution_submit()` | 提交任务执行 | 是 |
| `agentrt_execution_query()` | 查询任务状态 | 是 |
| `agentrt_execution_wait()` | 等待任务完成 | 是 |
| `agentrt_execution_cancel()` | 取消任务 | 是 |
| `agentrt_execution_get_result()` | 获取任务结果 | 是 |
| `agentrt_execution_set_feedback_callback()` | 设置反馈回调 | 是 |
...
| 接口 | 功能 |
|------|------|
| `agentrt_compensation_create()` | 创建补偿事务管理器 |
| `agentrt_compensation_register()` | 注册可补偿操作 |
| `agentrt_compensation_compensate()` | 执行补偿（回滚） |
| `agentrt_compensation_get_human_queue()` | 获取待人工介入的队列 |

### 记忆引擎接口 (memory.h)

| 接口 | 功能 | 线程安全 |
|------|------|----------|
| `agentrt_memory_create()` | 创建记忆引擎 | 否 |
| `agentrt_memory_destroy()` | 销毁记忆引擎 | 否 |
| `agentrt_memory_write()` | 写入记忆记录 | 是 |
| `agentrt_memory_query()` | 查询记忆（语义搜索） | 是 |
| `agentrt_memory_get()` | 根据 ID 获取记录 | 是 |
| `agentrt_memory_mount()` | 挂载记忆到当前上下文 | 是 |
| `agentrt_memory_evolve()` | 触发记忆进化（模式挖掘） | 否 |
| `agentrt_memory_health_check()` | 健康检查 | 是 |
...
int main(void) {
    ...
    agentrt_loop_config_t config = {0};
    ...
    char *task_id;
    agentrt_loop_submit(loop, "分析这份文档并生成摘要",
                        strlen("分析这份文档并生成摘要"), &task_id);

    char *result;
    size_t result_len;
    agentrt_loop_wait(loop, task_id, 60000, &result, &result_len);

    agentrt_mem_free(result);
    agentrt_mem_free(task_id);
