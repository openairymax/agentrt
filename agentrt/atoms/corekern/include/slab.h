 ...
 *   - P3.15.2: agentrt_slab_create/destroy/alloc/free API
...
#ifndef AGENTRT_COREKERN_SLAB_H
#define AGENTRT_COREKERN_SLAB_H
...
/** 默认每个 slab 页的对象数 */
#define AGENTRT_SLAB_DEFAULT_OBJS_PER_PAGE  64

/** 最大 CPU 数 */
#define AGENTRT_SLAB_MAX_CPUS               64

/** 默认 slab 名称最大长度 */
#define AGENTRT_SLAB_NAME_MAX               32
 ...
 */
typedef void *(*agentrt_slab_ctor_fn)(void *obj, void *arg);
 ...
 * @param arg  用户自定义参数
 */
typedef void (*agentrt_slab_dtor_fn)(void *obj, void *arg);
...
typedef struct {
    ...
    agentrt_slab_ctor_fn   ctor;             /**< 构造回调（可为 NULL） */
    agentrt_slab_dtor_fn   dtor;             /**< 析构回调（可为 NULL） */
    void                  *ctor_arg;         /**< 构造回调参数 */
    void                  *dtor_arg;         /**< 析构回调参数 */
    bool                   enable_stats;     /**< 是否启用统计 */
} agentrt_slab_config_t;
 ...
 *
 * 使用示例：
 *   agentrt_slab_config_t cfg = {
 *       .name = "ipc_msg",
 *       .obj_size = sizeof(ipc_msg_t),
 *   };
 *   agentrt_slab_t *slab = agentrt_slab_create(&cfg);
 */
AGENTRT_API agentrt_slab_t *agentrt_slab_create(const agentrt_slab_config_t *config);
 ...
 * @param slab slab 句柄
 */
AGENTRT_API void agentrt_slab_destroy(agentrt_slab_t *slab);
 ...
 */
AGENTRT_API void *agentrt_slab_alloc(agentrt_slab_t *slab);
 ...
 * @param obj  要释放的对象指针
 */
AGENTRT_API void agentrt_slab_free(agentrt_slab_t *slab, void *obj);
 ...
 */
AGENTRT_API void agentrt_slab_get_stats(agentrt_slab_t *slab,
 ...
 * @param slab slab 句柄
 */
AGENTRT_API void agentrt_slab_reset_stats(agentrt_slab_t *slab);
 ...
 */
AGENTRT_API size_t agentrt_slab_obj_size(agentrt_slab_t *slab);
 ...
 */
AGENTRT_API const char *agentrt_slab_name(agentrt_slab_t *slab);

#ifdef __cplusplus
}
#endif

#endif /* AGENTRT_COREKERN_SLAB_H */