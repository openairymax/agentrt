...
{
    /* 使用 agentrt_time_ms() 如果可用，否则回退到简单实现 */
    /* 使用平台时间函数获取毫秒时间戳 */
#ifdef _WIN32
 ...
 * @brief 获取压力级别名称（用于日志）
 */
static const char *oom_pressure_name(agentrt_mem_pressure_level_t level)
{
    switch (level) {
    case AGENTRT_MEM_PRESSURE_NORMAL:   return "NORMAL";
    case AGENTRT_MEM_PRESSURE_WARNING:  return "WARNING";
    case AGENTRT_MEM_PRESSURE_DEGRADED: return "DEGRADED";
    case AGENTRT_MEM_PRESSURE_CRITICAL: return "CRITICAL";
    case AGENTRT_MEM_PRESSURE_FATAL:    return "FATAL";
    default:                            return "UNKNOWN";
    }
}
 ...
 * @brief 基于内存使用率计算压力级别
 */
static agentrt_mem_pressure_level_t oom_calc_pressure_level(
    size_t current_allocated, size_t total_memory)
{
    if (total_memory == 0) {
        return AGENTRT_MEM_PRESSURE_NORMAL;
    }

    double usage = (double)current_allocated / (double)total_memory;

    if (usage > OOM_PRESSURE_THRESHOLD_FATAL)      return AGENTRT_MEM_PRESSURE_FATAL;
    else if (usage > OOM_PRESSURE_THRESHOLD_CRITICAL) return AGENTRT_MEM_PRESSURE_CRITICAL;
    else if (usage > OOM_PRESSURE_THRESHOLD_DEGRADED) return AGENTRT_MEM_PRESSURE_DEGRADED;
    else if (usage > OOM_PRESSURE_THRESHOLD_WARNING)  return AGENTRT_MEM_PRESSURE_WARNING;
    else                                               return AGENTRT_MEM_PRESSURE_NORMAL;
}
    ...
    agentrt_mem_pressure_level_t new_level)
{
    ...
    for (int level = start; level <= end; level++) {
        for (int i = 0; i < AGENTRT_PRESSURE_MAX_CALLBACKS; i++) {
...
agentrt_error_t agentrt_oom_init(size_t total_system_memory)
{
    /* 防止重复初始化 */
    ...
    oom_handler_t *handler =
        (oom_handler_t *)AGENTRT_CALLOC(1, sizeof(oom_handler_t));
    if (!handler) {
    ...
    /* 初始化扩展统计 */
    handler->ext_stats = (memory_stats_extended_t *)AGENTRT_CALLOC(
    ...
    /* 初始化分配跟踪环形缓冲区 */
    int ret = agentrt_memory_stats_extended_init(
        handler->ext_stats, OOM_TRACKER_CAPACITY);
    if (ret != 0) {
        AGENTRT_FREE(handler->ext_stats);
        AGENTRT_FREE(handler);
        return AGENTRT_ENOMEM;
    }
    ...
    /* 初始化压力分级系统 */
    handler->current_pressure = AGENTRT_MEM_PRESSURE_NORMAL;
    handler->pressure_denied_count = 0;
    handler->total_allocated_bytes = 0;
    agentrt_mutex_init(&handler->pressure_mutex);
    AGENTRT_MEMSET(handler->pressure_callbacks, 0,
                   sizeof(handler->pressure_callbacks));

    g_oom_handler = handler;

    /* SEC-13: Initialize pre-allocation pool for critical paths */
    if (agentrt_prealloc_init() != 0) {
        AGENTRT_LOG_WARN("[OOM] Pre-allocation pool init failed; "
                "critical path buffers unavailable");
    }

    AGENTRT_LOG_INFO("[OOM] OOM handler initialized: system_memory=%zu bytes, "
            "tracker_capacity=%d",
...
void agentrt_oom_destroy(void)
{
    ...
    /* 销毁扩展统计 */
    if (handler->ext_stats) {
        agentrt_memory_stats_extended_destroy(handler->ext_stats);
        AGENTRT_FREE(handler->ext_stats);
    ...
    /* 清理压力分级系统 */
    agentrt_mutex_destroy(&handler->pressure_mutex);
    AGENTRT_MEMSET(handler->pressure_callbacks, 0,
                   sizeof(handler->pressure_callbacks));
    handler->current_pressure = AGENTRT_MEM_PRESSURE_NORMAL;
    ...
    agentrt_prealloc_shutdown();

    AGENTRT_LOG_INFO("[OOM] OOM handler destroyed");
}

oom_handler_t *agentrt_oom_get_handler(void)
{
...
int agentrt_oom_handle(size_t requested, size_t available)
{
    if (!g_oom_handler) {
        /* 未初始化时直接返回 WARNING */
        return OOM_RESPONSE_WARNING;
    ...
    int response = agentrt_oom_determine_response(handler->current_watermark);

    AGENTRT_LOG_ERROR("[OOM] OOM event #%llu: requested=%zu, available=%zu, "
    ...
    case OOM_RESPONSE_DEGRADED:
        /* 触发降级回调 */
        agentrt_oom_degrade(old_level, handler->current_watermark);
        break;
    ...
    case OOM_RESPONSE_FATAL_TERMINATE:
        /* 最后尝试降级，然后终止 */
        agentrt_oom_degrade(old_level, handler->current_watermark);
        AGENTRT_LOG_ERROR("[OOM] FATAL: terminating process. "
...
agentrt_error_t agentrt_register_degradation(degradation_handler_t *handler)
{
    ...
    AGENTRT_LOG_INFO("[OOM] Degradation handler registered: %s (trigger=%s)",
...
void agentrt_unregister_degradation(degradation_handler_t *handler)
{
    ...
    while (*pp) {
        if (*pp == handler) {
            *pp = handler->next;
            ...
            AGENTRT_LOG_INFO("[OOM] Degradation handler unregistered: %s",
                    handler->feature_name);
            return;
        }
        pp = &(*pp)->next;
...
void agentrt_oom_degrade(watermark_level_t old_level,
    ...
    while (handler) {
        ...
        if (new_level >= handler->trigger_level && !handler->is_degraded) {
            ...
            if (handler->on_degrade) {
                AGENTRT_LOG_WARN("[OOM] Degrading: %s (level %s → %s)",
                        handler->feature_name,
        ...
        /* 水位回落：检查是否需要恢复 */
            ...
            if (handler->on_restore) {
                AGENTRT_LOG_INFO("[OOM] Restoring: %s (level %s → %s)",
                        handler->feature_name,
                        oom_watermark_name(old_level),
                        oom_watermark_name(new_level));
...
void agentrt_oom_update_watermark(size_t current_allocated)
{
    if (!g_oom_handler) {
        return;
    }
    ...
    if (new_level != old_level) {
        ...
        AGENTRT_LOG_WARN("[OOM] Watermark changed: %s → %s "
                "(allocated=%zu/%zu, %.1f%%)",
                oom_watermark_name(old_level),
                oom_watermark_name(new_level),
                current_allocated, oom->total_system_memory,
        ...
        /* 触发降级/恢复回调 */
        agentrt_oom_degrade(old_level, new_level);
    }
}

watermark_level_t agentrt_oom_get_watermark(void)
{
    if (!g_oom_handler) {
        return WATERMARK_NORMAL;
    }
...
bool agentrt_oom_is_degraded(void)
{
    if (!g_oom_handler) {
        return false;
    }
...
agentrt_mem_pressure_level_t agentrt_oom_get_pressure(void)
{
    if (!g_oom_handler) {
        return AGENTRT_MEM_PRESSURE_NORMAL;
    }
    ...
    agentrt_mutex_lock(&handler->pressure_mutex);
    ...
    agentrt_mem_pressure_level_t old_level = handler->current_pressure;
    agentrt_mem_pressure_level_t new_level = oom_calc_pressure_level(
        current_allocated, handler->total_system_memory);

    if (new_level != old_level) {
        handler->current_pressure = new_level;

        AGENTRT_LOG_WARN("[OOM] Pressure changed: %s → %s "
                "(allocated=%zu/%zu, %.1f%%)",
    ...
    agentrt_mem_pressure_level_t result = handler->current_pressure;
    agentrt_mutex_unlock(&handler->pressure_mutex);

    return result;
}

void agentrt_oom_set_pressure(agentrt_mem_pressure_level_t level)
{
    if (!g_oom_handler) {
        return;
    }
    ...
    if (level != old_level) {
        AGENTRT_LOG_WARN("[OOM] Pressure manually set: %s → %s",
                oom_pressure_name(old_level),
...
int agentrt_oom_register_callback(
    agentrt_mem_pressure_level_t level,
    void (*callback)(agentrt_mem_pressure_level_t, void *),
    void *user_data)
{
    if (!callback || level < 0 || level > AGENTRT_MEM_PRESSURE_FATAL) {
        return AGENTRT_EINVAL;
    }
    ...
    for (int i = 0; i < AGENTRT_PRESSURE_MAX_CALLBACKS; i++) {
        if (!handler->pressure_callbacks[level][i].active) {
            ...
            AGENTRT_LOG_INFO("[OOM] Pressure callback registered: level=%s, slot=%d",
    ...
    AGENTRT_LOG_WARN("[OOM] Pressure callback registration failed: "
            "level=%s, no free slots",
            oom_pressure_name(level));
    return AGENTRT_EBUSY;
}

int agentrt_oom_check_allocation(size_t requested_size)
{
    if (!g_oom_handler) {
        /* 未初始化时允许分配 */
        return 0;
    ...
    agentrt_mem_pressure_level_t pressure = handler->current_pressure;

    int result = 0; /* 0 = 允许 */

    switch (pressure) {
    case AGENTRT_MEM_PRESSURE_NORMAL:
        /* 正常操作，允许所有分配 */
        break;

    case AGENTRT_MEM_PRESSURE_WARNING:
        /* 允许分配，记录警告 */
        AGENTRT_LOG_WARN("[OOM] Allocation under WARNING pressure: "
                "size=%zu, total_allocated=%zu",
                requested_size,
                handler->total_allocated_bytes);
        break;

    case AGENTRT_MEM_PRESSURE_DEGRADED:
        /* 允许分配，但建议减少非必要分配 */
        AGENTRT_LOG_WARN("[OOM] Allocation under DEGRADED pressure: "
                "size=%zu, total_allocated=%zu",
                requested_size,
                handler->total_allocated_bytes);
        break;

    case AGENTRT_MEM_PRESSURE_CRITICAL:
        /* 拒绝非必要分配 */
        handler->pressure_denied_count++;
        result = AGENTRT_ENOMEM;
        AGENTRT_LOG_ERROR("[OOM] Allocation DENIED under CRITICAL pressure: "
    ...
    case AGENTRT_MEM_PRESSURE_FATAL:
        /* 拒绝所有分配 */
        handler->pressure_denied_count++;
        result = AGENTRT_ENOMEM;
        AGENTRT_LOG_ERROR("[OOM] Allocation DENIED under FATAL pressure: "
                "size=%zu, total_allocated=%zu, denied_count=%zu",
                requested_size,
                handler->total_allocated_bytes,
                handler->pressure_denied_count);
...
void agentrt_oom_report_stats(void)
{
    if (!g_oom_handler) {
        AGENTRT_LOG_INFO("[OOM] OOM handler not initialized");
        return;
    }
    ...
    AGENTRT_LOG_INFO("[OOM] Pressure Stats: level=%s, usage=%.1f%%, "
    ...
    /* 各级别回调统计 */
    for (int level = 0; level <= AGENTRT_MEM_PRESSURE_FATAL; level++) {
        int active_count = 0;
        for (int i = 0; i < AGENTRT_PRESSURE_MAX_CALLBACKS; i++) {
            if (handler->pressure_callbacks[level][i].active) {
                active_count++;
        ...
        if (active_count > 0) {
            AGENTRT_LOG_INFO("[OOM]   %s callbacks: %d/%d",
                    oom_pressure_name((agentrt_mem_pressure_level_t)level),
                    active_count, AGENTRT_PRESSURE_MAX_CALLBACKS);
        }
    }
...
/* 当前活跃配置 */
static agentrt_oom_config_t g_oom_config;

void agentrt_oom_config_defaults(agentrt_oom_config_t *config)
{
    if (!config) return;
...
int agentrt_oom_config_load(const char *config_path,
                             agentrt_oom_config_t *config)
{
    if (!config_path || !config) return AGENTRT_EINVAL;

    /* 先填充默认值 */
    agentrt_oom_config_defaults(config);
    ...
    if (cfg_ctx) {
        ...
        if (source) {
            ...
            if (err == CONFIG_SUCCESS) {
                ...
                AGENTRT_LOG_INFO("[OOM] Config loaded from: %s", config_path);
            } else {
                AGENTRT_LOG_WARN("[OOM] Failed to parse YAML config: %s, using defaults",
                        config_path);
            }
            config_source_destroy(source);
        } else {
            AGENTRT_LOG_WARN("[OOM] Cannot open config file: %s, using defaults",
                    config_path);
        }
        config_context_destroy(cfg_ctx);
    } else {
        AGENTRT_LOG_WARN("[OOM] Config system unavailable, using defaults: %s",
                config_path);
    }
    return 0;
}

int agentrt_oom_config_apply(const agentrt_oom_config_t *config)
{
    if (!config) return AGENTRT_EINVAL;
    if (!g_oom_handler) return AGENTRT_ENOTINIT;
    ...
    if (config->warning_threshold >= config->degraded_threshold ||
        ...
        AGENTRT_LOG_ERROR("[OOM] Invalid threshold order in config");
        return AGENTRT_EINVAL;
    }
    ...
    AGENTRT_LOG_INFO("[OOM] Config applied: warning=%.0f%% degraded=%.0f%% "
...
static agentrt_daemon_oom_registration_t g_daemons_oom_callbacks[OOM_MAX_DAEMON_CALLBACKS];
static size_t g_daemons_oom_count = 0;

int agentrt_oom_register_daemon_callback(
    const agentrt_daemon_oom_registration_t *reg)
{
    if (!reg || !reg->daemon_name || !reg->callback) return AGENTRT_EINVAL;
    if (g_daemons_oom_count >= OOM_MAX_DAEMON_CALLBACKS) return AGENTRT_EBUSY;

    /* 检查重名 */
    for (size_t i = 0; i < g_daemons_oom_count; i++) {
        if (strcmp(g_daemons_oom_callbacks[i].daemon_name,
                   reg->daemon_name) == 0) {
            /* 更新已有注册 */
            g_daemons_oom_callbacks[i] = *reg;
            return 0;
        }
    }

    g_daemons_oom_callbacks[g_daemons_oom_count] = *reg;
    g_daemons_oom_count++;

    AGENTRT_LOG_INFO("[OOM] Daemon OOM callback registered: %s (priority=%d)",
            reg->daemon_name, reg->priority);
    return 0;
}

int agentrt_oom_unregister_daemon_callback(const char *daemon_name)
{
    if (!daemon_name) return AGENTRT_EINVAL;

    for (size_t i = 0; i < g_daemons_oom_count; i++) {
        if (strcmp(g_daemons_oom_callbacks[i].daemon_name,
                   daemon_name) == 0) {
            g_daemons_oom_callbacks[i] =
                g_daemons_oom_callbacks[g_daemons_oom_count - 1];
            g_daemons_oom_count--;
            return 0;
        }
    }
...
size_t agentrt_oom_fire_daemon_callbacks(agentrt_mem_pressure_level_t pressure)
{
    ...
    for (int pass = 0; pass < 2; pass++) {
        for (size_t i = 0; i < g_daemons_oom_count; i++) {
            agentrt_daemon_oom_registration_t *reg =
                &g_daemons_oom_callbacks[i];
            ...
            if (freed > 0) {
                total_freed += freed;
                AGENTRT_LOG_INFO("[OOM] Daemon %s released %zu bytes "
                        "under %s pressure",
...
static agentrt_oom_recovery_cb_t g_recovery_callbacks[OOM_MAX_RECOVERY_CALLBACKS];
...
int agentrt_oom_register_recovery_callback(
    agentrt_oom_recovery_cb_t callback, void *user_data)
{
    if (!callback) return AGENTRT_EINVAL;
    if (g_recovery_count >= OOM_MAX_RECOVERY_CALLBACKS) return AGENTRT_EBUSY;
...
bool agentrt_oom_should_recover(void)
{
    if (!g_oom_handler) return false;

    /* 仅在压力低于 WARNING 时考虑恢复 */
    if (g_oom_handler->current_pressure >= AGENTRT_MEM_PRESSURE_WARNING) {
        return false;
    }
...
int agentrt_oom_recover(void)
{
    if (!g_oom_handler) return AGENTRT_ENOTINIT;

    agentrt_mem_pressure_level_t old_level = g_oom_handler->current_pressure;
    agentrt_mem_pressure_level_t new_level = AGENTRT_MEM_PRESSURE_NORMAL;
    ...
    AGENTRT_LOG_INFO("[OOM] Recovery completed: %s → %s",
            oom_pressure_name(old_level),
            oom_pressure_name(new_level));
