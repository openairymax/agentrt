...
static guard_block_t *create_guarded_block(size_t size)
{
    ...
    guard_block_t *block = (guard_block_t *)AGENTRT_MALLOC(total);
    if (!block) {
...
static void validate_guard(guard_block_t *block, const char *operation)
{
    ...
    if (!check_guard(block->front)) {
        ...
        AGENTRT_LOG_ERROR("GUARD VIOLATION [%s]: front guard corrupted (underflow) at %p",
...
#ifdef AGENTRT_ENABLE_MEMORY_DEBUG
        AGENTRT_LOG_ERROR("GUARD VIOLATION [%s]: back guard corrupted (overflow) at %p, size=%zu",
