...
/** @brief 分配记录链表头 */
static agentrt_mem_alloc_info_t *alloc_list = NULL;

/** @brief 统计数据互斥锁 */
static agentrt_mutex_t *mem_stats_mutex = NULL;

/** @brief 初始化状态标志（原子操作保护） */
static atomic_int mem_initialized = 0;

/** @brief 初始化锁（用于双重检查锁定） */
static agentrt_mutex_t *init_lock = NULL;
...
static int ensure_initialized(void)
{
    ...
    if (state == 0) {
        ...
        if (atomic_compare_exchange_strong_explicit(&mem_initialized, &expected, 2,
            ...
            /* 当前线程获得初始化权 */
            init_lock = agentrt_mutex_create();
            if (!init_lock) {
                atomic_store_explicit(&mem_initialized, 0, memory_order_release);
                ALLOC_RET_ERR(AGENTRT_EINVAL);
            }

            mem_stats_mutex = agentrt_mutex_create();
            if (!mem_stats_mutex) {
 ...
 * @return AGENTRT_SUCCESS 成功，AGENTRT_ENOMEM 内存不足
 * @note 线程安全，可多次调用
 */
agentrt_error_t agentrt_mem_init(size_t heap_size)
{
    /* 设置堆大小上限（0表示无限制） */
    atomic_store_explicit(&max_heap_size, heap_size, memory_order_release);

    if (ensure_initialized() != 0) {
        ALLOC_RET_ERR(AGENTRT_ENOMEM);
    }
...
void agentrt_mem_cleanup(void)
{
    ...
    agentrt_mem_check_leaks();

    agentrt_mutex_lock(mem_stats_mutex);
    while (alloc_list) {
        agentrt_mem_alloc_info_t *info = alloc_list;
        alloc_list = info->next;
        AGENTRT_FREE(info);
    }
    agentrt_mutex_unlock(mem_stats_mutex);

    if (mem_stats_mutex) {
        agentrt_mutex_free(mem_stats_mutex);
...
{
    agentrt_mem_alloc_info_t *info =
        (agentrt_mem_alloc_info_t *)AGENTRT_MALLOC(sizeof(agentrt_mem_alloc_info_t));
    if (info) {
        info->ptr = ptr;
        info->size = size;
        info->file = file;
...
static void remove_alloc_info_unlocked(void *ptr)
{
    if (!ptr)
        return;
    agentrt_mem_alloc_info_t *prev = NULL;
    agentrt_mem_alloc_info_t *curr = alloc_list;
    while (curr) {
        if (curr->ptr == ptr) {
            if (prev) {
                prev->next = curr->next;
 ...
 */
void *agentrt_mem_alloc_ex(size_t size, const char *file, int line)
{
 ...
 * @return 内存指针，失败返回 NULL
 */
void *agentrt_mem_alloc(size_t size)
{
    return agentrt_mem_alloc_ex(size, __FILE__, __LINE__);
}
 ...
 * @return 内存指针，失败返回 NULL
 */
void *agentrt_mem_aligned_alloc_ex(size_t size, size_t alignment, const char *file, int line)
{
    void *ptr = NULL;
#ifdef _WIN32
 ...
 * @return 内存指针，失败返回 NULL
 */
void *agentrt_mem_aligned_alloc(size_t size, size_t alignment)
{
    return agentrt_mem_aligned_alloc_ex(size, alignment, __FILE__, __LINE__);
}
 ...
 * @param ptr 内存指针
 */
void agentrt_mem_aligned_free(void *ptr)
{
    if (!ptr)
        return;
 ...
 */
void *agentrt_mem_realloc_ex(void *ptr, size_t new_size, const char *file, int line)
{
    if (!ptr)
        return agentrt_mem_alloc_ex(new_size, file, line);
    if (new_size == 0) {
 ...
 * @return 新内存指针，失败返回 NULL
 */
void *agentrt_mem_realloc(void *ptr, size_t new_size)
{
    return agentrt_mem_realloc_ex(ptr, new_size, __FILE__, __LINE__);
}
 ...
 */
void agentrt_mem_stats(size_t *out_total, size_t *out_used, size_t *out_peak)
{
    if (atomic_load_explicit(&mem_initialized, memory_order_acquire) == 1 && mem_stats_mutex) {
        agentrt_mutex_lock(mem_stats_mutex);
    }
 ...
 * @return 泄漏的分配数量
 */
size_t agentrt_mem_check_leaks(void)
{
    size_t leak_count = 0;
    size_t leak_size = 0;
    ...
    agentrt_mem_alloc_info_t *info = alloc_list;
#ifdef AGENTRT_ENABLE_MEMORY_DEBUG
    while (info) {
        leak_count++;
        leak_size += info->size;
        AGENTRT_LOG_ERROR("Memory leak: %p, size: %zu, file: %s, line: %d", info->ptr, info->size,
                          info->file, info->line);
        info = info->next;
    }

    if (leak_count > 0) {
        AGENTRT_LOG_ERROR("Total memory leaks: %zu allocations, %zu bytes", leak_count, leak_size);
    } else {
        AGENTRT_LOG_INFO("No memory leaks detected");
    }
#else
    /* 无调试输出，但仍计数 */
    while (info) {
