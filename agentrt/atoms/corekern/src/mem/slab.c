 ...
 */
struct agentrt_slab {
    char                      name[AGENTRT_SLAB_NAME_MAX]; /**< 名称 */
    size_t                    obj_size;                    /**< 对象大小 */
    size_t                    obj_aligned;                 /**< 对齐后对象大小 */
    uint32_t                  objs_per_page;               /**< 每页对象数 */
    uint32_t                  page_size;                   /**< 页总大小 */
    agentrt_slab_ctor_fn      ctor;                        /**< 构造回调 */
    agentrt_slab_dtor_fn      dtor;                        /**< 析构回调 */
    void                     *ctor_arg;                    /**< 构造参数 */
    void                     *dtor_arg;                    /**< 析构参数 */
 ...
 * @brief 分配新 slab 页
 */
static slab_page_t *slab_page_alloc(agentrt_slab_t *slab, uint32_t page_id)
{
    size_t total = sizeof(slab_page_t) + slab->page_size;
    slab_page_t *page = (slab_page_t *)agentrt_mem_alloc(total);
    if (!page) {
        AGENTRT_LOG_ERROR("Slab[%s]: P3.15 PAGE-ALLOC-FAIL size=%zu page_id=%u",
    ...
    AGENTRT_LOG_DEBUG("Slab[%s]: P3.15 PAGE-ALLOC id=%u objs=%u "
 ...
 * @brief 释放 slab 页
 */
static void slab_page_free(agentrt_slab_t *slab, slab_page_t *page)
{
    if (!page) return;

    AGENTRT_LOG_DEBUG("Slab[%s]: P3.15 PAGE-FREE id=%u free_count=%u/%u",
                      slab->name, page->page_id,
                      page->free_count, page->obj_count);

    agentrt_mem_free(page);
}
 ...
 * 如果所有页都已满，返回 NULL。
 */
static void *partial_list_pop(agentrt_slab_t *slab)
{
    ...
    while (page) {
        if (page->free_count > 0) {
            ...
            if (idx >= 0) {
                ...
                AGENTRT_LOG_DEBUG("Slab[%s]: P3.15 PARTIAL-POP page=%u idx=%d "
                                  "free=%u/%u",
    ...
    AGENTRT_LOG_DEBUG("Slab[%s]: P3.15 PARTIAL-POP-MISS (no free objects in partial list)",
                      slab->name);
 ...
 */
static void partial_list_push(agentrt_slab_t *slab, void *obj)
{
    if (!obj) {
        AGENTRT_LOG_WARN("Slab[%s]: P3.15 PARTIAL-PUSH-NULL ignored", slab->name);
        return;
    }
    ...
    if (!found) {
        AGENTRT_LOG_WARN("Slab[%s]: P3.15 PARTIAL-PUSH unknown obj=%p", slab->name, obj);
        return;
    }
    ...
    if (idx >= page->obj_count) {
        AGENTRT_LOG_ERROR("Slab[%s]: P3.15 PARTIAL-PUSH bad index %u (max=%u)",
    ...
    AGENTRT_LOG_DEBUG("Slab[%s]: P3.15 PARTIAL-PUSH page=%u idx=%u "
                      "free=%u/%u was_full=%d",
                      slab->name, page->page_id, idx,
    ...
    if (page->free_count == page->obj_count &&
        ...
        AGENTRT_LOG_INFO("Slab[%s]: P3.15 PAGE-RECLAIM id=%u free=%u/%u "
...
agentrt_slab_t *agentrt_slab_create(const agentrt_slab_config_t *config)
{
    if (!config) {
        AGENTRT_LOG_ERROR("Slab: P3.15 CREATE-FAIL — NULL config");
        return NULL;
    }
    if (config->obj_size == 0) {
        AGENTRT_LOG_ERROR("Slab: P3.15 CREATE-FAIL — obj_size must be > 0");
        return NULL;
    }

    agentrt_slab_t *slab = (agentrt_slab_t *)AGENTRT_CALLOC(1, sizeof(*slab));
    if (!slab) {
        AGENTRT_LOG_ERROR("Slab: P3.15 CREATE-FAIL — OOM allocating slab struct");
        return NULL;
    }

    /* 复制名称 */
    if (config->name && config->name[0]) {
        AGENTRT_STRNCPY_TERM(slab->name, config->name, sizeof(slab->name));
    } else {
    ...
    slab->objs_per_page = config->objs_per_page > 0
                          ...
                          : AGENTRT_SLAB_DEFAULT_OBJS_PER_PAGE;
    ...
    if (pthread_mutex_init(&slab->partial_lock, NULL) != 0) {
        AGENTRT_LOG_ERROR("Slab[%s]: P3.15 CREATE-FAIL — mutex_init failed",
                          slab->name);
        AGENTRT_FREE(slab);
        return NULL;
    }
    ...
    if (pthread_mutex_init(&slab->init_lock, NULL) != 0) {
        AGENTRT_LOG_ERROR("Slab[%s]: P3.15 CREATE-FAIL — init_lock failed",
                          slab->name);
    ...
    AGENTRT_LOG_INFO("Slab[%s]: P3.15 CREATE obj_size=%zu aligned=%zu "
...
{
    if (!slab) {
        AGENTRT_LOG_WARN("Slab: P3.15 DESTROY — NULL slab ignored");
        return;
    }
    if (slab->destroyed) {
        AGENTRT_LOG_WARN("Slab[%s]: P3.15 DESTROY — already destroyed", slab->name);
        return;
    }
    ...
    AGENTRT_LOG_INFO("Slab[%s]: P3.15 DESTROY allocs=%llu frees=%llu "
    ...
    if (active > 0) {
        AGENTRT_LOG_WARN("Slab[%s]: P3.15 DESTROY — %u objects still active "
    ...
    AGENTRT_LOG_INFO("Slab[%s]: P3.15 DESTROY — freed %zu pages from partial list",
...
{
    if (!slab) {
        AGENTRT_LOG_ERROR("Slab: P3.15 ALLOC-FAIL — NULL slab");
        return NULL;
    }
    if (!slab->initialized || slab->destroyed) {
        AGENTRT_LOG_ERROR("Slab[%s]: P3.15 ALLOC-FAIL — not initialized or destroyed",
                          slab->name);
        return NULL;
    }
    ...
    int32_t idx = bitmap_find_free(page->bitmap, page->obj_count);
    if (idx < 0) {
        AGENTRT_LOG_ERROR("Slab[%s]: P3.15 ALLOC-FAIL — new page has no free obj",
                          slab->name);
        slab_page_free(slab, page);
        goto alloc_fail;
    }
...
alloc_success:
    ...
    if (slab->ctor) {
        ...
        if (!constructed) {
            /* 构造失败，回收对象 */
            AGENTRT_LOG_WARN("Slab[%s]: P3.15 ALLOC — ctor failed for obj=%p",
                             slab->name, obj);
    ...
    AGENTRT_LOG_DEBUG("Slab[%s]: P3.15 ALLOC obj=%p active=%u peak=%u",
                      slab->name, obj, active, peak);

    return obj;

alloc_fail:
    AGENTRT_LOG_ERROR("Slab[%s]: P3.15 ALLOC-FAIL — no memory available", slab->name);
    return NULL;
}
...
{
    if (!slab) {
        AGENTRT_LOG_ERROR("Slab: P3.15 FREE-FAIL — NULL slab");
        return;
    }
    if (!obj) {
        AGENTRT_LOG_WARN("Slab[%s]: P3.15 FREE — NULL obj ignored", slab->name);
        return;
    }
    ...
    if (g_cpu_cache.count < SLAB_CPU_FREELIST_MAX) {
        ...
        AGENTRT_LOG_DEBUG("Slab[%s]: P3.15 FREE obj=%p cpu_freelist=%u",
                          slab->name, obj, g_cpu_cache.count);
        return;
    }

    /* 步骤 3: per-CPU freelist 已满，批量归还到全局 partial 链 */
    AGENTRT_LOG_DEBUG("Slab[%s]: P3.15 FREE — cpu_freelist full (%u), "
    ...
    AGENTRT_LOG_DEBUG("Slab[%s]: P3.15 FREE obj=%p batch_returned=%u "
...
void agentrt_slab_get_stats(agentrt_slab_t *slab,
                             agentrt_slab_stats_t *out_stats)
{
    if (!out_stats) {
        AGENTRT_LOG_WARN("Slab: P3.15 STATS — NULL out_stats ignored");
        return;
    }
    ...
    AGENTRT_LOG_DEBUG("Slab[%s]: P3.15 STATS allocs=%llu frees=%llu "
...
void agentrt_slab_reset_stats(agentrt_slab_t *slab)
{
    if (!slab) return;

    AGENTRT_LOG_INFO("Slab[%s]: P3.15 STATS-RESET", slab->name);
...
size_t agentrt_slab_obj_size(agentrt_slab_t *slab)
{
    return slab ? slab->obj_size : 0;
}

const char *agentrt_slab_name(agentrt_slab_t *slab)
{
