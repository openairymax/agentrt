...
agentrt_error_t are_ipc_header_init(are_ipc_message_header_t *header,
                                    ...
                                    const char *target)
{
    ...
    header->timestamp_ns = agentrt_time_realtime_ns();
...
agentrt_error_t are_ipc_header_validate(const are_ipc_message_header_t *header,
                                        const void *payload,
                                        size_t payload_len)
{
    if (!header) {
    ...
    if (header->checksum != expected) {
        return AGENTRT_ERR_CHECKSUM;
    }

    return AGENTRT_SUCCESS;
}

agentrt_error_t are_ipc_message_create(are_msg_type_t msg_type,
                                       are_proto_t protocol,
                                       uint64_t msg_id,
                                       uint64_t trace_id,
                                       const char *source,
                                       ...
                                       are_ipc_message_t **out_msg)
{
    ...
    *out_msg = NULL;
    ...
    are_ipc_message_t *msg =
        (are_ipc_message_t *)AGENTRT_CALLOC(1, sizeof(are_ipc_message_t));
    if (!msg) {
        return AGENTRT_ERR_OUT_OF_MEMORY;
    }

    /* 初始化 header */
    agentrt_error_t err = are_ipc_header_init(&msg->header, msg_type, protocol, msg_id,
