...
static void *posix_thread_entry_wrapper(void *param)
{
    ...
    /* 设置任务状态为运行?*/
    info->state = AGENTRT_TASK_STATE_RUNNING;

    /* 调用用户提供的线程入口函?*/
    info->retval = info->entry(info->arg);

    /* 设置任务状态为已终?*/
    info->state = AGENTRT_TASK_STATE_TERMINATED;
 ...
 *
 * AgentRT优先级范围为AGENTRT_TASK_PRIORITY_MIN到AGENTRT_TASK_PRIORITY_MAX? *
 * 需要映射到POSIX调度优先级范围? *
 * @param agentrt_priority AgentOS优先? * @param sched_policy 调度策略（输出参数）
 * @param sched_param 调度参数（输出参数）
 * @return 0 成功?1 失败
 */
static int map_priority_to_posix(int agentrt_priority, int *sched_policy, struct sched_param *sp)
{
    ...
    *sched_policy = SCHED_OTHER;
    ...
    sp->sched_priority = min_prio + (int)((max_prio - min_prio) * (double)agentrt_priority / 100.0);
...
static void *posix_thread_create(task_info_core_t *info, size_t stack_size)
{
    ...
    /* 分配平台特定数据 */
    data = (posix_task_data_t *)AGENTRT_CALLOC(1, sizeof(posix_task_data_t));
    if (!data) {
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
    }

    /* CROSS-01: 使用平台抽象层创建线程 */
    /* NOTE: agentrt_platform_thread_create 当前使用默认栈大小。
     * 当需要自定义栈大小时（stack_size > 0），需扩展平台抽象层 API，
     * 例如新增 agentrt_platform_thread_create_ex() 支持 pthread_attr_setstacksize。
     * 当前阶段使用默认栈大小，忽略 stack_size 参数。 */
    (void)stack_size;
    data->has_custom_stack_size = 0;

    ret = agentrt_platform_thread_create(&data->thread_handle, posix_thread_entry_wrapper, info);
    ...
    /* 设置线程优先级（如果支持）*/
    if (info->priority != AGENTRT_TASK_PRIORITY_NORMAL) {
        int sched_policy;
...
static int posix_thread_join(void *platform_handle, void **retval)
{
    ...
    int ret = agentrt_platform_thread_join(data->thread_handle, retval);
...
static uintptr_t posix_get_current_thread_id(void)
{
    ...
    return (uintptr_t)agentrt_thread_id();
}
 ...
 * 注意：POSIX标准没有提供获取系统线程ID的便携方法， 这里返回agentrt_thread_t作为系统线程ID的近似? *
...
static uintptr_t posix_get_thread_system_id(void *platform_handle)
{
    ...
    /* agentrt_thread_t 在 POSIX 上可视为系统线程ID的近似 */
