 ...
 * @param agentrt_priority AgentOS优先? * @return Windows优先级常? */
static int map_priority_to_windows(int agentrt_priority)
{
    /* AgentRT优先级范围映射到Windows优先?*/
    if (agentrt_priority >= AGENTRT_TASK_PRIORITY_HIGH) {
        return THREAD_PRIORITY_HIGHEST;
    } else if (agentrt_priority <= AGENTRT_TASK_PRIORITY_LOW) {
...
static void *windows_thread_create(task_info_core_t *info, size_t stack_size)
{
    ...
    /* 分配平台特定数据 */
    data = (windows_task_data_t *)AGENTRT_CALLOC(1, sizeof(windows_task_data_t));
    if (!data) {
