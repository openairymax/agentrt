...
/* ---- 健康检查回调 ---- */
static agentrt_health_status_t mock_health_check(void *user_data)
{
    (void)user_data;
...
void test_observability_init_shutdown(void)
{
    ...
    int ret = agentrt_observability_init(&config);
    /* 可能返回0或错误码（取决于依赖） */
    (void)ret;
...
void test_metric_counter(void)
{
    ...
    int ret = agentrt_metric_counter_create("test_requests_total", "{\"method\":\"GET\"}");
    /* 记录创建结果 */
    (void)ret;

    ret = agentrt_metric_counter_inc("test_requests_total", "{\"method\":\"GET\"}", 1.0);
    /* 计数器应能成功递增 */
    (void)ret;

    ret = agentrt_metric_counter_inc("test_requests_total", "{\"method\":\"GET\"}", 5.0);
    /* 再次递增 */
    (void)ret;
...
void test_metric_gauge(void)
{
    ...
    int ret = agentrt_metric_gauge_create("test_temperature", "{\"unit\":\"celsius\"}", 20.0);
    (void)ret;

    ret = agentrt_metric_gauge_set("test_temperature", "{\"unit\":\"celsius\"}", 25.5);
    (void)ret;

    ret = agentrt_metric_gauge_set("test_temperature", "{\"unit\":\"celsius\"}", -10.0);
    (void)ret;
...
void test_health_check(void)
{
    ...
    int ret = agentrt_health_check_register("mock_check", mock_health_check, NULL);
    (void)ret;

    agentrt_health_status_t status = agentrt_health_check_run(5000);
    /* 状态应为PASS/WARN/FAIL之一 */
    assert(status == AGENTRT_HEALTH_PASS || status == AGENTRT_HEALTH_WARN ||
           status == AGENTRT_HEALTH_FAIL);
...
void test_trace_span(void)
{
    ...
    int ret = agentrt_trace_span_start(&ctx, "test_service", "test_operation");
    ...
    ret = agentrt_trace_set_tag(&ctx, "http.method", "GET");
    (void)ret;

    ret = agentrt_trace_log(&ctx, "Test log message");
    (void)ret;

    ret = agentrt_trace_span_end(&ctx, 0);
    /* span end应成功 */
    if (ret == 0) {
        assert(ctx.end_ns > 0);
...
void test_trace_span_error(void)
{
    ...
    int ret = agentrt_trace_span_start(&ctx, "test_service", "failing_operation");
    (void)ret;

    ret = agentrt_trace_span_end(&ctx, 500);
    /* 带错误码结束 */
    if (ret == 0) {
...
void test_performance_metrics(void)
{
    ...
    int ret = agentrt_performance_get_metrics(&cpu_usage, &memory_usage, &thread_count);
    /* 在Linux上应成功 */
    if (ret == 0) {
...
void test_export_prometheus(void)
{
    ...
    /* 先创建一些指标 */
    agentrt_metric_counter_create("export_test_counter", "{}");
    agentrt_metric_counter_inc("export_test_counter", "{}", 42.0);

    char buffer[4096];
    memset(buffer, 0, sizeof(buffer));

    int bytes = agentrt_observability_export_prometheus(buffer, sizeof(buffer));
    /* 应输出一些数据 */
...
void test_export_health_status(void)
{
    ...
    int bytes = agentrt_health_export_status(buffer, sizeof(buffer));
    /* 应输出一些数据 */
...
void test_multiple_counters(void)
{
    ...
    agentrt_metric_counter_create("counter_a", "{\"service\":\"s1\"}");
    agentrt_metric_counter_create("counter_b", "{\"service\":\"s2\"}");

    agentrt_metric_counter_inc("counter_a", "{\"service\":\"s1\"}", 10.0);
    agentrt_metric_counter_inc("counter_b", "{\"service\":\"s2\"}", 20.0);
...
void test_gauge_edge_cases(void)
{
    ...
    agentrt_metric_gauge_create("gauge_zero", "{}", 0.0);
    agentrt_metric_gauge_set("gauge_zero", "{}", 0.0);

    agentrt_metric_gauge_create("gauge_large", "{}", 1e15);
    agentrt_metric_gauge_set("gauge_large", "{}", -1e15);
