...
static void test_header_init(void)
{
    ...
    agentrt_error_t err = are_ipc_header_init(&h, ARE_MSG_REQUEST, ARE_PROTO_JSON_RPC,
                                              0x123456789ABCDEF0ull, 0xCAFEBABEull,
                                              "gateway_d", "sched_d");
    TEST_ASSERT(err == AGENTRT_SUCCESS, "header_init returns SUCCESS", "init failed");
...
static void test_header_init_null(void)
{
    are_ipc_message_header_t h;
    /* NULL source → "unknown" */
    agentrt_error_t err = are_ipc_header_init(&h, ARE_MSG_NOTIFY, ARE_PROTO_MCP,
                                              1, 0, NULL, NULL);
    TEST_ASSERT(err == AGENTRT_SUCCESS, "init with NULL source/target succeeds", "failed");
    TEST_ASSERT(strcmp(h.source, "unknown") == 0, "NULL source → 'unknown'", "wrong");
    TEST_ASSERT(strcmp(h.target, "*") == 0, "NULL target → '*'", "wrong");

    /* NULL header → AGENTRT_ERR_INVALID_PARAM */
    err = are_ipc_header_init(NULL, ARE_MSG_REQUEST, ARE_PROTO_JSON_RPC, 1, 0, "a", "b");
    TEST_ASSERT(err == AGENTRT_ERR_INVALID_PARAM, "NULL header → EINVAL", "wrong error");

    /* 非法 msg_type → AGENTRT_ERR_INVALID_PARAM */
    err = are_ipc_header_init(&h, (are_msg_type_t)99, ARE_PROTO_JSON_RPC, 1, 0, "a", "b");
    TEST_ASSERT(err == AGENTRT_ERR_INVALID_PARAM, "invalid msg_type → EINVAL", "wrong error");

    /* 非法 protocol → AGENTRT_ERR_INVALID_PARAM */
    err = are_ipc_header_init(&h, ARE_MSG_REQUEST, (are_proto_t)99, 1, 0, "a", "b");
    TEST_ASSERT(err == AGENTRT_ERR_INVALID_PARAM, "invalid protocol → EINVAL", "wrong error");
...
{
    are_ipc_message_header_t h;
    const char *long_source = "this_is_a_very_long_daemons_name_exceeding_31_chars_xxxxxx";
    agentrt_error_t err = are_ipc_header_init(&h, ARE_MSG_REQUEST, ARE_PROTO_JSON_RPC,
                                              1, 0, long_source, "sched_d");
    TEST_ASSERT(err == AGENTRT_SUCCESS, "init with long source succeeds", "failed");
...
static void test_checksum_compute_consistency(void)
{
    ...
    TEST_ASSERT(err == AGENTRT_SUCCESS, "init succeeds", "failed");
...
static void test_validate_valid(void)
{
    ...
    agentrt_error_t err = are_ipc_message_create(ARE_MSG_REQUEST, ARE_PROTO_JSON_RPC,
                                                 100, 0x7FACE123ull, "llm_d", "sched_d",
                                                 payload, payload_len, &msg);
    TEST_ASSERT(err == AGENTRT_SUCCESS, "message_create succeeds", "failed");
    TEST_ASSERT(msg != NULL, "msg != NULL", "null");

    err = are_ipc_header_validate(&msg->header, msg->payload, msg->header.payload_len);
    TEST_ASSERT(err == AGENTRT_SUCCESS, "validate returns SUCCESS", "validation failed");
...
static void test_validate_bad_magic(void)
{
    ...
    agentrt_error_t err = are_ipc_message_create(ARE_MSG_REQUEST, ARE_PROTO_JSON_RPC,
                                                 1, 0, "a", "b", "x", 1, &msg);
    TEST_ASSERT(err == AGENTRT_SUCCESS, "create succeeds", "failed");

    /* 篡改 magic */
    msg->header.magic = 0xDEADBEEFu;
    err = are_ipc_header_validate(&msg->header, msg->payload, msg->header.payload_len);
    TEST_ASSERT(err == AGENTRT_ERR_PROTOCOL, "bad magic → ERR_PROTOCOL", "wrong error");
...
static void test_validate_bad_version(void)
{
    ...
    err = are_ipc_header_validate(&msg->header, msg->payload, msg->header.payload_len);
    TEST_ASSERT(err == AGENTRT_ERR_PROTOCOL, "bad version → ERR_PROTOCOL", "wrong error");
...
static void test_validate_bad_reserved(void)
{
    ...
    err = are_ipc_header_validate(&msg->header, msg->payload, msg->header.payload_len);
    TEST_ASSERT(err == AGENTRT_ERR_PROTOCOL, "non-zero reserved → ERR_PROTOCOL", "wrong error");
...
static void test_validate_bad_abi_pad(void)
{
    ...
    msg->header.checksum = are_ipc_checksum_compute(&msg->header, msg->payload, msg->header.payload_len);
    err = are_ipc_header_validate(&msg->header, msg->payload, msg->header.payload_len);
    TEST_ASSERT(err == AGENTRT_ERR_PROTOCOL, "non-zero _abi_pad → ERR_PROTOCOL", "wrong error");
...
static void test_validate_bad_checksum(void)
{
    ...
    err = are_ipc_header_validate(&msg->header, msg->payload, msg->header.payload_len);
    TEST_ASSERT(err == AGENTRT_ERR_CHECKSUM, "bad checksum → ERR_CHECKSUM", "wrong error");
...
static void test_message_lifecycle(void)
{
    ...
    agentrt_error_t err = are_ipc_message_create(ARE_MSG_RESPONSE, ARE_PROTO_A2A,
    ...
    /* 立即验证创建的消息 */
    err = are_ipc_header_validate(&msg->header, msg->payload, msg->header.payload_len);
    TEST_ASSERT(err == AGENTRT_SUCCESS, "created message validates", "validation failed");
    ...
    err = are_ipc_message_create(ARE_MSG_NOTIFY, ARE_PROTO_MCP, 1, 0, "a", "b",
                                 NULL, 0, &msg);
    TEST_ASSERT(err == AGENTRT_SUCCESS, "create with NULL payload succeeds", "failed");
    ...
    TEST_ASSERT(err == AGENTRT_ERR_INVALID_PARAM, "oversize payload → EINVAL", "wrong");
    TEST_ASSERT(msg == NULL, "msg remains NULL on error", "non-null");

    /* 测试 payload_len > 0 但 payload=NULL */
    err = are_ipc_message_create(ARE_MSG_REQUEST, ARE_PROTO_JSON_RPC, 1, 0, "a", "b",
                                 NULL, 10, &msg);
    TEST_ASSERT(err == AGENTRT_ERR_INVALID_PARAM, "NULL payload with len>0 → EINVAL", "wrong");
...
static void test_trace_id_echo(void)
{
    ...
    TEST_ASSERT(err == AGENTRT_SUCCESS, "create REQUEST", "failed");
    ...
    err = are_ipc_message_create(ARE_MSG_RESPONSE, ARE_PROTO_JSON_RPC,
                                 ...
                                 "sched_d", "gateway_d",
                                 "{\"result\":\"ok\"}", 15, &resp);
    TEST_ASSERT(err == AGENTRT_SUCCESS, "create RESPONSE", "failed");
