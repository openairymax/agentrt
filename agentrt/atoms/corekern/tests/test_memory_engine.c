 ...
 * - Error Code 兼容性（验证 ERR-03 修复：AGENTRT_ERR_* 值正确性/唯一性/描述覆盖）
 * - Types 兼容性（TASK_STATUS_* == AGENTRT_TASK_* 别名验证）
 ...
 * agentrt_malloc/free/realloc 依赖内存子系统初始化，
 * 单元测试中直接使用标准库实现
 */
#undef AGENTRT_MALLOC
#define AGENTRT_MALLOC(size)         malloc(size)
#undef AGENTRT_FREE
#define AGENTRT_FREE(ptr)            free(ptr)
#undef AGENTRT_REALLOC
#define AGENTRT_REALLOC(ptr, size)   realloc((ptr), (size))
#undef AGENTRT_CALLOC
#define AGENTRT_CALLOC(n, size)      calloc((n), (size))
...
static agentrt_error_t bench_dummy_cb(agentrt_ipc_channel_t *ch,
...
static agentrt_error_t bench_empty_cb(agentrt_ipc_channel_t *c,
                                      const agentrt_kernel_ipc_message_t *m, void *u)
{
    (void)c;
    (void)m;
    (void)u;
...
static agentrt_error_t bench_long_cb(agentrt_ipc_channel_t *c,
                                     const agentrt_kernel_ipc_message_t *m, void *u)
{
    (void)c;
    (void)m;
...
static void *thread_alloc_fn(void *arg)
{
    ...
    for (int i = 0; i < ALLOCS_PER_THREAD; i++) {
        td->ptrs[i] = AGENTRT_MALLOC(64 + (size_t)(td->index * 17 + i) % 256);
        if (td->ptrs[i])
...
static void test_memory_provider_init(void)
{
    ...
    TEST_PASS("AGENTRT_MALLOC/FREE 可直接使用，无需显式init");
}
...
static void test_memory_provider_alloc_free(void)
{
    ...
    void *p2 = AGENTRT_MALLOC(4096);
    assert(p2 != NULL);
    TEST_PASS("alloc(4096) 大块分配成功");
    if (p2)
        AGENTRT_FREE(p2);

    void *p3 = AGENTRT_MALLOC(1);
    assert(p3 != NULL);
...
static void test_memory_provider_alloc_null(void)
{
    ...
    void *big = AGENTRT_MALLOC((size_t)512 << 20); /* 512MB, large allocation stress test */
    if (big)
        AGENTRT_FREE(big);
...
static void test_memory_provider_stats(void)
{
    ...
    void *a = AGENTRT_MALLOC(1024);
    void *b = AGENTRT_MALLOC(2048);
    void *c = AGENTRT_MALLOC(512);
...
static void test_error_codes_match(void)
{
    ...
    const int32_t std_errs[] = {
        AGENTRT_EINVAL,   AGENTRT_ENOMEM,  AGENTRT_EBUSY,
        AGENTRT_ENOENT,   AGENTRT_EPERM,   AGENTRT_ETIMEDOUT,
        AGENTRT_EIO,      AGENTRT_EEXIST,  AGENTRT_ENOTINIT,
        AGENTRT_ECANCELLED, AGENTRT_ENOTSUP, AGENTRT_EOVERFLOW,
        AGENTRT_EUNKNOWN, AGENTRT_EINTR,   AGENTRT_EBADF,
        AGENTRT_ERESOURCE
    };
    ...
    const int32_t kern_errs[] = {
        AGENTRT_ENOSYS, AGENTRT_ECYCLE, AGENTRT_EFAIL
    };
    ...
    (void)success_unique;
    for (int i = 0; i < n_std; i++) {
        if (AGENTRT_SUCCESS == std_errs[i]) success_unique = 0;
    }
    for (int i = 0; i < n_kern; i++) {
        if (AGENTRT_SUCCESS == kern_errs[i]) success_unique = 0;
    }
...
static void test_error_codes_unique(void)
{
    ...
    const int32_t codes[] = {
        ...
        AGENTRT_EBUSY,        AGENTRT_ENOENT,       AGENTRT_EPERM,
        AGENTRT_ETIMEDOUT,    AGENTRT_EIO,          AGENTRT_EEXIST,
        AGENTRT_ENOTINIT,     AGENTRT_ECANCELLED,   AGENTRT_ENOTSUP,
        AGENTRT_EOVERFLOW,    AGENTRT_EPROTO,       AGENTRT_ENOTCONN,
        AGENTRT_ECONNRESET,   AGENTRT_ENOSYS,       AGENTRT_EFAIL,
        AGENTRT_ENOTFOUND,    AGENTRT_EPLATFORM,    AGENTRT_EPROTONOSUPPORT,
        AGENTRT_ESERVICE,     AGENTRT_EUNKNOWN,     AGENTRT_EINTR,
        AGENTRT_EBADF,        AGENTRT_ERESOURCE,    AGENTRT_ECYCLE
    };
...
static void test_error_strerror_coverage(void)
{
    ...
    const int32_t check_codes[] = {
        AGENTRT_SUCCESS,    AGENTRT_EINVAL,  AGENTRT_ENOMEM, AGENTRT_EBUSY,
        AGENTRT_ENOENT,     AGENTRT_EPERM,   AGENTRT_EIO,    AGENTRT_EUNKNOWN
    };
    ...
    for (int i = 0; i < n; i++) {
        const char *desc = agentrt_strerror(check_codes[i]);
        if (!desc || desc[0] == '\0')
            all_covered = 0;
    }
    assert(all_covered);
    ...
    const char *unknown_desc = agentrt_strerror(-9999);
    (void)unknown_desc;
...
{
    printf("\n--- [Types] TASK_STATUS_* == AGENTRT_TASK_* 别名 ---\n");

    assert((int)TASK_STATUS_PENDING == (int)AGENTRT_TASK_PENDING);
    TEST_PASS("PENDING alias 匹配");
    assert((int)TASK_STATUS_RUNNING == (int)AGENTRT_TASK_RUNNING);
    TEST_PASS("RUNNING alias 匹配");
    assert((int)TASK_STATUS_SUCCEEDED == (int)AGENTRT_TASK_SUCCEEDED);
    TEST_PASS("SUCCEEDED alias 匹配");
    assert((int)TASK_STATUS_FAILED == (int)AGENTRT_TASK_FAILED);
    TEST_PASS("FAILED alias 匹配");
    assert((int)TASK_STATUS_CANCELLED == (int)AGENTRT_TASK_CANCELLED);
    TEST_PASS("CANCELLED alias 匹配");
    assert((int)TASK_STATUS_TIMEOUT == (int)AGENTRT_TASK_TIMEOUT);
    TEST_PASS("TIMEOUT alias 匹配");
    assert((int)TASK_STATUS_RETRYING == (int)AGENTRT_TASK_RETRYING);
...
static void test_task_status_enum_values(void)
{
    ...
    assert((int)AGENTRT_TASK_PENDING == 0);
    TEST_PASS("PENDING == 0");
    assert((int)AGENTRT_TASK_RUNNING == 1);
    TEST_PASS("RUNNING == 1");
    assert((int)AGENTRT_TASK_SUCCEEDED == 2);
    TEST_PASS("SUCCEEDED == 2");
    assert((int)AGENTRT_TASK_FAILED == 3);
    TEST_PASS("FAILED == 3");
    assert((int)AGENTRT_TASK_CANCELLED == 4);
    TEST_PASS("CANCELLED == 4");
    assert((int)AGENTRT_TASK_TIMEOUT == 5);
    TEST_PASS("TIMEOUT == 5");
    assert((int)AGENTRT_TASK_RETRYING == 6);
    TEST_PASS("RETRYING == 6");

    assert((int)AGENTRT_TASK_PENDING >= 0 && (int)AGENTRT_TASK_RETRYING <= 15);
...
#define MAX_ALLOCATIONS 256
    ...
    for (int i = 0; i < MAX_ALLOCATIONS; i++) {
        ptrs[i] = AGENTRT_MALLOC(64 + (size_t)(i * 7) % 1024);
        if (ptrs[i]) {
...
static void test_boundary_empty_name(void)
{
    ...
    char *empty_str = AGENTRT_MALLOC(1);
    assert(empty_str != NULL);
    empty_str[0] = '\0';
    assert(strlen(empty_str) == 0);
    TEST_PASS("零长度字符串处理正常");
    AGENTRT_FREE(empty_str);

    void *zero_size = AGENTRT_MALLOC(0);
    (void)zero_size;
    TEST_PASS("malloc(0) 不崩溃");
    if (zero_size)
        AGENTRT_FREE(zero_size);
...
static void test_boundary_long_name(void)
{
    ...
    for (int i = 0; i < n; i++) {
        ptrs[i] = AGENTRT_MALLOC(large_sizes[i]);
        if (ptrs[i]) {
...
static void test_resource_cleanup(void)
{
    ...
    for (int i = 0; i < 32; i++) {
        buffers[i] = AGENTRT_MALLOC(256 + (size_t)i * 64);
        if (buffers[i]) {
    ...
    void *post_alloc = AGENTRT_MALLOC(100);
    assert(post_alloc != NULL);
    TEST_PASS("后续分配仍可正常工作");
    if (post_alloc)
        AGENTRT_FREE(post_alloc);
