...
typedef struct {
    ...
    agentrt_mem_pressure_level_t pressure_level;
} test_context_t;
...
static void test_pressure_callback(agentrt_mem_pressure_level_t level, void *user_data)
{
...
static void *alloc_until_pressure(size_t chunk_size, int target_level)
{
    ...
    while (iterations < max_iterations) {
        ptr = agentrt_mem_alloc(chunk_size);
        if (!ptr) {
...
static void test_oom_init(void)
{
    ...
    /* 初始化 OOM 处理器 */
    err = agentrt_oom_init(0);
    TEST_ASSERT(err == AGENTRT_SUCCESS,
                "oom_init: returned %d", (int)err);

    oom_handler_t *handler = agentrt_oom_get_handler();
    TEST_ASSERT(handler != NULL, "oom_get_handler: non-NULL");

    /* 初始化后应为 NORMAL */
    watermark_level_t wl = agentrt_oom_get_watermark();
    TEST_ASSERT(wl == WATERMARK_NORMAL,
                "oom_get_watermark: %d (expected NORMAL)", (int)wl);

    agentrt_mem_pressure_level_t pl = agentrt_oom_get_pressure();
    TEST_ASSERT(pl == AGENTRT_MEM_PRESSURE_NORMAL,
...
static void test_pressure_transition(void)
{
    ...
    /* 初始为 NORMAL */
    agentrt_mem_pressure_level_t p0 = agentrt_oom_get_pressure();
    TEST_ASSERT(p0 == AGENTRT_MEM_PRESSURE_NORMAL,
    ...
    for (int i = 0; i < 50 && block_count < 50; i++) {
        blocks[block_count] = agentrt_mem_alloc(block_size);
...
static void test_degradation_callbacks(void)
{
    ...
    agentrt_error_t err = agentrt_register_degradation(&handler);
    TEST_ASSERT(err == AGENTRT_SUCCESS,
    ...
    err = agentrt_register_degradation(&handler2);
    TEST_ASSERT(err == AGENTRT_SUCCESS,
    ...
    err = agentrt_register_degradation(&handler3);
    TEST_ASSERT(err == AGENTRT_SUCCESS,
                "register_degradation(3): returned %d", (int)err);

    /* 模拟降级: 手动触发 WARNING 级别 */
    printf("    Triggering WARNING level...\n");
    agentrt_oom_degrade(WATERMARK_NORMAL, WATERMARK_WARNING);
    ...
    agentrt_oom_degrade(WATERMARK_WARNING, WATERMARK_HIGH);
    ...
    agentrt_oom_degrade(WATERMARK_HIGH, WATERMARK_WARNING);
    ...
    /* 注销处理器 */
    agentrt_unregister_degradation(&handler);
    agentrt_unregister_degradation(&handler2);
    agentrt_unregister_degradation(&handler3);
...
static void test_pressure_callbacks(void)
{
    ...
    /* 注册 WARNING 级别回调 */
    int ret = agentrt_oom_register_callback(
        AGENTRT_MEM_PRESSURE_WARNING,
    ...
    /* 注册 DEGRADED 级别回调 */
    test_context_t ctx2 = {0};
    ret = agentrt_oom_register_callback(
        AGENTRT_MEM_PRESSURE_DEGRADED,
        test_pressure_callback,
        &ctx2);
    TEST_ASSERT(ret == 0,
    ...
    for (int i = 0; i < 40; i++) {
        blocks[i] = agentrt_mem_alloc(block_size);
        if (!blocks[i]) break;
        ...
        size_t total, used, peak;
        agentrt_mem_stats(&total, &used, &peak);
        agentrt_mem_pressure_level_t new_p = agentrt_oom_get_pressure();
...
static void test_allocation_check(void)
{
    ...
    /* NORMAL 级别应允许分配 */
    int allowed = agentrt_oom_check_allocation(4096);
    TEST_ASSERT(allowed == 0,
                "check_allocation(4096) at NORMAL: %d (expected 0=allow)", allowed);

    /* 设置压力级别为 CRITICAL */
    agentrt_oom_set_pressure(AGENTRT_MEM_PRESSURE_CRITICAL);

    /* CRITICAL 级别应拒绝非必要分配 */
    int critical_check = agentrt_oom_check_allocation(4096);
    ...
    /* 恢复 */
    agentrt_oom_set_pressure(AGENTRT_MEM_PRESSURE_NORMAL);

    allowed = agentrt_oom_check_allocation(4096);
    TEST_ASSERT(allowed == 0,
...
static void test_oom_stats(void)
{
    ...
    int resp = agentrt_oom_handle(1024 * 1024, 0);
...
static void *thread_alloc_func(void *arg)
{
    ...
    void *blocks[10];
    for (int i = 0; i < 10; i++) {
        blocks[i] = agentrt_mem_alloc(256 * 1024); /* 256KB */
        if (blocks[i]) {
...
static void test_thread_safety(void)
{
    ...
    /* 所有内存已释放，应回到 NORMAL */
    TEST_ASSERT(p == AGENTRT_MEM_PRESSURE_NORMAL,
...
static void test_oom_response_levels(void)
{
    ...
    /* NORMAL → WARNING 响应 */
    oom_response_level_t r0 = agentrt_oom_determine_response(WATERMARK_NORMAL);
    printf("    Response for NORMAL: %d\n", (int)r0);

    /* WARNING → DEGRADED 响应 */
    oom_response_level_t r1 = agentrt_oom_determine_response(WATERMARK_WARNING);
    printf("    Response for WARNING: %d\n", (int)r1);

    /* HIGH → CRITICAL 响应 */
    oom_response_level_t r2 = agentrt_oom_determine_response(WATERMARK_HIGH);
    printf("    Response for HIGH: %d\n", (int)r2);

    /* CRITICAL → FATAL 响应 */
    oom_response_level_t r3 = agentrt_oom_determine_response(WATERMARK_CRITICAL);
