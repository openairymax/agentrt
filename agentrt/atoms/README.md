...
Atoms 层是 AgentRT 的**核心系统层**，位于四层架构的最底层，直接构建于操作系统之上。它遵循微核心设计原则，以最小化核心、最大化可扩展性为宗旨，为上层 Daemon 服务和应用程序提供稳定、高效的底层运行时支撑。
...
AgentRT 采用四层分层架构设计，Atoms 层位于最底层：
...
Atoms 层直接构建于操作系统之上，通过封装系统资源为上层提供统一的智能体运行时抽象。它是整个 AgentRT 的基石，所有上层能力均依赖 Atoms 层提供的基础服务。
...
├── README.md                   # 本文件
├── corekern/                   # 微核心 — IPC/Binder、内存、调度、时间
...
│   ├── include/                # agentrt_frameworks.h 适配器接口 + 注册表 API
...
| 原则 | 说明 |
|------|------|
| **微核心** | 最小化核心，最大化服务化；内核仅含 IPC、内存、调度、时间四个原子能力 |
...
| 模块 | 依赖 | 被依赖 |
|------|------|--------|
| CoreKern | OS、commons、agentrt_platform | 所有模块 |
...
构建产物为静态库 `agentrt_core`（CoreKern）及各子模块库，通过 `agentrt_atoms` INTERFACE 目标统一导出头文件路径。
...
- CoreKern: `AGENTRT_CORE_API_VERSION` (1.0.0)
