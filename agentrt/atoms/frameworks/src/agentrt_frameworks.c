 ...
 *
 * @see agentrt_frameworks.h
 */

#include "agentrt_frameworks.h"
 ...
 */
typedef struct {
    const agentrt_framework_adapter_t *adapter;  /*!< 适配器指针（外部拥有） */
    bool used;                                   /*!< 槽位是否占用 */
} fw_adapter_slot_t;
 ...
 */
typedef struct {
    char name[AGENTRT_FW_MAX_NAME_LEN];          /*!< 实例名 */
    const agentrt_framework_adapter_t *adapter;  /*!< 关联适配器 */
    void *handle;                                /*!< 适配器返回的实例句柄 */
    agentrt_fw_state_t state;                    /*!< 当前状态 */
    agentrt_fw_config_t config;                  /*!< 配置副本 */
 ...
 */
static struct {
    fw_adapter_slot_t adapters[AGENTRT_FW_MAX_ADAPTERS];  /*!< 适配器注册表 */
    fw_instance_slot_t instances[AGENTRT_FW_MAX_ADAPTERS]; /*!< 实例表 */
...
{
    for (uint32_t i = 0; i < AGENTRT_FW_MAX_ADAPTERS; i++) {
...
int32_t agentrt_fw_registry_init(void)
{
    FW_LOCK();
    if (g_registry.initialized) {
        FW_UNLOCK();
        return AGENTRT_FW_OK;  /* 幂等 */
    }
    __builtin_memset(&g_registry, 0, sizeof(g_registry));
    g_registry.initialized = true;
    FW_UNLOCK();
    return AGENTRT_FW_OK;
}

int32_t agentrt_fw_registry_shutdown(void)
{
    FW_LOCK();
    if (!g_registry.initialized) {
        FW_UNLOCK();
    ...
    for (uint32_t i = 0; i < AGENTRT_FW_MAX_ADAPTERS; i++) {
        ...
        /* 先停止 */
        if (inst->state == AGENTRT_FW_STATE_RUNNING && inst->adapter && inst->adapter->stop) {
...
int32_t agentrt_fw_register_adapter(const agentrt_framework_adapter_t *adapter)
{
    if (!adapter || !adapter->name || !adapter->init || !adapter->destroy) {
        return AGENTRT_FW_INVALID_ARG;
    }

    FW_LOCK();
    if (!g_registry.initialized) {
        FW_UNLOCK();
        return AGENTRT_FW_NOT_INIT;
    }

    /* 检查重名 */
    if (find_adapter_index(adapter->name) >= 0) {
        FW_UNLOCK();
        return AGENTRT_FW_ALREADY_EXISTS;
    }
    ...
    if (slot < 0) {
        FW_UNLOCK();
        return AGENTRT_FW_CAPACITY_FULL;
    }
...
int32_t agentrt_fw_unregister_adapter(const char *name)
{
    if (!name)
        return AGENTRT_FW_INVALID_ARG;
...
const agentrt_framework_adapter_t *agentrt_fw_find_adapter(const char *name)
{
    if (!name)
        return NULL;
    ...
    FW_LOCK();
    int32_t idx = find_adapter_index(name);
    const agentrt_framework_adapter_t *result =
        (idx >= 0) ? g_registry.adapters[idx].adapter : NULL;
    FW_UNLOCK();
    return result;
}

int32_t agentrt_fw_list_adapters(char names[][AGENTRT_FW_MAX_NAME_LEN],
...
int32_t agentrt_fw_create_instance(const char *adapter_name,
                                    const agentrt_fw_config_t *config,
                                    char *out_instance_name)
{
    ...
    const agentrt_framework_adapter_t *adapter = g_registry.adapters[adapter_idx].adapter;
    fw_instance_slot_t *inst = &g_registry.instances[slot];

    /* 生成实例名 */
    __builtin_memset(inst, 0, sizeof(*inst));
    if (config && config->name[0]) {
        safe_strcpy(inst->name, config->name, AGENTRT_FW_MAX_NAME_LEN);
    } else {
        /* 自动命名：<adapter>-<seq> */
        snprintf(inst->name, AGENTRT_FW_MAX_NAME_LEN, "%s-%u",
    ...
    /* 存配置副本 */
    if (config) {
        __builtin_memcpy(&inst->config, config, sizeof(agentrt_fw_config_t));
    } else {
    ...
    inst->handle = handle;
    inst->state = AGENTRT_FW_STATE_INITIALIZED;
    ...
    if (out_instance_name) {
        safe_strcpy(out_instance_name, inst->name, AGENTRT_FW_MAX_NAME_LEN);
    }
...
int32_t agentrt_fw_start_instance(const char *instance_name)
{
    if (!instance_name)
        return AGENTRT_FW_INVALID_ARG;
    ...
    if (inst->state != AGENTRT_FW_STATE_INITIALIZED &&
        inst->state != AGENTRT_FW_STATE_PAUSED) {
        FW_UNLOCK();
        return AGENTRT_FW_NOT_INIT;
    }
    ...
    if (!inst->adapter || !inst->adapter->start) {
        FW_UNLOCK();
        return AGENTRT_FW_ERROR;
    }

    int32_t rc = inst->adapter->start(inst->handle);
    if (rc == AGENTRT_FW_OK) {
        inst->state = AGENTRT_FW_STATE_RUNNING;
    } else {
        inst->state = AGENTRT_FW_STATE_ERROR;
        inst->error_count++;
    }
    FW_UNLOCK();
    return rc;
...
int32_t agentrt_fw_stop_instance(const char *instance_name)
{
    if (!instance_name)
        return AGENTRT_FW_INVALID_ARG;
    ...
    fw_instance_slot_t *inst = &g_registry.instances[idx];
    if (inst->state == AGENTRT_FW_STATE_RUNNING) {
...
int32_t agentrt_fw_destroy_instance(const char *instance_name)
{
    if (!instance_name)
        return AGENTRT_FW_INVALID_ARG;
    ...
    inst->handle = NULL;
    inst->used = false;
    inst->state = AGENTRT_FW_STATE_SHUTDOWN;
    g_registry.instance_count--;
    FW_UNLOCK();
    return AGENTRT_FW_OK;
}

int32_t agentrt_fw_health_check(const char *instance_name)
{
    if (!instance_name)
        return AGENTRT_FW_INVALID_ARG;
    ...
    if (inst->state != AGENTRT_FW_STATE_RUNNING) {
        result = AGENTRT_FW_NOT_INIT;
    } else if (inst->adapter && inst->adapter->health_check) {
        result = inst->adapter->health_check(inst->handle);
        if (result != AGENTRT_FW_OK) {
            inst->error_count++;
        }
    } else {
        /* 无 health_check 回调则仅检查状态 */
        result = AGENTRT_FW_OK;
    }
    FW_UNLOCK();
    return result;
}

int32_t agentrt_fw_get_info(const char *instance_name, agentrt_fw_info_t *out_info)
{
    ...
    safe_strcpy(out_info->name, inst->name, AGENTRT_FW_MAX_NAME_LEN);
    if (inst->adapter) {
        safe_strcpy(out_info->adapter_name, inst->adapter->name, AGENTRT_FW_MAX_NAME_LEN);
        if (inst->adapter->version) {
            safe_strcpy(out_info->adapter_version, inst->adapter->version, AGENTRT_FW_MAX_VERSION_LEN);
        }
...
int32_t agentrt_fw_process_request(const char *instance_name,
                                    const agentrt_fw_request_t *request,
                                    agentrt_fw_response_t *response)
{
    ...
    if (!inst->adapter || !inst->adapter->process_request) {
        FW_UNLOCK();
        return AGENTRT_FW_ERROR;  /* 适配器不支持请求处理 */
    }
    ...
    const agentrt_framework_adapter_t *adapter = inst->adapter;
...
const char *agentrt_fw_state_to_string(agentrt_fw_state_t state)
{
    switch (state) {
    case AGENTRT_FW_STATE_UNINITIALIZED: return "Uninitialized";
    case AGENTRT_FW_STATE_INITIALIZED:   return "Initialized";
    case AGENTRT_FW_STATE_RUNNING:       return "Running";
    case AGENTRT_FW_STATE_PAUSED:        return "Paused";
    case AGENTRT_FW_STATE_ERROR:         return "Error";
    case AGENTRT_FW_STATE_SHUTDOWN:      return "Shutdown";
    default:                             return "Unknown";
    }
}

const char *agentrt_fw_error_to_string(int32_t code)
{
    switch (code) {
    case AGENTRT_FW_OK:             return "OK";
    case AGENTRT_FW_ERROR:          return "General error";
    case AGENTRT_FW_NOT_INIT:       return "Not initialized";
    case AGENTRT_FW_INVALID_ARG:    return "Invalid argument";
    case AGENTRT_FW_TIMEOUT:        return "Timeout";
    case AGENTRT_FW_BUSY:           return "Busy";
    case AGENTRT_FW_NOT_FOUND:      return "Not found";
    case AGENTRT_FW_DENIED:         return "Permission denied";
    case AGENTRT_FW_ALREADY_EXISTS: return "Already exists";
    case AGENTRT_FW_CAPACITY_FULL:  return "Registry capacity full";
    default:                         return "Unknown error";
    }
}

bool agentrt_fw_has_capability(const char *instance_name, agentrt_fw_capability_t capability)
{
    if (!instance_name)
        return false;
