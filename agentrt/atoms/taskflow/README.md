...
TaskFlow 是 AgentRT 的**任务流引擎**，基于 Pregel 超步模型提供大规模图计算的分布式执行框架。它将复杂工作流建模为有向无环图（DAG），支持任务的编排、调度、执行和监控。TaskFlow 不仅提供传统的 DAG 依赖管理，还实现了 Pregel 超步迭代计算模型、分布式图分区与消息传递、检查点容错和高性能批处理与流水线。
