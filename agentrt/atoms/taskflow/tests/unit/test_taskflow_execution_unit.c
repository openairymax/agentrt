...
{
    agentrt_execution_unit_t *unit = taskflow_unit_create(NULL);
...
static void test_register_unregister_unit(void)
{
    ...
    agentrt_error_t ec = agentrt_execution_create(4, &exec_engine);
    if (ec != AGENTRT_SUCCESS || !exec_engine) {
    ...
    /* 注册 taskflow 执行单元 */
    agentrt_error_t rc = taskflow_register_unit(exec_engine, "taskflow_default", &config);
    ...
    /* execution.h API: void agentrt_execution_destroy(agentrt_execution_engine_t *engine) */
...
static void test_register_null_engine(void)
{
    ...
    /* null engine 应安全处理 */
    agentrt_error_t rc = taskflow_register_unit(NULL, "test", &config);
    (void)rc;
...
static void test_parse_pack_task(void)
{
    ...
    /* parse_task_input 从 agentrt_task 解析输入 */
    ...
    /* pack_task_output 将输出打包到 agentrt_task */
    taskflow_task_output_t *output = taskflow_task_output_create();
    if (output) {
     ...
     * agentrt_task_t.task_output 是 void* (execution.h:66)，需强制转换。 */
    if (task.task_output) {
