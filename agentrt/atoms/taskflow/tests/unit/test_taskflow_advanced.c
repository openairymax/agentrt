...
static void test_register_workflow(void)
{
    ...
    AGENTRT_STRNCPY_TERM(wf.id, "wf_test_001", sizeof(wf.id));
    AGENTRT_STRNCPY_TERM(wf.name, "Test Workflow", sizeof(wf.name));
    AGENTRT_STRNCPY_TERM(wf.description, "A simple test workflow", sizeof(wf.description));
...
static void test_start_execution(void)
{
    ...
    taskflow_workflow_t wf;
    AGENTRT_MEMSET(&wf, 0, sizeof(wf));
    AGENTRT_STRNCPY_TERM(wf.id, "wf_start_test", sizeof(wf.id));
    AGENTRT_STRNCPY_TERM(wf.name, "Start Test", sizeof(wf.name));
...
static void test_cancel_pause_resume(void)
{
    ...
    taskflow_workflow_t wf;
    AGENTRT_MEMSET(&wf, 0, sizeof(wf));
    AGENTRT_STRNCPY_TERM(wf.id, "wf_cpr_test", sizeof(wf.id));
    AGENTRT_STRNCPY_TERM(wf.name, "CPR Test", sizeof(wf.name));
    wf.initial_node_id = strdup("n1");
    taskflow_engine_register_workflow(engine, &wf);
...
static void test_step_and_rtc(void)
{
    ...
    taskflow_workflow_t wf;
    AGENTRT_MEMSET(&wf, 0, sizeof(wf));
    AGENTRT_STRNCPY_TERM(wf.id, "wf_step_test", sizeof(wf.id));
    AGENTRT_STRNCPY_TERM(wf.name, "Step Test", sizeof(wf.name));
    wf.initial_node_id = strdup("n1");
    taskflow_engine_register_workflow(engine, &wf);
...
static void test_unregister_workflow(void)
{
    ...
    taskflow_workflow_t wf;
    AGENTRT_MEMSET(&wf, 0, sizeof(wf));
    AGENTRT_STRNCPY_TERM(wf.id, "wf_unregister", sizeof(wf.id));
    AGENTRT_STRNCPY_TERM(wf.name, "Workflow for Unregister", sizeof(wf.name));
    AGENTRT_STRNCPY_TERM(wf.version, "1.0", sizeof(wf.version));
    wf.initial_node_id = strdup("node_start");

    taskflow_node_t node;
    AGENTRT_MEMSET(&node, 0, sizeof(node));
    AGENTRT_STRNCPY_TERM(node.id, "node_start", sizeof(node.id));
    AGENTRT_STRNCPY_TERM(node.name, "Start", sizeof(node.name));
