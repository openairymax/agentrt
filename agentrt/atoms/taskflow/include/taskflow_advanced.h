...
#ifndef AGENTRT_TASKFLOW_ADVANCED_H
#define AGENTRT_TASKFLOW_ADVANCED_H
 ...
 *       数值碰撞。AGENTRT_CALLOC 零初始化的结构体字段默认值为 NONE
 *       （未配置策略），语义合理。
 */
typedef enum {
 ...
 * @return 0表示成功，非0表示失败
 */
typedef int (*taskflow_task_handler_t)(taskflow_engine_t *engine, const char *node_id,
 ...
 */
typedef bool (*taskflow_condition_fn)(const char *expression, const char *variables_json,
                                      void *user_data);
 ...
 * @param user_data 用户自定义数据
 */
typedef void (*taskflow_progress_callback_t)(const char *execution_id, const char *node_id,
                                             taskflow_state_t state, double progress,
                                             void *user_data);
 ...
 * @param user_data 用户自定义数据
 */
typedef void (*taskflow_event_callback_t)(const char *execution_id, const char *event_type,
...
#endif /* AGENTRT_TASKFLOW_ADVANCED_H */
