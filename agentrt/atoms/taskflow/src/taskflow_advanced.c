...
#ifdef AGENTRT_TASKFLOW_HAVE_CJSON
...
{
    taskflow_engine_t *engine = (taskflow_engine_t *)AGENTRT_CALLOC(1, sizeof(taskflow_engine_t));
...
void taskflow_engine_destroy(taskflow_engine_t *engine)
{
    ...
    for (size_t i = 0; i < engine->execution_count; i++) {
        if (engine->executions[i].execution.current_node_id)
            AGENTRT_FREE(engine->executions[i].execution.current_node_id);
        if (engine->executions[i].execution.input_json)
            AGENTRT_FREE(engine->executions[i].execution.input_json);
        if (engine->executions[i].execution.output_json)
            AGENTRT_FREE(engine->executions[i].execution.output_json);
        if (engine->executions[i].execution.error_message)
            AGENTRT_FREE(engine->executions[i].execution.error_message);
        if (engine->executions[i].execution.variables_json)
            AGENTRT_FREE(engine->executions[i].execution.variables_json);
        if (engine->executions[i].variables_json)
            AGENTRT_FREE(engine->executions[i].variables_json);
        for (size_t j = 0; j < engine->executions[i].checkpoint_count; j++) {
            if (engine->executions[i].checkpoints[j].snapshot_json)
                AGENTRT_FREE(engine->executions[i].checkpoints[j].snapshot_json);
        }
    }
    for (size_t i = 0; i < engine->workflow_count; i++) {
        if (engine->workflows[i].workflow.nodes) {
            for (size_t j = 0; j < engine->workflows[i].workflow.node_count; j++) {
                ...
                AGENTRT_FREE(n->task_handler_name);
                AGENTRT_FREE(n->config_json);
                AGENTRT_FREE(n->input_transform_json);
                AGENTRT_FREE(n->output_transform_json);
                AGENTRT_FREE(n->fallback_handler_name);
                AGENTRT_FREE(n->condition_expr);
                AGENTRT_FREE(n->subflow_id);
                AGENTRT_FREE(n->loop_condition_expr);
                AGENTRT_FREE(n->loop_foreach_json);
                AGENTRT_FREE(n->event_type);
            }
            AGENTRT_FREE(engine->workflows[i].workflow.nodes);
        }
        if (engine->workflows[i].workflow.edges)
            AGENTRT_FREE(engine->workflows[i].workflow.edges);
        AGENTRT_FREE(engine->workflows[i].workflow.initial_node_id);
        AGENTRT_FREE(engine->workflows[i].workflow.input_schema_json);
        AGENTRT_FREE(engine->workflows[i].workflow.output_schema_json);
    }
    AGENTRT_FREE(engine);
}
                                     ...
                                     taskflow_task_handler_t handler, void *user_data)
{
    if (!engine || !name || !handler)
        AGENTRT_ERROR(AGENTRT_EINVAL, "failed to register handler: null engine, name, or handler");
    if (engine->handler_count >= TASKFLOW_MAX_PARALLEL)
        ATM_RET_ERR(AGENTRT_ERR_OVERFLOW);
...
AGENTRT_STRNCPY_TERM(engine->handlers[engine->handler_count].name, name, sizeof(engine->handlers[0].name));
...
{
    if (!engine || !workflow)
        AGENTRT_ERROR(AGENTRT_EINVAL, "failed to register workflow: null engine or workflow");
...
static void taskflow_free_workflow_fields(taskflow_workflow_t *wf)
{
    ...
    if (wf->edges) {
        AGENTRT_FREE(wf->edges);
        wf->edges = NULL;
        wf->edge_count = 0;
    }
    AGENTRT_FREE(wf->initial_node_id);
    wf->initial_node_id = NULL;
    AGENTRT_FREE(wf->input_schema_json);
    wf->input_schema_json = NULL;
    AGENTRT_FREE(wf->output_schema_json);
    wf->output_schema_json = NULL;
}

#endif /* AGENTRT_TASKFLOW_HAVE_CJSON */

int taskflow_engine_load_workflow_json(taskflow_engine_t *engine, const char *workflow_json)
{
    if (!engine || !workflow_json)
        AGENTRT_ERROR(AGENTRT_EINVAL, "failed to load workflow json: null engine or workflow_json");
    if (engine->workflow_count >= TASKFLOW_MAX_SUBFLOWS)
        ATM_RET_ERR(AGENTRT_ERR_OVERFLOW);

#ifndef AGENTRT_TASKFLOW_HAVE_CJSON
    ...
    AGENTRT_LOG_WARN("taskflow: load_workflow_json requires cJSON (not available), "
                     "use taskflow_engine_register_workflow instead");
    ATM_RET_ERR(AGENTRT_ENOTSUP);
#else
    ...
    if (!root) {
        AGENTRT_LOG_WARN("taskflow: load_workflow_json JSON parse failed");
        ATM_RET_ERR(AGENTRT_EINVAL);
    }
    ...
    #define TF_READ_STR_FIELD(obj, dst, field, maxsize) do { \
        ...
        if (_item && cJSON_IsString(_item)) { \
            AGENTRT_STRNCPY_TERM(dst, _item->valuestring, maxsize); \
        } \
    } while (0)
    ...
    #define TF_READ_STR_DUP(obj, dst_ptr, field) do { \
        ...
        if (_item && cJSON_IsString(_item) && _item->valuestring) { \
            dst_ptr = AGENTRT_STRDUP(_item->valuestring); \
        } \
    } while (0)
    ...
    /* nodes 数组 */
    {
        ...
        if (nodes_arr && cJSON_IsArray(nodes_arr)) {
            ...
            if (n_count < 0 || (size_t)n_count > TASKFLOW_MAX_NODES) {
                ...
                AGENTRT_LOG_WARN("taskflow: load_workflow_json nodes count overflow (%d)", n_count);
                ATM_RET_ERR(AGENTRT_ERR_OVERFLOW);
            }
            if (n_count > 0) {
                wf.nodes = (taskflow_node_t *)AGENTRT_CALLOC((size_t)n_count, sizeof(taskflow_node_t));
                if (!wf.nodes) {
                    cJSON_Delete(root);
                    taskflow_free_workflow_fields(&wf);
                    ATM_RET_ERR(AGENTRT_ERR_OUT_OF_MEMORY);
                }
    ...
    /* edges 数组 */
    {
        ...
        if (edges_arr && cJSON_IsArray(edges_arr)) {
            ...
            if (e_count < 0 || (size_t)e_count > TASKFLOW_MAX_EDGES) {
                cJSON_Delete(root);
                taskflow_free_workflow_fields(&wf);
                AGENTRT_LOG_WARN("taskflow: load_workflow_json edges count overflow (%d)", e_count);
                ATM_RET_ERR(AGENTRT_ERR_OVERFLOW);
            }
            if (e_count > 0) {
                wf.edges = (taskflow_edge_t *)AGENTRT_CALLOC((size_t)e_count, sizeof(taskflow_edge_t));
                if (!wf.edges) {
                    cJSON_Delete(root);
                    taskflow_free_workflow_fields(&wf);
                    ATM_RET_ERR(AGENTRT_ERR_OUT_OF_MEMORY);
    ...
    if (wf.id[0] == '\0') {
        taskflow_free_workflow_fields(&wf);
        AGENTRT_LOG_WARN("taskflow: load_workflow_json missing required field 'id'");
        ATM_RET_ERR(AGENTRT_EINVAL);
    }
    ...
    if (rc != 0) {
        taskflow_free_workflow_fields(&wf);
        AGENTRT_LOG_WARN("taskflow: load_workflow_json register_workflow failed (rc=%d)", rc);
        return rc;
    }

    AGENTRT_LOG_INFO("taskflow: workflow '%s' loaded from JSON (nodes=%zu edges=%zu)",
                          ...
                          const char *input_json, char **execution_id)
{
    ...
    if (!engine || !workflow_id || !execution_id)
        AGENTRT_ERROR(AGENTRT_EINVAL, "failed to start execution: null engine, workflow_id, or execution_id");
    taskflow_workflow_t *wf = find_workflow(engine, workflow_id);
    if (!wf)
        ATM_RET_ERR(AGENTRT_ERR_INVALID_PARAM);
    ...
    snprintf(ee->execution.execution_id, sizeof(ee->execution.execution_id), "exec_%lu",
             (unsigned long)eid);
AGENTRT_STRNCPY_TERM(ee->execution.workflow_id, workflow_id, sizeof(ee->execution.workflow_id));
    ee->execution.state = TASKFLOW_STATE_RUNNING;
    ee->execution.input_json = input_json ? AGENTRT_STRDUP(input_json) : NULL;
    ee->execution.progress = 0.0;
    ee->execution.superstep = 0;
    ee->execution.completed_nodes = 0;
    ee->execution.total_nodes = wf->node_count;
    ee->execution.started_at = (uint64_t)(time_t)(agentrt_time_ms() / 1000ULL);
    ee->active = 1;
    ee->checkpoint_count = 0;
    ee->variables_json = AGENTRT_STRDUP("{}");

    if (wf->initial_node_id) {
        ee->execution.current_node_id = AGENTRT_STRDUP(wf->initial_node_id);
    } else if (wf->node_count > 0) {
        ee->execution.current_node_id = AGENTRT_STRDUP(wf->nodes[0].id);
    }

    if (execution_id) {
        *execution_id = AGENTRT_STRDUP(ee->execution.execution_id);
    }
        ...
        if (start_node && start_node->task_handler_name) {
            ...
            if (h && h->handler) {
                ...
                if (ret == 0) {
                    ...
                    ee->execution.error_message = (char *)AGENTRT_MALLOC(128);
    ...
    if (ee->execution.completed_nodes >= ee->execution.total_nodes &&
        ...
        ee->execution.completed_at = (uint64_t)(time_t)(agentrt_time_ms() / 1000ULL);
    ...
    for (size_t i = 0; i < engine->execution_count; i++) {
        if (strcmp(engine->executions[i].execution.execution_id, execution_id) == 0) {
            ...
            engine->executions[i].execution.completed_at =
                (uint64_t)(time_t)(agentrt_time_ms() / 1000ULL);
            return 0;
        }
    }
    return 1;
                                  ...
                                  taskflow_execution_t **execution)
{
    if (!engine || !execution_id || !execution)
        AGENTRT_ERROR(AGENTRT_EINVAL, "failed to get execution: null engine, execution_id, or execution");
    for (size_t i = 0; i < engine->execution_count; i++) {
        if (strcmp(engine->executions[i].execution.execution_id, execution_id) == 0) {
...
int taskflow_engine_step(taskflow_engine_t *engine, const char *execution_id)
{
    ...
    for (size_t i = 0; i < engine->execution_count; i++) {
        ...
        if (!wf)
            ATM_RET_ERR(AGENTRT_ERR_NULL_POINTER);
...
{
    if (!engine || !execution_id)
        AGENTRT_ERROR(AGENTRT_EINVAL, "failed to create checkpoint: null engine or execution_id");
    for (size_t i = 0; i < engine->execution_count; i++) {
        execution_entry_t *ee = &engine->executions[i];
        if (strcmp(ee->execution.execution_id, execution_id) != 0)
            continue;
...
AGENTRT_STRNCPY_TERM(cp->execution_id, execution_id, sizeof(cp->execution_id));
AGENTRT_STRNCPY_TERM(cp->workflow_id, ee->execution.workflow_id, sizeof(cp->workflow_id));
        if (ee->execution.current_node_id)
AGENTRT_STRNCPY_TERM(cp->node_id, ee->execution.current_node_id, sizeof(cp->node_id));
        cp->state = ee->execution.state;
        size_t snap_len = 256 + (ee->variables_json ? strlen(ee->variables_json) : 0);
        cp->snapshot_json = (char *)AGENTRT_MALLOC(snap_len);
        if (cp->snapshot_json) {
            snprintf(
        ...
        cp->timestamp = (uint64_t)(time_t)(agentrt_time_ms() / 1000ULL);

        if (checkpoint_id) {
            *checkpoint_id = AGENTRT_STRDUP(cp->id);
        }
        ee->checkpoint_count++;
        return 0;
    }
    ...
    for (size_t i = 0; i < engine->execution_count; i++) {
        ...
        for (size_t j = 0; j < ee->checkpoint_count; j++) {
            if (strcmp(ee->checkpoints[j].id, checkpoint_id) == 0) {
                ...
                if (ee->execution.current_node_id)
                    AGENTRT_FREE(ee->execution.current_node_id);
                ee->execution.current_node_id = AGENTRT_STRDUP(ee->checkpoints[j].node_id);
                ee->execution.state = TASKFLOW_STATE_RUNNING;
                return 0;
            }
        }
                                     ...
                                     taskflow_checkpoint_t **checkpoints, size_t *count)
{
    if (!engine || !execution_id || !checkpoints || !count)
        AGENTRT_ERROR(AGENTRT_EINVAL, "failed to list checkpoints: null engine, execution_id, checkpoints, or count");
    for (size_t i = 0; i < engine->execution_count; i++) {
        execution_entry_t *ee = &engine->executions[i];
        if (strcmp(ee->execution.execution_id, execution_id) != 0)
            continue;
                                 ...
                                 const char *key, const char *value_json)
{
    if (!engine || !execution_id || !key || !value_json)
        AGENTRT_ERROR(AGENTRT_EINVAL, "failed to set variable: null engine, execution_id, key, or value_json");
    for (size_t i = 0; i < engine->execution_count; i++) {
        execution_entry_t *ee = &engine->executions[i];
        if (strcmp(ee->execution.execution_id, execution_id) != 0)
            continue;
        ...
        char *new_vars = (char *)AGENTRT_MALLOC(new_len);
        if (!new_vars)
            ATM_RET_ERR(AGENTRT_ERR_OUT_OF_MEMORY);
        if (old_len <= 2) {
        ...
        if (ee->variables_json)
            AGENTRT_FREE(ee->variables_json);
                                 ...
                                 const char *key, char **value_json)
{
    ...
    *value_json = NULL;
    for (size_t i = 0; i < engine->execution_count; i++) {
        ...
        *value_json = (char *)AGENTRT_MALLOC(val_len + 1);
        if (*value_json) {
...
void taskflow_workflow_destroy(taskflow_workflow_t *workflow)
{
    ...
    if (workflow->nodes) {
        for (size_t i = 0; i < workflow->node_count; i++) {
            AGENTRT_FREE(workflow->nodes[i].task_handler_name);
            AGENTRT_FREE(workflow->nodes[i].config_json);
            AGENTRT_FREE(workflow->nodes[i].input_transform_json);
            AGENTRT_FREE(workflow->nodes[i].output_transform_json);
            AGENTRT_FREE(workflow->nodes[i].fallback_handler_name);
            AGENTRT_FREE(workflow->nodes[i].condition_expr);
            AGENTRT_FREE(workflow->nodes[i].subflow_id);
            AGENTRT_FREE(workflow->nodes[i].loop_condition_expr);
            AGENTRT_FREE(workflow->nodes[i].loop_foreach_json);
            AGENTRT_FREE(workflow->nodes[i].event_type);
        }
        AGENTRT_FREE(workflow->nodes);
    }
    AGENTRT_FREE(workflow->edges);
    AGENTRT_FREE(workflow->initial_node_id);
    AGENTRT_FREE(workflow->input_schema_json);
    AGENTRT_FREE(workflow->output_schema_json);
}
...
void taskflow_execution_destroy(taskflow_execution_t *execution)
{
    ...
    if (execution->current_node_id)
        AGENTRT_FREE(execution->current_node_id);
    if (execution->input_json)
        AGENTRT_FREE(execution->input_json);
    if (execution->output_json)
        AGENTRT_FREE(execution->output_json);
    if (execution->error_message)
        AGENTRT_FREE(execution->error_message);
    if (execution->variables_json)
        AGENTRT_FREE(execution->variables_json);
}
...
void taskflow_checkpoint_destroy(taskflow_checkpoint_t *checkpoint)
{
    ...
    if (checkpoint->snapshot_json)
        AGENTRT_FREE(checkpoint->snapshot_json);
}
