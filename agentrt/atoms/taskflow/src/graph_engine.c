...
{
    // 质数表，用于减少哈希冲突
    static const size_t primes[] = {53,        97,        193,      389,       769,       1543,
...
{
    void **table = (void **)AGENTRT_CALLOC(size, sizeof(void *));
    return table;
}
...
static bool add_vertex_index(struct graph_engine_s *e, vertex_id_t vertex_id, size_t vertex_idx)
{
    ...
    vertex_index_entry_t *new_entry =
        (vertex_index_entry_t *)AGENTRT_CALLOC(1, sizeof(vertex_index_entry_t));
    if (!new_entry)
        return false;
...
static bool add_edge_index(struct graph_engine_s *e, edge_id_t edge_id, size_t edge_idx)
{
    ...
    edge_index_entry_t *new_entry = (edge_index_entry_t *)AGENTRT_CALLOC(1, sizeof(edge_index_entry_t));
    if (!new_entry)
        return false;
...
static bool add_out_edge_index(struct graph_engine_s *e, vertex_id_t source, size_t edge_idx)
{
    ...
    out_edge_index_entry_t *new_entry =
        (out_edge_index_entry_t *)AGENTRT_CALLOC(1, sizeof(out_edge_index_entry_t));
    if (!new_entry)
        return false;
...
static bool add_in_edge_index(struct graph_engine_s *e, vertex_id_t target, size_t edge_idx)
{
    ...
    in_edge_index_entry_t *new_entry =
        (in_edge_index_entry_t *)AGENTRT_CALLOC(1, sizeof(in_edge_index_entry_t));
    if (!new_entry)
        return false;
...
// 更新顶点度数
static void update_vertex_degree(struct graph_engine_s *e, vertex_id_t vertex_id, bool is_out_edge,
                                 int delta)
{
    ...
    if (is_out_edge) {
        if (delta > 0)
            e->vertices[idx].out_degree++;
        else if (e->vertices[idx].out_degree > 0)
            e->vertices[idx].out_degree--;
    } else {
        if (delta > 0)
            e->vertices[idx].in_degree++;
        else if (e->vertices[idx].in_degree > 0)
            e->vertices[idx].in_degree--;
    }
}
...
static void remove_index_entry(void **table, size_t hash, void *target_id, size_t id_offset,
                               size_t next_offset, void (*free_fn)(void *))
{
...
static void remove_edge_from_all_indexes(struct graph_engine_s *e, edge_id_t edge_id,
                                         vertex_id_t source, vertex_id_t target, size_t removed_idx)
{
    size_t hash;
...
{
    if (!e->out_edge_index || vertex_id == 0)
        return 0;
...
// 收集入边
static size_t collect_in_edges(struct graph_engine_s *e, vertex_id_t vertex_id, graph_edge_t *edges,
                               size_t max_edges)
{
    if (!e->in_edge_index || vertex_id == 0)
        return 0;
...
static size_t collect_neighbors(struct graph_engine_s *e, vertex_id_t vertex_id,
                                vertex_id_t *neighbors, size_t max_neighbors, bool out_neighbors)
{
    if (vertex_id == 0)
        return 0;
...
{
    vertex_queue_t *q = (vertex_queue_t *)AGENTRT_CALLOC(1, sizeof(vertex_queue_t));
    return q;
}
...
static void queue_enqueue(vertex_queue_t *q, vertex_id_t vertex_id)
{
    ...
    queue_node_t *node = (queue_node_t *)AGENTRT_CALLOC(1, sizeof(queue_node_t));
    if (!node)
        return;
...
{
    vertex_stack_t *s = (vertex_stack_t *)AGENTRT_CALLOC(1, sizeof(vertex_stack_t));
    return s;
}
...
static void stack_push(vertex_stack_t *s, vertex_id_t vertex_id)
{
    ...
    stack_node_t *node = (stack_node_t *)AGENTRT_CALLOC(1, sizeof(stack_node_t));
    if (!node)
        return;
...
{
    vertex_hashset_t *set = (vertex_hashset_t *)AGENTRT_CALLOC(1, sizeof(vertex_hashset_t));
    if (!set) return NULL;

    set->size = size;
    set->buckets = (hashset_node_t **)AGENTRT_CALLOC(size, sizeof(hashset_node_t *));
    if (!set->buckets) {
        AGENTRT_FREE(set);
...
static bool hashset_add(vertex_hashset_t *set, vertex_id_t vertex_id)
{
    ...
    // 添加新节点
    hashset_node_t *new_node = (hashset_node_t *)AGENTRT_CALLOC(1, sizeof(hashset_node_t));
    if (!new_node)
        return false;
...
graph_engine_handle_t graph_engine_create(const taskflow_config_t *config)
{
    ...
    struct graph_engine_s *engine =
        (struct graph_engine_s *)AGENTRT_CALLOC(1, sizeof(struct graph_engine_s));
    if (!engine) {
    ...
    engine->vertices = (graph_vertex_t *)AGENTRT_CALLOC(engine->vertex_capacity, sizeof(graph_vertex_t));
    if (!engine->vertices) {
        AGENTRT_FREE(engine);
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
    }

    engine->edges = (graph_edge_t *)AGENTRT_CALLOC(engine->edge_capacity, sizeof(graph_edge_t));
    if (!engine->edges) {
        AGENTRT_FREE(engine->vertices);
        AGENTRT_FREE(engine);
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
    }
    ...
    if (!engine->vertex_index || !engine->edge_index || !engine->out_edge_index ||
        !engine->in_edge_index) {
        // 清理已分配的资源
        if (engine->vertex_index)
            AGENTRT_FREE(engine->vertex_index);
        if (engine->edge_index)
            AGENTRT_FREE(engine->edge_index);
        if (engine->out_edge_index)
            AGENTRT_FREE(engine->out_edge_index);
        if (engine->in_edge_index)
            AGENTRT_FREE(engine->in_edge_index);
        AGENTRT_FREE(engine->vertices);
        AGENTRT_FREE(engine->edges);
        AGENTRT_FREE(engine);
...
void graph_engine_destroy(graph_engine_handle_t engine)
{
    ...
    // 释放顶点存储
    if (e->vertices) {
        AGENTRT_FREE(e->vertices);
    }

    // 释放边存储
    if (e->edges) {
        AGENTRT_FREE(e->edges);
    }
...
taskflow_error_t graph_engine_add_vertex(graph_engine_handle_t engine, const graph_vertex_t *vertex)
{
    if (!engine || !vertex) {
        return TASKFLOW_ERROR_INVALID_ARG;
    }
...
#define MAX_NEIGHBORS_PER_BATCH 32
        ...
        size_t offset = 0;
        while (true) {
            size_t count = collect_neighbors(e, current, neighbors + offset,
                                             MAX_NEIGHBORS_PER_BATCH - offset, true);
            if (count == 0)
                break;
...
#define MAX_NEIGHBORS_PER_BATCH_DFS 32
        ...
        while (true) {
            size_t count = collect_neighbors(e, current, neighbors + offset,
                                             MAX_NEIGHBORS_PER_BATCH_DFS - offset, true);
            if (count == 0)
                break;
