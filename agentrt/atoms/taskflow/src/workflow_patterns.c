                                          ...
                                          vertex_id_t node_id)
{
    ...
    if (state_idx != SIZE_MAX) {
        internal_state->node_states[state_idx].start_time = agentrt_time_ns();
    ...
    if (node->task_executor && node->task_data) {
        ...
        if (state_idx != SIZE_MAX) {
            ...
            internal_state->node_states[state_idx].end_time = agentrt_time_ns();
        }
    }
...
#define MAX_PARALLEL 32
    ...
    for (size_t i = 0; i < context->edge_count && parallel_count < MAX_PARALLEL; i++) {
        if (context->edges[i].source_node == context->start_node &&
            ...
            for (size_t j = 0; j < parallel_count; j++) {
                if (parallel_nodes[j] == context->edges[i].target_node) {
                    duplicate = true;
                    break;
                }
            }
    ...
    internal_state->state =
        (overall_result == TASKFLOW_SUCCESS) ? EXECUTION_COMPLETED : EXECUTION_FAILED;
                                            ...
                                            taskflow_handle_t taskflow_engine)
{
    ...
    workflow_context_t *context = (workflow_context_t *)AGENTRT_CALLOC(1, sizeof(workflow_context_t));
    ...
    if (config->max_nodes > 0) {
        context->nodes = (workflow_node_t *)AGENTRT_CALLOC(config->max_nodes, sizeof(workflow_node_t));
        if (!context->nodes) {
    ...
    if (config->max_edges > 0) {
        context->edges = (workflow_edge_t *)AGENTRT_CALLOC(config->max_edges, sizeof(workflow_edge_t));
        if (!context->edges) {
            if (context->nodes)
                AGENTRT_FREE(context->nodes);
            AGENTRT_FREE(context);
            AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
        }
    }
...
void workflow_context_destroy(workflow_context_t *context)
{
    ...
    if (context->nodes) {
        for (size_t i = 0; i < context->node_count; i++) {
            if (context->nodes[i].node_name)
                AGENTRT_FREE(context->nodes[i].node_name);
            if (context->nodes[i].task_data)
                AGENTRT_FREE(context->nodes[i].task_data);
        }
        AGENTRT_FREE(context->nodes);
    }
    ...
    if (context->edges) {
        for (size_t i = 0; i < context->edge_count; i++) {
            if (context->edges[i].edge_label)
                AGENTRT_FREE(context->edges[i].edge_label);
        }
        AGENTRT_FREE(context->edges);
    }
    ...
    if (context->execution_state) {
        ...
        if (state->node_states)
            AGENTRT_FREE(state->node_states);
        AGENTRT_FREE(state);
    }
...
taskflow_error_t workflow_add_node(workflow_context_t *context, const workflow_node_t *node)
{
    ...
    // 复制节点名称
    if (node->node_name) {
        dest->node_name = AGENTRT_STRDUP(node->node_name);
        if (!dest->node_name)
    ...
    if (node->task_data && node->task_data_size > 0) {
        dest->task_data = AGENTRT_MALLOC(node->task_data_size);
        if (!dest->task_data) {
            if (dest->node_name)
                AGENTRT_FREE(dest->node_name);
            return TASKFLOW_ERROR_MEMORY;
        }
    ...
    // 添加到TaskFlow图
    graph_vertex_t vertex = {.id = node->node_id,
                             .state = VERTEX_ACTIVE,
                             .value = NULL,
                             .value_size = 0,
                             .out_degree = 0,
...
taskflow_error_t workflow_add_edge(workflow_context_t *context, const workflow_edge_t *edge)
{
    ...
    // 复制边标签
    if (edge->edge_label) {
        dest->edge_label = AGENTRT_STRDUP(edge->edge_label);
        if (!dest->edge_label)
            return TASKFLOW_ERROR_MEMORY;
    }

    // 添加到TaskFlow图
    graph_edge_t graph_edge = {.id = edge->edge_id,
                                           ...
                                           size_t node_count)
{
    ...
    for (size_t i = 0; i < node_count - 1; i++) {
        workflow_edge_t edge = {.edge_id = (edge_id_t)(1000 + i),  // 简单ID生成
                                         ...
                                         size_t parallel_count, vertex_id_t end_node_id)
{
    ...
    for (size_t i = 0; i < parallel_count; i++) {
        if (find_node_index(context, parallel_node_ids[i]) == SIZE_MAX)
            return TASKFLOW_ERROR_INVALID_ARG;
    }

    // 添加从起始节点到所有并行节点的边
    for (size_t i = 0; i < parallel_count; i++) {
        workflow_edge_t edge = {.edge_id = (edge_id_t)(2000 + i),
    ...
    // 添加从所有并行节点到结束节点的边
    for (size_t i = 0; i < parallel_count; i++) {
        workflow_edge_t edge = {.edge_id = (edge_id_t)(3000 + i),
                           ...
                           void *condition_context)
{
    ...
    if (find_node_index(context, condition_node_id) == SIZE_MAX)
        return TASKFLOW_ERROR_INVALID_ARG;
    if (find_node_index(context, true_branch_node_id) == SIZE_MAX)
        return TASKFLOW_ERROR_INVALID_ARG;
    if (find_node_index(context, false_branch_node_id) == SIZE_MAX)
        return TASKFLOW_ERROR_INVALID_ARG;
    if (merge_node_id != 0 && find_node_index(context, merge_node_id) == SIZE_MAX)
        return TASKFLOW_ERROR_INVALID_ARG;

    workflow_edge_t true_edge = {.edge_id = (edge_id_t)(4000),
    ...
    workflow_edge_t false_edge = {.edge_id = (edge_id_t)(4001),
                                  .source_node = condition_node_id,
    ...
    if (merge_node_id != 0) {
        workflow_edge_t merge_true = {.edge_id = (edge_id_t)(4002),
        ...
        workflow_edge_t merge_false = {.edge_id = (edge_id_t)(4003),
                                     ...
                                     void *condition_context)
{
    ...
    if (find_node_index(context, loop_start_node_id) == SIZE_MAX)
        return TASKFLOW_ERROR_INVALID_ARG;
    if (find_node_index(context, loop_body_node_id) == SIZE_MAX)
        return TASKFLOW_ERROR_INVALID_ARG;
    if (find_node_index(context, loop_condition_node_id) == SIZE_MAX)
        return TASKFLOW_ERROR_INVALID_ARG;
    if (loop_end_node_id != 0 && find_node_index(context, loop_end_node_id) == SIZE_MAX)
        return TASKFLOW_ERROR_INVALID_ARG;

    workflow_edge_t start_to_body = {.edge_id = (edge_id_t)(5000),
    ...
    workflow_edge_t body_to_cond = {.edge_id = (edge_id_t)(5001),
    ...
    workflow_edge_t cond_to_body = {.edge_id = (edge_id_t)(5002),
    ...
    if (loop_end_node_id != 0) {
        workflow_edge_t cond_to_end = {.edge_id = (edge_id_t)(5003),
                                       .source_node = loop_condition_node_id,
...
taskflow_error_t workflow_execute_sync(workflow_context_t *context, size_t max_iterations)
{
    ...
    if (context->execution_state) {
        ...
        internal_state = (workflow_internal_state_t *)AGENTRT_CALLOC(1, sizeof(workflow_internal_state_t));
        if (!internal_state)
            return TASKFLOW_ERROR_MEMORY;
        ...
        if (context->node_count > 0) {
            internal_state->node_states = (node_execution_state_t *)AGENTRT_CALLOC(
                context->node_count, sizeof(node_execution_state_t));
            if (!internal_state->node_states) {
                AGENTRT_FREE(internal_state);
                return TASKFLOW_ERROR_MEMORY;
            }
