...
typedef struct {
    ...
    agentrt_thread_t thread_handle;
    bool running;
} worker_context_t;
...
struct pregel_engine_s {
    ...
    agentrt_cond_t cond_var;
    agentrt_cond_t pause_cond;
...
{
    message_queue_t *queue = (message_queue_t *)AGENTRT_CALLOC(1, sizeof(message_queue_t));
    return queue;
}
...
// 向消息队列添加消息
static bool message_queue_enqueue(message_queue_t *queue, vertex_id_t target, const void *payload,
                                  size_t payload_size, superstep_t step)
{
    if (!queue)
        return false;

    message_queue_entry_t *entry =
        (message_queue_entry_t *)AGENTRT_CALLOC(1, sizeof(message_queue_entry_t));
    if (!entry)
        return false;
    ...
    if (payload_size > 0 && payload) {
        entry->payload = AGENTRT_MALLOC(payload_size);
        if (!entry->payload) {
            AGENTRT_FREE(entry);
            return false;
        }
...
static bool init_vertex_states(struct pregel_engine_s *engine)
{
    ...
    // 分配顶点状态数组
    engine->vertex_states =
        (pregel_vertex_state_t *)AGENTRT_CALLOC(vertex_count, sizeof(pregel_vertex_state_t));
    if (!engine->vertex_states)
        return false;
    ...
    // 尝试从图引擎获取真实顶点ID映射
    {
        vertex_id_t *real_ids = (vertex_id_t *)AGENTRT_CALLOC(vertex_count, sizeof(vertex_id_t));
        if (real_ids) {
            size_t fetched = 0;
            if (graph_engine_get_vertex_ids(engine->graph_engine, real_ids, vertex_count,
                                            &fetched) == TASKFLOW_SUCCESS &&
                fetched == vertex_count) {
                for (size_t i = 0; i < vertex_count; i++) {
                    engine->vertex_states[i].vertex_id = real_ids[i];
                }
            }
            AGENTRT_FREE(real_ids);
        }
    }
...
static void *worker_thread_func(void *arg)
{
    ...
    while (context->running) {
        agentrt_mutex_lock(&engine->mutex);

        while (context->running && engine->paused) {
            agentrt_cond_wait(&engine->pause_cond, &engine->mutex);
        }

        if (!context->running) {
            agentrt_mutex_unlock(&engine->mutex);
            break;
        }
        ...
        if (!has_work || engine->computation_done) {
            agentrt_cond_timedwait(&engine->cond_var, &engine->mutex, 1000);
            agentrt_mutex_unlock(&engine->mutex);
            continue;
        }

        for (size_t i = context->worker_id; i < engine->vertex_state_count;
             i += engine->worker_count) {
            if (engine->vertex_states[i].active && !engine->vertex_states[i].vote_to_halt &&
                engine->config.compute_func) {
                engine->config.compute_func(
                    engine->vertex_states[i].vertex_id, engine->vertex_states[i].value,
                    engine->vertex_states[i].value_size, engine->vertex_states[i].incoming_messages,
                    engine->vertex_states[i].incoming_message_count, engine->config.user_context);
            }
        }
...
pregel_engine_handle_t pregel_engine_create(const pregel_config_t *config)
{
    ...
    struct pregel_engine_s *engine =
        (struct pregel_engine_s *)AGENTRT_CALLOC(1, sizeof(struct pregel_engine_s));
    if (!engine) {
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
    }
    ...
    // 初始化消息队列数组
    engine->message_queues =
        (message_queue_t **)AGENTRT_CALLOC(engine->config.max_workers, sizeof(message_queue_t *));
    ...
    for (size_t i = 0; i < engine->config.max_workers; i++) {
        ...
        if (!engine->message_queues[i]) {
            ...
            AGENTRT_FREE(engine->message_queues);
            AGENTRT_FREE(engine);
            AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "null parameter");
        }
    }
...
void pregel_engine_destroy(pregel_engine_handle_t engine)
{
    ...
    if (e->vertex_states) {
        for (size_t i = 0; i < e->vertex_state_count; i++) {
            if (e->vertex_states[i].value) {
                AGENTRT_FREE(e->vertex_states[i].value);
            }
    ...
    AGENTRT_FREE(e->message_queues);

    message_queue_destroy(e->next_step_queue);

    // 释放工作线程上下文
    if (e->workers) {
        AGENTRT_FREE(e->workers);
    }

    if (e->initialized) {
        agentrt_mutex_destroy(&e->mutex);
        agentrt_cond_destroy(&e->cond_var);
        agentrt_cond_destroy(&e->pause_cond);
    }
    ...
    if (e->checkpoints) {
        for (size_t i = 0; i < e->checkpoint_count; i++) {
            if (e->checkpoints[i].snapshot_data) {
                AGENTRT_FREE(e->checkpoints[i].snapshot_data);
            }
        }
        AGENTRT_FREE(e->checkpoints);
    }
                                    ...
                                    graph_engine_handle_t graph_engine)
{
    ...
    agentrt_mutex_init(&e->mutex);
    agentrt_cond_init(&e->cond_var);
    agentrt_cond_init(&e->pause_cond);

    /* BAN-333: 0.1.1 强制 max_workers=1 串行降级，默认 1 */
    size_t num_workers = e->config.max_workers > 0 ? e->config.max_workers : 1;
    e->workers = (worker_context_t *)AGENTRT_CALLOC(num_workers, sizeof(worker_context_t));
    if (!e->workers) {
        agentrt_mutex_destroy(&e->mutex);
        agentrt_cond_destroy(&e->cond_var);
        agentrt_cond_destroy(&e->pause_cond);
...
taskflow_error_t pregel_engine_start(pregel_engine_handle_t engine, size_t max_supersteps)
{
    ...
    for (size_t i = 0; i < e->worker_count; i++) {
        ...
        if (agentrt_thread_create(&e->workers[i].thread_handle, worker_thread_func,
                                  &e->workers[i]) != 0) {
            e->workers[i].running = false;
            e->workers[i].thread_handle = AGENTRT_INVALID_THREAD;
            break;
        }
        started_count++;
    }
    ...
    if (started_count < e->worker_count) {
        ...
        agentrt_cond_broadcast(&e->cond_var);
        agentrt_cond_broadcast(&e->pause_cond);
        agentrt_mutex_unlock(&e->mutex);

        for (size_t i = 0; i < started_count; i++) {
            if (e->workers[i].thread_handle != AGENTRT_INVALID_THREAD) {
                agentrt_thread_join(e->workers[i].thread_handle, NULL);
                e->workers[i].thread_handle = AGENTRT_INVALID_THREAD;
            }
        }
        e->running = false;
                                            ...
                                            size_t payload_size)
{
    ...
    // 添加到消息队列
    if (!message_queue_enqueue(e->message_queues[worker_id], target, payload, payload_size,
                               e->current_superstep)) {
        return TASKFLOW_ERROR_MEMORY;
    }
...
uint64_t pregel_engine_create_checkpoint(pregel_engine_handle_t engine)
{
    ...
    void *snapshot = AGENTRT_MALLOC(snapshot_size);
    if (snapshot) {
        size_t offset = 0;
    ...
    if (e->checkpoint_count < 16) {
        e->checkpoints = (checkpoint_t *)AGENTRT_REALLOC(e->checkpoints, (e->checkpoint_count + 1) *
                                                                             sizeof(checkpoint_t));
        if (e->checkpoints) {
            size_t idx = e->checkpoint_count;
            e->checkpoints[idx].checkpoint_id = cp_id;
            e->checkpoints[idx].superstep = e->current_superstep;
            e->checkpoints[idx].timestamp = (uint64_t)(time_t)(agentrt_time_ms() / 1000ULL);
        ...
        if (e->checkpoints[0].snapshot_data)
            AGENTRT_FREE(e->checkpoints[0].snapshot_data);
        ...
        e->checkpoints[15].timestamp = (uint64_t)(time_t)(agentrt_time_ms() / 1000ULL);
    ...
    for (size_t i = 0; i < e->checkpoint_count; i++) {
        if (e->checkpoints[i].checkpoint_id == checkpoint_id) {
            ...
            if (e->checkpoint_count > 0) {
                checkpoint_t *new_arr = (checkpoint_t *)AGENTRT_REALLOC(
...
taskflow_error_t pregel_engine_get_stats(pregel_engine_handle_t engine, execution_stats_t *stats)
{
    ...
    *stats = e->stats;
    ...
    if (e->graph_engine) {
        ...
        graph_engine_get_stats(e->graph_engine, &vertex_count, &edge_count, &max_out_degree,
                               &max_in_degree);
                                                   ...
                                                   uint32_t timeout_ms)
{
    ...
    if (timeout_ms > 0 && timeout_ms != UINT32_MAX) {
        start_ms = agentrt_time_ms();
    }

    while (e->running && !e->computation_done && e->active_vertices > 0) {
        if (timeout_ms == 0 || timeout_ms == UINT32_MAX) {
            agentrt_cond_wait(&e->cond_var, &e->mutex);
        } else {
            uint64_t elapsed = agentrt_time_ms() - start_ms;
            if (elapsed >= timeout_ms) {
                agentrt_mutex_unlock(&e->mutex);
                return TASKFLOW_ERROR_TIMEOUT;
            }
            uint32_t remaining = (uint32_t)(timeout_ms - elapsed);
            int ret = agentrt_cond_timedwait(&e->cond_var, &e->mutex, remaining);
            if (ret != 0) {
                agentrt_mutex_unlock(&e->mutex);
                return TASKFLOW_ERROR_TIMEOUT;
            }
        ...
        if (vs->incoming_messages) {
            for (size_t j = 0; j < vs->incoming_message_count; j++) {
                if (vs->incoming_messages[j].payload) {
                    AGENTRT_FREE(vs->incoming_messages[j].payload);
                }
            }
            AGENTRT_FREE(vs->incoming_messages);
        ...
        while (!message_queue_is_empty(e->next_step_queue)) {
            ...
            if (entry) {
                ...
                if (target_idx != SIZE_MAX) {
                    ...
                    graph_message_t *new_msgs = (graph_message_t *)AGENTRT_REALLOC(
