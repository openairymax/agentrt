...
struct taskflow_engine_s {
    ...
    agentrt_mutex_t engine_mutex;
    agentrt_cond_t pause_cond;
    agentrt_thread_t worker_thread;
    bool worker_active;

    taskflow_message_t *message_queue;
    size_t message_count;
    size_t message_capacity;
    agentrt_thread_t async_thread;
    int async_thread_active;
    bool async_cancel_requested;
    agentrt_cond_t async_complete_cond;
...
/* 日志回调静态互斥锁（跨平台，参照 agentrt_frameworks.c 模式）。
...
taskflow_handle_t taskflow_engine_create_core(const taskflow_config_t *config)
{
    ...
    struct taskflow_engine_s *engine =
        (struct taskflow_engine_s *)AGENTRT_CALLOC(1, sizeof(struct taskflow_engine_s));
    if (!engine) {
        AGENTRT_ERROR_NULL(AGENTRT_ERR_UNKNOWN, "engine allocation failed");
    }
    ...
    if (!engine->graph_engine) {
        ...
        AGENTRT_ERROR_NULL(AGENTRT_ERR_INVALID_PARAM, "graph_engine_create failed");
    }
    ...
    // 初始化同步原语
    agentrt_mutex_init(&engine->engine_mutex);
    agentrt_cond_init(&engine->pause_cond);
    agentrt_cond_init(&engine->async_complete_cond);
    ...
    engine->message_queue =
        (taskflow_message_t *)AGENTRT_CALLOC(engine->message_capacity, sizeof(taskflow_message_t));
    ...
    if (e->message_queue) {
        for (size_t i = 0; i < e->message_count; i++) {
            if (e->message_queue[i].payload)
                AGENTRT_FREE(e->message_queue[i].payload);
        }
        AGENTRT_FREE(e->message_queue);
    }

    // 销毁同步原语
    agentrt_mutex_destroy(&e->engine_mutex);
    agentrt_cond_destroy(&e->pause_cond);
    agentrt_cond_destroy(&e->async_complete_cond);
...
static void *engine_worker_thread(void *arg)
{
    ...
    while (e->worker_active) {
        agentrt_mutex_lock(&e->engine_mutex);

        while (e->paused && e->worker_active) {
            agentrt_cond_wait(&e->pause_cond, &e->engine_mutex);
        }

        if (!e->worker_active) {
            agentrt_mutex_unlock(&e->engine_mutex);
            break;
        }
...
taskflow_error_t taskflow_engine_start_core(taskflow_handle_t engine)
{
    ...
    int ret = agentrt_thread_create(&e->worker_thread, engine_worker_thread, e);
    if (ret != 0) {
...
taskflow_error_t taskflow_engine_stop_core(taskflow_handle_t engine)
{
    ...
    agentrt_thread_join(e->worker_thread, NULL);
...
taskflow_graph_handle_t taskflow_graph_create(taskflow_handle_t engine)
{
    ...
    struct taskflow_graph_s *graph =
        (struct taskflow_graph_s *)AGENTRT_CALLOC(1, sizeof(struct taskflow_graph_s));
    if (!graph) {
        AGENTRT_ERROR_NULL(AGENTRT_ERR_UNKNOWN, "validation failed");
    }
    ...
    for (size_t step = 0; step < max_supersteps || max_supersteps == 0; step++) {
        ...
        if (e->pregel_engine) {
            ...
            if (perr != TASKFLOW_SUCCESS) {
                if (perr == TASKFLOW_ERROR_NO_ACTIVE_VERTICES)
                    break;
            }
        } else if (e->graph_engine && vertex_count > 0) {
            for (size_t v = 0; v < vertex_count; v++) {
                if (e->config.compute_func) {
                    e->config.compute_func((vertex_id_t)v, NULL, 0, NULL, 0,
                                           e->config.user_context);
                }
            }
        }

        e->stats.completed_supersteps++;
        if (max_supersteps > 0 && step >= max_supersteps - 1)
            break;
    }
...
static void *async_execute_worker(void *arg)
{
    ...
    agentrt_cond_broadcast(&e->async_complete_cond);
    agentrt_mutex_unlock(&e->engine_mutex);
                                        ...
                                        void *user_data)
{
    ...
    // 创建异步工作线程
    int ret = agentrt_thread_create(&e->async_thread, async_execute_worker, e);
    if (ret != 0) {
        e->async_thread_active = 0;
        agentrt_mutex_unlock(&e->engine_mutex);
        return TASKFLOW_ERROR_INTERNAL;
...
taskflow_error_t taskflow_execute_wait(taskflow_handle_t engine, uint32_t timeout_ms)
{
    ...
    if (timeout_ms == 0 || timeout_ms == UINT32_MAX) {
        while (e->async_thread_active) {
            agentrt_cond_wait(&e->async_complete_cond, &e->engine_mutex);
        }
        agentrt_mutex_unlock(&e->engine_mutex);
        return e->async_result;
    }

    uint64_t start_ms = agentrt_time_ms();
    while (e->async_thread_active) {
        ...
        int ret = agentrt_cond_timedwait(&e->async_complete_cond, &e->engine_mutex, remaining);
        if (ret != 0) {
            agentrt_mutex_unlock(&e->engine_mutex);
            return TASKFLOW_ERROR_TIMEOUT;
        }
...
taskflow_error_t taskflow_send_message(taskflow_handle_t engine, vertex_id_t source,
                                       vertex_id_t target, const void *payload, size_t payload_size)
{
    ...
    msg->payload = AGENTRT_MALLOC(payload_size);
    if (!msg->payload) {
        agentrt_mutex_unlock(&e->engine_mutex);
...
{
    if (!engine || vertex_id == 0 || !messages || max_count == 0)
        return 0;
...
{
    if (!engine || !checkpoints || max_count == 0)
        return 0;
...
const char *taskflow_error_to_string(taskflow_error_t error)
{
    static const char *error_strings[] = {
        "Success",         "Invalid argument",    "Memory allocation failed",
        "Not initialized", "Already initialized", "Graph too large",
        "Partition error", "Checkpoint error",    "Timeout",
        "Fault detected",  "Communication error", "Internal error"};
...
static taskflow_error_t taskflow_engine_validate_config(const taskflow_config_t *config)
{
    ...
    // 检查分区策略
    if (config->partition_strategy < PARTITION_HASH ||
        config->partition_strategy > PARTITION_CUSTOM) {
        return TASKFLOW_ERROR_INVALID_ARG;
    }
