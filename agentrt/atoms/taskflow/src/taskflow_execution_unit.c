...
typedef struct {
    ...
    agentrt_execution_unit_t unit;      // 执行单元接口
...
// 执行单元执行方法（接口实现）
static agentrt_error_t taskflow_unit_execute_impl(agentrt_execution_unit_t *unit,
                                                  const void *input_data, void **out_output)
{
    ...
    // 解析输入数据
    const agentrt_task_t *agentrt_task = (const agentrt_task_t *)input_data;
    taskflow_task_input_t *task_input = taskflow_parse_task_input(agentrt_task);
    if (!task_input) {
        return TASKFLOW_ERROR_INVALID_ARG;
    }
    ...
    taskflow_execution_context_t *context =
        (taskflow_execution_context_t *)AGENTRT_CALLOC(1, sizeof(taskflow_execution_context_t));
    if (!context) {
...
cleanup:
    ...
    *out_output = context->output;
     ...
     * 注：taskflow_error_t 与 agentrt_error_t 数值兼容
     * （TASKFLOW_SUCCESS = 0 = AGENTRT_ERR_OK），现有代码已混用此约定。 */
    return tf_result;
}

// 执行单元销毁方法（接口实现）
static void taskflow_unit_destroy_impl(agentrt_execution_unit_t *unit)
{
    if (!unit)
        return;
    ...
    AGENTRT_FREE(private);
    // 注意：不释放unit本身，因为它可能由调用者分配在栈上
}

// 执行单元获取元数据方法（接口实现）
static const char *taskflow_unit_get_metadata_impl(agentrt_execution_unit_t *unit)
{
    if (!unit)
        return "TaskFlow Unit (NULL)";

    return "TaskFlow Graph Execution Unit v1.0 - Provides distributed graph computation based on "
           "Pregel model";
}
...
agentrt_execution_unit_t *taskflow_unit_create(const taskflow_unit_config_t *config)
{
    if (!config) return NULL;

    // 分配私有数据
    taskflow_unit_private_t *private =
        (taskflow_unit_private_t *)AGENTRT_CALLOC(1, sizeof(taskflow_unit_private_t));
...
void taskflow_unit_destroy(agentrt_execution_unit_t *unit)
{
    if (!unit)
        return;
                                                  ...
                                                  size_t edge_count, size_t max_supersteps)
{
    taskflow_task_input_t *input =
        (taskflow_task_input_t *)AGENTRT_CALLOC(1, sizeof(taskflow_task_input_t));
...
taskflow_task_output_t *taskflow_task_output_create(void)
{
    taskflow_task_output_t *output =
        (taskflow_task_output_t *)AGENTRT_CALLOC(1, sizeof(taskflow_task_output_t));
...
void taskflow_task_output_destroy(taskflow_task_output_t *output)
{
    ...
    if (output->result_data)
        AGENTRT_FREE(output->result_data);
    AGENTRT_FREE(output);
}

taskflow_task_input_t *taskflow_parse_task_input(const agentrt_task_t *agentrt_task)
{
    if (!agentrt_task || !agentrt_task->task_input) return NULL;

    const taskflow_task_input_t *src = (const taskflow_task_input_t *)agentrt_task->task_input;
...
taskflow_error_t taskflow_pack_task_output(const taskflow_task_output_t *output,
                                           agentrt_task_t *agentrt_task)
{
    if (!output || !agentrt_task)
        return TASKFLOW_ERROR_INVALID_ARG;
    ...
    if (output->result_data && output->result_data_size > 0) {
        packed->result_data = AGENTRT_MALLOC(output->result_data_size);
    ...
    if (agentrt_task->task_output) {
        taskflow_task_output_destroy((taskflow_task_output_t *)agentrt_task->task_output);
    }
    agentrt_task->task_output = packed;

    if (output->result == TASKFLOW_SUCCESS) {
        agentrt_task->task_status = TASK_STATUS_SUCCEEDED;
    } else {
        agentrt_task->task_status = TASK_STATUS_FAILED;
    }

    return TASKFLOW_SUCCESS;
}

agentrt_error_t taskflow_register_unit(agentrt_execution_engine_t *engine, const char *unit_name,
                                       const taskflow_unit_config_t *config)
{
    ...
    // 创建TaskFlow执行单元
    agentrt_execution_unit_t *unit = taskflow_unit_create(config);
    if (!unit) {
        return TASKFLOW_ERROR_MEMORY;
    }

    // 注册到执行引擎
    agentrt_error_t result = agentrt_execution_register_unit(engine, unit_name, *unit);

    // 注意：agentrt_execution_register_unit复制单元，所以我们可以释放本地副本
    taskflow_unit_destroy(unit);

    return result;
}

void taskflow_unregister_unit(agentrt_execution_engine_t *engine, const char *unit_name)
{
    if (!engine || !unit_name)
        return;

    agentrt_execution_unregister_unit(engine, unit_name);
}