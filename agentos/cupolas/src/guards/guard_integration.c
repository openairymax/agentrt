...
CUPOLAS_API int cupolas_guards_init(const guard_manager_config_t *config)
{
    ...
    if (!g_guard_audit_logger) {
        g_guard_audit_logger =
            audit_logger_create(AGENTOS_TMP_DIR "/cupolas_audit", "guard", 1024 * 1024, 10);
    }
...
CUPOLAS_API int cupolas_guards_register_hooks(void)
{
    ...
    AGENTOS_LOG_INFO("[GUARD] Registering safety hooks to Cupolas components...");
    ...
    if (permission_register_post_check_hook(permission_guard_hook) == 0) {
        registered_count++;
        AGENTOS_LOG_INFO("[GUARD] Permission post-check hook registered");
    } else {
        AGENTOS_LOG_ERROR("[GUARD] Failed to register permission hook");
    }
#endif
    ...
    if (workbench_register_pre_exec_hook(command_execution_guard_hook) == 0) {
        registered_count++;
        AGENTOS_LOG_INFO("[GUARD] Workbench pre-execution hook registered");
    } else {
        AGENTOS_LOG_ERROR("[GUARD] Failed to register workbench hook");
    }
#endif
    ...
    if (sanitizer_register_post_process_hook(sanitizer_guard_hook) == 0) {
        registered_count++;
        AGENTOS_LOG_INFO("[GUARD] Sanitizer post-process hook registered");
    } else {
        AGENTOS_LOG_ERROR("[GUARD] Failed to register sanitizer hook");
    }
#endif
    ...
    if (registered_count > 0) {
        hooks_registered = 1;
        g_guards_enabled = true;
        AGENTOS_LOG_INFO("[GUARD] Successfully registered %d safety hooks", registered_count);
        return CUPOLAS_OK;
    }

    AGENTOS_LOG_WARN("[GUARD] Warning: No hooks registered (component hook APIs not available)");
    AGENTOS_LOG_INFO("[GUARD] Guards will work in standalone mode (explicit checks only)");
    return CUPOLAS_OK;
}
...
{
    AGENTOS_LOG_INFO("[GUARD] Unregistering safety hooks from Cupolas components...");

#ifdef CUPOLAS_HAS_PERMISSION_HOOK
    permission_unregister_post_check_hook();
    AGENTOS_LOG_INFO("[GUARD] Permission hook unregistered");
#endif

#ifdef CUPOLAS_HAS_WORKBENCH_HOOK
    workbench_unregister_pre_exec_hook();
    AGENTOS_LOG_INFO("[GUARD] Workbench hook unregistered");
#endif

#ifdef CUPOLAS_HAS_SANITIZER_HOOK
    sanitizer_unregister_post_process_hook();
    AGENTOS_LOG_INFO("[GUARD] Sanitizer hook unregistered");
#endif

    g_guards_enabled = false;
    AGENTOS_LOG_INFO("[GUARD] All safety hooks unregistered");
}