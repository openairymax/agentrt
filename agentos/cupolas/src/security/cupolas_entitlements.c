    ...
    *count = 0;
    ...
    while (token) {
        ...
        if (*token != '\0') {
            if (*count >= capacity) {
                capacity *= 2;
                char **new_arr = (char **)AGENTOS_REALLOC(arr, capacity * sizeof(char *));
                if (!new_arr) {
                    AGENTOS_FREE(dup);
...
static int cupolas_parse_yaml_line(const char *line, char **key, char **value, int *indent)
{
    ...
    *key = cupolas_str_trim(*key);
    ...
    if (*v != '\0' && *v != '\n' && *v != '\r') {
        ...
        *value = (char *)AGENTOS_MALLOC(value_len + 1);
        if (!*value) {
            AGENTOS_FREE(*key);
            *key = NULL;
            return AGENTOS_EINVAL;
...
int cupolas_entitlements_load(const char *yaml_path, cupolas_entitlements_t **entitlements)
{
    ...
    char *content = (char *)AGENTOS_MALLOC(size + 1);
    if (!content) {
        fclose(f);
        return CUPOLAS_ENT_PARSE_ERROR;
    }
                                     ...
                                     cupolas_entitlements_t **entitlements)
{
    ...
    *entitlements = (cupolas_entitlements_t *)AGENTOS_CALLOC(1, sizeof(cupolas_entitlements_t));
    if (!*entitlements)
        return CUPOLAS_ENT_PARSE_ERROR;
...
void cupolas_entitlements_free(cupolas_entitlements_t *entitlements)
{
    ...
    AGENTOS_FREE(entitlements->raw_content);
    AGENTOS_FREE(entitlements->signature);

    AGENTOS_FREE(entitlements->info.agent_id);
    AGENTOS_FREE(entitlements->info.version);

    for (size_t i = 0; i < entitlements->info.fs_count; i++) {
        AGENTOS_FREE(entitlements->info.fs_permissions[i].path);
        cupolas_free_string_array(entitlements->info.fs_permissions[i].permissions,
                                  entitlements->info.fs_permissions[i].perm_count);
    }
    AGENTOS_FREE(entitlements->info.fs_permissions);

    for (size_t i = 0; i < entitlements->info.net_count; i++) {
        AGENTOS_FREE(entitlements->info.net_permissions[i].host);
        AGENTOS_FREE(entitlements->info.net_permissions[i].protocol);
        AGENTOS_FREE(entitlements->info.net_permissions[i].direction);
    }
    AGENTOS_FREE(entitlements->info.net_permissions);

    for (size_t i = 0; i < entitlements->info.ipc_count; i++) {
        AGENTOS_FREE(entitlements->info.ipc_permissions[i].target);
        cupolas_free_string_array(entitlements->info.ipc_permissions[i].permissions,
                                  entitlements->info.ipc_permissions[i].perm_count);
    }
    AGENTOS_FREE(entitlements->info.ipc_permissions);

    for (size_t i = 0; i < entitlements->info.vault_count; i++) {
        AGENTOS_FREE(entitlements->info.vault_permissions[i].cred_id);
    ...
    AGENTOS_FREE(entitlements->info.vault_permissions);
