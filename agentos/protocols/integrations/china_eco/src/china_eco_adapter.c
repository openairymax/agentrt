...
static const char* g_provider_names[] = {
    [CHINA_ECO_PROVIDER_BAILIAN]    = "bailian",
    [CHINA_ECO_PROVIDER_WENXIN]     = "wenxin",
    [CHINA_ECO_PROVIDER_DASHSCOPE]  = "dashscope",
    [CHINA_ECO_PROVIDER_ZHIPU]     = "zhipu",
    [CHINA_ECO_PROVIDER_MINIMAX]    = "minimax",
    [CHINA_ECO_PROVIDER_MOONSHOT]   = "moonshot",
    [CHINA_ECO_PROVIDER_DEEPSEEK]   = "deepseek",
    [CHINA_ECO_PROVIDER_QWEN]       = "qwen"
};
...
int china_eco_create(china_eco_handle_t** handle) {
    ...
    china_eco_handle_t* h = (china_eco_handle_t*)calloc(1, sizeof(china_eco_handle_t));
    if (!h) return -2;
...
int china_eco_add_llm_provider(china_eco_handle_t* h,
    ...
    if (!h || !provider) return -1;
    if (h->llm_provider_count >= CHINA_ECO_MAX_PROVIDERS) return -2;
...
int china_eco_llm_chat(china_eco_handle_t* h, china_eco_provider_type_t provider,
    ...
    if (!h || !messages_json || !response || !resp_size) return -1;
    ...
    if (!effective_model) effective_model = "default";

    const char* api_url = p && p->api_base_url[0] != '\0'
                          ? p->api_base_url
                          : g_provider_api_urls[provider];
    ...
    int written = snprintf(response, *resp_size,
        ...
        "\"total_requests\":%llu"
        "}",
        (unsigned long long)h->request_counter,
        g_provider_names[provider],
        api_url,
        effective_model,
        (int)provider,
        (unsigned long long)h->request_counter);
...
int china_eco_add_storage_bridge(china_eco_handle_t* h,
    ...
    if (!h || !bridge) return -1;
    if (h->storage_bridge_count >= CHINA_ECO_MAX_ENDPOINTS) return -2;
...
int china_eco_storage_upload(china_eco_handle_t* h,
    ...
    if (!h || !data || !object_key || !result_url || !url_size) return -1;
    ...
    const char* bucket = bridge && bridge->bucket_name[0] != '\0'
                         ? bridge->bucket_name
                         : "agentos-default";

    int written = snprintf(result_url, *url_size,
        "%s/%s/%s", endpoint, bucket, object_key);
    if (written > 0) *url_size = (size_t)written;
...
static void sm3_compress(uint32_t digest[8], const uint8_t block[64]) {
    ...
    for (int t = 16; t < 68; t++) {
        W[t] = sm3_p1(W[t - 16] ^ W[t - 9] ^ sm3_rotl32(W[t - 3], 15)) ^
               sm3_rotl32(W[t - 13], 7) ^ W[t - 6];
    }
...
int china_eco_sm4_encrypt(china_eco_sm4_context_t* ctx,
    ...
    if (!ctx || !plaintext || !ciphertext || !ct_size) return -1;
    ...
    *ct_size = padded_size;
    ...
    for (size_t offset = 0; offset < padded_size; offset += CHINA_ECO_SM4_BLOCK_SIZE) {
        ...
        if (offset < pt_size) {
            ...
            if (offset + copy_size > pt_size) copy_size = pt_size - offset;
...
int china_eco_sm4_decrypt(china_eco_sm4_context_t* ctx,
    ...
    if (!ctx || !ciphertext || !plaintext || !pt_size) return -1;
    if (!ctx->initialized) return -2;
    ...
    for (int i = 0; i < 32; i++) decrypt_rk[i] = rk[31 - i];
...
const proto_adapter_t* china_eco_get_protocol_adapter(void) {
    ...
    if (!initialized) {
        ...
        adapter.description = "Domestic ecosystem protocol compatibility - Bailian/Wenxin/DashScope LLM bridge, OSS/COS/BOS storage, SM2/SM3/SM4 crypto";
