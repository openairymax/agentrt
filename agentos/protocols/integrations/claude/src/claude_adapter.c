                           ...
                           char *out_buf, size_t buf_len)
{
    ...
    if (!curl)
        return AGENTOS_ERR_SYS_RESOURCE;
    ...
    AGENTOS_FREE(response_buf.data);
    return AGENTOS_ERR_LLM_PROVIDER_FAIL;
}
                                    ...
                                    size_t buf_len)
{
    ...
    cJSON_Delete(req);
    return result > 0 ? result : AGENTOS_ERR_LLM_PROVIDER_FAIL;
#endif
}
...
static int claude_proto_handle_request(void *context, const void *req, void **resp)
{
    ...
    response->payload = AGENTOS_STRDUP(resp_text);
...
claude_adapter_context_t *claude_adapter_create(const claude_config_t *config)
{
    ...
    claude_adapter_context_t *ctx =
        (claude_adapter_context_t *)AGENTOS_CALLOC(1, sizeof(claude_adapter_context_t));
    if (!ctx)
        return NULL;
    ...
    if (config->system_prompt)
        ctx->config.system_prompt = AGENTOS_STRDUP(config->system_prompt);
    if (config->metadata_json)
        ctx->config.metadata_json = AGENTOS_STRDUP(config->metadata_json);
                           ...
                           const char *system_prompt, claude_response_t *response)
{
    ...
    response->id = AGENTOS_STRDUP(resp_id);
    response->model = AGENTOS_STRDUP(model_name);
    ...
    if (block_count > 0) {
        response->content_blocks = (claude_content_block_t *)AGENTOS_CALLOC(
        ...
        for (int i = 0; i < block_count; i++) {
            ...
            if (type && type->valuestring && strcmp(type->valuestring, "text") == 0) {
                ...
                response->content_blocks[i].type = AGENTOS_STRDUP("text");
                response->content_blocks[i].content.text =
                    AGENTOS_STRDUP(text && text->valuestring ? text->valuestring : "");
            } else if (type && type->valuestring && strcmp(type->valuestring, "tool_use") == 0) {
                cJSON *id = cJSON_GetObjectItem(block, "id");
                cJSON *name = cJSON_GetObjectItem(block, "name");
                cJSON *input = cJSON_GetObjectItem(block, "input");
                response->content_blocks[i].type = AGENTOS_STRDUP("tool_use");
                response->content_blocks[i].content.tool_use.id =
                    AGENTOS_STRDUP(id && id->valuestring ? id->valuestring : "");
                response->content_blocks[i].content.tool_use.name =
                    AGENTOS_STRDUP(name && name->valuestring ? name->valuestring : "");
                char *input_str = input ? cJSON_PrintUnformatted(input) : AGENTOS_STRDUP("{}");
...
int claude_list_models(claude_adapter_context_t *ctx, claude_model_info_t **models, size_t *count)
{
    ...
    *models = (claude_model_info_t *)AGENTOS_CALLOC((size_t)g_builtin_model_count,
    ...
    for (int i = 0; i < g_builtin_model_count; i++) {
        ...
        (*models)[i].api_name = AGENTOS_STRDUP(g_builtin_models[i].api_name);
        (*models)[i].display_name = AGENTOS_STRDUP(g_builtin_models[i].display_name);
    }
    ...
    for (size_t i = 0; i < resp->block_count; i++) {
        if (resp->content_blocks[i].type) {
            if (strcmp(resp->content_blocks[i].type, "text") == 0)
                AGENTOS_FREE(resp->content_blocks[i].content.text);
            else if (strcmp(resp->content_blocks[i].type, "tool_use") == 0) {
                AGENTOS_FREE(resp->content_blocks[i].content.tool_use.id);
                AGENTOS_FREE(resp->content_blocks[i].content.tool_use.name);
                AGENTOS_FREE(resp->content_blocks[i].content.tool_use.input_json);
            } else if (strcmp(resp->content_blocks[i].type, "tool_result") == 0) {
                AGENTOS_FREE(resp->content_blocks[i].content.tool_result.tool_use_id);
                AGENTOS_FREE(resp->content_blocks[i].content.tool_result.content);
            }
            AGENTOS_FREE(resp->content_blocks[i].type);
        }
    }
