...
#define ASSERT_NEQ(a, b, msg) do { if ((a) == (b)) { FAIL(msg); return; } } while(0)
...
static void test_config_default(void) {
    ...
    ASSERT_TRUE(cfg.heartbeat_interval_sec > 0 || cfg.request_timeout_ms > 0, "default config should have valid intervals");
    PASS();
}
...
int main(void) {
    printf("=== OpenClaw Adapter Unit Tests ===\n\n");
