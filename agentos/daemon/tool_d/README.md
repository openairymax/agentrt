...
`daemon/tool_d/` 是 AgentRT 的工具执行守护进程，负责外部工具的注册、发现、安全执行和结果管理。它在沙箱环境中执行工具代码，提供参数校验、结果缓存和执行限流等安全保障，是 Agent 与外部世界交互的关键执行层。通过 daemon_security 集成，所有工具执行都经过输入清洗和权限检查。
