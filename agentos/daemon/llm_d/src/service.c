...
llm_service_t *llm_service_create(const char *config_path)
{
    ...
    AGENTOS_MEMSET(&base_cfg, 0, sizeof(base_cfg));
 ...
 */
static const char *route_by_complexity(llm_complexity_level_t level)
{
    switch (level) {
...
int svc_config_load(const char *config_path, service_config_t *cfg)
{
    ...
    if (ends_with(config_path, ".yaml") || ends_with(config_path, ".yml")) {
        ...
        AGENTOS_MEMSET(cfg, 0, sizeof(service_config_t));
    ...
    *out_count = 0;
    ...
    while (!done) {
        ...
        if (event.type == YAML_STREAM_END_EVENT) {
            ...
            if (in_model_item == 0 && model_count < 64) {
                ...
                if (n && p) {
                    AGENTOS_MEMSET(&models[model_count], 0, sizeof(model_entry_t));
    ...
    for (size_t i = 0; i < model_count; ++i) {
        ...
        if (j == prov_count) {
            if (prov_count >= 16)
                break;
            AGENTOS_MEMSET(&provs[prov_count], 0, sizeof(provider_agg_t));
