 ...
 * @brief 适配器初始化函数
 */
static agentos_error_t gateway_adapter_init(agentos_service_t service,
                                            const agentos_svc_config_t *config)
{
    ...
    // 创建网关服务实例
    agentos_error_t err = gateway_service_create(&ctx->gateway_svc, &ctx->gateway_cfg);
 ...
 * @return 错误码
 */
agentos_error_t gateway_service_adapter_create(agentos_service_t *out_service,
                                               const agentos_svc_config_t *config)
{
    if (!out_service) {
        return AGENTOS_EINVAL;
 ...
 * @return 错误码
 */
agentos_error_t gateway_service_adapter_wrap(agentos_service_t *out_service,
 ...
 * @brief 获取适配器服务统计信息
 */
agentos_error_t gateway_service_adapter_get_stats(agentos_service_t service,
...
bool gateway_service_adapter_supports_type(agentos_service_t service, gateway_daemon_type_t type)
{
    if (!service)
        return false;
    gateway_adapter_ctx_t *ctx = (gateway_adapter_ctx_t *)agentos_service_get_user_data(service);
...
agentos_error_t gateway_service_adapter_set_type_enabled(agentos_service_t service,
...
agentos_error_t gateway_service_adapter_create_from_config(agentos_service_t *out_service,
                                                           const char *config_path)
{
    ...
    gateway_adapter_ctx_t *ctx =
        (gateway_adapter_ctx_t *)agentos_service_get_user_data(*out_service);
    if (ctx) {
        gateway_service_get_default_config(&ctx->gateway_cfg);
    }
    return AGENTOS_SUCCESS;
...
agentos_error_t gateway_service_adapter_reload_config(agentos_service_t service,
                                                      const char *config_path)
{
    if (!service)
        return AGENTOS_EINVAL;
