...
int hook_registry_init(void)
{
    ...
    AGENTOS_MEMSET(&g_hook_registry, 0, sizeof(g_hook_registry));

    if (AGENTOS_RWLOCK_INIT(&g_hook_registry.rwlock, NULL) != 0) {
        return -1;
    }
...
void hook_registry_destroy(void)
{
    ...
    AGENTOS_RWLOCK_WRLOCK(&g_hook_registry.rwlock);
    ...
    AGENTOS_RWLOCK_UNLOCK(&g_hook_registry.rwlock);
    AGENTOS_RWLOCK_DESTROY(&g_hook_registry.rwlock);
...
int hook_registry_register(const hook_entry_t *entry)
{
    ...
    /* 检查重名 */
    AGENTOS_RWLOCK_RDLOCK(&g_hook_registry.rwlock);
    ...
    /* 分配新节点 */
    hook_node_t *new_node = (hook_node_t *)AGENTOS_CALLOC(1, sizeof(hook_node_t));
    if (!new_node)
        return -1;
