 ...
 *        agentos_hook_metrics_dump()，为 Prometheus 导出做准备。
 ...
 * 遵循 hook_service.h 契约）。处理器在 agentos_hook_init() 之后通过
 * agentos_hook_register_builtin_handlers() 注册，在 agentos_hook_shutdown()
 * 之前通过 agentos_hook_unregister_builtin_handlers() 注销。
 *
 * Copyright (C) 2025-2026 SPHARX Ltd. All Rights Reserved.
 * SPDX-License-Identifier: AGPL-3.0-or-later OR Apache-2.0
 */

#ifndef AGENTOS_HOOK_BUILTIN_HANDLERS_H
#define AGENTOS_HOOK_BUILTIN_HANDLERS_H

#include "hook_service.h"
#include "agentos_hook.h"   /* agentos_hook_register/unregister/trigger 等 inline API */
 ...
 *
 * 在 agentos_hook_init() 成功后调用。注册 12 个 hook entry：
 ...
 */
int agentos_hook_register_builtin_handlers(void);

/**
 * @brief 注销全部内置生产 Hook 处理器
 *
 * 在 agentos_hook_shutdown() 之前调用。幂等。
 */
void agentos_hook_unregister_builtin_handlers(void);
 ...
 */
int agentos_hook_metrics_dump(char *buf, size_t bufsize);
 ...
 */
uint64_t agentos_hook_metrics_get_count(hook_type_t type);

#ifdef __cplusplus
}
#endif

#endif /* AGENTOS_HOOK_BUILTIN_HANDLERS_H */
