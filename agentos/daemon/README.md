...
`agentos/daemon/` 是 AgentOS 的用户态服务层，由 10+ 个独立进程的守护进程（Daemon）组成，为智能体系统提供完整的后端服务支持。每个守护进程遵循**职责单一原则**，独立运行并通过统一的 IPC 服务总线协作通信，构成一个高可用、可扩展、可插拔的微服务架构。
...
|----------|------|------|-------------|
| **API 网关** | `gateway_d/` | 外部请求接入，协议转换（HTTP/WS/MCP/A2A/OpenAI API）与路由 | `agentos-gateway-d` |
| **LLM 服务** | `llm_d/` | 大语言模型调用、Token 计数、成本追踪、响应缓存 | `agentos-llm-d` |
| **工具执行** | `tool_d/` | 工具注册/发现、沙箱执行、参数校验、结果缓存 | `agentos-tool-d` |
| **任务调度** | `sched_d/` | 任务分发、4 种调度策略（轮询/加权/优先级/ML） | `agentos-sched-d` |
| **应用市场** | `market_d/` | Agent/Skill/Tool/Template 资源管理、安装、版本控制 | `agentos-market-d` |
| **监控告警** | `monit_d/` | 指标采集、健康检查、告警管理、Agent 死循环检测 | `agentos-monit-d` |
| **通道服务** | `channel_d/` | 通信通道管理与消息路由 | `agentos-channel-d` |
| **信息服务** | `info_d/` | 系统信息查询与状态报告 | `agentos-info-d` |
| **通知服务** | `notify_d/` | 多渠道通知推送（邮件/Slack/Discord） | `agentos-notify-d` |
| **观测服务** | `observe_d/` | OpenTelemetry 可观测性数据采集 | `agentos-observe-d` |
...
服务状态枚举（`agentos_svc_state_t`）：

| 状态 | 说明 |
|------|------|
| `AGENTOS_SVC_STATE_NONE` | 未初始化 |
