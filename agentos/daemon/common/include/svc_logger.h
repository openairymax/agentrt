 ...
 * @brief 日志回调函数类型
 */
typedef void (*agentos_log_callback_t)(agentos_log_level_t level, const char *timestamp,
...
static inline void agentos_trace_new(agentos_trace_context_t *ctx)
{
    ...
    snprintf(ctx->trace_id, sizeof(ctx->trace_id), "%08x-%04x-%04x-%04x-%012llx",
             (uint32_t)agentos_time_ns(), (uint16_t)(agentos_time_ns() >> 16),
             (uint16_t)(agentos_time_ns() >> 32), (uint16_t)(agentos_time_ns() >> 48),
...
static inline void agentos_trace_new_span(agentos_trace_context_t *ctx)
{
    if (!ctx)
        return;
    snprintf(ctx->span_id, sizeof(ctx->span_id), "%llx", (unsigned long long)agentos_time_ns());
}
 ...
 * @param ... [in] 可变参数
 */
static inline void agentos_logger_log(agentos_logger_t logger, agentos_log_level_t level,
 ...
 * @brief 写入带追踪上下文的日志
 */
static inline void agentos_logger_log_with_trace(agentos_logger_t logger, agentos_log_level_t level,
                                                 const agentos_trace_context_t *trace_ctx,
...
#define LOG_TRACE_T(ctx, ...) \
    AGENTOS_LOG_TRACE_IMPL(agentos_logger_default(), LOG_LEVEL_DEBUG, (ctx), __VA_ARGS__)
...
#define LOG_INFO_T(ctx, ...) \
    AGENTOS_LOG_TRACE_IMPL(agentos_logger_default(), LOG_LEVEL_INFO, (ctx), __VA_ARGS__)

/**
 * @brief 带追踪的警告级别日志
 */
#define LOG_WARN_T(ctx, ...) \
    AGENTOS_LOG_TRACE_IMPL(agentos_logger_default(), LOG_LEVEL_WARN, (ctx), __VA_ARGS__)

/**
 * @brief 带追踪的错误级别日志
 */
#define LOG_ERROR_T(ctx, ...) \
    AGENTOS_LOG_TRACE_IMPL(agentos_logger_default(), LOG_LEVEL_ERROR, (ctx), __VA_ARGS__)

/**
 * @brief 带追踪的致命级别日志
 */
#define LOG_FATAL_T(ctx, ...) \
    AGENTOS_LOG_TRACE_IMPL(agentos_logger_default(), LOG_LEVEL_FATAL, (ctx), __VA_ARGS__)
