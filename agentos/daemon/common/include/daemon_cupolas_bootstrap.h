 ...
 *
 * 调用契约：
 *   - main() 中 agentos_log_init() 之后、socket/service 创建之前调用 daemon_cupolas_init()
...
#ifndef AGENTOS_DAEMON_CUPOLAS_BOOTSTRAP_H
#define AGENTOS_DAEMON_CUPOLAS_BOOTSTRAP_H

#include "error.h" /* agentos_error_t */
...
extern "C"
{
     ...
     *
     * 在 daemon main() 中 agentos_log_init() 之后调用。
     * 初始化 permission_engine、sanitizer、audit_logger 三大子模块。
     *
     * @param daemon_name daemon 名称（如 "tool_d"、"llm_d"），用于审计日志标识
     * @return AGENTOS_SUCCESS 成功；错误码失败（FATAL 日志已记录）
     *
     * @ownership daemon_name: BORROW (调用方保留所有权)
     */
    agentos_error_t daemon_cupolas_init(const char *daemon_name);
...
#endif /* AGENTOS_DAEMON_CUPOLAS_BOOTSTRAP_H */
