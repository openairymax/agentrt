...
static void test_add_credential_null_pool(void)
{
    ...
    ASSERT(ret == AGENTOS_ERR_INVALID_PARAM, "should return INVALID_PARAM");
    PASS();
}
...
static void test_add_credential_overflow(void)
{
    ...
    ASSERT(ret == AGENTOS_ERR_OVERFLOW, "should return OVERFLOW when full");
...
static void test_remove_credential_out_of_range(void)
{
    ...
    ASSERT(ret == AGENTOS_ERR_INVALID_PARAM, "should return INVALID_PARAM for out of range");

    ret = api_rec_remove_credential(pool, 1);
    ASSERT(ret == AGENTOS_ERR_INVALID_PARAM, "should return INVALID_PARAM for index == count");
...
static void test_mark_cred_empty_pool(void)
{
    ...
    int ret = api_rec_mark_cred_success(pool);
    ASSERT(ret == AGENTOS_ERR_INVALID_PARAM, "mark success on empty should fail");

    ret = api_rec_mark_cred_failure(pool, API_REC_ERR_NETWORK);
    ASSERT(ret == AGENTOS_ERR_INVALID_PARAM, "mark failure on empty should fail");
