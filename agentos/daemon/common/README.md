...
`daemon/common/` 是所有守护进程共享的基础库，提供兼容层、工具函数和通用服务组件。它为上层守护进程屏蔽了操作系统差异，提供统一的服务生命周期管理、IPC 通信、日志、配置、安全、缓存等基础设施，是整个 Daemon 层的基石。
...
| **agentos_event_loop** | `agentos_event_loop.h` | 事件循环，异步 I/O 处理 |
...
```c
agentos_error_t agentos_service_create(agentos_service_t *service, const char *name,
                                       const agentos_svc_interface_t *iface,
                                       const agentos_svc_config_t *config);
agentos_error_t agentos_service_init(agentos_service_t service);
agentos_error_t agentos_service_start(agentos_service_t service);
agentos_error_t agentos_service_stop(agentos_service_t service, bool force);
void agentos_service_destroy(agentos_service_t service);
agentos_svc_state_t agentos_service_get_state(agentos_service_t service);
bool agentos_service_is_ready(agentos_service_t service);
bool agentos_service_is_running(agentos_service_t service);
agentos_error_t agentos_service_healthcheck(agentos_service_t service);
agentos_error_t agentos_service_get_stats(agentos_service_t service,
...
agentos_error_t ipc_service_bus_start(ipc_service_bus_t bus);
agentos_error_t ipc_service_bus_stop(ipc_service_bus_t bus);
agentos_error_t ipc_service_bus_send(ipc_service_bus_t bus, const char *target_service,
                                     const ipc_bus_message_t *message);
agentos_error_t ipc_service_bus_request(ipc_service_bus_t bus, const char *target_service,
                                        const ipc_bus_message_t *request,
                                        ipc_bus_message_t *response, uint32_t timeout_ms);
agentos_error_t ipc_service_bus_broadcast(ipc_service_bus_t bus,
                                          const ipc_bus_message_t *message);
agentos_error_t ipc_service_bus_register_handler(ipc_service_bus_t bus,
...
agentos_error_t cb_execute_failover(circuit_breaker_t breaker, int32_t original_error,
...
agentos_service_create(&service, "my_daemon", &iface, &config);
agentos_service_init(service);
agentos_service_start(service);
```
...
if (cb_allow_request(cb)) {
    agentos_error_t err = call_llm_service();
    if (err == AGENTOS_OK) {
