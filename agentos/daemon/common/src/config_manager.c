...
AGENTOS_API int cm_set_namespaced(const char *namespace_, const char *key, const char *value,
                                  const char *source)
{
    if (!namespace_ || !key)
        return AGENTOS_ERR_INVALID_PARAM;
...
AGENTOS_API int cm_watch(const char *key_pattern, cm_change_callback_t callback, void *user_data)
{
    if (!callback)
        return AGENTOS_ERR_INVALID_PARAM;
    if (!g_cm.initialized)
...
AGENTOS_API int cm_get_history(const char *key, cm_change_record_t *records, uint32_t max_count,
