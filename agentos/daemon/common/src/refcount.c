...
void *refcount_alloc(size_t size, void (*deleter)(void *object))
{
    if (size < sizeof(refcounted_t)) {
        AGENTOS_LOG_ERROR("Refcount: size %zu too small (min %zu)",
                          size, sizeof(refcounted_t));
        return NULL;
    }

    void *ptr = AGENTOS_CALLOC(1, size);
    if (!ptr) {
        AGENTOS_LOG_ERROR("Refcount: OOM allocating %zu bytes", size);
        return NULL;
    }
...
uint32_t refcount_retain(refcounted_t *rc)
{
    ...
    if (old == 0) {
        ...
        AGENTOS_LOG_ERROR("Refcount: retain on freed object (type=%s, "
                          "old=%u, new=%u)",
    ...
    AGENTOS_LOG_TRACE("Refcount: retain (type=%s, %u → %u, "
                      "live=%llu)",
                      rc->type_name ? rc->type_name : "unknown",
                      old, new_count,
...
uint32_t refcount_release(refcounted_t *rc)
{
    ...
    if (old == 0) {
        AGENTOS_LOG_ERROR("Refcount: double-free on object (type=%s, "
                          "live=%llu)",
                          rc->type_name ? rc->type_name : "unknown",
                          (unsigned long long)atomic_load_explicit(
                              &g_current_live, memory_order_relaxed));
    ...
    AGENTOS_LOG_TRACE("Refcount: release (type=%s, %u → %u, "
                      "live=%llu)",
                      rc->type_name ? rc->type_name : "unknown",
                      old, new_count,
                      (unsigned long long)atomic_load_explicit(
    ...
    if (new_count == 0) {
        /* 引用计数归零，调用 deleter */
        AGENTOS_LOG_DEBUG("Refcount: object freed (type=%s, "
