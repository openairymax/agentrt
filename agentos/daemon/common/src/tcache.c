...
static int bin_batch_fill(tcache_t *tc, tcache_bin_t *bin)
{
    ...
    AGENTOS_LOG_DEBUG("Tcache: batch_fill size_class=%u (count=%u/%u, "
    ...
    for (uint32_t i = 0; i < to_fill; i++) {
        void *ptr = AGENTOS_MALLOC(bin->size_class);
        if (!ptr) {
            AGENTOS_LOG_ERROR("Tcache: batch_fill OOM at %u/%u "
    ...
    AGENTOS_LOG_DEBUG("Tcache: batch_fill done (size_class=%u, filled=%u/%u, "
...
static void bin_batch_flush(tcache_t *tc, tcache_bin_t *bin)
{
    ...
    AGENTOS_LOG_DEBUG("Tcache: batch_flush size_class=%u (count=%u, "
                      "to_flush=%u, cached_bytes=%zu)",
                      bin->size_class, bin->count, to_flush,
                      tc->total_cached_bytes);

    for (uint32_t i = 0; i < to_flush; i++) {
        AGENTOS_FREE(bin->entries[bin->count - 1 - i]);
        tc->total_cached_bytes -= bin->size_class;
    }
    ...
    AGENTOS_LOG_DEBUG("Tcache: batch_flush done (size_class=%u, "
...
{
    tcache_t *tc = (tcache_t *)AGENTOS_CALLOC(1, sizeof(tcache_t));
    if (!tc) {
        AGENTOS_LOG_ERROR("Tcache: OOM creating tcache");
        return NULL;
    }
    ...
    AGENTOS_LOG_DEBUG("Tcache: created (max_entries=%zu, batch=%zu, "
...
void tcache_destroy(tcache_t *tc)
{
    ...
    if (tc->config.enable_stats) {
        ...
        double hit_rate = total > 0
            ...
            : 0.0;
        AGENTOS_LOG_INFO("Tcache: destroyed (hits=%llu misses=%llu "
        ...
        AGENTOS_LOG_INFO("Tcache: destroyed (cached_at_exit=%zu objects/%zu bytes)",
                         cached_count, cached_bytes);
    }
...
void *tcache_alloc(tcache_t *tc, size_t size)
{
    if (!tc || size == 0) {
        if (!tc) {
            AGENTOS_LOG_ERROR("Tcache: tcache_alloc called with NULL tcache");
        }
        return NULL;
    }
    ...
    if (bin_idx < 0 || (uint32_t)size > tc->config.max_cache_size_class) {
        ...
        AGENTOS_LOG_TRACE("Tcache: oversized alloc (size=%zu, "
                          "oversized_count=%llu)",
                          size,
                          (unsigned long long)tc->stats.oversized_alloc);
        return AGENTOS_MALLOC(size);
    }
    ...
    if (bin->count == 0) {
        ...
        AGENTOS_LOG_DEBUG("Tcache: cache miss (size=%zu → size_class=%u, "
        ...
        if (bin_batch_fill(tc, bin) <= 0) {
            /* 批量填充失败，降级为直接分配 */
            AGENTOS_LOG_WARN("Tcache: batch_fill failed, fallback to MALLOC "
    ...
    AGENTOS_LOG_TRACE("Tcache: alloc hit (size=%zu → class=%u, ptr=%p, "
...
void tcache_free(tcache_t *tc, void *ptr, size_t size)
{
    if (!tc || !ptr) {
        if (!tc) {
            AGENTOS_LOG_WARN("Tcache: tcache_free called with NULL tcache "
    ...
    int bin_idx = find_size_class(size);
    if (bin_idx < 0 || (uint32_t)size > tc->config.max_cache_size_class) {
        AGENTOS_LOG_TRACE("Tcache: oversized free (size=%zu)", size);
        AGENTOS_FREE(ptr);
        return;
    }
    ...
    if (tc->config.max_total_cached_bytes > 0 &&
        ...
        AGENTOS_LOG_DEBUG("Tcache: cache full, direct free (size=%zu, "
                          "cached=%zu, max=%u)",
    ...
    if (bin->count >= bin->max_count) {
        AGENTOS_LOG_DEBUG("Tcache: bin full, triggering flush (size_class=%u, "
                          "count=%u/%u)",
                          bin->size_class, bin->count, bin->max_count);
    ...
    AGENTOS_LOG_TRACE("Tcache: free cached (size=%zu → class=%u, ptr=%p, "
...
void tcache_flush(tcache_t *tc)
{
    ...
    for (int i = 0; i < NUM_SIZE_CLASSES; i++) {
        ...
        if (bin->count > 0) {
            AGENTOS_LOG_DEBUG("Tcache: flush bin %d (size_class=%u, count=%u)",
                              i, bin->size_class, bin->count);
        }
        flushed_count += bin->count;
        while (bin->count > 0) {
            bin->count--;
            AGENTOS_FREE(bin->entries[bin->count]);
            tc->total_cached_bytes -= bin->size_class;
        }
    }
    ...
    AGENTOS_LOG_INFO("Tcache: flush complete (flushed=%zu objects, "
                     "%zu bytes)",
...
void tcache_purge(tcache_t *tc)
{
    ...
    AGENTOS_LOG_INFO("Tcache: purge (discarded %zu objects, %zu bytes)",
