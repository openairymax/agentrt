...
static long events_to_wsa(uint32_t events)
{
    ...
    if (events & AGENTOS_EVENT_TYPE_WRITE)
...
static uint32_t wsa_to_events(long wsa_events)
{
    ...
    if (wsa_events & (FD_READ | FD_ACCEPT | FD_CLOSE))
        ev |= AGENTOS_EVENT_TYPE_READ;
    if (wsa_events & (FD_WRITE | FD_CONNECT))
        ev |= AGENTOS_EVENT_TYPE_WRITE;
    return ev;
}
                           ...
                           agentos_event_callback_t cb, void *user_data, bool level_triggered)
{
    ...
    if (free_idx < 0) {
        AGENTOS_ERROR(AGENTOS_ERR_NOT_FOUND, "no free fd slot available");
    }
...
int agentos_event_loop_mod_fd(agentos_event_loop_t *loop, int fd, uint32_t events)
{
    ...
    int idx = find_fd_entry(loop, sock);
    if (idx < 0) {
        AGENTOS_ERROR(AGENTOS_ERR_NOT_FOUND, "fd not found for modify");
    }
...
void agentos_event_loop_remove_fd(agentos_event_loop_t *loop, int fd)
{
    ...
    AGENTOS_MEMSET(&loop->fd_entries[idx], 0, sizeof(fd_entry_t));
    loop->fd_count--;
}
...
int agentos_event_loop_cancel_timer(agentos_event_loop_t *loop, uint64_t timer_id)
{
    ...
    AGENTOS_ERROR(AGENTOS_ERR_NOT_FOUND, "timer not found");
}
...
int agentos_event_loop_wakeup(agentos_event_loop_t *loop)
{
    ...
    if (!SetEvent(loop->wakeup_event)) {
        AGENTOS_ERROR(AGENTOS_ERR_IO, "wakeup SetEvent failed");
    }
    return 0;
}
...
agentos_event_loop_t *agentos_event_loop_create(int max_events)
{
    ...
    loop->max_events = max_events;
    loop->epoll_events =
        (struct epoll_event *)AGENTOS_CALLOC((size_t)max_events, sizeof(struct epoll_event));
    loop->fd_entries = (fd_entry_t *)AGENTOS_CALLOC((size_t)max_events, sizeof(fd_entry_t));
...
int agentos_event_loop_mod_fd(agentos_event_loop_t *loop, int fd, uint32_t events)
{
    ...
    if (epoll_ctl(loop->epoll_fd, EPOLL_CTL_MOD, fd, &ev) < 0) {
        AGENTOS_ERROR(AGENTOS_ERR_IO, "epoll_ctl MOD failed");
    }
...
void agentos_event_loop_remove_fd(agentos_event_loop_t *loop, int fd)
{
    ...
    AGENTOS_MEMSET(&loop->fd_entries[fd], 0, sizeof(fd_entry_t));
}
    ...
    while (!loop->stop_requested) {
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (revents & (EPOLLIN | EPOLLHUP | EPOLLERR))
                user_events |= AGENTOS_EVENT_TYPE_READ;
            if (revents & EPOLLOUT)
                user_events |= AGENTOS_EVENT_TYPE_WRITE;
...
int agentos_event_loop_wakeup(agentos_event_loop_t *loop)
{
    ...
    if (write(loop->wakeup_fd, &val, sizeof(val)) < 0) {
        AGENTOS_ERROR(AGENTOS_ERR_IO, "wakeup write failed");
    }
    return 0;
}
