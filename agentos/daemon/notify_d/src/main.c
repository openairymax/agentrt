...
static int notify_d_compute_ws_accept_key(const char* client_key,
    ...
    if (!client_key || !out_key || out_size < 64) return -1;
    ...
    sha1[4] = (unsigned char)(h1 >> 24); sha1[5] = (unsigned char)(h1 >> 16);
    sha1[6] = (unsigned char)(h1 >> 8); sha1[7] = (unsigned char)(h1);
    sha1[8] = (unsigned char)(h2 >> 24); sha1[9] = (unsigned char)(h2 >> 16);
    sha1[10] = (unsigned char)(h2 >> 8); sha1[11] = (unsigned char)(h2);
    sha1[12] = (unsigned char)(h3 >> 24); sha1[13] = (unsigned char)(h3 >> 16);
    sha1[14] = (unsigned char)(h3 >> 8); sha1[15] = (unsigned char)(h3);
    sha1[16] = (unsigned char)(h4 >> 24); sha1[17] = (unsigned char)(h4 >> 16);
    sha1[18] = (unsigned char)(h4 >> 8); sha1[19] = (unsigned char)(h4);

    static const char* b64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    size_t off = 0;
    for (int i = 0; i < 20 && off < out_size - 4; i += 3) {
        unsigned int val = ((unsigned int)sha1[i] << 16) |
                           ((unsigned int)sha1[i+1] << 8) | (unsigned int)sha1[i+2];
...
static int notify_d_handle_ws_upgrade(notify_d_service_t* svc,
                                       notify_client_t* client,
                                       const char* request) {
...
static int notify_d_send_ws_frame(notify_client_t* client, const char* payload,
                                   size_t payload_len) {
    if (!client || !payload || client->fd == AGENTOS_INVALID_SOCKET) return -1;
...
static int notify_d_broadcast_event(notify_d_service_t* svc,
                                     const notify_event_t* event) {
    if (!svc || !event) return -1;
    ...
    int msg_len = snprintf(json_msg, sizeof(json_msg),
        ...
        event->channel ? event->channel : "default",
    ...
    for (size_t i = 0; i < svc->client_count; i++) {
        ...
        if (client->type == NOTIFY_CLIENT_WEBSOCKET && client->handshake_done) {
            ...
            char sse_msg[8448];
            int sse_len = snprintf(sse_msg, sizeof(sse_msg),
                "event: %s\ndata: %s\n\n",
                event->event_type ? event->event_type : "message",
                json_msg);
...
static int notify_d_enqueue(notify_d_service_t* svc, const char* msg,
                             const char* channel, const char* event_type) {
    if (!svc || !msg) return -1;
...
static notify_client_t* notify_d_find_client_slot(notify_d_service_t* svc) {
    for (size_t i = 0; i < NOTIFY_D_MAX_CLIENTS; i++) {
        if (!svc->clients[i].active) return &svc->clients[i];
    }
...
static int notify_d_stop(notify_d_service_t* svc, int force) {
    ...
    SVC_LOG_INFO("notify_d: service stopped (force=%d, pending=%zu, clients=%zu)",
                 force, svc->pending_count, svc->client_count);
    return AGENTOS_SUCCESS;
}
...
static int notify_d_healthcheck(notify_d_service_t* svc) {
    ...
    if (svc->error_count > svc->notified_count / 2 && svc->notified_count > 10) healthy = 0;
...
static void notify_d_handle_request(notify_d_service_t* svc,
    ...
    if (client->type == NOTIFY_CLIENT_SOCKET) {
        ...
        snprintf(response, sizeof(response),
            ...
            notify_d_healthcheck(svc) ? "true" : "false");
