...
`daemon/channel_d/` 是 AgentRT 的进程间通信通道管理守护进程，负责管理 IPC 通道的创建、销毁、数据收发和状态监控。它支持 Unix Socket、共享内存和命名管道三种通道类型，为 AgentRT 各守护进程间的高效通信提供统一的通道抽象层（IMP-08），是 IPC Service Bus 的底层传输基础设施。
...
```c
agentos_error_t channel_service_create(channel_service_t *service,
                                       const channel_service_config_t *config);
void channel_service_destroy(channel_service_t service);
agentos_error_t channel_service_start(channel_service_t service);
agentos_error_t channel_service_stop(channel_service_t service);
```

### 通道操作接口

```c
agentos_error_t channel_open(channel_service_t service, channel_type_t type,
                             const char *name, channel_handle_t *handle);
agentos_error_t channel_close(channel_service_t service, channel_handle_t handle);
agentos_error_t channel_send(channel_service_t service, channel_handle_t handle,
                             const void *data, size_t len);
agentos_error_t channel_receive(channel_service_t service, channel_handle_t handle,
...
```c
agentos_error_t channel_list(channel_service_t service, char **out_json);
agentos_error_t channel_get_info(channel_service_t service, channel_handle_t handle,
                                 channel_info_t *info);
agentos_error_t channel_set_callback(channel_service_t service, channel_handle_t handle,
                                     channel_event_cb_t callback, void *user_data);
agentos_error_t channel_ping(channel_service_t service, channel_handle_t handle,
...
```
channel_d
├── common (svc_common, agentos_common)
└── Threads::Threads
```
