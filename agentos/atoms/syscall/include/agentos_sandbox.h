 ...
 * 典型用法（tool_d 执行工具）：
 * ```c
 * agentos_sandbox_manager_init();           // 幂等，进程级初始化
 * agentos_sandbox_t *sb = NULL;
 * agentos_sandbox_create_default("tool_d", &sb);  // 默认配置
 * agentos_sandbox_add_rule(sb, SYS_TOOL_EXECUTE, PERM_ALLOW, NULL);
 * void *args[1] = { &tool_args };
 * void *out = NULL;
 * agentos_error_t rc = agentos_sandbox_invoke(sb, SYS_TOOL_EXECUTE, args, 1, &out);
 * if (rc != AGENTOS_SUCCESS) { // fail-closed: 拒绝执行
 * }
...
#ifndef AGENTOS_SANDBOX_H
#define AGENTOS_SANDBOX_H
 ...
 * 以便外部调用方（如 tool_d）通过 agentos_sandbox_add_rule 配置权限。
 ...
 * @return AGENTOS_SUCCESS 成功（含重复初始化）；AGENTOS_ENOMEM 内存不足
 * @note 必须在 agentos_sandbox_create_default 之前调用。重复调用安全。
 */
 ...
 * @return AGENTOS_SUCCESS 成功；AGENTOS_ENOTINIT 管理器未初始化；
 *         AGENTOS_EBUSY 无可用槽位；AGENTOS_ENOMEM 内存不足
 *
 * @details 默认配置：
 * - priority: 0（普通）
 ...
 * - 启用输入净化和资源监控
 *
 * @note 此函数是 agentos_sandbox_create(config, out) 的便捷封装，
 *       内部构造 sandbox_config_t 后调用真实 create。非桩函数，功能完整。
 */
AGENTOS_API agentos_error_t agentos_sandbox_create_default(const char *name,
                                                            const char *owner_id,
 ...
 * @return AGENTOS_SUCCESS 成功；AGENTOS_EINVAL sandbox 为 NULL；AGENTOS_ENOMEM 内存不足
 *
 * @details 规则按 syscall_num 匹配。DENY 规则优先于 ALLOW/ASK。
 * 同一 syscall 可添加多条规则，DENY 任一即拒绝。
 */
AGENTOS_API agentos_error_t agentos_sandbox_add_rule(agentos_sandbox_t *sandbox, int syscall_num,
 ...
 * 4. agentos_syscall_invoke 分发到具体 syscall 处理函数
 * 5. 审计日志记录（成功/失败均记录）
 * 6. 性能统计更新（耗时、调用次数）
 *
 * @note fail-closed 原则：sandbox 为 NULL 时调用方必须拒绝执行（由调用方检查）。
 */
AGENTOS_API agentos_error_t agentos_sandbox_invoke(agentos_sandbox_t *sandbox, int syscall_num,
