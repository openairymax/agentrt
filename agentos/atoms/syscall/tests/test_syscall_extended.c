...
static void test_task_submit_zero_timeout(void)
{
    ...
    ASSERT(e != 0 || e == AGENTOS_SUCCESS, "task_submit zero timeout returns error or success");
    if (out)
        agentos_sys_free(out);
}
...
static void test_memory_write_valid_data(void)
{
    ...
    ASSERT(e == AGENTOS_SUCCESS || e == AGENTOS_ENOTINIT || e != AGENTOS_EINVAL,
           "memory_write valid data accepted");
    if (rid)
        agentos_sys_free(rid);
}
...
static void test_session_create_valid(void)
{
    ...
    ASSERT(e == AGENTOS_SUCCESS || e == AGENTOS_ENOTINIT,
