...
static agentos_error_t persist_session_with_retry(const char *session_id, const char *metadata,
                                                  uint64_t created_ns, uint64_t last_active_ns)
{
    ...
    AGENTOS_LOG_ERROR("Session persist failed after %d attempts: %d", retry_count, last_err);
    return last_err;
}
...
static agentos_error_t persist_delete_with_retry(const char *session_id)
{
    ...
    while (retry_count <= g_persist_config.max_retries) {
        ...
        retry_count++;
        if (retry_count <= g_persist_config.max_retries) {
            AGENTOS_LOG_WARN("Session delete attempt %d failed: %d, retrying in %d ms", retry_count,
                             last_err, delay_ms);
            agentos_time_nanosleep((uint64_t)delay_ms * 1000000ULL);
            delay_ms =
                MIN(delay_ms * g_persist_config.backoff_multiplier, g_persist_config.max_delay_ms);
    ...
    AGENTOS_LOG_ERROR("Session delete failed after %d attempts: %d", retry_count, last_err);
    return last_err;
}
...
static void ensure_lock(void)
{
    agentos_mutex_t *current =
        (agentos_mutex_t *)atomic_load_ptr((_Atomic void **)&session_lock, memory_order_acquire);
    if (!current) {
    ...
    if (g_persist_config.enabled) {
        ...
        if (persist_err == AGENTOS_SUCCESS) {
            ...
            if (!g_persist_config.fail_fast) {
                AGENTOS_LOG_ERROR("Session creation failed due to persistence error: %d",
                                  persist_err);
                agentos_mutex_lock(session_lock);
        ...
        if (strcmp((*p)->session_id, session_id) == 0) {
            ...
            *p = tmp->next;
            ...
            if (g_persist_config.enabled && saved_id) {
                ...
                if (persist_err != AGENTOS_SUCCESS) {
                    AGENTOS_LOG_WARN("Session delete from heapstore failed after retries: %d",
                                     persist_err);
                }
...
agentos_error_t agentos_sys_session_get_persist_status(const char *session_id,
