...
typedef agentos_error_t (*skill_executor_fn)(const char *skill_id, const char *input,
...
static agentos_mutex_t *executor_lock = NULL;
...
{
    if (!executor_lock) {
        executor_lock = agentos_mutex_create();
    }
}
...
static agentos_error_t http_get(const char *url, http_response_buffer_t *buf, long timeout_ms)
{
    CURL *curl = curl_easy_init();
    if (!curl)
        return AGENTOS_ENOMEM;

    buf->data = (char *)AGENTOS_MALLOC(4096);
    if (!buf->data) {
    ...
    if (res != CURLE_OK) {
        AGENTOS_LOG_WARN("HTTP GET failed: %s (url=%s)", curl_easy_strerror(res), url);
        return AGENTOS_EIO;
    }
    if (http_code != 200) {
        AGENTOS_LOG_WARN("HTTP GET returned %ld (url=%s)", http_code, url);
        return AGENTOS_EIO;
    }
...
static agentos_error_t http_post_json(const char *url, const char *json_body,
                                       ...
                                       long timeout_ms)
{
    ...
    if (res != CURLE_OK) {
        AGENTOS_LOG_WARN("HTTP POST failed: %s (url=%s)", curl_easy_strerror(res), url);
        return AGENTOS_EIO;
    }
    if (http_code != 200 && http_code != 201) {
        AGENTOS_LOG_WARN("HTTP POST returned %ld (url=%s)", http_code, url);
        return AGENTOS_EIO;
    }
...
static char *json_escape_string(const char *src, size_t len)
{
    ...
    char *escaped = (char *)AGENTOS_MALLOC(escaped_len);
    if (!escaped)
        return NULL;
    size_t pos = 0;
                                                ...
                                                size_t input_len, char **out_output)
{
    ...
    if (!encoded_query) {
        char *err = (char *)AGENTOS_MALLOC(256);
        if (!err)
            return AGENTOS_ENOMEM;
        snprintf(err, 256,
    ...
    agentos_error_t err = http_get(search_url, &response, 15000);
    ...
    while (cursor && *cursor && result_count < max_results && pos < result_max - 512) {
        ...
        if (text_len > 0 && text_len < 500) {
            ...
            char *text_copy = (char *)AGENTOS_MALLOC(text_len + 1);
                                               ...
                                               size_t input_len, char **out_output)
{
    ...
    if (fd < 0) {
        char *r = (char *)AGENTOS_MALLOC(256);
        if (!r)
            return AGENTOS_ENOMEM;
        snprintf(r, 256,
    ...
    char *output = (char *)AGENTOS_MALLOC(output_capacity);
    if (!output) {
        close(pipefd[0]);
        unlink(tmpfile);
        return AGENTOS_ENOMEM;
        ...
        if (retval > 0) {
            ...
            if (output_size + n + 1 > output_capacity) {
                ...
                while (new_cap < output_size + n + 1)
                    new_cap *= 2;
                char *new_output = (char *)AGENTOS_REALLOC(output, new_cap);
                if (!new_output)
                    break;
                output = new_output;
                                                    ...
                                                    size_t input_len, char **out_output)
{
    ...
    /* 转移所有权（cJSON 使用 malloc，我们使用 AGENTOS_MALLOC，需要复制） */
...
static agentos_error_t skill_execute_file_io(const char *skill_id, const char *input,
                                             size_t input_len, char **out_output)
{
#ifdef AGENTOS_HAS_CJSON
    ...
    /* 沙箱根目录限制 */
    const char *sandbox_root = getenv("AGENTOS_RUNTIME_DIR");
    if (!sandbox_root)
    ...
    if (strcmp(operation, "read") == 0) {
        ...
        if (!fp) {
            ...
            if (fsize > 0 && fsize < 1024 * 1024) { /* 限制 1MB */
                char *content = (char *)AGENTOS_MALLOC(fsize + 1);
                if (content) {
...
static agentos_error_t skill_execute_image_gen(const char *skill_id, const char *input,
                                               size_t input_len, char **out_output)
{
    (void)input_len;
    ...
    const char *endpoint = getenv("AGENTOS_IMAGE_GEN_ENDPOINT");
    const char *api_key = getenv("AGENTOS_IMAGE_GEN_API_KEY");
    ...
    if (!endpoint) {
        ...
        snprintf(result, 512,
                 ...
                 "\"message\":\"Set AGENTOS_IMAGE_GEN_ENDPOINT environment variable\"}",
                 skill_id);
        *out_output = result;
        return AGENTOS_SUCCESS;
    }
...
static agentos_error_t skill_execute_audio_process(const char *skill_id, const char *input,
                                                   size_t input_len, char **out_output)
{
    (void)input_len;

#ifdef AGENTRT_HAS_CURL
    const char *endpoint = getenv("AGENTOS_AUDIO_PROCESS_ENDPOINT");
    const char *api_key = getenv("AGENTOS_AUDIO_PROCESS_API_KEY");
    ...
    if (!endpoint) {
        ...
        snprintf(result, 512,
                 ...
                 "\"error\":\"endpoint_not_configured\","
                 "\"message\":\"Set AGENTOS_AUDIO_PROCESS_ENDPOINT environment variable\"}",
                 skill_id);
        *out_output = result;
        return AGENTOS_SUCCESS;
    }
...
static agentos_error_t skill_execute_custom(const char *skill_id, const char *skill_type,
                                            ...
                                            char **out_output)
{
    ...
    agentos_mutex_lock(executor_lock);
    ...
    agentos_mutex_unlock(executor_lock);
    ...
    snprintf(result, result_max,
             ...
             "\"message\":\"Register an executor via agentos_sys_skill_register_executor()\","
             "\"input_length\":%zu}",
...
void agentos_sys_skill_cleanup(void)
{
    ...
    if (executor_lock) {
        ...
        while (ex) {
            ...
            AGENTOS_FREE(ex->type_name);
            AGENTOS_FREE(ex);
            ex = next;
        }
        executor_list = NULL;
        agentos_mutex_unlock(executor_lock);
        agentos_mutex_free(executor_lock);
        executor_lock = NULL;
    }
}
...
agentos_error_t agentos_sys_skill_register_executor(const char *type_name,
                                                     skill_executor_fn executor)
{
    ...
    while (e) {
        if (strcmp(e->type_name, type_name) == 0) {
            ...
            e->executor = executor;
            agentos_mutex_unlock(executor_lock);
            AGENTOS_LOG_INFO("Skill executor replaced: %s", type_name);
            return AGENTOS_SUCCESS;
        }
        e = e->next;
    }
    ...
    skill_executor_entry_t *entry =
        (skill_executor_entry_t *)AGENTOS_CALLOC(1, sizeof(skill_executor_entry_t));
    if (!entry) {
        agentos_mutex_unlock(executor_lock);
        return AGENTOS_ENOMEM;
    }

    entry->type_name = AGENTOS_STRDUP(type_name);
    if (!entry->type_name) {
        AGENTOS_FREE(entry);
        agentos_mutex_unlock(executor_lock);
        return AGENTOS_ENOMEM;
    ...
    agentos_mutex_unlock(executor_lock);
    AGENTOS_LOG_INFO("Skill executor registered: %s", type_name);
    return AGENTOS_SUCCESS;
}
