...
static void ensure_agent_lock(void)
{
    agentos_mutex_t *current =
        (agentos_mutex_t *)atomic_load_ptr((_Atomic void **)&agent_lock, memory_order_acquire);
    if (!current) {
...
static agentos_error_t agent_unit_execute(agentos_execution_unit_t *unit, const void *input,
                                          void **out_output)
{
    ...
    uint64_t end_ns = agentos_time_monotonic_ns();
    AGENTOS_LOG_INFO("Agent fallback execution: role=%s, elapsed=%lu ms", role,
                     (unsigned long)((end_ns - start_ns) / 1000000));
 ...
 * 6. 返回结果
 */
agentos_error_t agentos_sys_agent_invoke(const char *agent_id, const char *input, size_t input_len,
                                         char **out_output)
{
    ...
    *out_output = (char *)output;
    AGENTOS_LOG_DEBUG("Agent invoked: %s, output_length=%zu", agent_id,
