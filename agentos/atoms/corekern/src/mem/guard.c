...
static guard_block_t *create_guarded_block(size_t size)
{
    ...
    AGENTOS_MEMSET(block->user_ptr, 0xCD, size);
    return block;
}
...
static void validate_guard(guard_block_t *block, const char *operation)
{
    ...
    if (!check_guard(block->front)) {
        ...
        AGENTOS_LOG_ERROR("GUARD VIOLATION [%s]: front guard corrupted (underflow) at %p",
...
#ifdef AGENTOS_ENABLE_MEMORY_DEBUG
        AGENTOS_LOG_ERROR("GUARD VIOLATION [%s]: back guard corrupted (overflow) at %p, size=%zu",
