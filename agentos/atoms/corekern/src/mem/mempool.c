...
#define MEMPOOL_DEFAULT_EMERGENCY_RESERVE \
    (AGENTOS_MEMPOOL_IPC_EMERGENCY_RESERVE_MB * 1024ULL * 1024ULL)
...
struct agentos_mempool {
    char                  name[AGENTOS_MEMPOOL_NAME_MAX]; /**< 名称 */
                               ...
                               uint32_t block_count)
{
    ...
    cls->block_memory = (uint8_t *)AGENTOS_CALLOC(1, alloc_size);
    if (!cls->block_memory) {
        return -1;
    }
...
agentos_mempool_t *agentos_mempool_create(const agentos_mempool_config_t *config)
{
    agentos_mempool_t *mp = (agentos_mempool_t *)AGENTOS_CALLOC(1, sizeof(*mp));
    if (!mp) {
        AGENTOS_LOG_ERROR("Mempool: P3.16 CREATE-FAIL — OOM allocating struct");
        return NULL;
    }

    /* 名称 */
    if (config && config->name && config->name[0]) {
        AGENTOS_STRNCPY_TERM(mp->name, config->name, sizeof(mp->name));
    } else {
    ...
    mp->max_obj_size = (config && config->max_obj_size > 0)
                       ? config->max_obj_size
                       : AGENTOS_MEMPOOL_DEFAULT_MAX_OBJ_SIZE;

    mp->enable_stats = config ? config->enable_stats : true;

    AGENTOS_LOG_INFO("Mempool[%s]: P3.16 CREATE emergency_reserve=%zuMB "
                     "max_obj_size=%zuKB",
                     mp->name,
    ...
    for (int i = 0; i < MEMPOOL_SIZE_CLASSES; i++) {
        ...
        AGENTOS_LOG_DEBUG("Mempool[%s]: P3.16 CLASS-INIT class[%d] "
        ...
        if (mempool_class_init(&mp->classes[i], class_sizes[i], block_count) != 0) {
            AGENTOS_LOG_ERROR("Mempool[%s]: P3.16 CREATE-FAIL — "
    ...
    AGENTOS_LOG_INFO("Mempool[%s]: P3.16 CREATE — emergency pool initialized "
    ...
    if (pthread_mutex_init(&mp->lock, NULL) != 0) {
        AGENTOS_LOG_ERROR("Mempool[%s]: P3.16 CREATE-FAIL — mutex_init failed",
                          mp->name);
        for (int i = 0; i < MEMPOOL_SIZE_CLASSES; i++) {
...
void agentos_mempool_destroy(agentos_mempool_t *mp)
{
    if (!mp) {
        AGENTOS_LOG_WARN("Mempool: P3.16 DESTROY — NULL ignored");
        return;
    }
    if (mp->destroyed) {
        AGENTOS_LOG_WARN("Mempool[%s]: P3.16 DESTROY — already destroyed", mp->name);
        return;
    }

    agentos_mempool_stats_t stats;
    agentos_mempool_get_stats(mp, &stats);

    AGENTOS_LOG_INFO("Mempool[%s]: P3.16 DESTROY normal=%zu/%zu emergency=%zu/%zu "
                     "oom=%llu rescue=%llu",
                     mp->name,
    ...
    if (stats.normal_allocated > 0 || stats.emergency_allocated > 0) {
        AGENTOS_LOG_WARN("Mempool[%s]: P3.16 DESTROY — %zu normal + %zu emergency "
...
void *agentos_mempool_alloc(agentos_mempool_t *mp, size_t size)
{
    if (!mp) {
        AGENTOS_LOG_ERROR("Mempool: P3.16 ALLOC-FAIL — NULL mempool");
        return NULL;
    }
    if (!mp->initialized || mp->destroyed) {
        AGENTOS_LOG_ERROR("Mempool[%s]: P3.16 ALLOC-FAIL — not initialized or destroyed",
                          mp->name);
        return NULL;
    }
    if (size == 0) {
        AGENTOS_LOG_WARN("Mempool[%s]: P3.16 ALLOC — zero size requested", mp->name);
        return NULL;
    }
    ...
    /* 正常模式：优先从普通池分配 */
    if (!is_emergency) {
        void *ptr = agentos_mem_alloc(size);
        if (ptr) {
            ...
            AGENTOS_LOG_DEBUG("Mempool[%s]: P3.16 ALLOC-NORMAL ptr=%p size=%zu "
        ...
        AGENTOS_LOG_WARN("Mempool[%s]: P3.16 ALLOC-FALLBACK — normal pool exhausted, "
    ...
    if (class_idx < 0) {
        ...
        AGENTOS_LOG_ERROR("Mempool[%s]: P3.16 ALLOC-FAIL — size=%zu exceeds "
    ...
    if (!cls->freelist || cls->free_blocks == 0) {
        ...
        AGENTOS_LOG_ERROR("Mempool[%s]: P3.16 ALLOC-FAIL — emergency class[%d] "
    ...
    AGENTOS_LOG_INFO("Mempool[%s]: P3.16 ALLOC-EMERGENCY ptr=%p class[%d] "
...
void agentos_mempool_free(agentos_mempool_t *mp, void *ptr)
{
    if (!mp) {
        AGENTOS_LOG_ERROR("Mempool: P3.16 FREE-FAIL — NULL mempool");
        return;
    }
    if (!ptr) {
        AGENTOS_LOG_WARN("Mempool[%s]: P3.16 FREE — NULL ptr ignored", mp->name);
        return;
    }
    ...
    if (block->magic == MEMPOOL_MAGIC && block->pool == 1) {
        ...
        if (class_idx < 0) {
            AGENTOS_LOG_ERROR("Mempool[%s]: P3.16 FREE-FAIL — invalid block size %zu",
                              mp->name, block_size);
            return;
        }
        ...
        AGENTOS_LOG_DEBUG("Mempool[%s]: P3.16 FREE-EMERGENCY ptr=%p class[%d] "
        ...
        AGENTOS_LOG_DEBUG("Mempool[%s]: P3.16 FREE-NORMAL ptr=%p", mp->name, ptr);
    }
}

void agentos_mempool_set_emergency(agentos_mempool_t *mp, bool enable)
{
    if (!mp) {
        AGENTOS_LOG_WARN("Mempool: P3.16 EMERGENCY-MODE — NULL mempool ignored");
        return;
    }

    bool current = atomic_load(&mp->emergency_mode);
    if (current == enable) {
        AGENTOS_LOG_DEBUG("Mempool[%s]: P3.16 EMERGENCY-MODE — already %s",
    ...
    AGENTOS_LOG_INFO("Mempool[%s]: P3.16 EMERGENCY-MODE %s %s",
                     mp->name,
                     enable ? "ENTERED" : "EXITED",
                     enable ? "(OOM — normal pool disabled)" : "(normal pool restored)");
}

void agentos_mempool_get_stats(agentos_mempool_t *mp,
                                agentos_mempool_stats_t *out_stats)
{
    if (!out_stats) {
        AGENTOS_LOG_WARN("Mempool: P3.16 STATS — NULL out_stats ignored");
        return;
    }
    ...
    AGENTOS_LOG_DEBUG("Mempool[%s]: P3.16 STATS normal=%zu/%zu emergency=%zu/%zu "
...
void agentos_mempool_reset_stats(agentos_mempool_t *mp)
{
    if (!mp) return;

    AGENTOS_LOG_INFO("Mempool[%s]: P3.16 STATS-RESET", mp->name);
...
size_t agentos_mempool_emergency_available(agentos_mempool_t *mp)
{
    if (!mp) return 0;
...
const char *agentos_mempool_name(agentos_mempool_t *mp)
{
