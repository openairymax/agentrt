...
struct agentos_tcache {
    ...
    agentos_tcache_stats_t stats;      /**< 统计信息 */
};

/* ==================== 公共 API ==================== */

agentos_tcache_t *agentos_tcache_create(
    ...
    tcache_global_free_fn global_free)
{
    ...
    agentos_tcache_t *tc = (agentos_tcache_t *)calloc(1, sizeof(agentos_tcache_t));
    if (!tc)
        return NULL;
...
void agentos_tcache_destroy(agentos_tcache_t *tcache)
{
    if (!tcache)
        return;
...
void *agentos_tcache_alloc(agentos_tcache_t *tcache)
{
    if (!tcache)
        return NULL;
...
void agentos_tcache_free(agentos_tcache_t *tcache, void *ptr)
{
    if (!tcache || !ptr)
        return;
...
void agentos_tcache_get_stats(agentos_tcache_t *tcache, agentos_tcache_stats_t *out_stats)
{
    if (!tcache || !out_stats)
        return;
...
void agentos_tcache_reset_stats(agentos_tcache_t *tcache)
{
    if (!tcache)
        return;
