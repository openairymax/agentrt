 ...
 *
 * **与 L1 内核 IPC 的关系**：
 * - L1（`ipc.h` 的 `agentos_kernel_ipc_message_t`，40 字节）：
 *   微内核内部高性能通信，零依赖，5 字段
 ...
 *   `agentos_kernel_ipc_message_t.data` 传入；接收方按 magic 字段
 *   判别协议版本
 *
 * **设计原则**：
...
#ifndef AGENTOS_ARE_IPC_H
#define AGENTOS_ARE_IPC_H
 ...
 * - _abi_pad 与 reserved 必须为 0，校验失败返回 AGENTOS_ERR_PROTOCOL
 */
#pragma pack(push, 1)
 ...
 * @reentrant 是
 */
AGENTOS_API agentos_error_t are_ipc_header_init(are_ipc_message_header_t *header,
 ...
 * @reentrant 是
 */
AGENTOS_API uint32_t are_ipc_checksum_compute(const are_ipc_message_header_t *header,
                                              const void *payload,
                                              size_t payload_len);
 ...
 *
 * 校验顺序：
 * 1. magic == ARE_IPC_MAGIC，否则返回 AGENTOS_ERR_PROTOCOL
 * 2. version == ARE_IPC_VERSION，否则返回 AGENTOS_ERR_PROTOCOL
 * 3. reserved == 0，否则返回 AGENTOS_ERR_PROTOCOL
 * 4. _abi_pad == 0，否则返回 AGENTOS_ERR_PROTOCOL
 * 5. payload_len ≤ ARE_IPC_MAX_PAYLOAD，否则返回 AGENTOS_ERR_INVALID_PARAM
 * 6. 调用方传入的 payload_len 必须与 header.payload_len 一致
 * 7. checksum 与重算结果一致，否则返回 AGENTOS_ERR_CHECKSUM
 *
 * @param header [in] 待校验的消息头
 * @param payload [in] payload 数据（可为 NULL 若 payload_len=0）
 * @param payload_len [in] payload 字节数
 * @return AGENTOS_SUCCESS 校验通过；AGENTOS_ERR_PROTOCOL magic/version/reserved/_abi_pad 错误；
 *         AGENTOS_ERR_INVALID_PARAM payload_len 超限或不一致；AGENTOS_ERR_CHECKSUM 校验和不匹配
 *
 * @threadsafe 是（纯函数）
 * @reentrant 是
 */
AGENTOS_API agentos_error_t are_ipc_header_validate(const are_ipc_message_header_t *header,
                                                    const void *payload,
                                                    size_t payload_len);
 ...
 * @return AGENTOS_SUCCESS 成功；AGENTOS_EINVAL 参数无效；AGENTOS_ENOMEM 内存不足
 *
 * @ownership out_msg 由调用者负责，通过 are_ipc_message_destroy() 释放
 * @threadsafe 是
 */
AGENTOS_API agentos_error_t are_ipc_message_create(are_msg_type_t msg_type,
                                                   are_proto_t protocol,
                                                   uint64_t msg_id,
                                                   uint64_t trace_id,
                                                   const char *source,
 ...
 * @threadsafe 是
 */
AGENTOS_API void are_ipc_message_destroy(are_ipc_message_t *msg);
 ...
 */
AGENTOS_API const char *are_ipc_msg_type_str(are_msg_type_t msg_type);
 ...
 * @return 字符串字面量（无需释放）；未知类型返回 "UNKNOWN"
 */
AGENTOS_API const char *are_ipc_proto_str(are_proto_t protocol);

#ifdef __cplusplus
}
#endif

#endif /* AGENTOS_ARE_IPC_H */
