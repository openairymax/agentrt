...
#ifndef AGENTOS_CORELOOPTHREE_PROMPT_LOADER_H
#define AGENTOS_CORELOOPTHREE_PROMPT_LOADER_H
...
/** 最大变量数量 */
#define AGENTOS_PROMPT_MAX_VARIABLES      64
/** 变量名最大长度 */
#define AGENTOS_PROMPT_VAR_NAME_MAX       64
/** 变量值最大长度 */
#define AGENTOS_PROMPT_VAR_VALUE_MAX      4096
/** 模板缓存最大条目数 */
#define AGENTOS_PROMPT_CACHE_MAX_ENTRIES  128
/** 模板名称最大长度 */
#define AGENTOS_PROMPT_NAME_MAX           128
/** 模板路径最大长度 */
#define AGENTOS_PROMPT_PATH_MAX           512
/** 默认模板目录 */
#define AGENTOS_PROMPT_DEFAULT_TEMPLATE_DIR "ecosystem/prompts/templates/"
 ...
 */
typedef struct {
    char name[AGENTOS_PROMPT_VAR_NAME_MAX];   /**< 变量名 */
    char value[AGENTOS_PROMPT_VAR_VALUE_MAX];  /**< 变量值 */
} agentos_prompt_variable_t;

/**
 * @brief Prompt 模板（从 YAML 加载）
 */
typedef struct {
    char name[AGENTOS_PROMPT_NAME_MAX];       /**< 模板名称 */
    ...
    /* 内部标记 */
    char source_path[AGENTOS_PROMPT_PATH_MAX]; /**< 模板文件路径 */
    bool is_loaded;                            /**< 是否已加载 */
} agentos_prompt_template_t;
...
} agentos_prompt_rendered_t;

/**
 * @brief Prompt 加载器配置
 */
typedef struct {
    char template_dir[AGENTOS_PROMPT_PATH_MAX]; /**< 模板目录 */
 ...
 * @param out_template 输出模板（需调用者通过 agentos_prompt_template_free 释放）
 * @return 0 成功，非0失败
 *
 ...
 * @param out_rendered 输出渲染结果（需调用者通过 agentos_prompt_rendered_free 释放）
 * @return 0 成功，非0失败
 *
...
#endif /* AGENTOS_CORELOOPTHREE_PROMPT_LOADER_H */