 ...
 * agentos_memory_provider_t 可拔插架构。
 *
 * 三种构建模式：
 *   AGENTOS_MEMORY_BUILTIN     → 内置免费提供商
 *   AGENTOS_MEMORY_MEMORYROVOL → MemoryRovol 商业提供商
 *   AGENTOS_MEMORY_HYBRID      → 混合模式（内置 + MemoryRovol 同步）
 *
 * 使用方式：
 * @code
 *   // 初始化 MemoryRovol 提供商
 ...
 *
 *   // 获取 agentos_memory_provider_t 接口
 *   agentos_memory_provider_t *provider = memoryrovol_bridge_get_provider(bridge);
 *
 *   // 注册为活跃提供商
 *   agentos_memory_provider_set_active(provider);
 *
 *   // 在 CoreLoopThree 中使用
...
#ifndef AGENTOS_CORELOOPTHREE_MEMORYROVOL_BRIDGE_H
#define AGENTOS_CORELOOPTHREE_MEMORYROVOL_BRIDGE_H
 ...
 *
 * 初始化 MemoryRovol 引擎并填充 agentos_memory_provider_t 函数指针表。
...
/**
 * @brief C-L12: 获取 MemoryRovol 的 agentos_memory_provider_t 接口
 *
 ...
 * @brief 切换提供商模式
 *
 * 根据 agentos.yaml 的 memory.provider 配置选择：
 *   - "builtin"   → 内置提供商
...
#endif /* AGENTOS_CORELOOPTHREE_MEMORYROVOL_BRIDGE_H */