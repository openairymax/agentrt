 ...
 * 配置管理接口。Manager 通过此适配器加载/重载 agentos.yaml 配置，
...
#ifndef AGENTOS_CORELOOPTHREE_MANAGER_ADAPTER_H
#define AGENTOS_CORELOOPTHREE_MANAGER_ADAPTER_H
 ...
 * @brief 触发配置重载
 *
 * 重新加载 agentos.yaml 并通知所有注册的回调。
 * 通常由 Manager 在修改 agentos.yaml 后调用。
 *
 * @param adapter 适配器句柄
 * @return 0 成功，非0 失败
 */
 ...
 *
 * 将 agentos_yaml_config_t 中的内核参数映射到 agentos_loop_config_t。
 *
 ...
 *
 * 当 agentos.yaml 热重载时调用，通知上层配置已变更。
 *
...
typedef void (*manager_adapter_config_change_cb_t)(
    const agentos_yaml_config_t *old_config,
...
#endif /* AGENTOS_CORELOOPTHREE_MANAGER_ADAPTER_H */