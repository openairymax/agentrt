 ...
 * @return 0 成功，非0失败
 *
 * @ownership config: OWNER（调用者负责生命周期，通过 agentos_yaml_config_free 释放）
 */
 ...
 */
typedef void (*agentos_config_reload_cb_t)(
 ...
 * @brief 启动配置文件监听（热重载）
 *
 * 后台线程监听 agentos.yaml 文件变更，
 * 变更时自动重新加载并调用所有注册的回调。
 *
 * @param yaml_path agentos.yaml 文件路径（NULL 使用默认路径）
...
/**
 * @brief 手动触发配置重载
 * @param yaml_path agentos.yaml 文件路径（NULL 使用上次路径）
 * @return 0 成功，非0失败
 */
