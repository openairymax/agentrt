...
static agentos_error_t ml_dispatch(const agentos_task_node_t *task, const void **candidates,
...
agentos_error_t agentos_dispatching_ml_report_outcome(agentos_dispatching_strategy_t *strategy,
                                                      float reward)
{
...
float agentos_dispatching_ml_get_avg_reward(const agentos_dispatching_strategy_t *strategy)
{
    if (!strategy || !strategy->data)
        return 0.0f;
    ml_dispatch_data_t *data = (ml_dispatch_data_t *)strategy->data;
