...
static void priority_destroy(agentos_dispatching_strategy_t *strategy)
{
    if (!strategy)
        return;
    struct agentos_priority_dispatch *priority = (struct agentos_priority_dispatch *)strategy->data;
    if (priority)
 ...
 */
static agentos_error_t priority_select(const agentos_task_node_t __attribute__((unused)) * task,
                                       const void **candidates, size_t count, void *context,
                                       char **out_agent_id)
{

    struct agentos_priority_dispatch *priority = (struct agentos_priority_dispatch *)context;
