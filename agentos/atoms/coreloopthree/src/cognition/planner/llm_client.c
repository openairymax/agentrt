 ...
 *
 * 提供 agentos_llm_service 的内置实现。当 MemoryRovol 商业 LLM 仓库不可用时，
 * 作为 CoreLoopThree 的本地 LLM 客户端适配层。
 *
 * 设计：
 * - agentos_llm_service_t 是一个轻量句柄，持有一个可选的 IPC adapter 指针
...
#include "logger.h"
#include "memory_compat.h" /* P2.7: AGENTOS_MALLOC/CALLOC/FREE 宏定义 */
...
agentos_error_t agentos_llm_service_create(const agentos_llm_config_t *config,
                                           agentos_llm_service_t **out_service)
{
    ...
    *out_service = NULL;
    ...
    agentos_llm_service_t *svc =
        (agentos_llm_service_t *)AGENTOS_CALLOC(1, sizeof(agentos_llm_service_t));
    if (!svc) {
        AGENTOS_LOG_ERROR("LLM service alloc failed (errno=ENOMEM)");
    ...
    if (config && config->model_name) {
        AGENTOS_LOG_INFO("LLM service created (model=%s, builtin adapter not attached)",
                         config->model_name);
    } else {
        AGENTOS_LOG_INFO("LLM service created (no config, adapter not attached)");
    }
...
{
    return agentos_llm_service_create(NULL, out_service);
}
...
agentos_error_t agentos_llm_service_set_adapter(agentos_llm_service_t *service, void *adapter)
{
    ...
    if (adapter) {
        ...
        AGENTOS_LOG_INFO("LLM service: adapter injected (connected=%s, available=%d)",
                         connected ? "yes" : "no", service->available);
    } else {
        service->available = 0;
        AGENTOS_LOG_INFO("LLM service: adapter detached");
    }
                                         ...
                                         char **out_response)
{
    ...
    *out_response = NULL;
    ...
    if (ret != 0 || !resp || !resp->choices || resp->choice_count == 0) {
        AGENTOS_LOG_WARN("LLM service_call: IPC complete failed (ret=%d resp=%p)",
                         ret, (void *)resp);
        if (resp)
    ...
    char *text = (char *)AGENTOS_MALLOC(len + 1);
    if (!text) {
        llm_response_free(resp);
        return AGENTOS_ENOMEM;
    }
                                     ...
                                     agentos_llm_response_t **out_response)
{
    ...
    *out_response = NULL;
    ...
    int ret = llm_svc_adapter_complete(adapter, &cfg, &resp);
    if (ret != 0 || !resp || !resp->choices || resp->choice_count == 0) {
        AGENTOS_LOG_WARN("LLM complete: IPC complete failed (ret=%d resp=%p)",
                         ret, (void *)resp);
        if (resp)
            llm_response_free(resp);
        return AGENTOS_ESERVICE;
    ...
    agentos_llm_response_t *result =
        (agentos_llm_response_t *)AGENTOS_CALLOC(1, sizeof(agentos_llm_response_t));
    if (!result) {
        AGENTOS_FREE(text);
        llm_response_free(resp);
        return AGENTOS_ENOMEM;
 ...
 */
struct agentos_dual_think_config {
 ...
 * @brief 双思考结果的内部定义
 */
struct agentos_dual_think_result {
...
static const agentos_dual_think_config_t g_dual_think_default_config = {
    .s1_verify_enabled = 1,
 ...
 * 返回 AGENTOS_MALLOC 分配的文本，调用方负责释放。
 */
static agentos_error_t llm_call_adapter(llm_svc_adapter_t *adapter,
                                       ...
                                       agentos_dual_think_result_t **out_result)
{
    ...
    *out_result = NULL;
    ...
    const agentos_dual_think_config_t *cfg = config ? config : &g_dual_think_default_config;
    llm_svc_adapter_t *adapter = (llm_svc_adapter_t *)service->adapter;
    ...
    if (cfg->s2_generate_enabled) {
        ...
        if (err != AGENTOS_SUCCESS) {
            AGENTOS_LOG_WARN("dual_think: S2 generate failed (err=%d)", (int)err);
        ...
        result->s2_generate_output = (char *)AGENTOS_MALLOC(pl + 1);
    ...
    if (cfg->s1_verify_enabled && result->s2_generate_output) {
        ...
        char *verify_prompt = (char *)AGENTOS_MALLOC(prompt_len);
        if (!verify_prompt) {
            agentos_llm_dual_result_free(result);
            return AGENTOS_ENOMEM;
        }
        ...
        agentos_error_t err = llm_call_adapter(adapter, DT_S1_VERIFY_SYSTEM_PROMPT,
                                               verify_prompt, &result->s1_verify_output);
        AGENTOS_FREE(verify_prompt);
        if (err != AGENTOS_SUCCESS) {
            AGENTOS_LOG_WARN("dual_think: S1 verify failed (err=%d)", (int)err);
...
void agentos_llm_dual_result_free(agentos_dual_think_result_t *result)
{
    ...
    if (result->s1_verify_output)
        AGENTOS_FREE(result->s1_verify_output);
    if (result->s2_generate_output)
        AGENTOS_FREE(result->s2_generate_output);
    AGENTOS_FREE(result);
}
