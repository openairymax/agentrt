...
agentos_error_t agentos_tc_context_window_create(size_t max_tokens,
...
void agentos_tc_context_window_destroy(agentos_context_window_t *window)
{
    if (!window)
        return;
    if (window->buffer)
        AGENTOS_FREE(window->buffer);
    AGENTOS_FREE(window);
}

ssize_t agentos_tc_context_window_append(agentos_context_window_t *window, const char *data,
                                         size_t len)
{
...
agentos_error_t agentos_tc_context_window_get_recent(agentos_context_window_t *window,
...
int agentos_tc_context_window_has_space(agentos_context_window_t *window, size_t needed_tokens)
{
...
agentos_error_t agentos_tc_working_memory_create(size_t capacity,
...
void agentos_tc_working_memory_destroy(agentos_working_memory_t *mem)
{
    ...
    for (size_t i = 0; i < mem->count; i++) {
        ...
        if (mem->entries[i].type)
            AGENTOS_FREE(mem->entries[i].type);
    }
    AGENTOS_FREE(mem->entries);
...
static void wm_evict_one(agentos_working_memory_t *mem)
{
    ...
    if (victim != (size_t)-1) {
        ...
        if (mem->entries[victim].type)
            AGENTOS_FREE(mem->entries[victim].type);
...
agentos_error_t agentos_tc_working_memory_store(agentos_working_memory_t *mem, const char *key,
                                                ...
                                                const char *type, int pin)
{
    ...
    if (existing != (size_t)-1) {
        ...
        if (type) {
            ...
            if (t) {
                AGENTOS_FREE(mem->entries[existing].type);
...
agentos_error_t agentos_tc_working_memory_retrieve(agentos_working_memory_t *mem, const char *key,
...
agentos_error_t agentos_tc_working_memory_remove(agentos_working_memory_t *mem, const char *key)
{
    ...
    if (mem->entries[idx].type)
        AGENTOS_FREE(mem->entries[idx].type);
...
agentos_error_t agentos_tc_step_create(agentos_thinking_chain_t *chain, tc_step_type_t type,
...
agentos_error_t agentos_tc_step_complete(agentos_thinking_step_t *step, const char *content,
                                         size_t content_len, float confidence, const char *role)
{
    ...
    if (role)
        step->role = AGENTOS_STRDUP(role);

    return AGENTOS_SUCCESS;
}

agentos_error_t agentos_tc_step_verify(agentos_thinking_step_t *step, int *is_valid,
...
agentos_error_t agentos_tc_step_correct(agentos_thinking_step_t *step,
...
agentos_error_t agentos_tc_chain_create(const char *goal, size_t max_tokens, size_t wm_capacity,
...
void agentos_tc_chain_destroy(agentos_thinking_chain_t *chain)
{
    ...
    for (size_t i = 0; i < chain->step_count; i++) {
        ...
        if (s->raw_input)
            AGENTOS_FREE(s->raw_input);
        if (s->content)
            AGENTOS_FREE(s->content);
        if (s->critique)
            AGENTOS_FREE(s->critique);
        if (s->role)
            AGENTOS_FREE(s->role);
        if (s->depends_on)
            AGENTOS_FREE(s->depends_on);
        if (s->dependents)
            AGENTOS_FREE(s->dependents);
...
agentos_error_t agentos_tc_chain_next_ready_step(agentos_thinking_chain_t *chain,
                                                 agentos_thinking_step_t **out_step)
{
            ...
            if (ready > 0) {
                ...
                *out_step = s;
                ...
                if (chain->ctx_window && s->raw_input) {
                    agentos_tc_context_window_append(chain->ctx_window, s->raw_input,
                                                     s->raw_input_len);
                }
...
agentos_error_t agentos_tc_chain_stats(agentos_thinking_chain_t *chain, char **out_json,
                                       size_t *out_len)
{
...
void agentos_tc_chain_set_memory(agentos_thinking_chain_t *chain, agentos_memory_engine_t *memory)
{
    if (!chain)
        return;
    chain->memory = memory;
...
agentos_error_t agentos_tc_context_window_prepopulate(agentos_thinking_chain_t *chain,
                                                      ...
                                                      uint32_t limit)
{
    ...
    for (size_t i = 0; i < result->memory_result_count; i++) {
        agentos_memory_record_t *rec = result->memory_result_items[i]->memory_result_item_record;
...
agentos_error_t agentos_tc_working_memory_sync_to_persistent(agentos_thinking_chain_t *chain,
...
agentos_error_t agentos_tc_step_write_to_memory(agentos_thinking_chain_t *chain,
...
agentos_error_t agentos_tc_metacognition_inform_memory(agentos_thinking_chain_t *chain,
                                                       const void *eval,
                                                       agentos_thinking_step_t *step)
{
    ...
    if (chain->working_mem && step->confidence >= 0.7f) {
        ...
        agentos_tc_working_memory_store(chain->working_mem, key, val_buf, (size_t)vlen + 1,
                                        "evaluation_score", 1);
    }
...
agentos_error_t agentos_tc_step_monitor(const agentos_thinking_step_t *step,
...
agentos_error_t agentos_tc_step_recover(agentos_thinking_chain_t *chain,
                                        agentos_thinking_step_t *failed_step,
                                        const tc_monitor_result_t *monitor_result,
                                        agentos_error_t (*corrector_fn)(const char *, size_t,
                                                                        char **, size_t *, void *),
                                        void *user_data, tc_recovery_result_t *out_result)
{
    ...
    if (corrector_fn && failed_step->raw_input && failed_step->raw_input_len > 0) {
        for (uint32_t attempt = 0; attempt < TC_MAX_RECOVERY_ATTEMPTS; attempt++) {
            ...
            size_t new_len = 0;
            agentos_error_t err = corrector_fn(failed_step->raw_input, failed_step->raw_input_len,
            ...
            if (err == AGENTOS_SUCCESS && new_content && new_len > 0) {
                ...
                if (post_mon.anomaly == TC_ANOMALY_NONE || post_mon.severity_score < 0.4f) {
                    ...
                    if (post_mon.description)
                        AGENTOS_FREE(post_mon.description);
                    goto recovery_done;
                }
                if (post_mon.description)
                    AGENTOS_FREE(post_mon.description);
...
uint32_t agentos_tc_chain_checkpoint(agentos_thinking_chain_t *chain)
{
    ...
    AGENTOS_LOG_INFO("TC Checkpoint#%u created at step_count=%zu", next_id, chain->step_count);
    return next_id;
}

size_t agentos_tc_chain_rollback(agentos_thinking_chain_t *chain, uint32_t checkpoint_id)
{
    ...
    for (size_t i = target_steps; i < chain->step_count; i++) {
        ...
        if (step->content)
            AGENTOS_FREE(step->content);
        if (step->raw_input)
            AGENTOS_FREE(step->raw_input);
        if (step->critique)
            AGENTOS_FREE(step->critique);
        if (step->role)
            AGENTOS_FREE(step->role);
        if (step->depends_on) {
            AGENTOS_FREE(step->depends_on);
        }
        if (step->dependents)
            AGENTOS_FREE(step->dependents);
    ...
    AGENTOS_LOG_INFO("TC Rollback to cp#%u: removed %zu steps (now %zu)", checkpoint_id, removed,
                     chain->step_count);
    return removed;
}
...
agentos_error_t agentos_tc_allocate_attention(agentos_thinking_chain_t *chain,
                                              agentos_thinking_step_t *step,
...
float agentos_tc_compute_priority(const agentos_thinking_step_t *step,
                                  const agentos_thinking_chain_t *chain)
{
    if (!step)
        return 0.0f;
