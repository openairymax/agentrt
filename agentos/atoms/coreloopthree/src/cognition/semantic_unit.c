...
static agentos_error_t emit_unit(su_stream_detector_t *d, const char *text, size_t len,
...
agentos_error_t su_stream_detector_create(const su_config_t *config,
                                          su_stream_detector_t **out_detector)
{
    ...
    su_stream_detector_t *d =
        (su_stream_detector_t *)AGENTOS_CALLOC(1, sizeof(su_stream_detector_t));
    if (!d)
        return AGENTOS_ENOMEM;
...
void su_stream_detector_destroy(su_stream_detector_t *detector)
{
    ...
    if (detector->buffer)
        AGENTOS_FREE(detector->buffer);
    AGENTOS_FREE(detector);
}

agentos_error_t su_stream_detector_feed(su_stream_detector_t *detector, const char *tokens,
...
agentos_error_t su_stream_detector_flush(su_stream_detector_t *detector)
{
    ...
    agentos_error_t err = emit_unit(detector, detector->buffer, detector->buffer_used, bt);
    if (err == AGENTOS_SUCCESS) {
        detector->total_tokens_processed += detector->current_token_estimate;
        detector->buffer_used = 0;
        detector->current_token_estimate = 0;
...
agentos_error_t su_stream_detector_pop_pending(su_stream_detector_t *detector,
...
agentos_error_t su_stream_detector_stats(const su_stream_detector_t *detector, char **out_json)
{
