...
TEST(int05_2_weight_calibration_baseline)
{
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        mc_evaluation_result_t result;
        agentos_error_t        e = agentos_mc_evaluate_step(mc, &step, NULL, 0, &result);
        assert(e == AGENTOS_SUCCESS);
...
TEST(int05_3_self_correction_mode_detection)
{
    ...
    should_correct = agentos_mc_should_self_correct(mc, TC_STEP_GENERATION);
    printf("    After mixed evaluations: should_self_correct=%d\n", should_correct);
    assert(should_correct == 1);

    /* 5. 测试不同步骤类型的自我纠错检测 */
    int sc_decomp      = agentos_mc_should_self_correct(mc, TC_STEP_DECOMPOSITION);
    int sc_planning    = agentos_mc_should_self_correct(mc, TC_STEP_PLANNING);
    int sc_verification = agentos_mc_should_self_correct(mc, TC_STEP_VERIFICATION);
...
TEST(int05_4_confidence_calibration_deviation)
{
    ...
    float raw_conf   = 0.9f;
    float calibrated = agentos_mc_calibrate_confidence(mc, raw_conf);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        err = agentos_mc_feedback(mc, 0.9f, was_correct);
        assert(err == AGENTOS_SUCCESS);
    }
    ...
    /* 5. 校准后的置信度应该降低 */
    calibrated = agentos_mc_calibrate_confidence(mc, 0.9f);
    ...
    for (int i = 0; i < 10; i++) {
        /* 预测低置信度但实际全对 → 自信不足 */
        err = agentos_mc_feedback(mc2, 0.3f, 1);
        assert(err == AGENTOS_SUCCESS);
    }

    float cal2 = agentos_mc_calibrate_confidence(mc2, 0.3f);
