...
{
    void **table = (void **)AGENTOS_CALLOC(size, sizeof(void *));
    return table;
}
...
static bool add_vertex_index(struct graph_engine_s *e, vertex_id_t vertex_id, size_t vertex_idx)
{
    ...
    vertex_index_entry_t *new_entry =
        (vertex_index_entry_t *)AGENTOS_CALLOC(1, sizeof(vertex_index_entry_t));
    if (!new_entry)
        return false;
...
static bool add_edge_index(struct graph_engine_s *e, edge_id_t edge_id, size_t edge_idx)
{
    ...
    edge_index_entry_t *new_entry = (edge_index_entry_t *)AGENTOS_CALLOC(1, sizeof(edge_index_entry_t));
    if (!new_entry)
        return false;
...
static bool add_out_edge_index(struct graph_engine_s *e, vertex_id_t source, size_t edge_idx)
{
    ...
    out_edge_index_entry_t *new_entry =
        (out_edge_index_entry_t *)AGENTOS_CALLOC(1, sizeof(out_edge_index_entry_t));
    if (!new_entry)
        return false;
...
static bool add_in_edge_index(struct graph_engine_s *e, vertex_id_t target, size_t edge_idx)
{
    ...
    in_edge_index_entry_t *new_entry =
        (in_edge_index_entry_t *)AGENTOS_CALLOC(1, sizeof(in_edge_index_entry_t));
    if (!new_entry)
        return false;
...
{
    vertex_queue_t *q = (vertex_queue_t *)AGENTOS_CALLOC(1, sizeof(vertex_queue_t));
    return q;
}
...
static void queue_enqueue(vertex_queue_t *q, vertex_id_t vertex_id)
{
    ...
    queue_node_t *node = (queue_node_t *)AGENTOS_CALLOC(1, sizeof(queue_node_t));
    if (!node)
        return;
...
{
    vertex_stack_t *s = (vertex_stack_t *)AGENTOS_CALLOC(1, sizeof(vertex_stack_t));
    return s;
}
...
static void stack_push(vertex_stack_t *s, vertex_id_t vertex_id)
{
    ...
    stack_node_t *node = (stack_node_t *)AGENTOS_CALLOC(1, sizeof(stack_node_t));
    if (!node)
        return;
...
static bool hashset_add(vertex_hashset_t *set, vertex_id_t vertex_id)
{
    ...
    // 添加新节点
    hashset_node_t *new_node = (hashset_node_t *)AGENTOS_CALLOC(1, sizeof(hashset_node_t));
    if (!new_node)
        return false;
...
graph_engine_handle_t graph_engine_create(const taskflow_config_t *config)
{
    ...
    if (!engine->vertex_index || !engine->edge_index || !engine->out_edge_index ||
        ...
        if (engine->vertex_index)
            AGENTOS_FREE(engine->vertex_index);
        if (engine->edge_index)
            AGENTOS_FREE(engine->edge_index);
        if (engine->out_edge_index)
            AGENTOS_FREE(engine->out_edge_index);
        if (engine->in_edge_index)
            AGENTOS_FREE(engine->in_edge_index);
        AGENTOS_FREE(engine->vertices);
...
void graph_engine_destroy(graph_engine_handle_t engine)
{
    ...
    // 释放顶点存储
    if (e->vertices) {
        AGENTOS_FREE(e->vertices);
    }

    // 释放边存储
    if (e->edges) {
        AGENTOS_FREE(e->edges);
    }
