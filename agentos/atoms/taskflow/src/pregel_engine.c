...
{
    message_queue_t *queue = (message_queue_t *)AGENTOS_CALLOC(1, sizeof(message_queue_t));
    return queue;
}
                                  ...
                                  size_t payload_size, superstep_t step)
{
    ...
    message_queue_entry_t *entry =
        (message_queue_entry_t *)AGENTOS_CALLOC(1, sizeof(message_queue_entry_t));
    if (!entry)
        return false;
    ...
    if (payload_size > 0 && payload) {
        entry->payload = AGENTOS_MALLOC(payload_size);
        if (!entry->payload) {
            AGENTOS_FREE(entry);
            return false;
        }
...
static bool init_vertex_states(struct pregel_engine_s *engine)
{
    ...
    engine->vertex_states =
        (pregel_vertex_state_t *)AGENTOS_CALLOC(vertex_count, sizeof(pregel_vertex_state_t));
    ...
    // 尝试从图引擎获取真实顶点ID映射
    {
        vertex_id_t *real_ids = (vertex_id_t *)AGENTOS_CALLOC(vertex_count, sizeof(vertex_id_t));
        if (real_ids) {
            size_t fetched = 0;
...
void pregel_engine_destroy(pregel_engine_handle_t engine)
{
    ...
    if (e->vertex_states) {
        for (size_t i = 0; i < e->vertex_state_count; i++) {
            if (e->vertex_states[i].value) {
                AGENTOS_FREE(e->vertex_states[i].value);
            }
    ...
    AGENTOS_FREE(e->message_queues);

    message_queue_destroy(e->next_step_queue);

    // 释放工作线程上下文
    if (e->workers) {
        AGENTOS_FREE(e->workers);
    }
...
taskflow_error_t pregel_engine_start(pregel_engine_handle_t engine, size_t max_supersteps)
{
    ...
    for (size_t i = 0; i < e->worker_count; i++) {
        ...
        if (agentos_thread_create(&e->workers[i].thread_handle, worker_thread_func,
                                  &e->workers[i]) != 0) {
            e->workers[i].running = false;
...
uint64_t pregel_engine_create_checkpoint(pregel_engine_handle_t engine)
{
    ...
    if (e->checkpoint_count < 16) {
        e->checkpoints = (checkpoint_t *)AGENTOS_REALLOC(e->checkpoints, (e->checkpoint_count + 1) *
                                                                             sizeof(checkpoint_t));
        if (e->checkpoints) {
        ...
        if (vs->incoming_messages) {
            for (size_t j = 0; j < vs->incoming_message_count; j++) {
                if (vs->incoming_messages[j].payload) {
                    AGENTOS_FREE(vs->incoming_messages[j].payload);
                }
            }
            AGENTOS_FREE(vs->incoming_messages);
        ...
        while (!message_queue_is_empty(e->next_step_queue)) {
            ...
            if (entry) {
                ...
                if (target_idx != SIZE_MAX) {
                    ...
                    graph_message_t *new_msgs = (graph_message_t *)AGENTOS_REALLOC(
