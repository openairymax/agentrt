...
// 执行单元执行方法（接口实现）
static agentos_error_t taskflow_unit_execute_impl(agentos_execution_unit_t *unit,
                                                  const void *input_data, void **out_output)
{
    ...
    taskflow_execution_context_t *context =
        (taskflow_execution_context_t *)AGENTOS_CALLOC(1, sizeof(taskflow_execution_context_t));
    if (!context) {
...
taskflow_task_output_t *taskflow_task_output_create(void)
{
    ...
    AGENTOS_MEMSET(&output->stats, 0, sizeof(output->stats));
...
void taskflow_task_output_destroy(taskflow_task_output_t *output)
{
    ...
    if (output->result_data)
        AGENTOS_FREE(output->result_data);
    AGENTOS_FREE(output);
}
                                           ...
                                           agentos_task_t *agentos_task)
{
    ...
    if (output->result_data && output->result_data_size > 0) {
        packed->result_data = AGENTOS_MALLOC(output->result_data_size);
