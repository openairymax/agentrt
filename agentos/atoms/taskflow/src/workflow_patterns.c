                                            ...
                                            taskflow_handle_t taskflow_engine)
{
    ...
    if (config->max_edges > 0) {
        context->edges = (workflow_edge_t *)AGENTOS_CALLOC(config->max_edges, sizeof(workflow_edge_t));
        if (!context->edges) {
            if (context->nodes)
...
void workflow_context_destroy(workflow_context_t *context)
{
    ...
    if (context->nodes) {
        for (size_t i = 0; i < context->node_count; i++) {
            if (context->nodes[i].node_name)
                AGENTOS_FREE(context->nodes[i].node_name);
            if (context->nodes[i].task_data)
                AGENTOS_FREE(context->nodes[i].task_data);
        }
        AGENTOS_FREE(context->nodes);
    }
    ...
    if (context->edges) {
        for (size_t i = 0; i < context->edge_count; i++) {
            if (context->edges[i].edge_label)
                AGENTOS_FREE(context->edges[i].edge_label);
        }
        AGENTOS_FREE(context->edges);
    }
    ...
    if (context->execution_state) {
        ...
        if (state->node_states)
            AGENTOS_FREE(state->node_states);
        AGENTOS_FREE(state);
    }
...
taskflow_error_t workflow_add_node(workflow_context_t *context, const workflow_node_t *node)
{
    ...
    // 复制节点名称
    if (node->node_name) {
        dest->node_name = AGENTOS_STRDUP(node->node_name);
        if (!dest->node_name)
    ...
    if (node->task_data && node->task_data_size > 0) {
        dest->task_data = AGENTOS_MALLOC(node->task_data_size);
        if (!dest->task_data) {
            if (dest->node_name)
...
taskflow_error_t workflow_add_edge(workflow_context_t *context, const workflow_edge_t *edge)
{
    ...
    // 复制边标签
    if (edge->edge_label) {
        dest->edge_label = AGENTOS_STRDUP(edge->edge_label);
        if (!dest->edge_label)
            return TASKFLOW_ERROR_MEMORY;
    }
...
taskflow_error_t workflow_execute_sync(workflow_context_t *context, size_t max_iterations)
{
    ...
    if (context->execution_state) {
        ...
        internal_state = (workflow_internal_state_t *)AGENTOS_CALLOC(1, sizeof(workflow_internal_state_t));
        if (!internal_state)
            return TASKFLOW_ERROR_MEMORY;
        ...
        if (context->node_count > 0) {
            internal_state->node_states = (node_execution_state_t *)AGENTOS_CALLOC(
                context->node_count, sizeof(node_execution_state_t));
            if (!internal_state->node_states) {
                AGENTOS_FREE(internal_state);
                return TASKFLOW_ERROR_MEMORY;
            }
