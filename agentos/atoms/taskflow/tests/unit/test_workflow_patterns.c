...
static workflow_node_t make_node(vertex_id_t id, const char *name, workflow_node_type_t type)
{
     ...
     * 原实现 AGENTOS_STRNCPY_TERM(node.node_name, ...) 写入 NULL 指针导致 SEGV。
     * workflow_add_node 内部会 AGENTOS_STRDUP 复制 node_name（workflow_patterns.c:412），
...
static workflow_edge_t make_wf_edge(edge_id_t id, vertex_id_t src, vertex_id_t dst)
{
    ...
    AGENTOS_MEMSET(&edge, 0, sizeof(edge));
    edge.edge_id = id;
