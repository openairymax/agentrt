...
{
    builtin_retrieval_t *ret = (builtin_retrieval_t *)AGENTOS_CALLOC(1, sizeof(builtin_retrieval_t));
    return ret;
}
...
void builtin_retrieval_update_stats(builtin_retrieval_t *ret, size_t total_docs,
...
float builtin_retrieval_bm25_score(builtin_retrieval_t *ret, const char *query_tokens[],
                                   size_t query_token_count, const char *doc_text,
                                   size_t doc_length)
{

    if (!ret || !query_tokens || !doc_text)
        return 0.0f;
...
float builtin_retrieval_cosine_similarity(const float *vec_a, const float *vec_b, size_t dim)
{
...
size_t builtin_retrieval_tokenize(const char *text, char tokens[][MAX_TOKEN_LEN], size_t max_tokens)
{
