...
builtin_storage_t* builtin_storage_create(const char* base_path) {
    builtin_storage_t* st = (builtin_storage_t*)calloc(1, sizeof(builtin_storage_t));
    if (!st) return NULL;

    snprintf(st->base_path, sizeof(st->base_path), "%s", base_path ? base_path : "./data/agentos/memory");
    ...
    st->records = (storage_record_t*)calloc(st->record_capacity, sizeof(storage_record_t));
    if (!st->records) {
        free(st);
        return NULL;
    }
...
void builtin_storage_destroy(builtin_storage_t* st) {
    if (!st) return;
    if (st->records) free(st->records);
    free(st);
}

agentos_error_t builtin_storage_write(
    builtin_storage_t* st,
    const void* data, size_t len,
    const char* metadata_json,
    char** out_record_id) {

    if (!st || !data || len == 0 || !out_record_id) return AGENTOS_EINVAL;
    ...
    *out_record_id = strdup(rec->record_id);
    if (!*out_record_id) return AGENTOS_ENOMEM;

    st->record_count++;
    return AGENTOS_SUCCESS;
}

agentos_error_t builtin_storage_get(
    builtin_storage_t* st,
    const char* record_id,
    void** out_data, size_t* out_len) {

    if (!st || !record_id || !out_data || !out_len) return AGENTOS_EINVAL;
    ...
    for (size_t i = 0; i < st->record_count; i++) {
        if (strcmp(st->records[i].record_id, record_id) == 0) {
            ...
            if (nread != fsize) { free(buf); return AGENTOS_EIO; }
...
agentos_error_t builtin_storage_delete(
    builtin_storage_t* st,
    const char* record_id) {
    ...
    for (size_t i = 0; i < st->record_count; i++) {
        if (strcmp(st->records[i].record_id, record_id) == 0) {
            ...
            if (i < st->record_count - 1) {
                memmove(&st->records[i], &st->records[i + 1],
...
const storage_record_t* builtin_storage_get_record(
    const builtin_storage_t* st, size_t index) {
    if (!st || index >= st->record_count) return NULL;
    return &st->records[index];
}

const char* builtin_storage_get_metadata(
    const builtin_storage_t* st, const char* record_id) {
    if (!st || !record_id) return NULL;
    for (size_t i = 0; i < st->record_count; i++) {
        if (strcmp(st->records[i].record_id, record_id) == 0) {
