...
MemoryRovol 是 AgentRT 的商业桥接层，负责将外部 MemoryRovol PRO 仓库集成到 AgentRT 构建体系中。MemoryRovol PRO 实现 L1-L4 全功能记忆系统（原始存储、特征索引、结构绑定、模式识别），通过实现 `agentos_memory_provider_t` 接口无缝替换内置免费提供商。
...
├── src/
│   └── agentos_mr_force_includes.h   # 强制包含头文件（注入 AgentRT 兼容性）
└── README.md
```
...
| CMake 变量 | 路径 | 说明 |
|-------------|------|------|
| `AGENTOS_ROOT` | AgentRT 工作区根目录 | 供 MemoryRovol 定位 AgentRT 资源 |
| `AGENTOS_INCLUDE_DIR` | `agentos/` 目录 | AgentRT 头文件根目录 |
| `AGENTOS_ATOMS_DIR` | `agentos/atoms/` 目录 | Atoms 源码目录 |

#### 头文件搜索路径

为 `agentos_memoryrovol` 目标注入以下 AgentRT 头文件目录：
