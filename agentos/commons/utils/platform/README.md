...
Platform 模块提供面向应用层的高级跨平台工具集，封装了操作系统差异，为上层业务模块提供统一的文件系统操作、环境变量管理、进程管理、路径操作和时间工具。该模块位于 `utils/platform/`，与顶层 `platform/` 模块（系统层低级抽象）形成互补，专注于业务逻辑代码所需的跨平台能力。
...
### platform_exec_result_t — 命令执行结果

| 字段 | 类型 | 说明 |
|------|------|------|
| `exit_code` | `int` | 进程退出码 |
| `output` | `char *` | 命令标准输出（需调用 `platform_free_exec_result` 释放） |
| `output_length` | `size_t` | 输出长度 |
| `success` | `bool` | 是否执行成功（退出码为 0） |
...
| 函数 | 说明 |
|------|------|
| `platform_exec(command, timeout_ms)` | 执行系统命令（0 表示无超时） |
| `platform_free_exec_result(result)` | 释放命令执行结果 |
...
// 命令执行
platform_exec_result_t result = platform_exec("ls -la /tmp", 5000);
if (result.success) {
    printf("Command output:\n%.*s\n", (int)result.output_length, result.output);
}
platform_free_exec_result(&result);
...
|------|-------|---------|-------|
| 路径分隔符 | `/` | `\` | `/` |
| 命令执行 | `fork/exec` + `/bin/sh` | `CreateProcess` + `cmd.exe` | `fork/exec` + `/bin/sh` |
