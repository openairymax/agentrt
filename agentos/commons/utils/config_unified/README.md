...
Config Unified 是 AgentOS 的统一配置管理模块，提供从配置定义、加载、校验到运行时访问的完整解决方案。采用三层分离架构（Core→Source→Service），将配置定义、来源管理和运行时访问完全解耦，支持热重载、Schema 校验、配置加密和版本管理。
...
| `CONFIG_SOURCE_ENV` | 1 | 高 | `AGENTOS_` 前缀的环境变量 |
