                ...
                if (!validator->enum_values[i]) {
                    ...
                    for (size_t j = 0; j < i; j++) {
                        if (validator->enum_values[j])
                            AGENTOS_FREE(validator->enum_values[j]);
                    }
...
void config_validator_destroy(config_validator_t *validator)
{
    ...
    if (validator->enum_values) {
        for (size_t i = 0; i < validator->enum_count; i++) {
            if (validator->enum_values[i])
                AGENTOS_FREE(validator->enum_values[i]);
        }
        AGENTOS_FREE(validator->enum_values);
    }

    if (validator->error_message)
        AGENTOS_FREE(validator->error_message);
...
config_schema_t *config_schema_create(const char *name)
{
    ...
    schema->capacity = 16;
    schema->items =
        (schema_item_internal_t *)AGENTOS_CALLOC(schema->capacity, sizeof(schema_item_internal_t));
    if (!schema->items) {
...
void config_schema_destroy(config_schema_t *schema)
{
    ...
    for (size_t i = 0; i < schema->count; i++) {
        ...
        if (item->key)
            AGENTOS_FREE(item->key);
        if (item->description)
            AGENTOS_FREE(item->description);
        if (item->default_value)
            AGENTOS_FREE(item->default_value);
        if (item->validator)
...
config_error_t config_schema_add_item(config_schema_t *schema, const config_schema_item_t *item)
{
    ...
    if (item->default_value) {
        ...
        if (!new_item->default_value) {
            if (new_item->description)
                AGENTOS_FREE(new_item->description);
            AGENTOS_FREE(new_item->key);
            return CONFIG_ERROR_OUT_OF_MEMORY;
        }
    }
...
config_hot_reload_manager_create(config_context_t *ctx, config_source_manager_t *source_manager)
{
    ...
    manager->callbacks = (change_callback_item_t *)AGENTOS_CALLOC(manager->callback_capacity,
                                       ...
                                       uint32_t check_interval_ms)
{
    ...
    if (manager->thread_handle == NULL) {
        agentos_thread_t *thread = AGENTOS_MALLOC(sizeof(agentos_thread_t));
        if (!thread) {
            manager->running = false;
            return CONFIG_ERROR_OUT_OF_MEMORY;
        }
...
void config_version_manager_destroy(config_version_manager_t *manager)
{
    ...
    for (size_t i = 0; i < manager->count; i++) {
        ...
        if (version->author)
            AGENTOS_FREE(version->author);
        if (version->description)
            AGENTOS_FREE(version->description);
        if (version->snapshot) {
                                        ...
                                        const char *description)
{
    ...
    if (manager->count > manager->max_versions) {
        ...
        if (oldest->author)
            AGENTOS_FREE(oldest->author);
        if (oldest->description)
            AGENTOS_FREE(oldest->description);
        if (oldest->snapshot) {
        ...
        if (manager->versions[i].version == version) {
            ...
            for (size_t j = i + 1; j < manager->count; j++) {
                if (manager->versions[j].author)
                    AGENTOS_FREE(manager->versions[j].author);
                if (manager->versions[j].description)
                    AGENTOS_FREE(manager->versions[j].description);
