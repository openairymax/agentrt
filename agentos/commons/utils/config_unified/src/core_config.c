...
config_value_t *config_value_create_object(size_t capacity)
{
    ...
    if (val) {
        ...
        val->data.object_value.items = AGENTOS_CALLOC(val->data.object_value.capacity,
...
{
    if (!value) {
        AGENTOS_LOG_DEBUG("%*s(null)", indent, "");
        return;
    }

    switch (value->type) {
    case CONFIG_TYPE_NULL:
        AGENTOS_LOG_DEBUG("%*snull", indent, "");
        break;

    case CONFIG_TYPE_BOOL:
        AGENTOS_LOG_DEBUG("%*s%s", indent, "", value->data.bool_value ? "true" : "false");
        break;

    case CONFIG_TYPE_INT:
        AGENTOS_LOG_DEBUG("%*s%d", indent, "", value->data.int_value);
        break;

    case CONFIG_TYPE_INT64:
        AGENTOS_LOG_DEBUG("%*s%lld", indent, "", (long long)value->data.int64_value);
        break;

    case CONFIG_TYPE_DOUBLE:
        AGENTOS_LOG_DEBUG("%*s%g", indent, "", value->data.double_value);
        break;

    case CONFIG_TYPE_STRING:
        AGENTOS_LOG_DEBUG("%*s\"%s\"", indent, "", value->data.string_value.str);
        break;

    case CONFIG_TYPE_ARRAY:
        AGENTOS_LOG_DEBUG("%*s[", indent, "");
        for (size_t i = 0; i < value->data.array_value.count; i++) {
    ...
    case CONFIG_TYPE_OBJECT:
        AGENTOS_LOG_DEBUG("%*s{", indent, "");
        for (size_t i = 0; i < value->data.object_value.count; i++) {
            AGENTOS_LOG_DEBUG("%*s\"%s\": ", indent + 2, "", value->data.object_value.items[i].key);
    ...
    case CONFIG_TYPE_BINARY:
        AGENTOS_LOG_DEBUG("%*s<binary data, size=%zu>", indent, "", value->data.binary_value.size);
        break;
    }
}
