...
agentos_error_t ipc_channel_open(ipc_channel_t *channel)
{
    ...
    if (channel->config.buffer_size > 0) {
        channel->internal_buffer = AGENTOS_MALLOC(channel->config.buffer_size);
...
agentos_error_t ipc_channel_close(ipc_channel_t *channel)
{
    ...
    /* 释放内部缓冲区 */
    if (channel->internal_buffer) {
        AGENTOS_FREE(channel->internal_buffer);
        channel->internal_buffer = NULL;
    }
    channel->buffer_used = 0;
...
agentos_error_t ipc_send(ipc_channel_t *channel, const ipc_message_t *message)
{
    ...
    void *send_buffer = AGENTOS_MALLOC(total_size);
    if (!send_buffer) {
        return AGENTOS_ENOMEM;
    }
    ...
    if (message->payload && message->payload_size > 0) {
        memcpy((char *)send_buffer + sizeof(ipc_message_header_t), message->payload,
...
agentos_error_t ipc_receive(ipc_channel_t *channel, ipc_message_t *message, uint32_t timeout_ms)
{
    ...
    if (message->header.payload_len > 0 &&
        ...
        message->payload = AGENTOS_MALLOC(message->header.payload_len);
        if (!message->payload) {
            return AGENTOS_ENOMEM;
        }
...
agentos_error_t ipc_receive_data(ipc_channel_t *channel, void *buffer, size_t len, size_t *received)
{
    if (!channel || !buffer) {
        return AGENTOS_EINVAL;
    }
...
void ipc_server_destroy(ipc_server_t *server)
{
    ...
    AGENTOS_FREE(server->connections);
    AGENTOS_FREE(server);
}
...
agentos_error_t ipc_server_start(ipc_server_t *server)
{
    ...
    server->connections =
        (ipc_channel_t **)AGENTOS_CALLOC(server->max_connections, sizeof(ipc_channel_t *));
 ...
 */
static agentos_error_t ipc_mq_dequeue_message(ipc_mq_t *mq, void *buffer, size_t len,
...
agentos_error_t ipc_mq_send(ipc_mq_t *mq, const void *data, size_t len, unsigned int priority)
{
    ...
    // 创建新消息
    ipc_mq_message_t *msg = (ipc_mq_message_t *)AGENTOS_MALLOC(sizeof(ipc_mq_message_t));
    if (!msg) {
                                      ...
                                      size_t *written)
{
    ...
    if (message->payload && message->payload_size > 0) {
        memcpy((char *)buffer + sizeof(ipc_message_header_t), message->payload,
               message->payload_size);
    }
...
agentos_error_t ipc_message_deserialize(const void *buffer, size_t len, ipc_message_t *message)
{
    ...
    if (message->header.payload_len > 0) {
        ...
        memcpy(message->payload, (const char *)buffer + sizeof(ipc_message_header_t),
...
ipc_rpc_server_t *ipc_rpc_server_create(const ipc_rpc_server_config_t *config)
{
    ...
    if (config->service_name) {
        server->service_name = AGENTOS_STRDUP(config->service_name);
    }

    for (size_t i = 0; i < config->method_count; i++) {
        rpc_method_node_t *node = (rpc_method_node_t *)AGENTOS_CALLOC(1, sizeof(rpc_method_node_t));
        if (!node)
            continue;
        node->method_name = AGENTOS_STRDUP(config->methods[i].method_name);
...
void ipc_rpc_server_destroy(ipc_rpc_server_t *server)
{
    ...
    while (node) {
        ...
        AGENTOS_FREE(node->method_name);
        AGENTOS_FREE(node);
        node = next;
    }

    AGENTOS_FREE(server->service_name);
    AGENTOS_FREE(server);
}
                                               ...
                                               const ipc_rpc_method_t *method)
{
    ...
    node->method_name = AGENTOS_STRDUP(method->method_name);
...
agentos_error_t ipc_rpc_server_process(ipc_rpc_server_t *server, uint32_t timeout_ms)
{
    ...
    void *response_buf = AGENTOS_CALLOC(1, response_max);
    if (!response_buf) {
        ipc_message_free(&msg);
        return AGENTOS_ENOMEM;
    }
    ...
    void *combined_payload = AGENTOS_MALLOC(total_payload);
    if (combined_payload) {
        ...
        ipc_send(server->transport, &rsp_msg);
        AGENTOS_FREE(combined_payload);
    }

    AGENTOS_FREE(response_buf);
    ipc_message_free(&msg);
    return AGENTOS_SUCCESS;
}
                                  ...
                                  size_t response_max, size_t *response_len)
{
    ...
    void *request_buf = AGENTOS_MALLOC(total_payload);
    if (!request_buf)
        return AGENTOS_ENOMEM;
