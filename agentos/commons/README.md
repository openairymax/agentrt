...
Commons 是 AgentOS 的统一基础库，为整个系统提供跨平台、跨语言、跨模块的通用基础设施。Commons 不依赖任何上层模块，所有原子内核、守护进程、安全组件均可基于它构建。作为全项目的类型定义权威源（Authoritative Source），Commons 确保了模块间类型一致性，消除了跨模块类型冲突。

## 设计目标

- **零依赖抽象**：平台无关的类型系统与接口定义，确保内核与外围代码分离
- **统一错误契约**：全系统共享的错误码体系（`agentos_error_t`）与异常传播机制
...
| 类型 | 说明 |
|------|------|
| `agentos_error_t` | 统一错误码类型（`int32_t`，负值为错误，0 为成功） |
| `agentos_ipc_header_t` | 应用级 IPC 消息头（magic/version/type/flags/msg_id 等） |
| `agentos_ipc_message_t` | 应用级 IPC 消息结构（header + payload） |
| `agentos_task_id_t` | 任务 ID 类型（`uint64_t`） |
| `agentos_message_id_t` | 消息 ID 类型（`uint64_t`） |

统一错误码体系（`AGENTOS_E*`）覆盖：参数无效、内存不足、权限不足、超时、I/O 错误、协议错误、配额超限等 29 种标准错误。
...
- **文件系统**：路径规范化、文件操作抽象
- **线程与同步**：`agentos_thread_t`、`agentos_mutex_t`、`agentos_cond_t` 等
- **动态库加载**：跨平台 FFI 支持
...
> **BAN-12**：外部依赖由根 `CMakeLists.txt` 集中检测，子模块不得独立调用 `find_package`。检测结果通过 `AGENTOS_HAS_CJSON` 等 CMake 缓存变量传递。
...
- `AGENTOS_HAS_CJSON`：由根 CMakeLists 自动检测
- `AGENTOS_HAS_YAML`：由根 CMakeLists 自动检测
