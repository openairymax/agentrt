                        ...
                        const char *upload_data, size_t *upload_data_size, void **con_cls)
{
    ...
    /* 阶段 1: 初始化请求上下文 */
    if (!context) {
        context = AGENTOS_CALLOC(1, sizeof(http_request_context_t));
        if (!context) {
            return MHD_NO;
        }
