...
static char *route_task_methods(const char *method, cJSON *params, cJSON *request_id)
{
    ...
    if (strcmp(method, "agentos_sys_task_submit") == 0) {
        ...
        err = agentos_sys_task_submit(input->valuestring, strlen(input->valuestring), timeout_ms,
                                      &out_result);

        if (err == AGENTOS_SUCCESS && out_result) {
            result = cJSON_CreateObject();
            cJSON_AddStringToObject(result, "result", out_result);
            AGENTOS_FREE(out_result);
        }
...
static char *route_memory_methods(const char *method, cJSON *params, cJSON *request_id)
{
    ...
    if (strcmp(method, "agentos_sys_memory_write") == 0) {
        ...
        err = agentos_sys_memory_write(data->valuestring, strlen(data->valuestring), meta_str,
                                       &out_record_id);

        if (meta_str)
            AGENTOS_FREE((void *)meta_str);

        if (err == AGENTOS_SUCCESS && out_record_id) {
            result = cJSON_CreateObject();
            cJSON_AddStringToObject(result, "record_id", out_record_id);
            AGENTOS_FREE(out_record_id);
        }
...
static char *route_agent_methods(const char *method, cJSON *params, cJSON *request_id)
{
    ...
    if (strcmp(method, "agentos_sys_agent_spawn") == 0) {
        ...
        err = agentos_sys_agent_invoke(agent_id->valuestring, input_str, strlen(input_str),
                                       &out_output);

        if (err == AGENTOS_SUCCESS && out_output) {
            result = cJSON_CreateObject();
            cJSON_AddStringToObject(result, "output", out_output);
            AGENTOS_FREE(out_output);
        }
...
static void ht_destroy(hash_table_t *ht)
{
    ...
    for (size_t i = 0; i < ht->capacity; i++) {
        AGENTOS_FREE(ht->entries[i].key);
    }
    AGENTOS_FREE(ht->entries);
    ht->entries = NULL;
    ht->capacity = 0;
    ht->count = 0;
}
...
static bool ht_insert(hash_table_t *ht, const char *key, size_t index)
{
    ...
    for (size_t i = 0; i < ht->capacity; i++) {
        ...
        if (!ht->entries[pos].occupied) {
            ht->entries[pos].key = AGENTOS_STRDUP(key);
...
static void ht_remove(hash_table_t *ht, const char *key)
{
    ...
    for (size_t i = 0; i < ht->capacity; i++) {
        ...
        if (!ht->entries[pos].occupied)
            return;
        if (strcmp(ht->entries[pos].key, key) == 0) {
            AGENTOS_FREE(ht->entries[pos].key);
...
static void __attribute__((constructor)) runtime_init(void)
{
    ...
    g_runtime.tasks = (task_entry_t *)AGENTOS_CALLOC(g_max_tasks, sizeof(task_entry_t));
    g_runtime.records = (memory_record_t *)AGENTOS_CALLOC(g_max_records, sizeof(memory_record_t));
    g_runtime.sessions = (session_entry_t *)AGENTOS_CALLOC(g_max_sessions, sizeof(session_entry_t));
    g_runtime.agents = (agent_entry_t *)AGENTOS_CALLOC(g_max_agents, sizeof(agent_entry_t));
    if (!g_runtime.tasks || !g_runtime.records || !g_runtime.sessions || !g_runtime.agents) {
        AGENTOS_LOG_ERROR("syscall_router: runtime_init calloc failed");
        AGENTOS_FREE(g_runtime.tasks);
        AGENTOS_FREE(g_runtime.records);
        AGENTOS_FREE(g_runtime.sessions);
        AGENTOS_FREE(g_runtime.agents);
...
static const char *generate_uuid(void)
{
    ...
    snprintf(uuid, sizeof(uuid), "agentos-%016llx-%08llx", (unsigned long long)time(NULL),
             (unsigned long long)++counter);
    return uuid;
}

/* Task 管理 */
agentos_error_t agentos_sys_task_submit(const char *input, size_t len, uint32_t timeout_ms,
                                        char **out_result)
{
    if (!input || !out_result)
    ...
    task->task_id = AGENTOS_STRDUP(generate_uuid());
...
agentos_error_t agentos_sys_task_wait(const char *task_id, uint32_t timeout_ms, char **out_result)
{
    ...
    if (idx >= 0 && (size_t)idx < g_runtime.task_count) {
        ...
        g_runtime.tasks[idx].result = AGENTOS_STRDUP("{\"output\":\"processed\",\"exit_code\":0}");
...
/* Memory 管理 */
agentos_error_t agentos_sys_memory_write(const void *data, size_t len, const char *metadata,
                                         char **out_record_id)
{
    ...
    rec->record_id = AGENTOS_STRDUP(generate_uuid());
    rec->data = AGENTOS_MALLOC(len);
    if (!rec->data) {
        RUNTIME_UNLOCK();
        return AGENTOS_ERR_OUT_OF_MEMORY;
    }
...
agentos_error_t agentos_sys_memory_search(const char *query, uint32_t limit, char ***record_ids,
                                          float **scores, size_t *count)
{
    ...
    *record_ids = (char **)AGENTOS_CALLOC(max_results, sizeof(char *));
    *scores = (float *)AGENTOS_CALLOC(max_results, sizeof(float));
    ...
    for (size_t i = 0; i < g_runtime.record_count && found < max_results; i++) {
        if (!query || strlen(query) == 0 ||
            ...
            (*record_ids)[found] = AGENTOS_STRDUP(g_runtime.records[i].record_id);
...
agentos_error_t agentos_sys_memory_get(const char *record_id, void **out_data, size_t *out_len)
{
    ...
    if (idx >= 0 && (size_t)idx < g_runtime.record_count) {
        *out_data = AGENTOS_MALLOC(g_runtime.records[idx].len + 1);
        if (!*out_data) {
            RUNTIME_UNLOCK();
            return AGENTOS_ERR_OUT_OF_MEMORY;
        }
...
agentos_error_t agentos_sys_session_create(const char *metadata, char **out_session_id)
{
    ...
    sess->session_id = AGENTOS_STRDUP(generate_uuid());
    sess->metadata = metadata ? AGENTOS_STRDUP(metadata) : NULL;
    sess->created_at = time(NULL);
    sess->last_accessed = sess->created_at;
    *out_session_id = AGENTOS_STRDUP(sess->session_id);
...
agentos_error_t agentos_sys_session_list(char ***sessions, size_t *count)
{
    ...
    RUNTIME_LOCK();
    *sessions = (char **)AGENTOS_CALLOC(g_runtime.session_count, sizeof(char *));
    ...
    for (size_t i = 0; i < g_runtime.session_count; i++) {
        (*sessions)[i] = AGENTOS_STRDUP(g_runtime.sessions[i].session_id);
    }
...
agentos_error_t agentos_sys_agent_spawn(const char *spec, char **out_agent_id)
{
    ...
    agent->agent_id = AGENTOS_STRDUP(generate_uuid());
    agent->spec = AGENTOS_STRDUP(spec);
    agent->status = 1;
    agent->spawned_at = time(NULL);
    *out_agent_id = AGENTOS_STRDUP(agent->agent_id);
...
agentos_error_t agentos_sys_agent_invoke(const char *agent_id, const char *input, size_t len,
                                         char **out_output)
{
    ...
    RUNTIME_UNLOCK();
    *out_output = AGENTOS_STRDUP("{\"error\":\"Agent not found\"}");
    return AGENTOS_ERR_NOT_FOUND;
}
...
agentos_error_t agentos_sys_agent_list(char ***agent_ids, size_t *count)
{
    ...
    RUNTIME_LOCK();
    *agent_ids = (char **)AGENTOS_CALLOC(g_runtime.agent_count, sizeof(char *));
    ...
    for (size_t i = 0; i < g_runtime.agent_count; i++) {
        (*agent_ids)[i] = AGENTOS_STRDUP(g_runtime.agents[i].agent_id);
    }
