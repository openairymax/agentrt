...
|-----------------|---------------------|------|
| `pipeline/build-module.sh` | `atoms/`, `commons/`, `cupolas/`, `daemon/`, `gateway/`, `heapstore/` | 多模块并行/增量编译 |
| `pipeline/run-tests.sh` | 所有模块 | CTest + pytest 双引擎测试执行 |
| `pipeline/quality-gate.sh` | 所有模块 | 代码质量门禁检查 |
| `pipeline/security_check.py` | `atoms/`, `commons/`, `cupolas/` | C 语言安全编码静态检查 |
| `pipeline/security_regression.sh` | `atoms/`, `commons/` | 安全回归测试 |
| `pipeline/install-deps.sh` | 所有模块 | 跨平台依赖安装 |
| `pipeline/deploy-artifacts.sh` | 所有模块 | 构建产物归档与部署 |
...
```bash
# 依赖安装
scripts/ci/pipeline/install-deps.sh

# 模块编译
scripts/ci/pipeline/build-module.sh

# 测试执行
scripts/ci/pipeline/run-tests.sh

# 质量门禁
scripts/ci/pipeline/quality-gate.sh

# 安全检查
python scripts/ci/pipeline/security_check.py

# 安全回归测试
scripts/ci/pipeline/security_regression.sh

# 产物部署
scripts/ci/pipeline/deploy-artifacts.sh
```
...
| `pipeline/security_check.py` | Python 3.8+ | C 语言安全编码静态检查，纯 Python 实现 |
