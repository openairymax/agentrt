...
AgentOS 脚本工具集是项目全生命周期管理的核心基础设施，涵盖 CI/CD 流水线、开发环境搭建、运维部署、性能基准测试、Python 运维工具包和项目资源等六大领域。所有脚本按职能分类存放于五个子模块中，与 `agentos/` 核心模块紧密对应，形成从代码提交到生产部署的完整自动化链路。
...
AgentOS 运维和开发的 Python 工具集合，包含 15 个源模块：
