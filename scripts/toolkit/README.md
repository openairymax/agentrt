...
`toolkit/` 模块提供 AgentOS 运维和开发的 Python 工具集合，包含 15 个源模块，覆盖系统诊断、性能测试、多层记忆管理、Token 统计与预算、配置模板引擎、检查点管理、契约验证、CLI 增强、事件总线、日志、插件系统、安全模块和遥测系统。该模块是 AgentOS Python 生态的核心基础设施，为上层运维脚本和开发工具提供统一的工具类接口。

工具包的设计原则：

- **统一导出**：`__init__.py` 统一导出所有工具类，用户只需 `from scripts.toolkit import ...` 即可使用
- **模块化设计**：每个模块职责单一、接口清晰，可独立使用也可组合编排
- **预览版迭代**：当前为 v0.0.5 预览版，API 可能随版本迭代发生变化
- **Prometheus 兼容**：遥测模块兼容 Prometheus 指标格式，可无缝集成到现有监控体系

> **版本**：v0.0.5
> **状态**：🔶 预览版，API 可能随版本迭代发生变化。
