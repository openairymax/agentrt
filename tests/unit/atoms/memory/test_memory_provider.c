...
static void test_builtin_provider_init_destroy(void) {
    ...
    agentos_error_t err = provider->init(provider, test_dir);
    TEST_ASSERT(err == AGENTOS_SUCCESS, "builtin provider init succeeds");
...
static void test_builtin_provider_write_get(void) {
    ...
    TEST_ASSERT(err == AGENTOS_SUCCESS, "write_raw succeeds");
    ...
    if (record_id) {
        ...
        TEST_ASSERT(err == AGENTOS_SUCCESS, "get_raw succeeds");
        ...
        if (out_data) AGENTOS_FREE(out_data);
...
static void test_builtin_provider_delete(void) {
    ...
    if (record_id) {
        ...
        TEST_ASSERT(err == AGENTOS_SUCCESS, "delete_raw succeeds");

        void* out_data = NULL;
        size_t out_len = 0;
        err = provider->get_raw(provider, record_id, &out_data, &out_len);
        TEST_ASSERT(err != AGENTOS_SUCCESS, "get_raw fails after delete");
...
static void test_builtin_provider_query(void) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        err = provider->write_raw(provider, entries[i], strlen(entries[i]),
                                   metadata, &rid);
        TEST_ASSERT(err == AGENTOS_SUCCESS, "write_raw for query test data succeeds");
        if (rid) AGENTOS_FREE(rid);
    }
    ...
    TEST_ASSERT(err == AGENTOS_SUCCESS, "query succeeds");
...
static void test_builtin_provider_multiple_writes(void) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        err = provider->write_raw(provider, data, strlen(data), metadata,
                                   &record_ids[i]);
        if (err == AGENTOS_SUCCESS && record_ids[i]) {
            success_count++;
        } else {
            record_ids[i] = NULL;
        }
...
static void test_builtin_provider_null_params(void) {
    ...
    TEST_ASSERT(err != AGENTOS_SUCCESS, "write_raw with NULL provider returns error");

    err = provider->write_raw(provider, NULL, 4, "{}", NULL);
    TEST_ASSERT(err != AGENTOS_SUCCESS, "write_raw with NULL data returns error");

    err = provider->get_raw(NULL, "id", NULL, NULL);
    TEST_ASSERT(err != AGENTOS_SUCCESS, "get_raw with NULL provider returns error");

    err = provider->delete_raw(NULL, "id");
    TEST_ASSERT(err != AGENTOS_SUCCESS, "delete_raw with NULL provider returns error");

    err = provider->query(NULL, "text", 10, NULL, NULL, NULL);
    TEST_ASSERT(err != AGENTOS_SUCCESS, "query with NULL provider returns error");
...
static void test_builtin_provider_stats(void) {
    ...
    if (provider->get_stats) {
        ...
        TEST_ASSERT(err == AGENTOS_SUCCESS, "get_stats succeeds");
    ...
    if (provider->get_stats) {
        err = provider->get_stats(provider, &stats);
        TEST_ASSERT(err == AGENTOS_SUCCESS, "get_stats after write succeeds");
