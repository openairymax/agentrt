...
| `python/` | `daemon/`, `commons/` | Python 性能基准测试（守护进程/公共库性能指标） |
| `atoms/` | `agentos/atoms/` | Atoms 层组件性能测试（内核 IPC/记忆/系统调用延迟） |
| `concurrency/` | `daemon/`, `gateway/` | 并发压力测试（负载测试/报告输出） |
| `cupolas/` | `agentos/cupolas/` | Cupolas 安全基准与压力测试（权限检查/清洗吞吐/并发安全） |
...
| **并发处理** | `daemon/`, `gateway/` | QPS、响应时间 P50/P95/P99、错误率 | `concurrency/load_test.py` |
...
| **Python 性能** | `daemon/`, `commons/` | 服务调用延迟、配置加载时间、序列化吞吐 | `python/benchmark_performance.py` |
...
| `daemon/` | `python/benchmark_performance.py`, `concurrency/load_test.py` | pytest-benchmark | 服务调用延迟、并发 QPS |
